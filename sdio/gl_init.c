...
#if CFG_THERMAL_API_SUPPORT
BOOLEAN g_fgIsWifiEnabled = FALSE;
...
MODULE_AUTHOR("Yogi Hermawan <yogist.xda@gmail.com>");
...
int kalDcSetWow(void)
{
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucWow && prGlueInfo->prAdapter->rWowCtrl.fgWowEnable) {
		DBGLOG(INIT, EVENT, "Rekey offload wow enable\n");
		setRekeyOffloadEnterWow(prGlueInfo);

		if (kalGetMediaStateIndicated(prGlueInfo) == PARAM_MEDIA_STATE_CONNECTED) {
			/* when connected AIS bss enter wow power mode,
			 *  default fast power-saving
			 */
	...
	while (prGlueInfo->rHifInfo.state != USB_STATE_PRE_SUSPEND_DONE) {
		if (count > 25) {
			DBGLOG(HAL, ERROR, "pre_suspend timeout, return -ETIME in the end\n");
			ret = -ETIME;
			break;
		}
		msleep(20);
...
static struct ieee80211_channel mtk_2ghz_channels[] = {
	CHAN2G(1, 2412, 0),  CHAN2G(2, 2417, 0),  CHAN2G(3, 2422, 0),  CHAN2G(4, 2427, 0),  CHAN2G(5, 2432, 0),
	CHAN2G(6, 2437, 0),  CHAN2G(7, 2442, 0),  CHAN2G(8, 2447, 0),  CHAN2G(9, 2452, 0),  CHAN2G(10, 2457, 0),
	CHAN2G(11, 2462, 0), CHAN2G(12, 2467, 0), CHAN2G(13, 2472, 0), CHAN2G(14, 2484, 0),
};

#define CHAN5G(_channel, _flags) \
	{ \
		.band = KAL_BAND_5GHZ, .center_freq = 5000 + (5 * (_channel)), .hw_value = (_channel), \
		.flags = (_flags), .max_antenna_gain = 0, .max_power = 30, \
	}
static struct ieee80211_channel mtk_5ghz_channels[] = {
	CHAN5G(36, 0),	CHAN5G(40, 0),	CHAN5G(44, 0),	CHAN5G(48, 0),	CHAN5G(52, 0),	CHAN5G(56, 0),	CHAN5G(60, 0),
	CHAN5G(64, 0),	CHAN5G(100, 0), CHAN5G(104, 0), CHAN5G(108, 0), CHAN5G(112, 0), CHAN5G(116, 0), CHAN5G(120, 0),
	CHAN5G(124, 0), CHAN5G(128, 0), CHAN5G(132, 0), CHAN5G(136, 0), CHAN5G(140, 0), CHAN5G(144, 0), CHAN5G(149, 0),
...
static struct ieee80211_rate mtk_rates[] = {
	...
	RATETAB_ENT(60, 0x2000, 0),  RATETAB_ENT(90, 0x2001, 0),  RATETAB_ENT(120, 0x2002, 0),
	RATETAB_ENT(180, 0x2003, 0), RATETAB_ENT(240, 0x2004, 0), RATETAB_ENT(360, 0x2005, 0),
	RATETAB_ENT(480, 0x2006, 0), RATETAB_ENT(540, 0x2007, 0), /* 802.11a/g */
};
...
#define WLAN_MCS_INFO \
	{ \
		.rx_mask = { 0xff, 0, 0, 0, 0, 0, 0, 0, 0, 0 }, .rx_highest = 0, \
		.tx_params = IEEE80211_HT_MCS_TX_DEFINED, \
	}

#define WLAN_VHT_MCS_INFO \
	{ \
		.rx_mcs_map = 0xFFFA, .rx_highest = cpu_to_le16(867), .tx_mcs_map = 0xFFFA, \
...
#define WLAN_VHT_CAP \
	{ \
		...
		.cap = IEEE80211_VHT_CAP_RXLDPC | IEEE80211_VHT_CAP_SUPP_CHAN_WIDTH_MASK | \
		       IEEE80211_VHT_CAP_MAX_MPDU_LENGTH_11454 | IEEE80211_VHT_CAP_RXLDPC | \
...
static const struct wiphy_vendor_command mtk_wlan_vendor_ops[] = {
	...
	{ { .vendor_id = GOOGLE_OUI, .subcmd = WIFI_SUBCMD_SET_COUNTRY_CODE },
	  ...
	  VENDOR_OPS_SET_POLICY(VENDOR_CMD_RAW_DATA) },
	{ { .vendor_id = GOOGLE_OUI, .subcmd = WIFI_SUBCMD_GET_ROAMING_CAPABILITIES },
	  .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
	...
	{ { .vendor_id = GOOGLE_OUI, .subcmd = WIFI_SUBCMD_SELECT_TX_POWER_SCENARIO },
	  .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
	...
	{ { .vendor_id = GOOGLE_OUI, .subcmd = LOGGER_GET_VER },
	  ...
	  VENDOR_OPS_SET_POLICY(VENDOR_CMD_RAW_DATA) },
	{ { .vendor_id = GOOGLE_OUI, .subcmd = WIFI_SUBCMD_SET_PNO_RANDOM_MAC_OUI },
	  .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
static const struct nl80211_vendor_cmd_info mtk_wlan_vendor_events[] = {
	...
	{ .vendor_id = GOOGLE_OUI, .subcmd = WIFI_EVENT_MAGIC_PACKET_RECEIVED }
#endif
};
#endif
...
static void glLoadNvram(IN P_GLUE_INFO_T prGlueInfo, OUT P_REG_INFO_T prRegInfo)
{
	...
	if (kalCfgDataRead16(prGlueInfo, sizeof(WIFI_CFG_PARAM_STRUCT) - sizeof(UINT_16), (PUINT_16)aucTmp) == TRUE) {
		...
		kalCfgDataRead16(prGlueInfo, OFFSET_OF(WIFI_CFG_PARAM_STRUCT, fgRssiCompensationVaildbit),
				 (PUINT_16)aucTmp);
		...
		if (aucTmp[0]) {
			...
			kalCfgDataRead16(prGlueInfo, OFFSET_OF(WIFI_CFG_PARAM_STRUCT, cBandEdgeMaxPwrOFDM20),
					 (PUINT_16)aucTmp);
...
int wlanDoIOCTL(struct net_device *prDev, struct ifreq *prIfReq, int i4Cmd)
{
	...
	if ((i4Cmd >= SIOCIWFIRST) && (i4Cmd < SIOCIWFIRSTPRIV)) {
		...
		ret = alps_android_private_support_driver_cmd(prDev, prIfReq, i4Cmd);
#else
...
static void wlanSetMulticastListWorkQueue(struct work_struct *work)
{
	...
	if (kalIoctl(prGlueInfo, wlanoidSetCurrentPacketFilter, &u4PacketFilter, sizeof(u4PacketFilter), FALSE, FALSE,
		     TRUE, &u4SetInfoLen) != WLAN_STATUS_SUCCESS) {
		return;
	}
	...
	if (u4PacketFilter & PARAM_PACKET_FILTER_MULTICAST) {
		...
		kalIoctl(prGlueInfo, wlanoidSetMulticastList, prMCAddrList, (i * ETH_ALEN), FALSE, FALSE, TRUE,
			 &u4SetInfoLen);
...
#if CFG_SUPPORT_CFG80211_AUTH
#if CFG_SUPPORT_CFG80211_QUEUE
	INIT_DELAYED_WORK(&cfg80211_workq, wlanSchedCfg80211WorkQueue);
#endif
#endif
...
VOID wlanUpdateChannelTable(P_GLUE_INFO_T prGlueInfo)
{
	...
	rlmDomainGetChnlList(prGlueInfo->prAdapter, BAND_NULL, FALSE,
			     ARRAY_SIZE(mtk_2ghz_channels) + ARRAY_SIZE(mtk_5ghz_channels), &ucNumOfChannel,
			     aucChannelList);
...
static INT_32 wlanNetRegister(struct wireless_dev *prWdev)
{
	...
	do {
		...
		wlanBindBssIdxToNetInterface(prGlueInfo, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex,
					     (PVOID)prWdev->netdev);
#else
		wlanBindBssIdxToNetInterface(prGlueInfo, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex,
					     (PVOID)prWdev->netdev);
...
#if KERNEL_VERSION(3, 17, 0) <= CFG80211_VERSION_CODE
	prGlueInfo->prDevHandler = alloc_netdev_mq(sizeof(NETDEV_PRIVATE_GLUE_INFO), prInfName, NET_NAME_PREDICTABLE,
						   ether_setup, CFG_MAX_TXQ_NUM);
#else
...
int set_p2p_mode_handler(struct net_device *netdev, PARAM_CUSTOM_P2P_SET_STRUCT_T p2pmode)
{
	...
	rWlanStatus = kalIoctl(prGlueInfo, wlanoidSetP2pMode, (PVOID)&rSetP2P, sizeof(PARAM_CUSTOM_P2P_SET_STRUCT_T),
...
#define WIFI_CFG_FN "wifi.cfg"
	...
	u4ConfigReadLen = 0;
	if (pucConfigBuf) {
		if (kalRequestFirmware(WIFI_CFG_FN, pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE, &u4ConfigReadLen,
				       prAdapter->prGlueInfo->prDev) == 0) {
			/* ToDo:: Nothing */
		} else if (kalReadToFile("/storage/sdcard0/wifi.cfg", pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE,
					 &u4ConfigReadLen) == 0) {
			/* ToDo:: Nothing */
		} else if (kalReadToFile("/data/misc/wifi.cfg", pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE,
					 &u4ConfigReadLen) == 0) {
			/* ToDo:: Nothing */
		} else if (kalReadToFile("/data/misc/wifi/wifi.cfg", pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE,
					 &u4ConfigReadLen) == 0) {
			/* ToDo:: Nothing */
		}
...
WLAN_STATUS wlanExtractBufferBin(P_ADAPTER_T prAdapter)
{
	...
	if (prAdapter->fgIsSupportPowerOnSendBufferModeCMD == TRUE) {
		...
		if ((prAdapter->rWifiVar.ucEfuseBufferModeCal == LOAD_EEPROM_BIN) &&
			...
			if (kalRequestFirmware(aucEeprom, pucConfigBuf, MAX_EEPROM_BUFFER_SIZE, &u4ContentLen,
					       prGlueInfo->prDev) == 0) {
...
WLAN_STATUS wlanDownloadBufferBin(P_ADAPTER_T prAdapter)
{
	...
	if (prAdapter->fgIsSupportPowerOnSendBufferModeCMD == TRUE) {
		...
		if (prAdapter->rWifiVar.ucEfuseBufferModeCal == LOAD_AUTO) {
			...
			if (prGlueInfo->prAdapter->aucEepromVaule[1] == (chip_id >> 8)) {
		...
		rStatus = kalIoctlTimeout(prGlueInfo, wlanoidSetEfusBufferMode, (PVOID)prSetEfuseBufModeInfo,
					  sizeof(PARAM_CUSTOM_EFUSE_BUFFER_MODE_T), FALSE, TRUE, TRUE, i4OidTimeout,
					  &u4BufLen);
	}
...
#if 0
static INT_32 glEfuseCheck(P_ADAPTER_T prAdapter)
{
	UINT_32 u4BufLen = 0;
	UINT_8 uContent, uLoop, uAddrIndex;
	UINT_16 u2Addr, u2BlockIndex, u2NextBlockIndex;
	BOOLEAN bNeedUpdate = FALSE;
	PARAM_CUSTOM_ACCESS_EFUSE_T rAccessEfuseInfo;
	PARAM_CUSTOM_EFUSE_FREE_BLOCK_T rEfuseFreeBlock;
	WLAN_STATUS i4Status = WLAN_STATUS_SUCCESS;
	P_GLUE_INFO_T prGlueInfo = prAdapter->prGlueInfo;
	const UINT_16 efusecontent[][2] = { { 0x30, 0x10 },  { 0x100, 0x50 }, { 0x101, 0x30 }, { 0x102, 0x02 },
					    { 0x103, 0x81 }, { 0x104, 0x10 }, { 0xFFFF, 0xFF } };

	for (uLoop = 0; efusecontent[uLoop][0] != 0xFFFF; uLoop++) {
		u2Addr = efusecontent[uLoop][0];
		uContent = (UINT_8)efusecontent[uLoop][1];

		u2BlockIndex = (u2Addr / EFUSE_BLOCK_SIZE) * EFUSE_BLOCK_SIZE;
		if ((uLoop == 0) || (rAccessEfuseInfo.u4Address != u2BlockIndex)) {
			/*need to read new efuse block*/
			kalMemSet(&rAccessEfuseInfo, 0, sizeof(PARAM_CUSTOM_ACCESS_EFUSE_T));

			rAccessEfuseInfo.u4Address = u2BlockIndex;
			i4Status = kalIoctl(prGlueInfo, wlanoidQueryProcessAccessEfuseRead, &rAccessEfuseInfo,
					    sizeof(PARAM_CUSTOM_ACCESS_EFUSE_T), TRUE, TRUE, TRUE, &u4BufLen);

			if (i4Status != WLAN_STATUS_SUCCESS)
				break;

			kalMemCopy(rAccessEfuseInfo.aucData, prGlueInfo->prAdapter->aucEepromVaule, EFUSE_BLOCK_SIZE);
		}

		uAddrIndex = u2Addr % EFUSE_BLOCK_SIZE;
		if (rAccessEfuseInfo.aucData[uAddrIndex] != uContent) {
			DBGLOG(INIT, TRACE, "update 0X%2X form 0X%2X to 0X%2X\n", u2Addr,
			       rAccessEfuseInfo.aucData[uAddrIndex], uContent);
			rAccessEfuseInfo.aucData[uAddrIndex] = uContent;
			bNeedUpdate = TRUE;
		}
		...
		u2NextBlockIndex = (efusecontent[uLoop + 1][0] / EFUSE_BLOCK_SIZE);
		u2NextBlockIndex = u2NextBlockIndex * EFUSE_BLOCK_SIZE;
		if (u2NextBlockIndex != u2BlockIndex) {
			if (prAdapter->fgIsSupportGetFreeEfuseBlockCount) {
				kalMemSet(&rEfuseFreeBlock, 0, sizeof(PARAM_CUSTOM_EFUSE_FREE_BLOCK_T));

				i4Status = kalIoctl(prGlueInfo, wlanoidQueryEfuseFreeBlock, &rEfuseFreeBlock,
						    sizeof(PARAM_CUSTOM_EFUSE_FREE_BLOCK_T), TRUE, TRUE, TRUE,
						    &u4BufLen);

				if (i4Status != WLAN_STATUS_SUCCESS)
					break;

				if (prGlueInfo->prAdapter->u4FreeBlockNum < 2) {
					DBGLOG(INIT, ERROR, "no block\n");
					i4Status = WLAN_STATUS_FAILURE;
					break;
				}
			}
			DBGLOG(INIT, TRACE, "write block %d\n", u2BlockIndex);
			i4Status = kalIoctl(prGlueInfo, wlanoidQueryProcessAccessEfuseWrite, &rAccessEfuseInfo,
...
static INT_32 wlanProbe(struct sdio_func *pvData, PVOID pvDriverData)
{
	...
	dev_info(&pvData->dev, "Mediatek MT7668S Driver probe start\n");
	...
	do {
		...
		if (prGlueInfo->prAdapter->rWifiVar.ucThreadPriority > 0) {
			const struct sched_param param = { .sched_priority = prAdapter->rWifiVar.ucThreadPriority };
			sched_setscheduler(prGlueInfo->main_thread, prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling,
					   &param);
#if CFG_SUPPORT_MULTITHREAD
			sched_setscheduler(prGlueInfo->hif_thread, prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling,
					   &param);
			sched_setscheduler(prGlueInfo->rx_thread, prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling,
					   &param);
#endif
		...
		/* set MAC address */
		if (prAdapter) {
			kalMemCopy(prGlueInfo->prDevHandler->dev_addr, prAdapter->rWifiVar.aucMacAddress, ETH_ALEN);
#if CFG_MESON_G12A_PATCH
			prGlueInfo->prDevHandler->mtu = 1408;
#endif
			...
			DBGLOG(INIT, INFO, "MAC address: " MACSTR, MAC2STR(prAdapter->rWifiVar.aucMacAddress));
#endif
		} else {
			DBGLOG(INIT, WARN, "set MAC addr fail.\n");
			prGlueInfo->u4ReadyFlag = 0;
		}
		...
		if (prAdapter->fgIsSupportCsumOffload) {
			...
			rStatus = kalIoctl(prGlueInfo, wlanoidSetCSUMOffload, (PVOID)&u4CSUMFlags, sizeof(UINT_32),
	...
	if (i4Status != 0) {
		...
		dev_info(&pvData->dev, "Mediatek MT7668S Driver probe FAILED [%d]\n", i4Status);
...
#if CFG_SUPPORT_QA_TOOL
#if CFG_SUPPORT_BUFFER_MODE
	if (prAdapter->rWifiVar.fgEfuseCheck && prAdapter->fgIsHwACDisabled)
		i4Status = glEfuseCheck(prAdapter);
#endif
#endif
#endif

	dev_info(&pvData->dev, "Mediatek MT7668S Driver probe Success\n");
...
static int initWlan(void)
{
	...
	do {
		...
		func_ptr pFunc2 = (func_ptr)kal_kallsyms_lookup_name(func_name2);

		if (pFunc) {
			DBGLOG(INIT, ERROR, "Register Wow callback1 success\n");
			pFunc(&kalDcSetWow);
		} else if (pFunc2) {
			DBGLOG(INIT, ERROR, "Register Wow callback2 success\n");
			pFunc2(&kalDcSetWow);
		} else
			DBGLOG(INIT, ERROR, "No Exported Func Found:%s\n", func_name);
	} while (0);
#endif
...
static VOID exitWlan(void)
{
	...
	procUninitProcFs();
#endif
	yhpgiUnregisterRebootNotifier();
	DBGLOG(INIT, INFO, "exitWlan\n");

} /* end of exitWlan() */

static int yhpgi_power_notify(struct notifier_block *nb, unsigned long event, void *unused)
{
	if (event == SYS_RESTART || event == SYS_DOWN || event == SYS_POWER_OFF) {
		pr_info("[yhpgi] [PowerEvent] Cleaning Mediatek MT7668S Driver...\n");
		glUnregisterBus(wlanRemove);
		kalUninitIOBuffer();
		wlanDestroyWirelessDevice();
		glP2pDestroyWirelessDevice();
		...
		procUninitProcFs();
#endif
		pr_info("[yhpgi] [PowerEvent] Mediatek MT7668S Driver Cleaned...\n");
	}
	return 0;
}

static struct notifier_block yhpgi_power_notifier = {
	.notifier_call = yhpgi_power_notify,
	.next = NULL,
	.priority = __INT_MAX__, /* try to be the first */
};

void yhpgiRegisterRebootNotifier(void)
{
	register_reboot_notifier(&yhpgi_power_notifier);
}

void yhpgiUnregisterRebootNotifier(void)
{
	unregister_reboot_notifier(&yhpgi_power_notifier);
}
...
static int mtk_wcn_temp_query_ctrl(void)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidQueryTemperature, &ucTemperature, sizeof(UINT_8), TRUE, TRUE, TRUE,
			   &u4BufLen);

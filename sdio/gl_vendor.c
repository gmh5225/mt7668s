...
#define WIFI_HAL_FEATURE_SET ((WIFI_FEATURE_P2P) | (WIFI_FEATURE_SOFT_AP) | (WIFI_FEATURE_TDLS))
...
int mtk_cfg80211_vendor_get_supported_feature_set(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data,
						  int data_len)
{
	...
	if (unlikely(nla_put_nohdr(skb, sizeof(u4FeatureSet), &u4FeatureSet) < 0)) {
...
int mtk_cfg80211_vendor_set_tx_power_scenario(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data,
					      int data_len)
{
	return -EOPNOTSUPP;
}

int mtk_cfg80211_vendor_packet_keep_alive_start(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data,
						int data_len)
{
	...
	prPkt = (struct PARAM_PACKET_KEEPALIVE_T *)kalMemAlloc(sizeof(struct PARAM_PACKET_KEEPALIVE_T), VIR_MEM_TYPE);
	if (!prPkt) {
		DBGLOG(REQ, ERROR, "Can not alloc memory for struct PARAM_PACKET_KEEPALIVE_T\n");
		return -ENOMEM;
	}
	...
	if (NLA_PARSE_NESTED(attr, MKEEP_ALIVE_ATTRIBUTE_PERIOD_MSEC, (struct nlattr *)(data - NLA_HDRLEN),
	...
	for (i = MKEEP_ALIVE_ATTRIBUTE_ID; i <= MKEEP_ALIVE_ATTRIBUTE_PERIOD_MSEC; i++) {
		if (attr[i]) {
			...
			case MKEEP_ALIVE_ATTRIBUTE_SRC_MAC_ADDR:
				kalMemCopy(prPkt->ucSrcMacAddr, nla_data(attr[i]), sizeof(uint8_t) * 6);
				break;
			case MKEEP_ALIVE_ATTRIBUTE_DST_MAC_ADDR:
				kalMemCopy(prPkt->ucDstMacAddr, nla_data(attr[i]), sizeof(uint8_t) * 6);
				break;
	...
	DBGLOG(REQ, TRACE, "prPkt->pIpPkt=0x%02x%02x%02x%02x\n", prPkt->pIpPkt[0], prPkt->pIpPkt[1], prPkt->pIpPkt[2],
	       prPkt->pIpPkt[3]);
	DBGLOG(REQ, TRACE, "%02x%02x%02x%02x, %02x%02x%02x%02x\n", prPkt->pIpPkt[4], prPkt->pIpPkt[5], prPkt->pIpPkt[6],
	       prPkt->pIpPkt[7], prPkt->pIpPkt[8], prPkt->pIpPkt[9], prPkt->pIpPkt[10], prPkt->pIpPkt[11]);
	DBGLOG(REQ, TRACE, "%02x%02x%02x%02x\n", prPkt->pIpPkt[12], prPkt->pIpPkt[13], prPkt->pIpPkt[14],
	       prPkt->pIpPkt[15]);
	DBGLOG(REQ, TRACE, "prPkt->srcMAC=%02x:%02x:%02x:%02x:%02x:%02x\n", prPkt->ucSrcMacAddr[0],
	       prPkt->ucSrcMacAddr[1], prPkt->ucSrcMacAddr[2], prPkt->ucSrcMacAddr[3], prPkt->ucSrcMacAddr[4],
	       prPkt->ucSrcMacAddr[5]);
	DBGLOG(REQ, TRACE, "dstMAC=%02x:%02x:%02x:%02x:%02x:%02x\n", prPkt->ucDstMacAddr[0], prPkt->ucDstMacAddr[1],
	       prPkt->ucDstMacAddr[2], prPkt->ucDstMacAddr[3], prPkt->ucDstMacAddr[4], prPkt->ucDstMacAddr[5]);

	prGlueInfo = (P_GLUE_INFO_T)wiphy_priv(wiphy);
	ASSERT(prGlueInfo);

	rStatus = kalIoctl(prGlueInfo, wlanoidPacketKeepAlive, prPkt, sizeof(struct PARAM_PACKET_KEEPALIVE_T), FALSE,
			   FALSE, TRUE, &u4BufLen);
	kalMemFree(prPkt, VIR_MEM_TYPE, sizeof(struct PARAM_PACKET_KEEPALIVE_T));
	return rStatus;
...
int mtk_cfg80211_vendor_packet_keep_alive_stop(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data,
					       int data_len)
{
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	uint32_t u4BufLen = 0;
...
int mtk_cfg80211_vendor_get_version(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
#define STR_HELPER(x) #x
#define STRR(x) STR_HELPER(x)
#define DriverVersionStr \
	STRR(NIC_DRIVER_MAJOR_VERSION) \
	"_" STRR(NIC_DRIVER_MINOR_VERSION) "_" STRR(NIC_DRIVER_SERIAL_VERSION) "-20240626"
	...
	if (attrlist->nla_type == LOGGER_ATTRIBUTE_DRIVER_VER) {
		...
		snprintf(aucVersionBuf, 256, "%s-%u.%u.%u[DEC] (%s)", aucBuf, (prVerInfo->u2FwOwnVersion >> 8),
			 (prVerInfo->u2FwOwnVersion & 0xff), prVerInfo->ucFwBuildNumber, aucDate);
...
int mtk_cfg80211_vendor_set_scan_mac_oui(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	ASSERT(wiphy);
	ASSERT(wdev);
...
int mtk_cfg80211_vendor_get_channel_list(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	if (band == 0) { /* 2.4G band */
		rlmDomainGetChnlList(prGlueInfo->prAdapter, BAND_2G4, TRUE, MAX_CHN_NUM, &ucNumOfChannel,
				     aucChannelList);
	} else { /* 5G band */
		rlmDomainGetChnlList(prGlueInfo->prAdapter, BAND_5G, TRUE, MAX_CHN_NUM, &ucNumOfChannel,
				     aucChannelList);
	}

	kalMemZero(channels, sizeof(channels));
	u2CountryCode = prGlueInfo->prAdapter->rWifiVar.rConnSettings.u2CountryCode;
	for (i = 0, j = 0; i < ucNumOfChannel; i++) {
		...
		if (channels[j] == 0)
			continue;
		else if ((u2CountryCode == COUNTRY_CODE_TW) && (channels[j] >= 5180 && channels[j] <= 5260)) {
	...
	if (unlikely(nla_put(skb, WIFI_ATTRIBUTE_CHANNEL_LIST, (sizeof(wifi_channel) * num_channels), channels) < 0))
		goto nla_put_failure;
...
int mtk_cfg80211_vendor_get_roaming_capabilities(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data,
						 int data_len)
{
	...
	if (unlikely(nla_put(skb, WIFI_ATTRIBUTE_ROAMING_CAPABILITIES, sizeof(maxNumOfList), maxNumOfList) < 0))
		goto nla_put_failure;
...
int mtk_cfg80211_vendor_config_roaming(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
...
int mtk_cfg80211_vendor_enable_roaming(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	UINT_32 u4ResultLen = 0;
	struct nlattr *attr;
...
int mtk_cfg80211_vendor_event_wowlan_magic_pkt(struct wiphy *wiphy, struct wireless_dev *wdev, UINT_32 num)
{
	struct sk_buff *skb;
	...
	skb = cfg80211_vendor_event_alloc(wiphy, wdev, sizeof(num), WIFI_EVENT_MAGIC_PACKET_RECEIVED, GFP_KERNEL);
#else
	skb = cfg80211_vendor_event_alloc(wiphy, sizeof(num), WIFI_EVENT_MAGIC_PACKET_RECEIVED, GFP_KERNEL);
	...
	/*NLA_PUT_U32(skb, WIFI_EVENT_MAGIC_PACKET_RECEIVED, num);*/
	{
		...
		if (unlikely(nla_put(skb, WIFI_EVENT_MAGIC_PACKET_RECEIVED, sizeof(unsigned int), &__tmp) < 0))
			goto nla_put_failure;
	}

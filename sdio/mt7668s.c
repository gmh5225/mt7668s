...
#define WIFI_INTERRUPT_NAME "mt7668s-wifi-eint"
#endif
...
static MTK_WCN_HIF_SDIO_FUNCINFO funcInfo[] = {
	{ MTK_WCN_HIF_SDIO_FUNC(0x037a, 0x7608, 0x1, 512) },
};

static MTK_WCN_SDIO_DRIVER_DATA_MAPPING sdio_driver_data_mapping[] = {
	{ 0x7608, &driver_data_mt7668 },
};
...
VOID mt7668ConstructFirmwarePrio(P_GLUE_INFO_T prGlueInfo, PPUINT_8 apucNameTable, PPUINT_8 apucName,
				 PUINT_8 pucNameIdx, UINT_8 ucMaxNameIdx)
{
	...
	for (sub_idx = 0; apucNameTable[sub_idx]; sub_idx++) {
		if (((*pucNameIdx) + 3) < ucMaxNameIdx) {
			...
			snprintf(*(apucName + (*pucNameIdx)), CFG_FW_NAME_MAX_LEN, "%s%x.bin", apucNameTable[sub_idx],
				 chip_id);
			(*pucNameIdx) += 1;

			/* Type 2. WIFI_RAM_CODE_MTxxxx */
			snprintf(*(apucName + (*pucNameIdx)), CFG_FW_NAME_MAX_LEN, "%s%x", apucNameTable[sub_idx],
				 chip_id);
			(*pucNameIdx) += 1;

			/* Type 3. WIFI_RAM_CODE_MTxxxx_Ex.bin */
			snprintf(*(apucName + (*pucNameIdx)), CFG_FW_NAME_MAX_LEN, "%s%x_E%u.bin",
				 apucNameTable[sub_idx], chip_id, wlanGetEcoVersion(prGlueInfo->prAdapter));
			(*pucNameIdx) += 1;

			/* Type 4. WIFI_RAM_CODE_MTxxxx_Ex */
			snprintf(*(apucName + (*pucNameIdx)), CFG_FW_NAME_MAX_LEN, "%s%x_E%u", apucNameTable[sub_idx],
...
static void wlan_register_irq(struct WOWLAN_DEV_NODE *node)
{
	...
	eint_node = of_find_compatible_node(NULL, NULL, WIFI_COMPATIBLE_NODE_NAME);
	if (eint_node) {
		node->wowlan_irq = irq_of_parse_and_map(eint_node, 0);
		DBGLOG(INIT, INFO, "%s, WOWLAN irq_number = %d\n", __func__, node->wowlan_irq);
		if (node->wowlan_irq) {
			of_property_read_u32_array(eint_node, "interrupts", interrupts, ARRAY_SIZE(interrupts));
			node->wowlan_irqlevel = interrupts[1];
			if (request_irq(node->wowlan_irq, wifi_wow_isr, node->wowlan_irqlevel, WIFI_INTERRUPT_NAME,
					node)) {
				DBGLOG(INIT, ERROR, "%s, WOWLAN irq NOT AVAILABLE!\n", __func__);
			} else {
				disable_irq_nosync(node->wowlan_irq);
			}
		} else {
			DBGLOG(INIT, ERROR, "%s, can't find wifi_ctrl irq\n", __func__);
		}

	} else {
		node->wowlan_irq = 0;
		DBGLOG(INIT, ERROR, "%s, can't find wifi_ctrl compatible node\n", __func__);
	}
}
...
static void mtk_sdio_eint_free_irq(struct sdio_func *func)
{
	...
	if (u4Irq) {
		...
		free_irq(u4Irq, &prGlueInfo->prAdapter->rWowlanDevNode);
	}
}
...
INT_32 mtk_sdio_probe(MTK_WCN_HIF_SDIO_CLTCTX cltCtx, const MTK_WCN_HIF_SDIO_FUNCINFO *prFuncInfo)
{
	...
	struct hif_driver_data *sdio_driver_data = NULL;

	prFunc = prFuncInfo;

	for (i = 0; i < dd_table_len; i++) {
		if (prFunc->card_id == sdio_driver_data_mapping[i].card_id) {
			sdio_driver_data = sdio_driver_data_mapping[i].driver_data;
			break;
		}
	}
...
static int mtk_sdio_pm_suspend(struct device *pDev)
{
	...
	if (prAdapter->rWowlanDevNode.wowlan_irq != 0 &&
	    atomic_read(&(prAdapter->rWowlanDevNode.irq_enable_count)) == 0) {
		DBGLOG(HAL, ERROR, "%s:enable WIFI IRQ:%d\n", __func__, prAdapter->rWowlanDevNode.wowlan_irq);
		...
		DBGLOG(HAL, ERROR, "%s:irq_enable count:%d\n", __func__,
		       atomic_read(&(prAdapter->rWowlanDevNode.irq_enable_count)));
	}
#endif
...
static int mtk_sdio_pm_resume(struct device *pDev)
{
	...
	prAdapter = prGlueInfo->prAdapter;
	if (prAdapter->rWowlanDevNode.wowlan_irq != 0 &&
	    atomic_read(&(prAdapter->rWowlanDevNode.irq_enable_count)) == 1) {
...
WLAN_STATUS glRegisterBus(probe_card pfProbe, remove_card pfRemove)
{
	...
	ret = ((mtk_wcn_hif_sdio_client_reg(&cltInfo) == HIF_SDIO_ERR_SUCCESS) ? WLAN_STATUS_SUCCESS :
										 WLAN_STATUS_FAILURE);
#else
...
BOOL kalDevRegRead_mac(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4Register, OUT PUINT_32 pu4Value)
{
	...
	do {
		...
		if (((u4Current > u4Time) && ((u4Current - u4Time) > HIF_SDIO_INTERRUPT_RESPONSE_TIMEOUT)) ||
		    (u4Current < u4Time &&
		     ((u4Current + (0xFFFFFFFF - u4Time)) > HIF_SDIO_INTERRUPT_RESPONSE_TIMEOUT))) {
		     ...
		     IN UINT_32 u4ValidInBufSize)
{
	...
	if (ret) {
		...
		P_ADAPTER_T prAdapter = NULL;
#endif
		kalSendAeeWarning(HIF_SDIO_ERR_TITLE_STR, HIF_SDIO_ERR_DESC_STR "sdio_writesb() reports error: %x",
				  ret);
...
BOOL kalDevWriteWithSdioCmd52(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4Addr, IN UINT_8 ucData)
{
	...
	if (ret) {
#if CFG_CHIP_RESET_SUPPORT
		P_ADAPTER_T prAdapter = container_of(&prGlueInfo, ADAPTER_T, prGlueInfo);
#endif

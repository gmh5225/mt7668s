 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 ResponseToQA(HQA_CMD_FRAME *HqaCmdFrame, IN union iwreq_data *prIwReqData, INT_32 i4Length,
			   INT_32 i4Status)
{
	...
	if (copy_to_user(prIwReqData->data.pointer, (UCHAR *)(HqaCmdFrame), prIwReqData->data.length)) {
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT copy_to_user() fail in %s\n", __func__);
		return -EFAULT;
	}
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA command(0x%04x)[Magic number(0x%08x)] is done\n",
...
static INT_32 ToDoFunction(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT ToDoFunction\n");
...
static INT_32 HQA_OpenAdapter(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_OpenAdapter\n");
	prGlueInfo = *((P_GLUE_INFO_T *)netdev_priv(prNetDev));

	/*
	 * g_ucEepromCurrentMode: For EEPROM content field of QA Tool
	 * 0 -> bin Mode
	 * 2 -> efuse Mode
	 */

	/*
	 * u4EepromMode: For EEPROM Type field of QA Tool
	 * 1 -> efuse Mode
	 * 2 -> flash Mode
	 * 3 -> eeprom Mode
	 * 4 -> bin Mode
	...
	/*
	 * Synchronize EEPROM mode related parameters from
	 * ucEfuseBufferModeCal
	 */
	if (prGlueInfo->prAdapter->rWifiVar.ucEfuseBufferModeCal == LOAD_EFUSE) {
		g_ucEepromCurrentMode = EFUSE_MODE;
		u4EepromMode = 1;
		DBGLOG(RFTEST, INFO, "QA_AGENT HQA_OpenAdapter EEPROM = eFuse Mode\n");
	} else if (prGlueInfo->prAdapter->rWifiVar.ucEfuseBufferModeCal == LOAD_EEPROM_BIN) {
		g_ucEepromCurrentMode = BUFFER_BIN_MODE;
		u4EepromMode = 4;
		DBGLOG(RFTEST, INFO, "QA_AGENT HQA_OpenAdapter EEPROM = Bin Mode\n");
	}
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_CloseAdapter(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CloseAdapter\n");
...
static INT_32 HQA_StartTx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartTx\n");
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartTx TxCount = %d\n", TxCount);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartTx TxLength = %d\n", TxLength);
...
static INT_32 HQA_StartTxExt(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartTxExt\n");
...
/*----------------------------------------------------------------------------*/
/* 1 todo not support yet */
static INT_32 HQA_StartTxContiTx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartTxContiTx\n");
...
static INT_32 HQA_StartTxCarrier(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartTxCarrier\n");
...
static INT_32 HQA_StartRx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartRx\n");
...
static INT_32 HQA_StopTx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StopTx\n");
...
static INT_32 HQA_StopContiTx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StopContiTx\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_StopTxCarrier(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StopTxCarrier\n");
...
static INT_32 HQA_StopRx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StopRx\n");
...
static INT_32 HQA_SetTxPath(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxPath\n");
...
static INT_32 HQA_SetRxPath(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRxPath\n");
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRxPath Value : %d\n", Value);
...
static INT_32 HQA_SetTxIPG(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxIPG\n");

	memcpy(&u4Aifs, HqaCmdFrame->Data + 4 * 0, 4);
	u4Aifs = ntohs(u4Aifs);

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxIPG u4Aifs : %d\n", u4Aifs);
...
static INT_32 HQA_SetTxPower0(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxPower0\n");
...
static INT_32 HAQ_SetTxPower1(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HAQ_SetTxPower1\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetTxPowerExt(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Power = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxPowerExt u4Power : %d\n", u4Power);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxPowerExt u4Dbdc_idx : %d\n", u4Dbdc_idx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxPowerExt u4Channel : %d\n", u4Channel);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxPowerExt u4Band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxPowerExt u4Ant_idx : %d\n", u4Ant_idx);
...
static INT_32 HQA_SetOnOff(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetOnOff\n");
...
static INT_32 HQA_AntennaSel(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_AntennaSel\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_FWPacketCMD_ClockSwitchDisable(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
						 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_FWPacketCMD_ClockSwitchDisable\n");
...
static INT_32 HQA_SetChannel(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetChannel\n");

	memcpy((PUCHAR)&i4SetChan, HqaCmdFrame->Data, 4);
	i4SetChan = ntohl(i4SetChan);

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetChannel Channel = %d\n", i4SetChan);
...
static INT_32 HQA_SetPreamble(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetPreamble\n");

	memcpy((PUCHAR)&i4Mode, HqaCmdFrame->Data, 4);
	i4Mode = ntohl(i4Mode);

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetPreamble Mode = %d\n", i4Mode);
...
static INT_32 HQA_SetRate(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRate\n");
...
static INT_32 HQA_SetNss(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetNss\n");
...
static INT_32 HQA_SetSystemBW(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetSystemBW\n");

	memcpy((PUCHAR)&i4BW, HqaCmdFrame->Data, 4);
	i4BW = ntohl(i4BW);

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetSystemBW BW = %d\n", i4BW);
...
static INT_32 HQA_SetPerPktBW(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetPerPktBW\n");

	memcpy((PUCHAR)&u4Perpkt_bw, HqaCmdFrame->Data, 4);
	u4Perpkt_bw = ntohl(u4Perpkt_bw);

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetPerPktBW u4Perpkt_bw = %d\n", u4Perpkt_bw);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetPrimaryBW(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Pri_sel = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetPrimaryBW u4Pri_sel : %d\n", u4Pri_sel);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetFreqOffset(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetFreqOffset u4FreqOffset : %d\n", u4FreqOffset);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetAutoResponder(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetAutoResponder\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetTssiOnOff(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTssiOnOff\n");
...
static INT_32 HQA_SetRxHighLowTemperatureCompensation(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
						      HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRxHighLowTemperatureCompensation\n");
...
static INT_32 HQA_LowPower(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_LowPower\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_ResetTxRxCounter(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	UINT_32 i4Status;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_ResetTxRxCounter\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetStatistics(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetStatistics\n");
...
static INT_32 HQA_GetRxOKData(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetRxOKData\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetRxOKOther(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetRxOKOther\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetRxAllPktCount(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetRxAllPktCount\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetTxTransmitted(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetTxTransmitted\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetHwCounter(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetHwCounter\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_CalibrationOperation(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CalibrationOperation\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_CalibrationBypassExt(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Status = 0;
	UINT_32 u4Item = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CalibrationBypassExt u4Item : 0x%08x\n", u4Item);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CalibrationBypassExt u4Band_idx : %d\n", u4Band_idx);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetRXVectorIdx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 band_idx = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRXVectorIdx band_idx : %d\n", band_idx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRXVectorIdx Group_1 : %d\n", Group_1);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRXVectorIdx Group_2 : %d\n", Group_2);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRXVectorIdx rRfATInfo.u4FuncData : 0x%08x\n",
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetFAGCRssiPath(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4band_idx = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetFAGCRssiPath u4band_idx : %d\n", u4band_idx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetFAGCRssiPath u4FAGC_Path : %d\n", u4FAGC_Path);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_MacBbpRegRead(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MacBbpRegRead\n");

	memcpy(&u4Offset, HqaCmdFrame->Data, 4);
	u4Offset = ntohl(u4Offset);

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MacBbpRegRead Offset = 0x%08x\n", u4Offset);
	...
	if (i4Status == 0) {
		...
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT Address = 0x%08x, Result = 0x%08x\n", u4Offset, u4Value);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_MacBbpRegWrite(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	/*	INT_32 i4Ret = 0; */
	UINT_32 u4Offset, u4Value;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MacBbpRegWrite\n");
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MacBbpRegWrite Offset = 0x%08x\n", u4Offset);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MacBbpRegWrite Value = 0x%08x\n", u4Value);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_MACBbpRegBulkRead(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MACBbpRegBulkRead\n");
	...
	for (u4Index = 0; u4Index < u2Len; u4Index++) {
		...
		i4Status = kalIoctl(prGlueInfo, wlanoidQueryMcrRead, &rMcrInfo, sizeof(rMcrInfo), TRUE, TRUE, TRUE,
				    &u4BufLen);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_RfRegBulkRead(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkRead\n");
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkRead WfSel  = %d\n", u4WfSel);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkRead Offset = 0x%08x\n", u4Offset);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkRead Length = %d\n", u4Length);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_RfRegBulkWrite(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkWrite\n");
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkWrite WfSel  = %d\n", u4WfSel);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkWrite Offset = 0x%08x\n", u4Offset);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkWrite Length = %d\n", u4Length);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RfRegBulkWrite Value  = 0x%08x\n", u4Value);
...
static INT_32 HQA_ReadEEPROM(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadEEPROM\n");
	...
	if (rStatus == WLAN_STATUS_SUCCESS) {
		DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadEEPROM u4Value = %x\n", u4Value);
...
static INT_32 HQA_WriteEEPROM(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	/* Write */
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_WriteEEPROM\n");
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucEfuseBufferModeCal == LOAD_EEPROM_BIN) {
		...
		if (u4Index >= EFUSE_BLOCK_SIZE - 1) {
			DBGLOG(INIT, ERROR, "MT7668S : efuse Offset error\n");
			return -EINVAL;
		}
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_ReadBulkEEPROM(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_16 Offset;
	...
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadBulkEEPROM\n");
	...
	Len = ntohs(Len);
	tmp = Offset;
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadBulkEEPROM Offset : %d\n", Offset);
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadBulkEEPROM Len : %d\n", Len);

#if (CFG_EEPROM_PAGE_ACCESS == 1)
	rAccessEfuseInfo.u4Address = (Offset / EFUSE_BLOCK_SIZE) * EFUSE_BLOCK_SIZE;

	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadBulkEEPROM Address : %d\n", rAccessEfuseInfo.u4Address);

	if ((g_ucEepromCurrentMode == EFUSE_MODE) && (prGlueInfo->prAdapter->fgIsSupportQAAccessEfuse == TRUE)) {
		/* Read from Efuse */
		DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadBulkEEPROM Efuse Mode\n");
		rStatus = kalIoctl(prGlueInfo, wlanoidQueryProcessAccessEfuseRead, &rAccessEfuseInfo,
				   sizeof(PARAM_CUSTOM_ACCESS_EFUSE_T), TRUE, TRUE, TRUE, &u4BufLen);

		if (rStatus == WLAN_STATUS_FAILURE)
			DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadBulkEEPROM kal fail\n");

		Offset = Offset % EFUSE_BLOCK_SIZE;

#if 0
		for (u4Loop = 0; u4Loop < 16; u4Loop++) {
			DBGLOG(INIT, INFO, "MT7668:QA_AGENT HQA_ReadBulkEEPROM Efuse Offset=%x u4Loop=%d u4Value=%x\n",
		...
		for (u4Loop = 0; u4Loop < Len; u4Loop += 2) {
			...
			Buffer = ntohs(Buffer);
			DBGLOG(INIT, INFO, "MT7668S :From Efuse  u4Loop=%d  Buffer=%x\n", u4Loop, Buffer);
		...
		for (u4Loop = 0; u4Loop < Len; u4Loop += 2) {
			...
			Buffer = ntohs(Buffer);
			memcpy(HqaCmdFrame->Data + 2 + u4Loop, &Buffer, 2);
			DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_ReadBulkEEPROM u4Loop=%d  u4Value=%x\n", u4Loop,
			       uacEEPROMImage[Offset + u4Loop]);
		}
	}
#endif
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_WriteBulkEEPROM(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_16 Offset;
	...
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_WriteBulkEEPROM\n");
	...
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_WriteBulkEEPROM Offset : %x\n", Offset);
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_WriteBulkEEPROM Len : %d\n", Len);
	...
	if (prGlueInfo->prAdapter->fgIsSupportQAAccessEfuse == TRUE) {
		...
		for (u4Loop = 0; u4Loop < (Len)/2; u4Loop++) {
			...
			DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_WriteBulkEEPROM u4Loop=%d  u4Value=%x\n",
		...
		if (g_ucEepromCurrentMode == BUFFER_BIN_MODE) {
			...
			/* EFUSE */
			/* Read */
			DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_WriteBulkEEPROM  Read\n");
			kalMemSet(&rAccessEfuseInfoRead, 0, sizeof(PARAM_CUSTOM_ACCESS_EFUSE_T));
			rAccessEfuseInfoRead.u4Address = (Offset / EFUSE_BLOCK_SIZE) * EFUSE_BLOCK_SIZE;
			...
			if (Len > 2) {
				...
				u4Index = Offset % EFUSE_BLOCK_SIZE;
				DBGLOG(INIT, INFO, "MT7668:QA_AGENT HQA_WriteBulkEEPROM Wr,u4Index=%x,Buffer=%x\n",
				       u4Index, testBuffer);
				...
				*Buffer = ntohs(*Buffer);
				DBGLOG(INIT, INFO, "MT7668S : Buffer[0]=%x, Buffer[0]&0xff=%x\n", Buffer[0],
				       Buffer[0] & 0xff);
				DBGLOG(INIT, INFO, "MT7668S : Buffer[0] >> 8 & 0xff=%x\n", Buffer[0] >> 8 & 0xff);
			...
			for (u4Loop = 0; u4Loop < (EFUSE_BLOCK_SIZE); u4Loop++) {
				DBGLOG(INIT, INFO, "MT7668S :  Loop=%d  aucData=%x\n", u4Loop,
		...
		if (Len == 2) {
			...
			if (!g_BufferDownload) {
				...
				rStatus = kalIoctl(prGlueInfo, wlanoidSetEfusBufferMode, (PVOID)prSetEfuseBufModeInfo,
						   sizeof(PARAM_CUSTOM_EFUSE_BUFFER_MODE_T), FALSE, FALSE, TRUE,
						   &u4BufLen);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_CheckEfuseMode(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 Value = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CheckEfuseMode\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetFreeEfuseBlock(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT HQA_GetFreeEfuseBlock\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetEfuseBlockNr(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetEfuseBlockNr\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_WriteEFuseFromBuffer(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_WriteEFuseFromBuffer\n");
...
static INT_32 HQA_GetTxPower(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetTxPower u4Channel : %d\n", u4Channel);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetTxPower u4Band : %d\n", u4Band);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetTxPower u4Ch_Band : %d\n", u4Ch_Band);
...
static INT_32 HQA_SetCfgOnOff(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetCfgOnOff Type : %d\n", Type);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetCfgOnOff Enable : %d\n", Enable);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetCfgOnOff Band : %d\n", Band);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetFreqOffset(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4FreqOffset = 0;
	...
	i4Ret = kalIoctl(prGlueInfo, wlanoidRftestQueryAutoTest, &rRfATInfo, sizeof(rRfATInfo), TRUE, TRUE, TRUE,
			 &u4BufLen);

	if (i4Ret == 0) {
		u4FreqOffset = rRfATInfo.u4FuncData;

		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetFreqOffset u4FreqOffset = %d\n", u4FreqOffset);
...
static INT_32 HQA_DBDCTXTone(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone BandIdx = 0x%08x\n", i4BandIdx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone Control = 0x%08x\n", i4Control);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone AntIndex = 0x%08x\n", i4AntIndex);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone ToneType = 0x%08x\n", i4ToneType);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone ToneFreq = 0x%08x\n", i4ToneFreq);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone DcOffsetI = 0x%08x\n", i4DcOffsetI);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone DcOffsetQ = 0x%08x\n", i4DcOffsetQ);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone Band = 0x%08x\n", i4Band);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone RF_Power = 0x%08x\n", i4RF_Power);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCTXTone Digi_Power = 0x%08x\n", i4Digi_Power);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_DBDCContinuousTX(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4Band : %d\n", u4Band);		   /* ok */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4Control : %d\n", u4Control);	   /* ok */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4AntMask : %d\n", u4AntMask);	   /* ok */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4Phymode : %d\n", u4Phymode);	   /* ok */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4BW : %d\n", u4BW);		   /* ok */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4Pri_Ch : %d\n", u4Pri_Ch);	   /* ok */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4Rate : %d\n", u4Rate);		   /* ok */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4Central_Ch : %d\n", u4Central_Ch); /* ok */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DBDCContinuousTX u4TxfdMode : %d\n", u4TxfdMode);	   /* ok */
	...
	if (u4Control) {
		...
		if (u4Phymode == 0) {
			...
			DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT CCK/OFDM (normal preamble) rate : %d\n", u4Rate);
			...
			DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT CCK (short preamble) rate : %d\n", u4Rate);

			MT_ATESetRate(prNetDev, u4Rate);
		} else if (u4Phymode >= 2 && u4Phymode <= 4) {
			u4Rate |= 0x80000000;

			DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HT/VHT rate : %d\n", u4Rate);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetRXFilterPktLen(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRXFilterPktLen Band : %d\n", u4Band);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRXFilterPktLen Control : %d\n", u4Control);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRXFilterPktLen RxPktlen : %d\n", u4RxPktlen);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetRXFilterPktLen rRfATInfo.u4FuncData : 0x%08x\n",
	       rRfATInfo.u4FuncData);
...
static INT_32 HQA_GetTXInfo(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetTXInfo\n");

	rRfATInfo.u4FuncIndex = RF_AT_FUNCID_TXED_COUNT;
	rRfATInfo.u4FuncData = 0;

	i4Status = kalIoctl(prGlueInfo, wlanoidRftestQueryAutoTest, &rRfATInfo, sizeof(rRfATInfo), TRUE, TRUE, TRUE,
			    &u4BufLen);

	if (i4Status == 0) {
		u4Txed_band0 = rRfATInfo.u4FuncData;

		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT u4Txed_band0 packet count = %d\n", u4Txed_band0);
	...
	if (i4Status == 0) {
		...
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT u4Txed_band1 packet count = %d\n", u4Txed_band1);
...
static INT_32 HQA_GetCfgOnOff(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetCfgOnOff\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetBufferBin(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 Ret = 0;
	UINT_32 data = 0;
	...
	data = ntohl(data);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetBufferBin data=%x\n", data);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : ucEepromCurrentMode=%x\n", g_ucEepromCurrentMode);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_ReadTempReferenceValue(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_ReadTempReferenceValue\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetThermalValue(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	UINT_32 u4Value;
	UINT_32 u4BufLen = 0;
	...
	if (i4Status == 0) {
		...
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetThermalValue Value = %d\n", u4Value);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetSideBandOption(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetSideBandOption\n");
...
static INT_32 HQA_GetFWInfo(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetFWInfo\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_StartContinousTx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartContinousTx\n");
...
static INT_32 HQA_SetSTBC(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetSTBC\n");
...
static INT_32 HQA_SetShortGI(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetShortGI u4ShortGi = %d\n", u4ShortGi);
...
static INT_32 HQA_SetDPD(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetDPD\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_GetRxStatisticsAll(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				     HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	P_GLUE_INFO_T prGlueInfo = NULL;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetRxStatisticsAll\n");

	rRxStatisticsTest.u4SeqNum = u4RxStatSeqNum;
	rRxStatisticsTest.u4TotalNum = HQA_RX_STATISTIC_NUM + 6;

	i4Ret = kalIoctl(prGlueInfo, wlanoidQueryRxStatistics, &rRxStatisticsTest, sizeof(rRxStatisticsTest), TRUE,
			 TRUE, TRUE, &u4BufLen);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_StartContiTxTone(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StartContiTxTone\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_StopContiTxTone(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StopContiTxTone\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_CalibrationTestMode(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				      HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Mode = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CalibrationTestMode\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_DoCalibrationTestItem(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Status = 0;
	UINT_32 u4Item = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DoCalibrationTestItem item : 0x%08x\n", u4Item);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DoCalibrationTestItem band_idx : %d\n", u4Band_idx);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_eFusePhysicalWrite(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				     HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_eFusePhysicalWrite\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_eFusePhysicalRead(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_eFusePhysicalRead\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_eFuseLogicalRead(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_eFuseLogicalRead\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_eFuseLogicalWrite(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_eFuseLogicalWrite\n");
...
static INT_32 HQA_TMRSetting(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TMRSetting u4Setting : %d\n", u4Setting);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TMRSetting u4Version : %d\n", u4Version);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TMRSetting u4MPThres : %d\n", u4MPThres);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TMRSetting u4MPIter : %d\n", u4MPIter);
...
static INT_32 HQA_GetRxSNR(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetRxSNR\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_WriteBufferDone(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_16 u2InitAddr;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_WriteBufferDone Value : %d\n", Value);
...
static INT_32 HQA_FFT(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_FFT\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_SetTxTonePower(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetTxTonePower\n");
...
static INT_32 HQA_GetChipID(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetChipID\n");
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetChipID ChipId = 0x%08x\n", u4ChipId);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_MPSSetSeqData(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetSeqData u4Len : %d\n", u4Len);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetSeqData u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0; i < u4Len; i++) {
		...
		DBGLOG(RFTEST, INFO,
		       "MT7668S : QA_AGENT HQA_MPSSetSeqData mps_setting Case %d (Mode : %d / TX Path : %d / MCS : %d)\n",
		...
		DBGLOG(RFTEST, INFO,
		       "MT7668S : QA_AGENT HQA_MPSSetSeqData mps_setting Case %d (Mode : %d / TX Path : %d / MCS : %d)\n",
		       i, (int)((mps_setting[i] & BITS(24, 27)) >> 24), (int)((mps_setting[i] & BITS(8, 23)) >> 8),
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_MPSSetPayloadLength(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				      HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 *mps_setting = NULL;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPayloadLength u4Len : %d\n", u4Len);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPayloadLength u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0; i < u4Len; i++) {
		...
		DBGLOG(RFTEST, INFO,
		       "MT7668S : QA_AGENT HQA_MPSSetPayloadLength mps_setting Case %d (Payload Length : %d)\n", i,
		       mps_setting[i]);
	}
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_MPSSetPacketCount(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 *mps_setting = NULL;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPacketCount u4Len : %d\n", u4Len);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPacketCount u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0; i < u4Len; i++) {
		...
		DBGLOG(RFTEST, INFO,
		       "MT7668S : QA_AGENT HQA_MPSSetPacketCount mps_setting Case %d (Packet Count : %d)\n", i,
		       mps_setting[i]);
	}
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_MPSSetPowerGain(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 *mps_setting = NULL;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPowerGain u4Len : %d\n", u4Len);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPowerGain u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0; i < u4Len; i++) {
		...
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPowerGain mps_setting Case %d (Power : %d)\n", i,
		       mps_setting[i]);
	}
...
static INT_32 HQA_MPSStart(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSStart\n");
...
static INT_32 HQA_MPSStop(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSStop\n");
...
static INT_32 HQA_MPSSetNss(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetNss u4Len : %d\n", u4Len);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetNss u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0; i < u4Len; i++) {
		...
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetNss mps_setting Case %d (Nss : %d)\n", i,
		       mps_setting[i]);
	}
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_MPSSetPerpacketBW(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 *mps_setting = NULL;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPerpacketBW u4Len : %d\n", u4Len);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPerpacketBW u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0; i < u4Len; i++) {
		...
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MPSSetPerpacketBW mps_setting Case %d (BW : %d)\n", i,
		       mps_setting[i]);
	}
...
static INT_32 HQA_SetAIFS(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetAIFS SlotTime = %d\n", SlotTime);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetAIFS SifsTime = %d\n", SifsTime);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_CheckEfuseModeType(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				     HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 Value = u4EepromMode;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CheckEfuseModeType\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_CheckEfuseNativeModeType(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CheckEfuseNativeModeType\n");
...
static INT_32 HQA_SetBandMode(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetBandMode u4Band_mode : %d\n", u4Band_mode);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_SetBandMode u4Band_type : %d\n", u4Band_type);
...
static INT_32 HQA_GetBandMode(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetBandMode u4Band_idx : %d\n", u4Band_idx);
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_GetBandMode g_DBDCEnable = %d\n", g_DBDCEnable);
...
static INT_32 HQA_RDDStartExt(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RDDStartExt\n");
...
static INT_32 HQA_RDDStopExt(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_RDDStopExt\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_BssInfoUpdate(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_BssInfoUpdate OwnMacIdx : %d\n", OwnMacIdx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_BssInfoUpdate BssIdx : %d\n", BssIdx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_BssInfoUpdate addr1:%02x:%02x:%02x:%02x:%02x:%02x\n", ucAddr1[0],
	...
	DBGLOG(RFTEST, ERROR, "MT7668S prInBuf = %s\n", prInBuf);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_DevInfoUpdate(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DevInfoUpdate Band : %d\n", Band);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DevInfoUpdate OwnMacIdx : %d\n", OwnMacIdx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_DevInfoUpdate addr1:%02x:%02x:%02x:%02x:%02x:%02x\n", ucAddr1[0],
	       ucAddr1[1], ucAddr1[2], ucAddr1[3], ucAddr1[4], ucAddr1[5]);
...
static INT_32 HQA_LogOnOff(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_LogOnOff band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_LogOnOff log_type : %d\n", u4Log_type);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_LogOnOff log_ctrl : %d\n", u4Log_ctrl);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_LogOnOff log_size : %d\n", u4Log_size);
...
static INT_32 HQA_TxBfProfileTagInValid(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 invalid = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagInValid\n");
...
static INT_32 HQA_TxBfProfileTagPfmuIdx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 pfmuidx = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagPfmuIdx\n");
...
static INT_32 HQA_TxBfProfileTagBfType(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 bftype = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagBfType\n");
...
static INT_32 HQA_TxBfProfileTagBw(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 tag_bw = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagBw\n");
...
static INT_32 HQA_TxBfProfileTagSuMu(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				     HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 su_mu = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagSuMu\n");
...
static INT_32 HQA_TxBfProfileTagMemAlloc(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagMemAlloc\n");
...
static INT_32 HQA_TxBfProfileTagMatrix(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagMatrix\n");
...
static INT_32 HQA_TxBfProfileTagSnr(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagSnr\n");
...
static INT_32 HQA_TxBfProfileTagSmtAnt(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 smt_ant = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagSmtAnt\n");
...
static INT_32 HQA_TxBfProfileTagSeIdx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				      HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 se_idx = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagSeIdx\n");
...
static INT_32 HQA_TxBfProfileTagRmsdThrd(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 rmsd_thrd = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagRmsdThrd\n");
...
static INT_32 HQA_TxBfProfileTagMcsThrd(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagMcsThrd\n");
...
static INT_32 HQA_TxBfProfileTagTimeOut(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 bf_tout = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagTimeOut\n");
...
static INT_32 HQA_TxBfProfileTagDesiredBw(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 desire_bw = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagDesiredBw\n");
...
static INT_32 HQA_TxBfProfileTagDesiredNc(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 desire_nc = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagDesiredNc\n");
...
static INT_32 HQA_TxBfProfileTagDesiredNr(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 desire_nr = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagDesiredNr\n");
...
static INT_32 HQA_TxBfProfileTagWrite(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				      HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagWrite\n");
...
static INT_32 HQA_TxBfProfileTagRead(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				     HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfProfileTagRead\n");
...
static INT_32 HQA_StaRecCmmUpdate(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StaRecCmmUpdate\n");
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x", wlan_idx, bss_idx, aid, mac[0], mac[1],
...
static INT_32 HQA_StaRecBfUpdate(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_StaRecBfUpdate\n");
...
static INT_32 HQA_BFProfileDataRead(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_BFProfileDataRead\n");
...
static INT_32 HQA_BFProfileDataWrite(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				     HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_BFProfileDataWrite\n");
	...
	kalSprintf(prInBuf, "%02x:%03x:%03x:%02x:%03x:%02x:%03x:%02x:%03x:%02x:%03x:%02x:%03x:%02x:%02x:%02x:%02x:%02x",
		   pfmuid, subcarrier, phi11, psi21, phi21, psi31, phi31, psi41, phi22, psi32, phi32, psi42, phi33,
		   psi43, snr00, snr01, snr02, snr03);
...
static INT_32 HQA_BFSounding(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_BFSounding\n");
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x:%02x", su_mu, mu_num, snd_interval, wlan_id0, wlan_id1,
		   wlan_id2, wlan_id3);
...
static INT_32 HQA_TXBFSoundingStop(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				   HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TXBFSoundingStop\n");
...
static INT_32 HQA_TXBFProfileDataWriteAllExt(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
					     HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
...
static INT_32 HQA_TxBfTxApply(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_TxBfTxApply\n");
...
static INT_32 HQA_ManualAssoc(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_ManualAssoc\n");
...
static INT_32 HQA_MUGetInitMCS(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Gid = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUGetInitMCS\n");
...
static INT_32 HQA_MUCalInitMCS(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUCalInitMCS\n");
...
static INT_32 HQA_MUCalLQ(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUCalLQ\n");
...
static INT_32 HQA_MUGetLQ(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUGetLQ\n");
...
static INT_32 HQA_MUSetSNROffset(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Offset = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUSetSNROffset\n");
...
static INT_32 HQA_MUSetZeroNss(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUSetZeroNss\n");
...
static INT_32 HQA_MUSetSpeedUpLQ(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUSetSpeedUpLQ\n");
...
static INT_32 HQA_MUSetMUTable(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_8 *prTable;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUSetMUTable\n");
...
static INT_32 HQA_MUSetGroup(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUSetGroup\n");
	...
	kalSprintf(
		...
		u4User0UP, u4User1UP, u4User0MuPfId, u4User1MuPfId, u4User0InitMCS, u4User1InitMCS, ucAddr1[0],
		ucAddr1[1], ucAddr1[2], ucAddr1[3], ucAddr1[4], ucAddr1[5], ucAddr2[0], ucAddr2[1], ucAddr2[2],
		ucAddr2[3], ucAddr2[4], ucAddr2[5]);
...
static INT_32 HQA_MUGetQD(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUGetQD\n");
...
static INT_32 HQA_MUSetEnable(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUSetEnable\n");
...
static INT_32 HQA_MUSetGID_UP(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUSetGID_UP\n");
...
static INT_32 HQA_MUTriggerTx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_MUTriggerTx\n");
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 HQA_CapWiFiSpectrum(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4Control = %d\n", u4Control);
	...
	if (u4Control == 1 && u4Trigger == 1) {
		...
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4Trigger = %d\n", u4Trigger);
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4RingCapEn = %d\n", u4RingCapEn);
		/* AT Command #81 */
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4TriggerEvent = %d\n", u4TriggerEvent);
		/* AT Command #80 */
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4CaptureNode = %d\n", u4CaptureNode);
		/* AT Command #83 */
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4CaptureLen = %d\n", u4CaptureLen);
		/* AT Command #84 */
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4CapStopCycle = %d\n", u4CapStopCycle);
		/* AT Command #71 */
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4BW = %d\n", u4BW);
		...
		if (g_bCaptureDone) {
			DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum Done!!!!!!!!!!!!!!!!!\n");
			i4Ret = 0;
			/* Query whether ICAP Done */
			ResponseToQA(HqaCmdFrame, prIwReqData, 2, i4Ret);
		} else {
			DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum Wait!!!!!!!!!!!!!!!!!\n");
			i4Ret = 1;
			/* Query whether ICAP Done */
			ResponseToQA(HqaCmdFrame, prIwReqData, 2, i4Ret);
		}
		...
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4WFNum = %d\n", u4WFNum);
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_CapWiFiSpectrum u4IQ = %d\n", u4IQ);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 hqa_set_channel_ext(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Ext_id = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext central_ch0 : %d\n", u4Central_ch0);
	/* for BW80+80 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext central_ch1 : %d\n", u4Central_ch1);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext sys_bw : %d\n", u4Sys_bw);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext perpkt_bw : %d\n", u4Perpkt_bw);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext pri_sel : %d\n", u4Pri_sel);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext reason : %d\n", u4Reason);
	/* 0:2.4G    1:5G */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_channel_ext ch_band : %d\n", u4Ch_band);
	...
	if ((u4Central_ch0 == u4Central_ch1) && (u4Sys_bw == 6) && (u4Perpkt_bw == 6)) {
		DBGLOG(RFTEST, INFO, "MT7668S : Wrong Setting for POR Cal\n");
		goto exit;
	}
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 hqa_set_txcontent_ext(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Len = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext band_idx : %d\n", u4Band_idx);
	/* Frame Control...0800 : Beacon */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext FC : 0x%x\n", u4FC);
	/* Duration....NAV */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext dur : 0x%x\n", u4Dur);
	/* Sequence Control */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext seq : 0x%x\n", u4Seq);
	/* Normal:0,Repeat:1,Random:2 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext gen_payload_rule : %d\n", u4Gen_payload_rule);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext txlen : %d\n", u4Txlen);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext payload_len : %d\n", u4Payload_len);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext addr1:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucAddr1[0], ucAddr1[1], ucAddr1[2], ucAddr1[3], ucAddr1[4], ucAddr1[5]);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext addr2:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucAddr2[0], ucAddr2[1], ucAddr2[2], ucAddr2[3], ucAddr2[4], ucAddr2[5]);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_set_txcontent_ext addr3:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucAddr3[0], ucAddr3[1], ucAddr3[2], ucAddr3[3], ucAddr3[4], ucAddr3[5]);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext payload : 0x%x\n", ucPayload);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 hqa_start_tx_ext(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Ext_id = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext pkt_cnt : %d\n", u4Pkt_cnt);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext phymode : %d\n", u4Phymode);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext rate : %d\n", u4Rate);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext pwr : %d\n", u4Pwr);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext stbc : %d\n", u4Stbc);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext ldpc : %d\n", u4Ldpc);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext ibf : %d\n", u4iBF);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext ebf : %d\n", u4eBF);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext wlan_id : %d\n", u4Wlan_id);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext aifs : %d\n", u4Aifs);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext gi : %d\n", u4Gi);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext tx_path : %d\n", u4Tx_path);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_tx_ext nss : %d\n", u4Nss);
 ...
 */
/*----------------------------------------------------------------------------*/
static INT_32 hqa_start_rx_ext(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Ext_id = 0;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_rx_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_rx_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_rx_ext band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_rx_ext own_mac:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucOwn_mac[0], ucOwn_mac[1], ucOwn_mac[2], ucOwn_mac[3], ucOwn_mac[4], ucOwn_mac[5]);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_start_rx_ext rx_path : 0x%x\n", u4Rx_path);
...
static INT_32 hqa_stop_tx_ext(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_stop_tx_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_stop_tx_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_stop_tx_ext band_idx : %d\n", u4Band_idx);
...
static INT_32 hqa_stop_rx_ext(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_stop_rx_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_stop_rx_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_stop_rx_ext band_idx : %d\n", u4Band_idx);
...
static INT_32 HQA_iBFInit(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_iBFInit\n");
...
static INT_32 HQA_iBFSetValue(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_iBFSetValue\n");
...
static INT_32 HQA_iBFGetStatus(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_iBFGetStatus\n");
...
static INT_32 HQA_iBFChanProfUpdate(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				    HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_iBFChanProfUpdate\n");
...
static INT_32 HQA_iBFProfileRead(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				 HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;

	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_iBFProfileRead\n");
...
static INT_32 HQA_IRRSetADC(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetADC ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetADC u4WFIdx : %d\n", u4WFIdx);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetADC u4ChFreq : %d\n", u4ChFreq);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetADC u4BW : %d\n", u4BW);
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetADC u4Sx : %d\n", u4Sx); /* SX : 0, 2 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetADC u4Band : %d\n", u4Band);
	/* RunType : 0 -> QA, 1 -> ATE */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetADC u4RunType : %d\n", u4RunType);
	/* FType : 0 -> FI, 1 -> FD */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetADC u4FType : %d\n", u4FType);
...
static INT_32 HQA_IRRSetRxGain(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
			       HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4PgaLpfg;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetRxGain ext_id : %d\n", u4Ext_id);
	/* PGA is for MT663, LPFG is for MT7615 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetRxGain u4PgaLpfg : %d\n", u4PgaLpfg);
	/* 5 : UH, 4 : H, 3 : M, 2 : L, 1 : UL */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetRxGain u4Lna : %d\n", u4Lna);
	/* DBDC band0 or band1 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetRxGain u4Band : %d\n", u4Band);
	/* (each bit for each WF) */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetRxGain u4WF_inx : 0x%x\n", u4WF_inx);
	/* only for MT6632 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetRxGain u4Rfdgc : %d\n", u4Rfdgc);
...
static INT_32 HQA_IRRSetTTG(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTTG ext_id : %d\n", u4Ext_id);
	/* TTG Power Index:   Power index value 0~15 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTTG u4TTGPwrIdx : %d\n", u4TTGPwrIdx);
	/* Ch Freq: channel frequency value */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTTG u4ChFreq : %d\n", u4ChFreq);
	/* FI Tone Freq(float): driver calculate TTG Freq(TTG Freq = Ch_freq + FI tone freq) */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTTG u4FIToneFreq : %d\n", u4FIToneFreq);
	/* Band: DBDC band0 or band1 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTTG u4Band : %d\n", u4Band);
...
static INT_32 HQA_IRRSetTrunOnTTG(struct net_device *prNetDev, IN union iwreq_data *prIwReqData,
				  HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 u4Ext_id;
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTrunOnTTG ext_id : %d\n", u4Ext_id);
	/* TTG on/off:  0:off,   1: on */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTrunOnTTG u4TTGOnOff : %d\n", u4TTGOnOff);
	/* Band: DBDC band0 or band1 */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTrunOnTTG u4Band : %d\n", u4Band);
	/* (each bit for each WF) */
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT HQA_IRRSetTrunOnTTG u4WF_inx : %d\n", u4WF_inx);
...
static INT_32 hqa_ext_cmds(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_ext_cmds index : %d\n", i4Idx);
	...
	if (i4Idx < (sizeof(hqa_ext_cmd_set) / sizeof(HQA_CMD_HANDLER))) {
		if (hqa_ext_cmd_set[i4Idx] != NULL)
			...
			DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_ext_cmds cmd idx %d is NULL\n", i4Idx);
	} else
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT hqa_ext_cmds cmd idx %d is not supported\n", i4Idx);
		  ...
		  IN union iwreq_data *prIwReqData, IN char *pcExtra)
{
	...
	default:
		i4Status = -EINVAL;
		DBGLOG(RFTEST, INFO, "MT7668S : QA_AGENT ATEMagicNum Error!!!\n");
		break;
	}

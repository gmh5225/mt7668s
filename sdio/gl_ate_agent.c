...
ATE_PRIV_CMD rAtePrivCmdTable[] = { { "ResetCounter", Set_ResetStatCounter_Proc },
				    { "ATE", SetATE },
#if 0
	{"ADCDump", SetADCDump},
...
int Set_ResetStatCounter_Proc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set_ResetStatCounter_Proc\n");
...
int SetATE(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetATE\n");

	if (!strcmp(prInBuf, "ATESTART")) {
		DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetATE - ATESTART\n");
		i4Status = MT_ATEStart(prNetDev, prInBuf);
	} else if (!strcmp(prInBuf, "ICAPSTART")) {
		DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetATE - ICAPSTART\n");
		i4Status = MT_ICAPStart(prNetDev, prInBuf);
	} else if (!strcmp(prInBuf, "ATESTOP")) {
		DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetATE - ATESTOP\n");
		i4Status = MT_ATEStop(prNetDev, prInBuf);
	} else if (!strcmp(prInBuf, "TXFRAME")) {
		DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetATE - TXFRAME\n");
		i4Status = MT_ATEStartTX(prNetDev, prInBuf);
	} else if (!strcmp(prInBuf, "TXSTOP")) {
		DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetATE - TXSTOP\n");
		i4Status = MT_ATEStopTX(prNetDev, prInBuf);
	} else if (!strcmp(prInBuf, "RXFRAME")) {
		DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetATE - RXFRAME\n");
		i4Status = MT_ATEStartRX(prNetDev, prInBuf);
	} else if (!strcmp(prInBuf, "RXSTOP")) {
		DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetATE - RXSTOP\n");
...
int SetATEDa(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S : ATE_AGENT iwpriv SetDa\n");
	/* xx:xx:xx:xx:xx:xx */
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x", &addr[0], &addr[1], &addr[2], &addr[3], &addr[4], &addr[5]);
	if (rv == 6) {
		DBGLOG(RFTEST, ERROR, "MT7668S : ATE_AGENT iwpriv SetATEDa Sa:%02x:%02x:%02x:%02x:%02x:%02x\n", addr[0],
...
int SetATESa(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S : ATE_AGENT iwpriv SetSa\n");
	/* xx:xx:xx:xx:xx:xx */
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x", &addr[0], &addr[1], &addr[2], &addr[3], &addr[4], &addr[5]);
	if (rv == 6) {
		DBGLOG(RFTEST, ERROR, "MT7668S : ATE_AGENT iwpriv SetATESa Sa:%02x:%02x:%02x:%02x:%02x:%02x\n", addr[0],
		       addr[1], addr[2], addr[3], addr[4], addr[5]);
...
int SetATEChannel(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetChannel\n");
...
int SetATETxPower0(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetTxPower0\n");
...
int SetATETxGi(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetTxGi\n");
...
int SetATETxBw(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetSystemBW\n");
...
int SetATETxMode(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetTxMode\n");
...
int SetATETxLength(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetTxLength\n");
...
int SetATETxCount(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetTxCount\n");
...
int SetATETxMcs(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetTxMcs\n");
...
int SetATEIpg(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv SetIpg\n");
...
int Set_TxBfProfileTag_InValid(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_InValid\n");

	rv = kstrtoint(prInBuf, 0, &ucInValid);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_InValid prInBuf = %s, ucInValid = %d\n", prInBuf,
		       ucInValid);
...
int Set_TxBfProfileTag_PfmuIdx(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_PfmuIdx\n");

	rv = kstrtoint(prInBuf, 0, &ucProfileIdx);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_PfmuIdx prInBuf = %s, ucProfileIdx = %d\n", prInBuf,
		       ucProfileIdx);
...
int Set_TxBfProfileTag_BfType(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_BfType\n");

	rv = kstrtoint(prInBuf, 0, &ucBFType);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_BfType prInBuf = %s, ucBFType = %d\n", prInBuf,
		       ucBFType);
...
int Set_TxBfProfileTag_DBW(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_DBW\n");

	rv = kstrtoint(prInBuf, 0, &ucBW);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_DBW prInBuf = %s, ucBW = %d\n", prInBuf, ucBW);
...
int Set_TxBfProfileTag_SuMu(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_SuMu\n");

	rv = kstrtoint(prInBuf, 0, &ucSuMu);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_SuMu prInBuf = %s, ucSuMu = %d\n", prInBuf, ucSuMu);
...
int Set_TxBfProfileTag_Mem(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_Mem\n");
	...
	if (rv == 8) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S : ATE_AGENT iwpriv Set_TxBfProfileTag_Mem aucInput:%d:%d:%d:%d:%d:%d:%d:%d\n",
...
int Set_TxBfProfileTag_Matrix(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_Matrix\n");
	...
	if (rv == 6) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S : ATE_AGENT iwpriv Set_TxBfProfileTag_Matrix aucInput:%d:%d:%d:%d:%d:%d\n", aucInput[0],
		       aucInput[1], aucInput[2], aucInput[3], aucInput[4], aucInput[5]);
		...
		i4Status = TxBfProfileTag_Matrix(prNetDev, &g_rPfmuTag1, ucNrow, ucNcol, ucNgroup, ucLM, ucCodeBook,
						 ucHtcExist);
	} else
		return -EINVAL;
...
int Set_TxBfProfileTag_SNR(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_SNR\n");

	rv = sscanf(prInBuf, "%d:%d:%d:%d", &aucInput[0], &aucInput[1], &aucInput[2], &aucInput[3]);
	if (rv == 4) {
		DBGLOG(RFTEST, ERROR, "MT7668S : ATE_AGENT iwpriv Set_TxBfProfileTag_SNR aucInput:%d:%d:%d:%d\n",
...
int Set_TxBfProfileTag_SmartAnt(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_SmartAnt\n");

	rv = kstrtoint(prInBuf, 0, &ucSmartAnt);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_SmartAnt prInBuf = %s, ucSmartAnt = %d\n", prInBuf,
		       ucSmartAnt);
...
int Set_TxBfProfileTag_SeIdx(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_SeIdx\n");

	rv = kstrtoint(prInBuf, 0, &ucSeIdx);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S TxBfProfileTag_SeIdx prInBuf = %s, ucSeIdx = %d\n", prInBuf, ucSeIdx);
...
int Set_TxBfProfileTag_RmsdThrd(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_RmsdThrd\n");

	rv = kstrtoint(prInBuf, 0, &ucRmsdThrd);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_RmsdThrd prInBuf = %s, ucRmsdThrd = %d\n", prInBuf,
		       ucRmsdThrd);
...
int Set_TxBfProfileTag_McsThrd(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_McsThrd\n");

	rv = sscanf(prInBuf, "%d:%d:%d:%d:%d:%d", &aucInput[0], &aucInput[1], &aucInput[2], &aucInput[3], &aucInput[4],
		    &aucInput[5]);
	if (rv == 6) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S : ATE_AGENT iwpriv Set_TxBfProfileTag_McsThrd aucInput:%d:%d:%d:%d:%d:%d\n",
...
int Set_TxBfProfileTag_TimeOut(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_TimeOut\n");

	rv = kstrtouint(prInBuf, 0, &ucTimeOut);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_TimeOut prInBuf = %s, ucTimeOut = %d\n", prInBuf,
		       ucTimeOut);
...
int Set_TxBfProfileTag_DesiredBW(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_DesiredBW\n");

	rv = kstrtoint(prInBuf, 0, &ucDesiredBW);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_DesiredBW prInBuf = %s, ucDesiredBW = %d\n", prInBuf,
		       ucDesiredBW);
...
int Set_TxBfProfileTag_DesiredNc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_DesiredNc\n");

	rv = kstrtoint(prInBuf, 0, &ucDesiredNc);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_DesiredNc prInBuf = %s, ucDesiredNc = %d\n", prInBuf,
		       ucDesiredNc);
...
int Set_TxBfProfileTag_DesiredNr(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_DesiredNr\n");

	rv = kstrtoint(prInBuf, 0, &ucDesiredNr);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTag_DesiredNr prInBuf = %s, ucDesiredNr = %d\n", prInBuf,
		       ucDesiredNr);
...
int Set_TxBfProfileTagWrite(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTagWrite\n");

	rv = kstrtoint(prInBuf, 0, &profileIdx);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTagWrite prInBuf = %s, profileIdx = %d\n", prInBuf,
		       profileIdx);
...
int Set_TxBfProfileTagRead(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTagRead\n");

	rv = sscanf(prInBuf, "%d:%d", &profileIdx, &fgBFer);
	if (rv == 2) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileTagRead prInBuf = %s, profileIdx = %d, fgBFer = %d\n",
...
int Set_TxBfProfileDataRead(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfileDataRead\n");

	rv = sscanf(prInBuf, "%d:%d:%x:%x", &profileIdx, &fgBFer, &subcarrierIdxMsb, &subcarrierIdxLsb);
	if (rv == 4) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S Set_TxBfProfileDataRead prInBuf = %s, profileIdx = %d, fgBFer = %d, subcarrierIdxMsb:%x, subcarrierIdxLsb:%x\n",
...
int Set_TxBfProfileDataWrite(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S TxBfProfileDataWrite\n");

	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x", &u4ProfileIdx, &u4SubcarrierIdx,
		    &au4Phi[0], &au4Psi[0], &au4Phi[1], &au4Psi[1], &au4Phi[2], &au4Psi[2], &au4Phi[3], &au4Psi[3],
		    &au4Phi[4], &au4Psi[4], &au4Phi[5], &au4Psi[5], &au4DSnr[0], &au4DSnr[1], &au4DSnr[2], &au4DSnr[3]);

	if (rv == 18) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S TxBfProfileDataWrite prInBuf = %s, u4ProfileIdx = %x, u4SubcarrierIdx = %x, au4Phi[0]:%x, au4Phi[1]:%x, au4Phi[2]:%x, au4Phi[3]:%x, au4Phi[4]:%x, au4Phi[5]:%x, au4Psi[0]:%x, au4Psi[1]:%x, au4Psi[2]:%x, au4Psi[3]:%x, au4Psi[4]:%x, au4Psi[5]:%x,au4DSnr[0]:%x, au4DSnr[1]:%x, au4DSnr[2]:%x, au4DSnr[3]:%x\n",
		       prInBuf, u4ProfileIdx, u4SubcarrierIdx, au4Phi[0], au4Phi[1], au4Phi[2], au4Phi[3], au4Phi[4],
		       au4Phi[5], au4Psi[0], au4Psi[1], au4Psi[2], au4Psi[3], au4Psi[4], au4Psi[5], au4DSnr[0],
		       au4DSnr[1], au4DSnr[2], au4DSnr[3]);
		for (i = 0; i < 6; i++) {
			au2Phi[i] = au4Phi[i];
			aucPsi[i] = au4Psi[i];
		}
...
int Set_TxBfProfilePnRead(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfilePnRead\n");

	rv = kstrtoint(prInBuf, 0, &profileIdx);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_TxBfProfilePnRead prInBuf = %s, profileIdx = %d\n", prInBuf,
		       profileIdx);
...
int Set_TxBfProfilePnWrite(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S TxBfProfilePnWrite\n");

	rv = sscanf(prInBuf, "%d:%hd:%hd:%hd:%hd:%hd:%hd:%hd:%hd:%hd:%hd:%hd:%hd:%hd", &ucProfileIdx, &u2bw,
		    &au2XSTS[0], &au2XSTS[1], &au2XSTS[2], &au2XSTS[3], &au2XSTS[4], &au2XSTS[5], &au2XSTS[6],
		    &au2XSTS[7], &au2XSTS[8], &au2XSTS[9], &au2XSTS[10], &au2XSTS[11]);
	if (rv == 14) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S TxBfProfilePnWrite prInBuf = %s, ucProfileIdx = %d, u2bw = %dau2XSTS[0]:%d, au2XSTS[1]:%d, au2XSTS[2]:%d, au2XSTS[3]:%d, au2XSTS[4]:%d, au2XSTS[5]:%d, au2XSTS[6]:%d, au2XSTS[7]:%d, au2XSTS[8]:%d, au2XSTS[9]:%d, au2XSTS[10]:%d, au2XSTS[11]:%d\n",
		       prInBuf, ucProfileIdx, u2bw, au2XSTS[0], au2XSTS[1], au2XSTS[2], au2XSTS[3], au2XSTS[4],
		       au2XSTS[5], au2XSTS[6], au2XSTS[7], au2XSTS[8], au2XSTS[9], au2XSTS[10], au2XSTS[11]);
...
int Set_Trigger_Sounding_Proc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_Trigger_Sounding_Proc\n");

	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x:%x", &ucSuMu, &ucNumSta, &ucSndInterval, &ucWLan0, &ucWLan1, &ucWLan2,
		    &ucWLan3);
	if (rv == 7) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S Set_Trigger_Sounding_Proc prInBuf = %s, ucSuMu = %d, ucNumSta = %d, ucSndInterval = %d, ucWLan0 = %d, ucWLan1 = %d, ucWLan2:%d, ucWLan3:%d\n",
...
int Set_Stop_Sounding_Proc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_Stop_Sounding_Proc\n");
...
int Set_TxBfTxApply(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S TxBfTxApply\n");

	rv = sscanf(prInBuf, "%d:%d:%d:%d", &u4WlanId, &u4ETxBf, &u4ITxBf, &u4MuTxBf);
	if (rv == 4) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S TxBfTxApply prInBuf = %s, u4WlanId = %d, u4ETxBf = %d, u4ITxBf = %d, u4MuTxBf = %d\n",
...
int Set_TxBfManualAssoc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S TxBfManualAssoc\n");

	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x:%x:%d:%x:%x:%x:%x:%d:%x:%x:%x:%d:%x", &au4Mac[0], &au4Mac[1],
		    &au4Mac[2], &au4Mac[3], &au4Mac[4], &au4Mac[5], &u4Type, &u4Wtbl, &u4Ownmac, &u4PhyMode, &u4Bw,
		    &u4Nss, &u4PfmuId, &u4Mode, &u4Marate, &u4SpeIdx, &ucaid, &u4Rv);
	if (rv == 18) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S TxBfManualAssoc au4Mac[0] = %x, au4Mac[1] = %x, au4Mac[2] = %xau4Mac[3] = %x, au4Mac[4] = %x, au4Mac[5] = %x, u4Type = %x, u4Wtbl = %d, u4Ownmac = %x, u4PhyMode = %x u4Bw = %x, u4Nss = %x, u4PfmuId = %d, u4Mode = %x, u4Marate = %x, u4SpeIdx = %d, ucaid = %d, u4Rv = %x",
		...
		i4Status = TxBfManualAssoc(prNetDev, aucMac, u4Type, u4Wtbl, u4Ownmac, u4Mode, u4Bw, u4Nss, u4PfmuId,
...
int Set_TxBfPfmuMemAlloc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S TxBfPfmuMemAlloc\n");

	rv = sscanf(prInBuf, "%d:%d", &ucSuMuMode, &ucWlanIdx);
	if (rv == 2) {
		DBGLOG(RFTEST, ERROR, "MT7668S TxBfPfmuMemAlloc ucSuMuMode = %d, ucWlanIdx = %d", ucSuMuMode,
		       ucWlanIdx);
...
int Set_TxBfPfmuMemRelease(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S TxBfPfmuMemRelease\n");

	rv = kstrtoint(prInBuf, 0, &ucWlanId);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S TxBfPfmuMemRelease ucWlanId = %d", ucWlanId);
...
int Set_DevInfoUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S DevInfoUpdate\n");
	...
	if (rv == 8) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S DevInfoUpdate prInBuf = %s, u4OwnMacIdx = %x, fgBand = %x,OwnMacAddr[0]:%x, OwnMacAddr[1]:%x, OwnMacAddr[2]:%x, OwnMacAddr[3]:%x, OwnMacAddr[4]:%x, OwnMacAddr[5]:%x,",
...
int Set_BssInfoUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S BssInfoUpdate\n");

	rv = sscanf(prInBuf, "%d:%d:%x:%x:%x:%x:%x:%x", &u4OwnMacIdx, &u4BssIdx, &au4BssId[0], &au4BssId[1],
		    &au4BssId[2], &au4BssId[3], &au4BssId[4], &au4BssId[5]);
	if (rv == 8) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S BssInfoUpdate prInBuf = %s, u4OwnMacIdx = %x, u4BssIdx = %x,au4BssId[0]:%x, au4BssId[1]:%x, au4BssId[2]:%x, au4BssId[3]:%x, au4BssId[4]:%x, au4BssId[5]:%x,",
...
int Set_StaRecCmmUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_StaRecCmmUpdate\n");
	...
	if (rv == 9) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S Set_StaRecCmmUpdate prInBuf = %s, u4WlanId = %x, u4BssId = %x, u4Aid = %x,aucMacAddr[0]:%x, aucMacAddr[1]:%x, aucMacAddr[2]:%x, aucMacAddr[3]:%x, aucMacAddr[4]:%x, aucMacAddr[5]:%x,",
...
int Set_StaRecBfUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_StaRecBfUpdate\n");
	...
	if (rv == 23) {
		/*
		 *DBGLOG(RFTEST, ERROR,
		 *"MT7668S Set_StaRecBfUpdate prInBuf = %s, u4WlanId = %x, u4BssId = %x, u4Aid = %x,
...
int Set_MUGetInitMCS(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUGetInitMCS\n");
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Test\n");
		DBGLOG(RFTEST, ERROR, "MT7668S Set_MUGetInitMCS prInBuf = %s, u4groupIdx = %x", prInBuf, u4groupIdx);
		...
		i4Status = kalIoctl(prGlueInfo, wlanoidMuMimoAction, &rMuMimoActionInfo, sizeof(rMuMimoActionInfo),
...
int Set_MUCalInitMCS(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUCalInitMCS\n");
	...
	if (rv == 9) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S Set_MUCalInitMCS prInBuf = %s, u4NumOfUser = %x, u4Bandwidth = %x, u4NssOfUser0 = %x, u4NssOfUser1 = %x, u4PfMuIdOfUser0 = %x, u4PfMuIdOfUser1 = %x, u4NumOfTxer = %x, u4SpeIndex = %x, u4GroupIndex = %x",
...
int Set_MUCalLQ(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUCalLQ\n");
	...
	if (rv == 9) {
		DBGLOG(RFTEST, ERROR,
		       "MT7668S Set_MUCalLQ prInBuf = %s, u4NumOfUser = %x, u4Bandwidth = %x, u4NssOfUser0 = %x, u4NssOfUser1 = %x, u4PfMuIdOfUser0 = %x, u4PfMuIdOfUser1 = %x, u4NumOfTxer = %x, u4SpeIndex = %x, u4GroupIndex = %x",
		       prInBuf, u4NumOfUser, u4Bandwidth, u4NssOfUser0, u4NssOfUser1, u4PfMuIdOfUser0, u4PfMuIdOfUser1,
		       u4NumOfTxer, u4SpeIndex, u4GroupIndex);
...
int Set_MUGetLQ(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUGetLQ\n");
	...
	/* { */
	/* DBGLOG(RFTEST, ERROR, "MT7668S Set_MUGetLQ prInBuf = %s, u4Type = %x", prInBuf, u4Type); */
...
int Set_MUSetSNROffset(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetSNROffset\n");
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetSNROffset prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetZeroNss(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetZeroNss\n");
	...
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetZeroNss prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetSpeedUpLQ(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetSpeedUpLQ\n");
	...
	rv = kstrtouint(prInBuf, 0, &u4Val);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetSpeedUpLQ prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetMUTable(struct net_device *prNetDev, UINT_8 *prTable)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetMUTable\n");
	...
	/* { */
	/* DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetMUTable prInBuf = %s, */
...
int Set_MUSetGroup(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetGroup\n");
...
int Set_MUGetQD(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUGetQD\n");
	...
	if (sscanf(prInBuf, "%x:%x", &u4SubcarrierIndex, &u4Length) == 2) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_MUGetQD prInBuf = %s, u4SubcarrierIndex = %x, u4Length = %x",
...
int Set_MUSetEnable(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetEnable\n");
	...
	rv = kstrtouint(prInBuf, 0, &u4Val);
	if (rv == 0) {
		DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetEnable prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetGID_UP(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUSetGID_UP\n");
	...
	if (sscanf(prInBuf, "%x:%x:%x:%x:%x:%x", &rMuMimoActionInfo.unMuMimoParam.rMuSetGidUp.au4Gid[0],
		...
		DBGLOG(RFTEST, ERROR,
		       "MT7668S Set_MUSetGID_UP prInBuf = %s, au4Gid[0] = %x, au4Gid[1] = %x, au4Up[0] = %x, au4Up[1] = %x, au4Up[2] = %x, au4Up[3] = %x",
...
int Set_MUTriggerTx(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(RFTEST, ERROR, "MT7668S Set_MUTriggerTx\n");
	...
	if (sscanf(prInBuf, "%d:%x:%x:%x:%d:%d:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x", &u4IsRandomPattern,
		   &u4MsduPayloadLength0, &u4MsduPayloadLength1, &u4MuPacketCount, &u4NumOfSTAs, &au4MacAddrs[0][0],
		   &au4MacAddrs[0][1], &au4MacAddrs[0][2], &au4MacAddrs[0][3], &au4MacAddrs[0][4], &au4MacAddrs[0][5],
		   &au4MacAddrs[1][0], &au4MacAddrs[1][1], &au4MacAddrs[1][2], &au4MacAddrs[1][3], &au4MacAddrs[1][4],
		   &au4MacAddrs[1][5]) == 17) {
		DBGLOG(RFTEST, ERROR,
...
int WriteEfuse(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv WriteEfuse, buf: %s\n", prInBuf);

	rv = sscanf(prInBuf, "%x:%x", &addr[0], &addr[1]);

	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv WriteEfuse, prInBuf: %s\n", prInBuf);
	DBGLOG(INIT, ERROR, "MT7668S : ATE_AGENT iwpriv WriteEfuse :%02x:%02x\n", addr[0], addr[1]);
...
int SetTxTargetPower(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set Tx Target Power, buf: %s\n", prInBuf);

	/* rv = sscanf(prInBuf, "%u", &addr);*/
	rv = kstrtoint(prInBuf, 0, &addr);

	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set Tx Target Power, prInBuf: %s\n", prInBuf);
	DBGLOG(INIT, ERROR, "MT7668S : ATE_AGENT iwpriv Set Tx Target Power :%02x\n", addr);
...
int SetRddReport(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set RDD Report, buf: %s\n", prInBuf);

	/* rv = sscanf(prInBuf, "%u", &addr);*/
	rv = kstrtoint(prInBuf, 0, &dbdcIdx);

	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set RDD Report, prInBuf: %s\n", prInBuf);
	DBGLOG(INIT, ERROR, "MT7668S : ATE_AGENT iwpriv Set RDD Report : Band %d\n", dbdcIdx);
...
int SetByPassCac(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set By Pass Cac, buf: %s\n", prInBuf);

	rv = kstrtoint(prInBuf, 0, &i4ByPassCacTime);

	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set By Pass Cac, prInBuf: %s\n", prInBuf);
	DBGLOG(INIT, ERROR, "MT7668S : ATE_AGENT iwpriv Set By Pass Cac : %dsec\n", i4ByPassCacTime);
...
int SetRadarDetectMode(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set Radar Detect Mode, buf: %s\n", prInBuf);

	rv = kstrtoint(prInBuf, 0, &radarDetectMode);

	DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv Set Radar Detect Mode, prInBuf: %s\n", prInBuf);
	DBGLOG(INIT, ERROR, "MT7668S : ATE_AGENT iwpriv Set Radar Detect Mode : %d\n", radarDetectMode);
...
int AteCmdSetHandle(struct net_device *prNetDev, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	while ((this_char = strsep((char **)&prInBuf, ",")) != NULL) {
		if (!*this_char)
			continue;
		DBGLOG(RFTEST, ERROR, "MT7668S : ATE_AGENT iwpriv this_char = %s\n", this_char);
		DBGLOG(RFTEST, INFO, "MT7668S : ATE_AGENT iwpriv this_char = %s\n", this_char);

		value = strchr(this_char, '=');
		if (value != NULL)
			*value++ = 0;

		DBGLOG(REQ, INFO, "MT7668S : ATE_AGENT iwpriv cmd = %s, value = %s\n", this_char, value);

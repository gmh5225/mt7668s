...
static UINT_8 aucDbModuleName[][PROC_DBG_LEVEL_MAX_DISPLAY_STR_LEN] = {
	"INIT", "HAL", "INTR", "REQ", "TX",   "RX",	 "RFTEST", "EMU", "SW1", "SW2", "SW3",
	"SW4",	"HEM", "AIS",  "RLM", "MEM",  "CNM",	 "RSN",	   "BSS", "SCN", "SAA", "AAA",
	"P2P",	"QM",  "SEC",  "BOW", "WAPI", "ROAMING", "TDLS",   "PF",  "OID", "NIC"
};
...
static ssize_t procCSIDataPrepare(UINT_8 *buf, struct CSI_INFO_T *prCSIInfo, struct CSI_DATA_T *prCSIData)
{
	INT_32 i4Pos = 0;
	UINT_8 *tmpBuf = buf;
	...
	if (prCSIInfo->ucValue1[CSI_CONFIG_INFO] & CSI_INFO_RSVD1) {
		...
		i4Pos += 2;
		kalMemCopy((tmpBuf + i4Pos), prCSIData->ai4Rsvd1, u2Rsvd1Size);
		i4Pos += u2Rsvd1Size;
		...
		put_unaligned(u2Rsvd1Size, (UINT_16 *)(tmpBuf + i4Pos));
		i4Pos += 2;
		kalMemCopy((tmpBuf + i4Pos), prCSIData->au4Rsvd2, u2Rsvd1Size);
		i4Pos += u2Rsvd1Size;
...
static ssize_t procCSIDataRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (prCSIInfo->bIncomplete == FALSE) {
		wait_event_interruptible(prCSIInfo->waitq, prCSIInfo->u4CSIBufferUsed != 0);

		/*
		 * No older CSI data in buffer waiting for reading out, so prepare a new one
		 * for reading.
		 */
		if (wlanPopCSIData(g_prGlueInfo_proc->prAdapter, &rTmpCSIData))
			i4Pos = procCSIDataPrepare(temp, prCSIInfo, &rTmpCSIData);
...
static ssize_t procDbgLevelRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	for (i = 0; i < u2ModuleNum; i += 2)
		i4Pos += scnprintf((temp + i4Pos), (u4BufMax - i4Pos),
				   ...
				   &aucDbModuleName[i][0], i, aucDebugModule[i], &aucDbModuleName[i + 1][0], i + 1,
				   aucDebugModule[i + 1]);

	if ((sizeof(aucDbModuleName) / PROC_DBG_LEVEL_MAX_DISPLAY_STR_LEN) & 0x1)
		i4Pos += scnprintf((temp + i4Pos), (u4BufMax - i4Pos), "DBG_%s_IDX\t(0x%02x):\t0x%02x\n",
...
{
	static const struct seq_operations procEfuseDump_ops = { .start = procEfuseDump_start,
...
#define BUFFER_RESERVE_BYTE 50
	...
	for (i = 0; i < WLAN_CFG_ENTRY_NUM_MAX; i++) {
		...
		i4Pos += scnprintf((temp + i4Pos), (u4BufMax - i4Pos), "%s|%s\n", prWlanCfgEntry->aucKey,
	...
	for (i = 0; i < WLAN_CFG_REC_ENTRY_NUM_MAX; i++) {
		...
		i4Pos += scnprintf((temp + i4Pos), (u4BufMax - i4Pos), "D:%s|%s\n", prWlanCfgEntry->aucKey,
				   prWlanCfgEntry->aucValue);
...
static ssize_t procDriverCmdWrite(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (count > sizeof(g_aucProcBuf) - 1) {
		DBGLOG(INIT, ERROR, "input count %zu over local buffer size %zu\n", count, sizeof(g_aucProcBuf));
		return 0;
	}
...
#define TXPWR_TABLE_ENTRY(_siso_mcs, _cdd_mcs, _mimo_mcs, _idx) \
	{ \
		.mcs[STREAM_SISO] = _siso_mcs, .mcs[STREAM_CDD] = _cdd_mcs, .mcs[STREAM_MIMO] = _mimo_mcs, \
		.idx = (_idx), \
	}
...
static struct txpwr_table txpwr_tables[] = {
	{ "Legacy", dsss, ARRAY_SIZE(dsss) }, { "11g", ofdm, ARRAY_SIZE(ofdm) }, { "11a", ofdm, ARRAY_SIZE(ofdm) },
	{ "HT20", ht, ARRAY_SIZE(ht) },	      { "HT40", ht, ARRAY_SIZE(ht) },	 { "VHT20", vht, ARRAY_SIZE(vht) },
...
void print_txpwr_tbl(struct txpwr_table *txpwr_tbl, unsigned char ch, unsigned char *tx_pwr[], char pwr_offset[],
		     char *stream_buf[], unsigned int stream_pos[])
{
	...
	for (i = 0; i < txpwr_tbl->n_tables; i++) {
		...
		for (stream_idx = 0; stream_idx < STREAM_NUM; stream_idx++) {
			...
			*pos += kalScnprintf(buf + *pos, TMP_SZ - *pos, "%s, %d, %s, %s, ", prefix, ch,
					     txpwr_tbl->phy_mode, tmp_tbl[i].mcs[stream_idx]);
			...
			for (pwr_idx = 0; pwr_idx < TXPWR_TBL_NUM; pwr_idx++) {
				...
				*pos += kalScnprintf(buf + *pos, TMP_SZ - *pos, "%d.%d%s", tmp_pwr / 2, tmp_pwr % 2 * 5,
						     tmp);
			}
		}
	}
...
static ssize_t procGetTxpwrTblRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	pos = kalScnprintf(buffer, buf_len, "\n%s", "spatial stream, Channel, bw, modulation, ");
	...
	for (i = 0; i < ARRAY_SIZE(txpwr_tables); i++) {
		...
		print_txpwr_tbl(&txpwr_tables[i], pwr_tbl.ucCenterCh, tx_pwr, pwr_offset, stream_buf, stream_pos);
...
#if CFG_DISCONN_DEBUG_FEATURE
static INT_32 parseTxRateInfo(IN char *pcBuffer, IN int i4Size, IN P_TX_VECTOR_BBP_LATCH_T prTxV)
{
	...
	if (prTxV->u4TxVector1 == 0xFFFFFFFF) {
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s%s\n",
					       "Last TX Rate", " = ", "N/A");
	} else {
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s",
					       "Last TX Rate", " = ");

		if (txmode == TX_RATE_MODE_CCK)
			i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%s, ",
			...
			i4BytesWritten +=
				kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "MCS%d, ", rate);
		else
			i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten,
		...
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%s%s%s\n",
...
static INT_32 parseRxRateInfo(IN char *pcBuffer, IN int i4Size, IN P_STA_RECORD_T prStaRec)
{
	...
	i4BytesWritten +=
		kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s", "Last RX Rate", " = ");
	...
	if (txmode == TX_RATE_MODE_CCK)
		...
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "MCS%d, ", rate);
	else
		i4BytesWritten +=
			kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "NSS%d_MCS%d, ", nsts, rate);
	...
	i4BytesWritten +=
		kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%s", stbc == 0 ? " " : " STBC, ");

	if (mu) {
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%s, %s, %s (%d)\n",
					       txmode < 5 ? HW_TX_MODE_STR[txmode] : HW_TX_MODE_STR[5],
					       ldpc == 0 ? "BCC" : "LDPC", "MU", groupid);
	} else {
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%s, %s\n",
					       txmode < 5 ? HW_TX_MODE_STR[txmode] : HW_TX_MODE_STR[5],
					       ldpc == 0 ? "BCC" : "LDPC");
	}
...
static INT_32 parseRxRssiInfo(IN char *pcBuffer, IN int i4Size, IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	if (prAdapter->rWifiVar.ucNSS > 2) {
		...
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s%d %d\n",
					       "Last RX Data RSSI", " = ", i4RSSI0, i4RSSI1);

	return i4BytesWritten;
}

static UINT_32 parseRxRespRssiInfo(IN char *pcBuffer, IN int i4Size, IN P_ADAPTER_T prAdapter,
...
static INT_32 parseTxPerInfo(IN char *pcBuffer, IN int i4Size, IN P_PARAM_HW_WLAN_INFO_T prHwWlanInfo,
			     IN P_PARAM_GET_STA_STATISTICS prQueryStaStatistics)
{
	...
	if (ucSkipAr) {
		...
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s%d.%1d%%\n",
					       "instant PER", " = ", u4InstantPer / 10, u4InstantPer % 10);
	} else {
		u4InstantPer = (prQueryStaStatistics->ucPer == 0) ? (0) : (prQueryStaStatistics->ucPer);

		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s%d%%\n",
					       "instant PER", " = ", u4InstantPer);
	}

	return i4BytesWritten;
}

static INT_32 parseRxPerInfo(IN char *pcBuffer, IN int i4Size, IN P_ADAPTER_T prAdapter,
			     IN P_PARAM_GET_STA_STATISTICS prQueryStaStatistics)
{
	...
	for (ucDbdcIdx = 0; ucDbdcIdx < ENUM_BAND_NUM; ucDbdcIdx++) {
		u4InstantPer[ucDbdcIdx] =
			((prMibInfo[ucDbdcIdx].u4RxMpduCnt + prMibInfo[ucDbdcIdx].u4FcsError) == 0) ?
				(0) :
				(1000 * prMibInfo[ucDbdcIdx].u4FcsError /
				 (prMibInfo[ucDbdcIdx].u4RxMpduCnt + prMibInfo[ucDbdcIdx].u4FcsError));
	}

	if (!prAdapter->rWifiVar.fgDbDcModeEn) {
		i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s%d.%1d%%\n",
					       "instant PER", " = ", u4InstantPer[ENUM_BAND_0] / 10,
		...
		for (ucDbdcIdx = 0; ucDbdcIdx < ENUM_BAND_NUM; ucDbdcIdx++) {
			...
			i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten,
						       "%-26s%s%d.%1d%%\n", "instant PER", " = ",
						       u4InstantPer[ucDbdcIdx] / 10, u4InstantPer[ucDbdcIdx] % 10);
		}
	}
...
static UINT_32 parseTrigger(IN char *pcBuffer, IN int i4Size, IN UINT_8 trigger)
{
	INT_32 i4BytesWritten = 0;
	PCHAR pString;
...
static UINT_32 parseDisconnReasonCode(IN char *pcBuffer, IN int i4Size, IN UINT_8 ucReason)
{
	INT_32 i4BytesWritten = 0;
	PCHAR pString;
	...
	i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s%s\n",
				       "Disconnect reason", " = ", pString);

	return i4BytesWritten;
}

static UINT_32 parseBcnTimeoutReasonCode(IN char *pcBuffer, IN int i4Size, IN UINT_8 ucReason)
{
	INT_32 i4BytesWritten = 0;
	PCHAR pString;
	...
	case BEACON_TIMEOUT_DUE_2_HW_BEACON_LOST_NONADHOC:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_HW_BEACON_LOST_NONADHOC);
		break;
	case BEACON_TIMEOUT_DUE_2_HW_BEACON_LOST_ADHOC:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_HW_BEACON_LOST_ADHOC);
		break;
	case BEACON_TIMEOUT_DUE_2_HW_TSF_DRIFT:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_HW_TSF_DRIFT);
		break;
	case BEACON_TIMEOUT_DUE_2_NULL_FRAME_THRESHOLD:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_NULL_FRAME_THRESHOLD);
		break;
	...
	case BEACON_TIMEOUT_DUE_2_CONNECTION_FAIL:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_CONNECTION_FAIL);
		break;
	case BEACON_TIMEOUT_DUE_2_ALLOCAT_NULL_PKT_FAIL_THRESHOLD:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_ALLOCAT_NULL_PKT_FAIL_THRESHOLD);
		break;
	case BEACON_TIMEOUT_DUE_2_NO_TX_DONE_EVENT:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_NO_TX_DONE_EVENT);
		break;
	case BEACON_TIMEOUT_DUE_2_UNSPECIF_REASON:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_UNSPECIF_REASON);
		break;
	case BEACON_TIMEOUT_DUE_2_SET_CHIP:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_SET_CHIP);
		break;
	case BEACON_TIMEOUT_DUE_2_RESERVED:
		pString = GET_VARNAME(BEACON_TIMEOUT_DUE_2_RESERVED);
		break;
	default:
		pString = "N/A";
		break;
	...
	i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s%s\n",
				       "Beacon timeout reason", " = ", pString);

	return i4BytesWritten;
}

static UINT_32 parseDisassocReasonCode(IN char *pcBuffer, IN int i4Size, IN UINT_8 ucReason)
{
	INT_32 i4BytesWritten = 0;
	PCHAR pString;
	...
	case REASON_CODE_RESERVED:
		pString = GET_VARNAME(REASON_CODE_RESERVED);
		break;
	case REASON_CODE_UNSPECIFIED:
		pString = GET_VARNAME(REASON_CODE_UNSPECIFIED);
		break;
	case REASON_CODE_PREV_AUTH_INVALID:
		pString = GET_VARNAME(REASON_CODE_PREV_AUTH_INVALID);
		break;
	case REASON_CODE_DEAUTH_LEAVING_BSS:
		pString = GET_VARNAME(REASON_CODE_DEAUTH_LEAVING_BSS);
		break;
	case REASON_CODE_DISASSOC_INACTIVITY:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_INACTIVITY);
		break;
	case REASON_CODE_DISASSOC_AP_OVERLOAD:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_AP_OVERLOAD);
		break;
	case REASON_CODE_CLASS_2_ERR:
		pString = GET_VARNAME(REASON_CODE_CLASS_2_ERR);
		break;
	case REASON_CODE_CLASS_3_ERR:
		pString = GET_VARNAME(REASON_CODE_CLASS_3_ERR);
		break;
	case REASON_CODE_DISASSOC_LEAVING_BSS:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_LEAVING_BSS);
		break;
	case REASON_CODE_ASSOC_BEFORE_AUTH:
		pString = GET_VARNAME(REASON_CODE_ASSOC_BEFORE_AUTH);
		break;
	case REASON_CODE_DISASSOC_PWR_CAP_UNACCEPTABLE:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_PWR_CAP_UNACCEPTABLE);
		break;
	case REASON_CODE_DISASSOC_SUP_CHS_UNACCEPTABLE:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_SUP_CHS_UNACCEPTABLE);
		break;
	case REASON_CODE_INVALID_INFO_ELEM:
		pString = GET_VARNAME(REASON_CODE_INVALID_INFO_ELEM);
		break;
	case REASON_CODE_MIC_FAILURE:
		pString = GET_VARNAME(REASON_CODE_MIC_FAILURE);
		break;
	case REASON_CODE_4_WAY_HANDSHAKE_TIMEOUT:
		pString = GET_VARNAME(REASON_CODE_4_WAY_HANDSHAKE_TIMEOUT);
		break;
	case REASON_CODE_GROUP_KEY_UPDATE_TIMEOUT:
		pString = GET_VARNAME(REASON_CODE_GROUP_KEY_UPDATE_TIMEOUT);
		break;
	case REASON_CODE_DIFFERENT_INFO_ELEM:
		pString = GET_VARNAME(REASON_CODE_DIFFERENT_INFO_ELEM);
		break;
	case REASON_CODE_MULTICAST_CIPHER_NOT_VALID:
		pString = GET_VARNAME(REASON_CODE_MULTICAST_CIPHER_NOT_VALID);
		break;
	case REASON_CODE_UNICAST_CIPHER_NOT_VALID:
		pString = GET_VARNAME(REASON_CODE_UNICAST_CIPHER_NOT_VALID);
		break;
	case REASON_CODE_AKMP_NOT_VALID:
		pString = GET_VARNAME(REASON_CODE_AKMP_NOT_VALID);
		break;
	case REASON_CODE_UNSUPPORTED_RSNE_VERSION:
		pString = GET_VARNAME(REASON_CODE_UNSUPPORTED_RSNE_VERSION);
		break;
	case REASON_CODE_INVALID_RSNE_CAPABILITIES:
		pString = GET_VARNAME(REASON_CODE_INVALID_RSNE_CAPABILITIES);
		break;
	case REASON_CODE_IEEE_802_1X_AUTH_FAILED:
		pString = GET_VARNAME(REASON_CODE_IEEE_802_1X_AUTH_FAILED);
		break;
	case REASON_CODE_CIPHER_REJECT_SEC_POLICY:
		pString = GET_VARNAME(REASON_CODE_CIPHER_REJECT_SEC_POLICY);
		break;
	case REASON_CODE_DISASSOC_UNSPECIFIED_QOS:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_UNSPECIFIED_QOS);
		break;
	case REASON_CODE_DISASSOC_LACK_OF_BANDWIDTH:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_LACK_OF_BANDWIDTH);
		break;
	case REASON_CODE_DISASSOC_ACK_LOST_POOR_CHANNEL:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_ACK_LOST_POOR_CHANNEL);
		break;
	case REASON_CODE_DISASSOC_TX_OUTSIDE_TXOP_LIMIT:
		pString = GET_VARNAME(REASON_CODE_DISASSOC_TX_OUTSIDE_TXOP_LIMIT);
		break;
	case REASON_CODE_PEER_WHILE_LEAVING:
		pString = GET_VARNAME(REASON_CODE_PEER_WHILE_LEAVING);
		break;
	case REASON_CODE_PEER_REFUSE_DLP:
		pString = GET_VARNAME(REASON_CODE_PEER_REFUSE_DLP);
		break;
	case REASON_CODE_PEER_SETUP_REQUIRED:
		pString = GET_VARNAME(REASON_CODE_PEER_SETUP_REQUIRED);
		break;
	case REASON_CODE_PEER_TIME_OUT:
		pString = GET_VARNAME(REASON_CODE_PEER_TIME_OUT);
		break;
	case REASON_CODE_PEER_CIPHER_UNSUPPORTED:
		pString = GET_VARNAME(REASON_CODE_PEER_CIPHER_UNSUPPORTED);
		break;
	default:
		pString = "N/A";
		break;
	...
	i4BytesWritten += kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "%-26s%s%s\n",
				       "Disassociation reason", " = ", pString);

	return i4BytesWritten;
}

static UINT_32 parseRssiInfo(IN char *pcBuffer, IN int i4Size, IN struct CMD_NOISE_HISTOGRAM_REPORT *prNoise)
{
	INT_32 i4BytesWritten = 0;
	...
	if (kalMemCmp(prNoise, &rEmptyNoise, sizeof(struct CMD_NOISE_HISTOGRAM_REPORT)) == 0) {
		DBGLOG(INIT, WARN, "Empty info\n");
		return i4BytesWritten;
	}

	i4BytesWritten +=
		kalScnprintf(pcBuffer + i4BytesWritten, i4Size - i4BytesWritten, "\n%s", "----- Noise Info  -----");
...
static ssize_t procDisconnInfoRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	P_GLUE_INFO_T prGlueInfo;
	INT_32 i4Count = 0;
	UINT_8 ucDbdcIdx;
	...
	for (cnt = 0; cnt < MAX_DISCONNECT_RECORD; cnt++) {
		...
		if (prDisconn->ucTrigger != DISCONNECT_TRIGGER_RESERVED) {
...
#endif
			kalScnprintf(date, sizeof(date), "%02d-%02d %02d:%02d:%02d.%ld", broken.tm_mon + 1,
			...
			i4Count += kalScnprintf(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, "%s %s %d %s %s %s %s",
						"===============", "Record", temp_idx, "(", date, ")",
						"===============");

			/* Dump misc info*/
			i4Count +=
				kalScnprintf(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, "\n%s", "<Misc Info>\n");

			i4Count += parseTrigger(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, prDisconn->ucTrigger);

			i4Count += parseDisconnReasonCode(temp + i4Count, sizeof(g_aucProcBuf) - i4Count,
							  prDisconn->ucDisConnReason);

			i4Count += parseBcnTimeoutReasonCode(temp + i4Count, sizeof(g_aucProcBuf) - i4Count,
							     prDisconn->ucBcnTimeoutReason);

			i4Count += parseDisassocReasonCode(temp + i4Count, sizeof(g_aucProcBuf) - i4Count,
							   prDisconn->ucDisassocReason);

			if (prDisconn->u2DisassocSeqNum != 0xFFFF) {
				i4Count += kalScnprintf(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, "%-26s%s%d\n",
							"Disassociation SeqNum", " = ", prDisconn->u2DisassocSeqNum);
			}

			/* Noise info*/
			i4Count += parseRssiInfo(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, &prDisconn->rNoise);

			/* Dump RX info */
			i4Count += kalScnprintf(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, "\n%s",
						"<Last Rx Info>\n");

			/* Last RX Rate info */
			i4Count += parseRxRateInfo(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, &prDisconn->rStaRec);

			/* RX PER info */
			i4Count += parseRxPerInfo(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, prGlueInfo->prAdapter,
						  &prDisconn->rStaStatistics);

			/* Last RX RSSI info*/
			i4Count += parseRxRssiInfo(temp + i4Count, sizeof(g_aucProcBuf) - i4Count,
						   prGlueInfo->prAdapter, &prDisconn->rStaRec);

			/* Last RX Resp RSSI */
			i4Count += parseRxRespRssiInfo(temp + i4Count, sizeof(g_aucProcBuf) - i4Count,
						       prGlueInfo->prAdapter, &prDisconn->rHwInfo);

			/* Last Beacon RSSI */
			if (prDisconn->rBcnRssi != 0) {
				i4Count += kalScnprintf(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, "%-26s%s%d\n",
							"Beacon RSSI", " = ", prDisconn->rBcnRssi);
			}
			...
			if (!prGlueInfo->prAdapter->rWifiVar.fgDbDcModeEn) {
				i4Count += parseTxRateInfo(temp + i4Count, sizeof(g_aucProcBuf) - i4Count,
							   &prDisconn->rStaStatistics.rTxVector[ENUM_BAND_0]);
				...
				for (ucDbdcIdx = 0; ucDbdcIdx < ENUM_BAND_NUM; ucDbdcIdx++) {
					...
					i4Count += parseTxRateInfo(temp + i4Count, sizeof(g_aucProcBuf) - i4Count,
								   &prDisconn->rStaStatistics.rTxVector[ucDbdcIdx]);
				}
			}

			/* TX PER info */
			i4Count += parseTxPerInfo(temp + i4Count, sizeof(g_aucProcBuf) - i4Count, &prDisconn->rHwInfo,
						  &prDisconn->rStaStatistics);
		}
	}
...
static ssize_t procMCRRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidQueryMcrRead, (PVOID)&rMcrInfo, sizeof(rMcrInfo), TRUE, TRUE, TRUE,
			   &u4BufLen);
	kalMemZero(g_aucProcBuf, sizeof(g_aucProcBuf));

	i4Count = scnprintf(temp, sizeof(g_aucProcBuf), "MCR (0x%08xh): 0x%08x\n", rMcrInfo.u4McrOffset,
			    rMcrInfo.u4McrData);
...
static ssize_t procMCRWrite(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	case 2:
		...
		/* if (IS_ALIGN_4(rMcrInfo.u4McrOffset)) */
		{
			...
			rStatus = kalIoctl(prGlueInfo, wlanoidSetMcrWrite, (PVOID)&rMcrInfo, sizeof(rMcrInfo), FALSE,
					   FALSE, TRUE, &u4BufLen);
		}
		break;
	case 1:
...
static ssize_t procRoamRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	rStatus = kalIoctl(g_prGlueInfo_proc, wlanoidGetRoamParams, g_aucProcBuf, sizeof(g_aucProcBuf), TRUE, FALSE,
			   TRUE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS) {
...
static ssize_t procRoamWrite(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (kalStrnCmp(g_aucProcBuf, "force_roam", 10) == 0)
		rStatus = kalIoctl(g_prGlueInfo_proc, wlanoidSetForceRoam, NULL, 0, FALSE, FALSE, TRUE, &u4BufLen);
	else
		rStatus = kalIoctl(g_prGlueInfo_proc, wlanoidSetRoamParams, g_aucProcBuf, kalStrLen(g_aucProcBuf),
...
static ssize_t procCountryWrite(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	rStatus =
		kalIoctl(g_prGlueInfo_proc, wlanoidSetCountryCode, &g_aucProcBuf[0], 2, FALSE, FALSE, TRUE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS) {
...
#if CFG_DISCONN_DEBUG_FEATURE
	prEntry = proc_create(PROC_DISCONN_INFO, 0664, gprProcRoot, &disconn_info_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR, "Unable to create /proc entry disconn_info\n\r");
		return -1;
	}
#endif

...
#else
VOID kalP2PSetRole(IN P_GLUE_INFO_T prGlueInfo, IN UINT_8 ucResult, IN PUINT_8 pucSSID, IN UINT_8 ucSSIDLen,
		   IN UINT_8 ucRole)
{
	union iwreq_data evt;
	UINT_8 aucBuffer[IW_CUSTOM_MAX];
...
UINT_16 kalP2PCalP2P_IELen(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4IEIdx, IN UINT_8 ucRoleIdx)
{
 ...
 */
VOID kalP2PGenP2P_IE(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4IEIdx, IN PUINT_8 pucBuffer, IN UINT_8 ucRoleIdx)
{
	P_GL_P2P_INFO_T prGlP2pInfo = (P_GL_P2P_INFO_T)NULL;
	...
	if (prGlP2pInfo->u2P2PIELen[u4IEIdx] > 0)
		kalMemCopy(pucBuffer, prGlP2pInfo->aucP2PIE[u4IEIdx], prGlP2pInfo->u2P2PIELen[u4IEIdx]);
}
 ...
 */
VOID kalP2PUpdateP2P_IE(IN P_GLUE_INFO_T prGlueInfo, IN PUINT_8 pucBuffer, IN UINT_16 u2BufferLength,
			IN UINT_8 ucRoleIdx)
{
	P_GL_P2P_INFO_T prGlP2pInfo = (P_GL_P2P_INFO_T)NULL;
	UINT_32 u4IEIdx;
	...
	for (u4IEIdx = 0; u4IEIdx < MAX_MULTI_P2P_IE_COUNT; u4IEIdx++) {
		if (prGlP2pInfo->u2P2PIELen[u4IEIdx] == 0) {
			DBGLOG(P2P, INFO, "ucRoleIdx=%d, u4IEIdx=%d, u2BufferLength=%d\n", ucRoleIdx, u4IEIdx,
			       u2BufferLength);
			kalMemCopy(prGlP2pInfo->aucP2PIE[u4IEIdx], pucBuffer, u2BufferLength);
...
VOID kalP2PIndicateMgmtTxStatus(IN P_GLUE_INFO_T prGlueInfo, IN P_MSDU_INFO_T prMsduInfo, IN BOOLEAN fgIsAck)
{
	...
	do {
		...
		pu8GlCookie = (PUINT_64)((ULONG)prMsduInfo->prPacket + (ULONG)prMsduInfo->u2FrameLength +
		...
		cfg80211_mgmt_tx_status(prNetdevice->ieee80211_ptr, /* struct net_device * dev, */
					*pu8GlCookie, (PUINT_8)((ULONG)prMsduInfo->prPacket + MAC_TX_RESERVED_FIELD),
...
VOID kalP2PIndicateRxMgmtFrame(IN P_GLUE_INFO_T prGlueInfo, IN P_SW_RFB_T prSwRfb, IN BOOLEAN fgIsDevInterface,
			       IN UINT_8 ucRoleIdx)
{
#define DBG_P2P_MGMT_FRAME_INDICATION 1
	...
	do {
		...
		if (fgIsDevInterface)
			...
			prNetdevice = prGlueP2pInfo->aprRoleHandler;
		if (prNetdevice == NULL) {
			DBGLOG(AIS, WARN, "prNetdevice is NULL!\n");
			break;
		}
		...
		cfg80211_rx_mgmt(prNetdevice->ieee80211_ptr, /* struct net_device * dev, */
				 i4Freq, RCPI_TO_dBm(nicRxGetRcpiValueFromRxv(RCPI_MODE_WF0, prSwRfb)),
...
VOID kalP2PGCIndicateConnectionStatus(IN P_GLUE_INFO_T prGlueInfo, IN UINT_8 ucRoleIndex,
				      IN P_P2P_CONNECTION_REQ_INFO_T prP2pConnInfo, IN PUINT_8 pucRxIEBuf,
				      IN UINT_16 u2RxIELen, IN UINT_16 u2StatusReason, IN WLAN_STATUS eStatus)
{
	P_GL_P2P_INFO_T prGlueP2pInfo = (P_GL_P2P_INFO_T)NULL;
	...
	do {
		...
		if (prP2pConnInfo) {
			cfg80211_connect_result(prGlueP2pInfo->aprRoleHandler,
						...
						prP2pConnInfo->u4BufLength, pucRxIEBuf, u2RxIELen, u2StatusReason,
						GFP_KERNEL);
			/* gfp_t gfp */ /* allocation flags */
...
VOID kalP2PGOStationUpdate(IN P_GLUE_INFO_T prGlueInfo, IN UINT_8 ucRoleIndex, IN P_STA_RECORD_T prCliStaRec,
			   IN BOOLEAN fgIsNew)
{
	...
	do {
		...
		if (fgIsNew) {
			...
			if (prP2pGlueInfo->aprRoleHandler && prP2pGlueInfo->aprRoleHandler->ieee80211_ptr &&
			    prP2pGlueInfo->aprRoleHandler->ieee80211_ptr->wiphy)
...
VOID kalP2PRddDetectUpdate(IN P_GLUE_INFO_T prGlueInfo, IN UINT_8 ucRoleIndex)
{
	...
	do {
		...
		netif_device_detach(prGlueInfo->prP2PInfo[ucRoleIndex]->prDevHandler);
			...
			if (UNEQUAL_MAC_ADDR(rbssid, aucNullAddr)) {
				if (UNEQUAL_MAC_ADDR(&(prGlueInfo->prP2PInfo[ucRoleIndex]->aucblackMACList[i]),
					...
					if (EQUAL_MAC_ADDR(&(prGlueInfo->prP2PInfo[ucRoleIndex]->aucblackMACList[i]),
							   aucNullAddr)) {
						COPY_MAC_ADDR(&(prGlueInfo->prP2PInfo[ucRoleIndex]->aucblackMACList[i]),
							      rbssid);
						fgIsValid = FALSE;
						return fgIsValid;
					}

...
int mtk_cfg80211_change_iface(struct wiphy *wiphy, struct net_device *ndev, enum nl80211_iftype type,
			      struct vif_params *params)
#else
int mtk_cfg80211_change_iface(struct wiphy *wiphy, struct net_device *ndev, enum nl80211_iftype type, u32 *flags,
			      struct vif_params *params)
#endif
{
	...
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_change_iface do nothing\n");
		return -EINVAL;
	}
#endif
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetInfrastructureMode, &eOpMode, sizeof(eOpMode), FALSE, FALSE, TRUE,
			   &u4BufLen);
			 ...
			 struct key_params *params)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(RSN, ERROR, "chip resetting, mtk_cfg80211_add_key do nothing\n");
		return -EINVAL;
	}
#endif
	...
	if (mac_addr) {
		DBGLOG(RSN, INFO, "keyIdx = %d pairwise = %d mac = " MACSTR "\n", key_index, pairwise,
		       MAC2STR(mac_addr));
	} else {
 ...
 */
/*----------------------------------------------------------------------------*/
int mtk_cfg80211_get_key(struct wiphy *wiphy, struct net_device *ndev, u8 key_index, bool pairwise, const u8 *mac_addr,
...
int mtk_cfg80211_del_key(struct wiphy *wiphy, struct net_device *ndev, u8 key_index, bool pairwise, const u8 *mac_addr)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetRemoveKey, &rRemoveKey, rRemoveKey.u4Length, FALSE, FALSE, TRUE,
			   &u4BufLen);
				 ...
				 bool multicast)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetDefaultKey, &rDefaultKey, sizeof(PARAM_DEFAULT_KEY_T), FALSE, FALSE,
			   TRUE, &u4BufLen);
	if (rStatus == WLAN_STATUS_SUCCESS)
		i4Rst = 0;
...
int mtk_cfg80211_get_station(struct wiphy *wiphy, struct net_device *ndev, const u8 *mac, struct station_info *sinfo)
{
	...
	if (prGlueInfo->eParamMediaStateIndicated != PARAM_MEDIA_STATE_CONNECTED) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidQueryLinkSpeed, &u4Rate, sizeof(u4Rate), TRUE, FALSE, FALSE,
				   &u4BufLen);
	...
	if (prGlueInfo->eParamMediaStateIndicated != PARAM_MEDIA_STATE_CONNECTED) {
		...
		rStatus =
			kalIoctl(prGlueInfo, wlanoidQueryRssi, &i4Rssi, sizeof(i4Rssi), TRUE, FALSE, FALSE, &u4BufLen);
	...
	if (prDevStats) {
		...
		if (!prQueryStaStatistics) {
			...
			kalMemZero(prQueryStaStatistics, sizeof(PARAM_GET_STA_STA_STATISTICS));
			...
			if (rStatus != WLAN_STATUS_SUCCESS) {
				...
				DBGLOG(REQ, INFO, "BSSID: [" MACSTR "] TxFailCount %d LifeTimeOut %d\n",
				       MAC2STR(arBssid), prQueryStaStatistics->u4TxFailCount,
				       prQueryStaStatistics->u4TxLifeTimeoutCount);

				u4TotalError = prQueryStaStatistics->u4TxFailCount +
					       prQueryStaStatistics->u4TxLifeTimeoutCount;
...
int mtk_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	...
	if (request->n_channels > MAXIMUM_OPERATION_CHANNEL_LIST) {
		...
		for (i = 0; i < request->n_channels; i++) {
			rScanRequest.arChnlInfoList[i].eBand = kalCfg80211ToMtkBand(request->channels[i]->band);
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetBssidListScanAdv, &rScanRequest, sizeof(PARAM_SCAN_REQUEST_ADV_T),
...
void mtk_cfg80211_abort_scan(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidAbortScan, NULL, 1, FALSE, FALSE, TRUE, &u4SetInfoLen);
	if (rStatus != WLAN_STATUS_SUCCESS) {
...
int mtk_cfg80211_auth(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_auth_request *req)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_auth do nothing\n");
		return -EINVAL;
	}
#endif
	...
	if (req->key_len != 0) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetAddWep, prWepKey, prWepKey->u4Length, FALSE, FALSE, TRUE,
				   &u4BufLen);
	...
	if (!prConnSettings->fgIsConnInitialized || fgNewAuthParam) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetConnect, (void *)&rNewSsid, sizeof(PARAM_CONNECT_T), FALSE,
				   FALSE, TRUE, &u4BufLen);
		 ...
		 */
		prStaRec = cnmGetStaRecByAddress(prGlueInfo->prAdapter, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex,
						 rNewSsid.pucBssid);
		if (prStaRec)
...
int mtk_cfg80211_connect(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_connect_params *sme)
{
	...
	if (sme->ie && sme->ie_len > 0) {
...
#if CFG_SUPPORT_WAPI
		rStatus = kalIoctl(prGlueInfo, wlanoidSetWapiAssocInfo, pucIEStart, sme->ie_len, FALSE, FALSE, FALSE,
				   &u4BufLen);

		if (rStatus != WLAN_STATUS_SUCCESS)
			DBGLOG(SEC, WARN, "[wapi] set wapi assoc info error:0x%x\n", rStatus);
#endif
#if CFG_SUPPORT_WPS2
		if (wextSrchDesiredWPSIE(pucIEStart, sme->ie_len, 0xDD, (PUINT_8 *)&prDesiredIE)) {
			...
			rStatus = kalIoctl(prGlueInfo, wlanoidSetWSCAssocInfo, prDesiredIE, IE_SIZE(prDesiredIE), FALSE,
					   FALSE, FALSE, &u4BufLen);
			if (rStatus != WLAN_STATUS_SUCCESS)
				DBGLOG(SEC, WARN, "[WSC] set WSC assoc info error:0x%x\n", rStatus);
		}
#endif
#if CFG_SUPPORT_PASSPOINT
		if (wextSrchDesiredHS20IE(pucIEStart, sme->ie_len, (PUINT_8 *)&prDesiredIE)) {
			rStatus = kalIoctl(prGlueInfo, wlanoidSetHS20Info, prDesiredIE, IE_SIZE(prDesiredIE), FALSE,
					   FALSE, TRUE, &u4BufLen);
#if 0
			if (rStatus != WLAN_STATUS_SUCCESS)
		...
		if (wextSrchDesiredInterworkingIE(pucIEStart, sme->ie_len, (PUINT_8 *)&prDesiredIE)) {
			rStatus = kalIoctl(prGlueInfo, wlanoidSetInterworkingInfo, prDesiredIE, IE_SIZE(prDesiredIE),
		...
		if (wextSrchDesiredRoamingConsortiumIE(pucIEStart, sme->ie_len, (PUINT_8 *)&prDesiredIE)) {
			rStatus = kalIoctl(prGlueInfo, wlanoidSetRoamingConsortiumIEInfo, prDesiredIE,
		...
		if (cfg80211_get_non_wfa_vendor_ie(prGlueInfo, pucIEStart, sme->ie_len) > 0) {
	...
	rStatus =
		kalIoctl(prGlueInfo, wlanoidSetAuthMode, &eAuthMode, sizeof(eAuthMode), FALSE, FALSE, FALSE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS)
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetEncryptionStatus, &eEncStatus, sizeof(eEncStatus), FALSE, FALSE, FALSE,
			   &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS)
	...
	rNewSsid.u4SsidLen = sme->ssid_len;
	rStatus = kalIoctl(prGlueInfo, wlanoidSetConnect, (PVOID)&rNewSsid, sizeof(PARAM_CONNECT_T), FALSE, FALSE, TRUE,
			   &u4BufLen);
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_disconnect do nothing\n");
		return -EINVAL;
	}
#endif
...
int mtk_cfg80211_deauth(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_deauth_request *req)
{
	...
	if (prGlueInfo->prAdapter->rWifiVar.rConnSettings.bss) {
		...
		cfg80211_assoc_timeout(ndev, prGlueInfo->prAdapter->rWifiVar.rConnSettings.bss);
#else
		cfg80211_send_assoc_timeout(ndev, prGlueInfo->prAdapter->rWifiVar.rConnSettings.bss->bssid);
#endif
...
int mtk_cfg80211_join_ibss(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_ibss_params *params)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_join_ibss do nothing\n");
		return -EINVAL;
	}
#endif
	...
	if (params->channel_fixed) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetFrequency, &u4ChnlFreq, sizeof(u4ChnlFreq), FALSE, FALSE,
				   FALSE, &u4BufLen);
		if (rStatus != WLAN_STATUS_SUCCESS)
			return -EFAULT;
	}
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetSsid, (PVOID)&rNewSsid, sizeof(PARAM_SSID_T), FALSE, FALSE, TRUE,
			   &u4BufLen);
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_leave_ibss do nothing\n");
		return -EINVAL;
	}
#endif
...
int mtk_cfg80211_set_power_mgmt(struct wiphy *wiphy, struct net_device *ndev, bool enabled, int timeout)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSet802dot11PowerSaveProfile, &rPowerMode, sizeof(PARAM_POWER_MODE_T),
			   FALSE, FALSE, TRUE, &u4BufLen);
#if CFG_CHIP_RESET_SUPPORT
	rst_data.entry_conut--;
	DBGLOG(INIT, TRACE, "entry_conut = %d\n", rst_data.entry_conut);
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_set_pmksa do nothing\n");
		return -EINVAL;
	}
#endif
...
void mtk_cfg80211_mgmt_frame_register(IN struct wiphy *wiphy, IN struct wireless_dev *wdev, IN u16 frame_type,
				      IN bool reg)
{
#if 0
 ...
 */
/*----------------------------------------------------------------------------*/
int mtk_cfg80211_remain_on_channel(struct wiphy *wiphy, struct wireless_dev *wdev, struct ieee80211_channel *chan,
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_remain_on_channel do nothing\n");
		return -EINVAL;
	}
#endif
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_cancel_remain_on_channel do nothing\n");
		return -EINVAL;
	}
#endif
...
int mtk_cfg80211_mgmt_tx(struct wiphy *wiphy, struct wireless_dev *wdev, struct cfg80211_mgmt_tx_params *params,
			 u64 *cookie)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	INT_32 i4Rslt = -EINVAL;
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_mgmt_tx do nothing\n");
		return -EINVAL;
	}
#endif
...
int mtk_cfg80211_mgmt_tx(struct wiphy *wiphy, struct wireless_dev *wdev, struct ieee80211_channel *channel,
			 bool offscan, unsigned int wait, const u8 *buf, size_t len, bool no_cck,
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting,mtk_cfg80211_mgmt_tx_cancel_wait do nothing\n");
		return -EINVAL;
	}
#endif
...
int mtk_cfg80211_testmode_hs20_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_testmode_hs20_cmd do nothing\n");
		return -EINVAL;
	}
#endif
	...
	if (prParams) {
		...
		case HS20_CMD_ID_SET_BSSID_POOL:
			DBGLOG(INIT, INFO, "[%s] fgBssidPoolIsEnable (%d)\n", __func__,
			       prParams->hs20_set_bssid_pool.fgBssidPoolIsEnable);
			DBGLOG(INIT, INFO, "[%s] ucNumBssidPool (%d)\n", __func__,
			...
			rstatus = kalIoctl(prGlueInfo, (PFN_OID_HANDLER_FUNC)wlanoidSetHS20BssidPool,
					   &prParams->hs20_set_bssid_pool, sizeof(struct param_hs20_set_bssid_pool),
...
int mtk_cfg80211_testmode_set_key_ext(IN struct wiphy *wiphy, IN void *data, IN int len)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_testmode_set_key_ext do nothing\n");
		return -EINVAL;
	}
#endif
	...
	if (prIWEncExt->alg == IW_ENCODE_ALG_SMS4) {
		...
		rstatus = kalIoctl(prGlueInfo, wlanoidSetAddKey, prWpiKey, sizeof(PARAM_KEY_T), FALSE, FALSE, TRUE,
				   &u4BufLen);
...
int mtk_cfg80211_testmode_get_sta_statistics(IN struct wiphy *wiphy, IN void *data, IN int len,
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_testmode_get_sta_statistics do nothing\n");
	}
#endif
...
int mtk_cfg80211_testmode_sw_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	...
	if (prParams) {
		if (prParams->set == 1) {
			rstatus = kalIoctl(prGlueInfo, (PFN_OID_HANDLER_FUNC)wlanoidSetSwCtrlWrite, &prParams->adr,
...
int mtk_cfg80211_sched_scan_start(IN struct wiphy *wiphy, IN struct net_device *ndev,
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_sched_scan_start do nothing\n");
		return -EINVAL;
	}
#endif
...
#endif
	rStatus = kalIoctl(prGlueInfo, wlanoidSetStartSchedScan, prSchedScanRequest, sizeof(PARAM_SCHED_SCAN_REQUEST),
			   FALSE, FALSE, TRUE, &u4BufLen);
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState()) {
		DBGLOG(REQ, ERROR, "chip resetting, mtk_cfg80211_sched_scan_stop do nothing\n");
		return -EINVAL;
	}
#endif
...
int mtk_cfg80211_assoc(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_assoc_request *req)
{
	...
	if (req->ie && req->ie_len > 0) {
...
#if CFG_SUPPORT_WAPI
		rStatus = kalIoctl(prGlueInfo, wlanoidSetWapiAssocInfo, pucIEStart, req->ie_len, FALSE, FALSE, FALSE,
				   &u4BufLen);
		...
		if (wextSrchDesiredWPAIE(pucIEStart, req->ie_len, 0x30, (PPUINT_8)&prDesiredIE)) {
			if (rsnParseRsnIE(prGlueInfo->prAdapter, (P_RSN_INFO_ELEM_T)prDesiredIE, &rRsnInfo)) {
				...
				if (rRsnInfo.u2PmkidCnt > 0)
					kalMemCopy(prConnSettings->rRsnInfo.aucPmkidList, rRsnInfo.aucPmkidList,
						   (rRsnInfo.u2PmkidCnt * RSN_PMKID_LEN));
			}
		}

		/* Find non-wfa vendor specific ies set from upper layer */
		if (cfg80211_get_non_wfa_vendor_ie(prGlueInfo, pucIEStart, req->ie_len) > 0) {
		...
		if (wextSrchDesiredWPAIE(pucIEStart, req->ie_len, 0xff, (PPUINT_8)&prDesiredIE)) {
			...
			kalMemSet(&prGlueInfo->prAdapter->rWifiVar.rConnSettings.rOweInfo, 0,
				  sizeof(struct OWE_INFO_T));
		}
#endif
#endif
	}
	...
	if (!prConnSettings->fgIsConnInitialized) {
		rStatus = kalIoctl(prGlueInfo, wlanoidSetBssid, (void *)req->bss->bssid, MAC_ADDR_LEN, FALSE, FALSE,
				   TRUE, &u4BufLen);
...
#else
	rStatus = kalIoctl(prGlueInfo, wlanoidSetBssid, (void *)req->bss->bssid, MAC_ADDR_LEN, FALSE, FALSE, TRUE,
			   &u4BufLen);
...
/*----------------------------------------------------------------------------*/
#if KERNEL_VERSION(3, 16, 0) <= CFG80211_VERSION_CODE
int mtk_cfg80211_change_station(struct wiphy *wiphy, struct net_device *ndev, const u8 *mac,
				struct station_parameters *params)
{
	...
	if (prAisBssInfo->fgTdlsIsChSwProhibited) {
		...
		rStatus = kalIoctl(prGlueInfo, TdlsSendChSwControlCmd, &TdlsSendChSwControlCmd,
...
#else
int mtk_cfg80211_change_station(struct wiphy *wiphy, struct net_device *ndev, u8 *mac,
				struct station_parameters *params)
{
	/* return 0; */
			   ...
			   enum nl80211_tdls_operation oper)
{
	...
	rCmdOper.oper = (ENUM_TDLS_LINK_OPER)((INT_32)oper);
	...
	kalIoctl(prGlueInfo, TdlsexLinkOper, &rCmdOper, sizeof(TDLS_CMD_LINK_OPER_T), FALSE, FALSE, FALSE, &u4BufLen);
...
void mtk_apply_custom_regulatory(IN struct wiphy *pWiphy, IN const struct ieee80211_regdomain *pRegdom)
{
	u32 band_idx, ch_idx;
...
void mtk_reg_notify(IN struct wiphy *pWiphy, IN struct regulatory_request *pRequest)
{
	...
	if (pRequest->initiator != NL80211_REGDOM_SET_BY_DRIVER) {
		...
		if (rlmDomainIsEfuseUsed()) {
			...
			/* iwpriv case */
			if (rlmDomainIsUsingLocalRegDomainDataBase() && (!rlmDomainIsEfuseUsed())) {
	...
	while (pos < end) {
		...
		if (ie_oui != WLAN_OUI_MICROSOFT && ie_oui != WLAN_OUI_WFA) {
			...
			if (max_len - (*ret_len) >= ie->len + 2) {
				DBGLOG(AIS, TRACE, "vendor ie(len=%d, oui=0x%06x)\n", ie->len, ie_oui);
				 ...
				 * and return.
				 */
				DBGLOG(AIS, ERROR, "insufficient buf for vendor ie, exit!\n");
				break;
			}
		}
cont:

...
BOOLEAN
mtk_p2p_cfg80211func_channel_sco_switch(IN enum nl80211_channel_type channel_type, IN P_ENUM_CHNL_EXT_T prChnlSco)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState())
		DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211func_channel_format_switch\n");
#endif
...
static void mtk_vif_destructor(struct net_device *dev)
{
	...
	if (dev) {
		...
		if (prWdev) {
			...
			for (u4Idx = 0; u4Idx < KAL_P2P_NUM; u4Idx++) {
				if ((prWdev == gprP2pRoleWdev[u4Idx]) && (gprP2pRoleWdev[u4Idx] != gprP2pWdev)) {
					DBGLOG(P2P, INFO, "mtk_newInf_destructor remove added Wd\n");
					gprP2pRoleWdev[u4Idx] = gprP2pWdev;
					if (prNetDevPriv) {
						prNetDevPriv->prGlueInfo->prP2PInfo[u4Idx]->aprRoleHandler =
							prNetDevPriv->prGlueInfo->prP2PInfo[u4Idx]->prDevHandler;
					}
				}
			}
			kfree(prWdev);
...
#else
struct wireless_dev *mtk_p2p_cfg80211_add_iface(struct wiphy *wiphy, const char *name, enum nl80211_iftype type,
...
#endif
{
	...
	do {
		...
		for (u4Idx = 0; u4Idx < KAL_P2P_NUM; u4Idx++) {
			if (prGlueInfo->prP2PInfo[u4Idx]->aprRoleHandler == prGlueInfo->prP2PInfo[u4Idx]->prDevHandler)
				break;
			if (prGlueInfo->prP2PInfo[u4Idx]->aprRoleHandler == NULL) {
		...
		prNewNetDevice = alloc_netdev_mq(sizeof(NETDEV_PRIVATE_GLUE_INFO), name, NET_NAME_PREDICTABLE,
		...
		*((P_GLUE_INFO_T *)netdev_priv(prNewNetDevice)) = prGlueInfo;
		...
		/* Switch OP MOde. */
		prSwitchModeMsg = (P_MSG_P2P_SWITCH_OP_MODE_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG,
		...
		prMsgActiveBss = (P_MSG_P2P_ACTIVE_DEV_BSS_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG,
...
int mtk_p2p_cfg80211_del_iface(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	prP2pDelIfaceMsg =
		(P_MSG_P2P_DEL_IFACE_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG, sizeof(MSG_P2P_DEL_IFACE_T));
...
int mtk_p2p_cfg80211_add_key(struct wiphy *wiphy, struct net_device *ndev, u8 key_index, bool pairwise,
...
int mtk_p2p_cfg80211_get_key(struct wiphy *wiphy, struct net_device *ndev, u8 key_index, bool pairwise,
...
int mtk_p2p_cfg80211_del_key(struct wiphy *wiphy, struct net_device *ndev, u8 key_index, bool pairwise,
			     const u8 *mac_addr)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
...
int mtk_p2p_cfg80211_set_default_key(struct wiphy *wiphy, struct net_device *netdev, u8 key_index, bool unicast,
				     bool multicast)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
...
int mtk_p2p_cfg80211_get_station(struct wiphy *wiphy, struct net_device *ndev, const u8 *mac,
...
int mtk_p2p_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	...
	do {
		...
		u4MsgSize = sizeof(MSG_P2P_SCAN_REQUEST_T) + (request->n_channels * sizeof(RF_CHANNEL_INFO_T)) +
		...
		for (u4Idx = 0; u4Idx < request->n_ssids; u4Idx++) {
			COPY_SSID(prSsidStruct->aucSsid, prSsidStruct->ucSsidLen, request->ssids->ssid,
		...
		kalMemCopy(&(prP2pGlueDevInfo->rBackupScanRequest), request, sizeof(struct cfg80211_scan_request));
...
int mtk_p2p_cfg80211_set_txpower(struct wiphy *wiphy, struct wireless_dev *wdev, enum nl80211_tx_power_setting type,
				 int mbm)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
...
int mtk_p2p_cfg80211_set_power_mgmt(struct wiphy *wiphy, struct net_device *ndev, bool enabled, int timeout)
{
	...
	/* p2p_set_power_save */
	kalIoctl(prGlueInfo, wlanoidSet802dot11PowerSaveProfile, &rPowerMode, sizeof(rPowerMode), FALSE, FALSE, TRUE,
		 &u4Leng);
...
int mtk_p2p_cfg80211_start_ap(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_ap_settings *settings)
{
	...
	do {
		...
		if (prGlueInfo->prP2PInfo[ucRoleIdx]->fgIsNetDevDetach) {
			...
			prGlueInfo->prP2PInfo[ucRoleIdx]->fgIsNetDevDetach = FALSE;
		}
		...
		if (chandef) {
			...
			/* Follow the channel info from wifi.cfg prior to hostapd.conf */
			{
				...
				if ((prWifiVar->ucApChannel != 0) && (prWifiVar->ucApChnlDefFromCfg != 0) &&
		...
		prP2pStartAPMsg = (P_MSG_P2P_START_AP_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG,
						       ...
						       struct cfg80211_chan_def *chandef, unsigned int cac_time_ms)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState())
		DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_start_radar_detection_impl\n");
#endif
	...
	do {
		...
		if (prGlueInfo->prP2PInfo[ucRoleIdx]->chandef == NULL) {
			prGlueInfo->prP2PInfo[ucRoleIdx]->chandef = (struct cfg80211_chan_def *)cnmMemAlloc(
				prGlueInfo->prAdapter, RAM_TYPE_BUF, sizeof(struct cfg80211_chan_def));

			prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan = (struct ieee80211_channel *)cnmMemAlloc(
				prGlueInfo->prAdapter, RAM_TYPE_BUF, sizeof(struct ieee80211_channel));
		}
		...
		prP2pDfsCacMsg =
			(P_MSG_P2P_DFS_CAC_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG, sizeof(*prP2pDfsCacMsg));
		...
		default:
			DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_start_radar_detection. !!!Bandwidth do not support!!!\n");
			ASSERT(FALSE);
			break;
		}
...
int mtk_p2p_cfg80211_start_radar_detection(struct wiphy *wiphy, struct net_device *dev,
					   struct cfg80211_chan_def *chandef, unsigned int cac_time_ms)
{
	return mtk_p2p_cfg80211_start_radar_detection_impl(wiphy, dev, chandef, cac_time_ms);
}
#else
int mtk_p2p_cfg80211_start_radar_detection(struct wiphy *wiphy, struct net_device *dev,
					   struct cfg80211_chan_def *chandef)
{
	return mtk_p2p_cfg80211_start_radar_detection_impl(wiphy, dev, chandef, IEEE80211_DFS_MIN_CAC_TIME_MS);
}
#endif
...
int mtk_p2p_cfg80211_channel_switch(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_csa_settings *params)
{
	...
	do {
		...
		mtk_p2p_cfg80211func_channel_format_switch(&params->chandef, params->chandef.chan, &rRfChnlInfo);

		p2pFuncSetChannel(prGlueInfo->prAdapter, ucRoleIdx, &rRfChnlInfo);

		DBGLOG(P2P, INFO, "mtk_p2p_cfg80211_channel_switch.(role %d)\n", ucRoleIdx);

		if (prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan->dfs_state == NL80211_DFS_AVAILABLE
#if KERNEL_VERSION(3, 15, 0) <= CFG80211_VERSION_CODE
		    && prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan->dfs_cac_ms != 0
#endif
		)
		...
		prP2pSetNewChannelMsg = (P_MSG_P2P_SET_NEW_CHANNEL_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG,
		...
		default:
			DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_channel_switch. !!!Bandwidth do not support!!!\n");
			ASSERT(FALSE);
			break;
		}
		...
		if ((params->beacon_csa.head_len != 0) || (params->beacon_csa.tail_len != 0)) {
			u4Len = sizeof(MSG_P2P_BEACON_UPDATE_T) + params->beacon_csa.head_len +
				params->beacon_csa.tail_len;
			prP2pBcnUpdateMsg =
				(P_MSG_P2P_BEACON_UPDATE_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG, u4Len);
			...
			mboxSendMsg(prGlueInfo->prAdapter, MBOX_ID_0, (P_MSG_HDR_T)prP2pBcnUpdateMsg,
				    MSG_SEND_METHOD_BUF);
...
int mtk_p2p_cfg80211_change_beacon(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_beacon_data *info)
{
	...
	do {
		...
		if ((info->head_len != 0) || (info->tail_len != 0)) {
			u4Len = sizeof(MSG_P2P_BEACON_UPDATE_T) + info->head_len + info->tail_len +
...
int mtk_p2p_cfg80211_remain_on_channel(struct wiphy *wiphy, struct wireless_dev *wdev, struct ieee80211_channel *chan,
				       unsigned int duration, u64 *cookie)
{
	...
	do {
		...
		*cookie = prGlueP2pDevInfo->u8Cookie++;
		...
		mtk_p2p_cfg80211func_channel_format_switch(NULL, chan, &prMsgChnlReq->rChannelInfo);
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState())
		DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_cancel_remain_on_channel\n");
#endif
...
int mtk_p2p_cfg80211_mgmt_tx(struct wiphy *wiphy, struct wireless_dev *wdev, struct cfg80211_mgmt_tx_params *params,
			     u64 *cookie)
{
	P_GLUE_INFO_T prGlueInfo = (P_GLUE_INFO_T)NULL;
	P_GL_P2P_INFO_T prGlueP2pInfo = (P_GL_P2P_INFO_T)NULL;
	...
	do {
		...
		*cookie = prGlueInfo->u8Cookie++;
		...
		if (params->offchan) {
			...
			mtk_p2p_cfg80211func_channel_format_switch(NULL, params->chan, &prMsgTxReq->rChannelInfo);
...
int mtk_p2p_cfg80211_mgmt_tx(struct wiphy *wiphy, struct wireless_dev *wdev, struct ieee80211_channel *chan,
			     bool offchan, unsigned int wait, const u8 *buf, size_t len, bool no_cck,
	...
	do {
		...
		*cookie = prGlueP2pDevInfo->u8Cookie++;
		...
		if (offchan) {
			...
			mtk_p2p_cfg80211func_channel_format_switch(NULL, chan, &prMsgTxReq->rChannelInfo);
			mtk_p2p_cfg80211func_channel_sco_switch(NL80211_CHAN_NO_HT, &prMsgTxReq->eChnlExt);
		} else {
			prMsgTxReq->fgIsOffChannel = FALSE;
		}
...
int mtk_p2p_cfg80211_del_station(struct wiphy *wiphy, struct net_device *dev, struct station_del_parameters *params)
{
	...
	do {
		...
		prDisconnectMsg = (P_MSG_P2P_CONNECTION_ABORT_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG,
...
int mtk_p2p_cfg80211_connect(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	do {
		...
		prConnReqMsg = (P_MSG_P2P_CONNECTION_REQUEST_T)cnmMemAlloc(
			prGlueInfo->prAdapter, RAM_TYPE_MSG, (sizeof(MSG_P2P_CONNECTION_REQUEST_T) + sme->ie_len));
		...
		mtk_p2p_cfg80211func_channel_format_switch(NULL, sme->channel, &prConnReqMsg->rChannelInfo);
...
int mtk_p2p_cfg80211_disconnect(struct wiphy *wiphy, struct net_device *dev, u16 reason_code)
{
	...
	do {
		...
		if (mtk_Netdev_To_RoleIdx(prGlueInfo, dev, &ucRoleIdx) < 0)
			break;
		/* prDisconnMsg = (P_MSG_P2P_CONNECTION_ABORT_T)kalMemAlloc(sizeof(P_MSG_P2P_CONNECTION_ABORT_T),
		 * VIR_MEM_TYPE); */
		prDisconnMsg = (P_MSG_P2P_CONNECTION_ABORT_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG,
									 sizeof(MSG_P2P_CONNECTION_ABORT_T));
...
#if KERNEL_VERSION(4, 12, 0) <= CFG80211_VERSION_CODE
int mtk_p2p_cfg80211_change_iface(IN struct wiphy *wiphy, IN struct net_device *ndev, IN enum nl80211_iftype type,
...
int mtk_p2p_cfg80211_set_bitrate_mask(IN struct wiphy *wiphy, IN struct net_device *dev, IN const u8 *peer,
...
void mtk_p2p_cfg80211_mgmt_frame_register(IN struct wiphy *wiphy, struct wireless_dev *wdev, IN u16 frame_type,
					  IN bool reg)
{
#if 0
...
#if GO_STA_SCC
		case 0x50:
			4Status = mtk_p2p_cfg80211_go_channel_switch(wiphy, wdev, data, len);
			break
#endif

				default : i4Status = -EINVAL;
			break;
		}
	}
...
int mtk_p2p_cfg80211_testmode_p2p_sigma_pre_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState())
		DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_testmode_p2p_sigma_pre_cmd\n");
#endif
	...
	case 106: /* P2P set sleep  */
	{
		...
		kalIoctl(prGlueInfo, wlanoidSet802dot11PowerSaveProfile, &rPowerMode, sizeof(rPowerMode), FALSE, FALSE,
			 TRUE, &u4Leng);
	} break;
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState())
		DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_testmode_p2p_sigma_cmd\n");
#endif
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState())
		DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_testmode_wfd_update_cmd\n");
#endif
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState())
		DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_testmode_hotspot_block_list_cmd\n");
#endif
...
int mtk_p2p_cfg80211_testmode_get_best_channel(IN struct wiphy *wiphy, IN void *data, IN int len)
{
...
#if CFG_CHIP_RESET_SUPPORT
	if (checkResetState())
		DBGLOG(P2P, ERROR, "mtk_p2p_cfg80211_testmode_get_best_channel\n");
#endif
	 ...
	 */
	rlmDomainGetChnlList(prGlueInfo->prAdapter, BAND_2G4, TRUE, MAX_2G_BAND_CHN_NUM, &ucNumOfChannel,
			     aucChannelList);
	...
	if (prQueryLteChn == NULL) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidQueryLteSafeChannel, prQueryLteChn, sizeof(PARAM_GET_CHN_INFO),
		...
		u4LteSafeChnBitMask_2G =
			prQueryLteChn->rLteSafeChnList
				.au4SafeChannelBitmask[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_2G_BASE_1 - 1];
...
#if GO_STA_SCC
int mtk_p2p_cfg80211_go_channel_switch(IN struct wiphy *wiphy, IN struct wireless_dev *wdev, IN void *data, IN int len)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	NT_32 i4Status = -EINVAL struct net_device *dev = (struct net_device *)NULL;
	P_MSG_P2P_SET_NEW_CHANNEL_T prP2pSetNewChannelMsg = (P_MSG_P2P_SET_NEW_CHANNEL_T)NULL;
	UINT_8 ucRoleIdx = 0;
	RF_CHANNEL_INFO_T rRfChnlInfo;
	...
	/* Copy chan def to local buffer*/
	prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->center_freq1 = chan_def->center_freq1;
	prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->center_freq2 = chan_def->center_freq2;
	prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->width = chan_def->width;
	memcpy(prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan, chan_def->chan, sizeof(struct ieee80211_channel));

	mtk_p2p_cfg80211func_channel_format_switch(chan_def, chan_def->chan, &rRfChnlInfo);
	p2pFuncSetChannel(prGlueInfo->prAdapter, ucRoleIdx, &rRfChnlInfo);
	/* Set new channel parameters */
	prP2pSetNewChannelMsg = (P_MSG_P2P_SET_NEW_CHANNEL_T)cnmMemAlloc(prGlueInfo->prAdapter, RAM_TYPE_MSG,
									 sizeof(*prP2pSetNewChannelMsg));
	...
	default:
		DBGLOG(P2P, ERROR, "Bandwidth do not support!!!\n");
		i4Status = -EINVAL;
		return i4Status;
	}

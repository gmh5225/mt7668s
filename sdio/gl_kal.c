...
static PUINT_8 apucPatchName[] = { (PUINT_8) "mt7668s_patch_e1_hdr.bin", (PUINT_8) "mt7666_patch_e1_hdr.bin", NULL };
...
VOID kalConstructDefaultFirmwarePrio(P_GLUE_INFO_T prGlueInfo, PPUINT_8 apucNameTable, PPUINT_8 apucName,
				     PUINT_8 pucNameIdx, UINT_8 ucMaxNameIdx)
{
	...
	for (sub_idx = 0; apucNameTable[sub_idx]; sub_idx++) {
		if ((*pucNameIdx + 3) < ucMaxNameIdx) {
			...
			snprintf(*(apucName + (*pucNameIdx)), FILE_NAME_MAX, "%s%x_E%u", apucNameTable[sub_idx],
				 chip_id, wlanGetEcoVersion(prGlueInfo->prAdapter));
			(*pucNameIdx) += 1;

			/* Type 2. WIFI_RAM_CODE_MTxxxx_Ex.bin */
			snprintf(*(apucName + (*pucNameIdx)), FILE_NAME_MAX, "%s%x_E%u.bin", apucNameTable[sub_idx],
				 chip_id, wlanGetEcoVersion(prGlueInfo->prAdapter));
			(*pucNameIdx) += 1;

			/* Type 3. WIFI_RAM_CODE_MTxxxx */
			snprintf(*(apucName + (*pucNameIdx)), FILE_NAME_MAX, "%s%x", apucNameTable[sub_idx], chip_id);
			(*pucNameIdx) += 1;

			/* Type 4. WIFI_RAM_CODE_MTxxxx.bin */
			snprintf(*(apucName + (*pucNameIdx)), FILE_NAME_MAX, "%s%x.bin", apucNameTable[sub_idx],
				 chip_id);
			(*pucNameIdx) += 1;
		} else {
...
PVOID
kalFirmwareImageMapping(IN P_GLUE_INFO_T prGlueInfo, OUT PPVOID ppvMapFileBuf, OUT PUINT_32 pu4FileLength,
			IN ENUM_IMG_DL_IDX_T eDlIdx)
{
	PPUINT_8 apucNameTable = NULL;
	PUINT_8 apucName[FILE_NAME_TOTAL + 1]; /* extra +1, for the purpose of
...
WLAN_STATUS kalRxIndicateOnePkt(IN P_GLUE_INFO_T prGlueInfo, IN PVOID pvPkt)
{
	...
	if (GLUE_GET_PKT_BSS_IDX(prSkb) < HW_BSSID_NUM) {
		prNetDev = (struct net_device *)wlanGetNetInterfaceByBssIdx(prGlueInfo, GLUE_GET_PKT_BSS_IDX(prSkb));
	} else {
		/* Bssindex from fw pkt is illegal */
		DBGLOG(RX, ERROR, "WLAN Error[%s-%d]:-----Bss IDX got from fw pakcet is wrong------.\n", __func__,
		       __LINE__);
	}
	...
	if (prSkb->protocol == NTOHS(ETH_P_8021Q) && !FEAT_SUP_LLC_VLAN_RX(prChipInfo)) {
		/*
 ...
 */
/*----------------------------------------------------------------------------*/
VOID kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf,
				  IN UINT_32 u4BufLen)
{
	UINT_32 bufLen;
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		do {
			...
			ssid.aucSsid[(ssid.u4SsidLen >= PARAM_MAX_LEN_SSID) ? (PARAM_MAX_LEN_SSID - 1) :
									      ssid.u4SsidLen] = '\0';
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			/* only loop 15 times to avoid dead loop */
			UINT_8 ucLoopCnt = 15;
			...
			bss = cfg80211_get_bss(priv_to_wiphy(prGlueInfo), prChannel, arBssid, ssid.aucSsid,
					       ssid.u4SsidLen, IEEE80211_BSS_TYPE_ESS, IEEE80211_PRIVACY_ANY);
#else
			bss = cfg80211_get_bss(priv_to_wiphy(prGlueInfo), prChannel, arBssid, ssid.aucSsid,
					       ssid.u4SsidLen, WLAN_CAPABILITY_ESS, WLAN_CAPABILITY_ESS);
#endif

			if (bss == NULL) {
				/* create BSS on-the-fly */
				prBssDesc = ((P_AIS_FSM_INFO_T)(&(prGlueInfo->prAdapter->rWifiVar.rAisFsmInfo)))
						    ->prTargetBssDesc;

				if (prChannel && prBssDesc != NULL) {
#if KERNEL_VERSION(3, 18, 0) <= CFG80211_VERSION_CODE
					bss = cfg80211_inform_bss(priv_to_wiphy(prGlueInfo), prChannel,
								  CFG80211_BSS_FTYPE_PRESP, arBssid, 0, /* TSF */
								  WLAN_CAPABILITY_ESS,
			...
			while (ucLoopCnt--) {
#if KERNEL_VERSION(4, 1, 0) <= CFG80211_VERSION_CODE
				bss_others = cfg80211_get_bss(wiphy, NULL, arBssid, ssid.aucSsid, ssid.u4SsidLen,
			...
			if (eStatus == WLAN_STATUS_ROAM_OUT_FIND_BEST) {
				struct ieee80211_channel *prChannel = NULL;
				UINT_8 ucChannelNum = wlanGetChannelNumberByNetwork(
					prGlueInfo->prAdapter, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex);
...
#if ((KERNEL_VERSION(4, 13, 0) <= CFG80211_VERSION_CODE) && (KERNEL_VERSION(4, 14, 0) >= CFG80211_VERSION_CODE))
				rRoamInfo.authorized = FALSE;
#endif
				cfg80211_roamed(prGlueInfo->prDevHandler, &rRoamInfo, GFP_KERNEL);
#else
				cfg80211_roamed_bss(prGlueInfo->prDevHandler, bss, prGlueInfo->aucReqIe,
						    prGlueInfo->u4ReqIeLength, prGlueInfo->aucRspIe,
						    prGlueInfo->u4RspIeLength, GFP_KERNEL);
#endif
			} else {
				cfg80211_connect_result(prGlueInfo->prDevHandler, arBssid, prGlueInfo->aucReqIe,
							prGlueInfo->u4ReqIeLength, prGlueInfo->aucRspIe,
	...
	case WLAN_STATUS_SCAN_ABORT:
		...
		kalCfg80211ScanDone(prGlueInfo->prScanRequest, eStatus == WLAN_STATUS_SCAN_ABORT ? TRUE : FALSE);
	...
	case WLAN_STATUS_MEDIA_SPECIFIC_INDICATION:
		if (pStatus) {
			...
			case ENUM_STATUS_TYPE_AUTHENTICATION:
				...
				if ((pAuth->arRequest[0].u4Flags == PARAM_AUTH_REQUEST_PAIRWISE_ERROR) ||
	...
	case WLAN_STATUS_JOIN_TIMEOUT: {
		...
		cfg80211_connect_result(prGlueInfo->prDevHandler, arBssid, prGlueInfo->aucReqIe,
					prGlueInfo->u4ReqIeLength, prGlueInfo->aucRspIe, prGlueInfo->u4RspIeLength,
 ...
 */
/*----------------------------------------------------------------------------*/
VOID kalUpdateReAssocReqInfo(IN P_GLUE_INFO_T prGlueInfo, IN PUINT_8 pucFrameBody, IN UINT_32 u4FrameBodyLen,
...
kalHardStartXmit(struct sk_buff *prOrgSkb, IN struct net_device *prDev, P_GLUE_INFO_T prGlueInfo, UINT_8 ucBssIndex)
{
	...
	if (GLUE_GET_REF_CNT(prGlueInfo->ai4TxPendingFrameNumPerQueue[ucBssIndex][u2QueueIdx]) >= u4MaxTxPendingNum) {
		netif_stop_subqueue(prDev, u2QueueIdx);

		DBGLOG(TX, TRACE, "Stop subqueue for BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%d] PER-Q_CNT[%d]\n",
		       ucBssIndex, u2QueueIdx, prSkb->len, GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum),
	...
	DBGLOG(TX, LOUD, "Enqueue frame for BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%d] PER-Q_CNT[%d]\n", ucBssIndex,
	       u2QueueIdx, prSkb->len, GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum),
	       GLUE_GET_REF_CNT(prGlueInfo->ai4TxPendingFrameNumPerQueue[ucBssIndex][u2QueueIdx]));
...
VOID kalSendCompleteAndAwakeQueue(IN P_GLUE_INFO_T prGlueInfo, IN PVOID pvPacket)
{
	...
	DBGLOG(TX, LOUD, "Release frame for BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%d] PER-Q_CNT[%d]\n", ucBssIndex,
	       u2QueueIdx, prSkb->len, GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum),
	       GLUE_GET_REF_CNT(prGlueInfo->ai4TxPendingFrameNumPerQueue[ucBssIndex][u2QueueIdx]));
	...
	if (fgIsValidDevice == TRUE) {
		...
		if (netif_subqueue_stopped(prDev, prSkb) &&
			...
			DBGLOG(TX, TRACE, "WakeUp Queue BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%d] PER-Q_CNT[%d]\n",
			       ucBssIndex, u2QueueIdx, prSkb->len, GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum),
			       GLUE_GET_REF_CNT(prGlueInfo->ai4TxPendingFrameNumPerQueue[ucBssIndex][u2QueueIdx]));
		}
	}
...
BOOLEAN
kalIPv4FrameClassifier(IN P_GLUE_INFO_T prGlueInfo, IN P_NATIVE_PACKET prPacket, IN PUINT_8 pucIpHdr,
		...
		if ((u2DstPort == IP_PORT_BOOTP_SERVER) || (u2DstPort == IP_PORT_BOOTP_CLIENT)) {
			...
			if (u4DhcpMagicCode == DHCP_MAGIC_NUMBER) {
				...
				DBGLOG(SW4, INFO, "DHCP PKT[0x%p] XID[0x%08x] OPT[%u] TYPE[%u]\n", prPacket, u4Xid,
				       prBootp->aucOptions[4], prBootp->aucOptions[6]);
...
/*----------------------------------------------------------------------------*/
BOOLEAN
kalQoSFrameClassifierAndPacketInfo(IN P_GLUE_INFO_T prGlueInfo, IN P_NATIVE_PACKET prPacket,
				   OUT P_TX_PACKET_INFO prTxPktInfo)
{
	UINT_32 u4PacketLen;
	...
	case ETH_WPI_1X:
		...
		if (u2EtherTypeLen == ETH_P_1X) {
			...
			if (ucEAPoLKey != ETH_EAPOL_KEY)
				...
				DBGLOG(RSN, STATE, "Tx EAPOL Frame[Len:%d, KeyInfo:%d]\n", u4PacketLen, u2KeyInfo);
				if (u2KeyInfo & BIT(3))
					prTxPktInfo->u2Flag |= BIT(ENUM_PKT_NON_PROTECTED_1X);
			}
		}
...
VOID kalOidComplete(IN P_GLUE_INFO_T prGlueInfo, IN BOOLEAN fgSetQuery, IN UINT_32 u4SetQueryInfoLen,
...
{
	return kalIoctlTimeout(prGlueInfo, pfnOidHandler, pvInfoBuf, u4InfoBufLen, fgRead, fgWaitResp, fgCmd, -1,
			       pu4QryInfoLen);
}
...
WLAN_STATUS
kalIoctlTimeout(IN P_GLUE_INFO_T prGlueInfo, IN PFN_OID_HANDLER_FUNC pfnOidHandler, IN PVOID pvInfoBuf,
		...
		OUT PUINT_32 pu4QryInfoLen)
{
	 ...
	 */
	if (prGlueInfo->u4TxThreadPid == KAL_GET_CURRENT_THREAD_ID() || in_interrupt()) {
		if (pfnOidHandler) {
			g_fgIsOid = FALSE;
	...
	while (TRUE) {
		...
		if (prGlueInfo->fgEnSdioTestPattern == TRUE) {
			...
			if (prGlueInfo->fgSdioReadWriteMode == TRUE) {
				/* read test */
				kalDevPortRead(prGlueInfo, MCR_WTMDR, 256, prGlueInfo->aucSdioTestBuffer,
					       sizeof(prGlueInfo->aucSdioTestBuffer));
			} else {
				/* write test */
				kalDevPortWrite(prGlueInfo, MCR_WTMDR, 172, prGlueInfo->aucSdioTestBuffer,
						sizeof(prGlueInfo->aucSdioTestBuffer));
			}
		}
#endif
			...
			if (test_and_clear_bit(GLUE_FLAG_OID_BIT, &prGlueInfo->ulFlag)) {
				...
				if (prIoReq->fgRead == FALSE) {
					prIoReq->rStatus = wlanSetInformation(prIoReq->prAdapter,
									      ...
									      prIoReq->pvInfoBuf, prIoReq->u4InfoBufLen,
									      prIoReq->pu4QryInfoLen);
				} else {
					prIoReq->rStatus = wlanQueryInformation(
						prIoReq->prAdapter, prIoReq->pfnOidHandler, prIoReq->pvInfoBuf,
...
BOOLEAN kalRetrieveNetworkAddress(IN P_GLUE_INFO_T prGlueInfo, IN OUT PARAM_MAC_ADDRESS *prMacAddr)
{
	...
	if (prGlueInfo->fgIsMacAddrOverride == FALSE) {
		...
		for (i = 0; i < MAC_ADDR_LEN; i += 2) {
			if (kalCfgDataRead16(prGlueInfo, OFFSET_OF(WIFI_CFG_PARAM_STRUCT, aucMacAddress) + i,
...
{
#if KERNEL_VERSION(4, 14, 0) <= LINUX_VERSION_CODE
	struct _GLUE_INFO_T *prGlueInfo = from_timer(prGlueInfo, timer, tickfn);
#else
 ...
 */
/*----------------------------------------------------------------------------*/
VOID kalGetConfigurationVersion(IN P_GLUE_INFO_T prGlueInfo, OUT PUINT_16 pu2Part1CfgOwnVersion,
				OUT PUINT_16 pu2Part1CfgPeerVersion, OUT PUINT_16 pu2Part2CfgOwnVersion,
 ...
 */
/*----------------------------------------------------------------------------*/
VOID kalUpdateRSSI(IN P_GLUE_INFO_T prGlueInfo, IN ENUM_KAL_NETWORK_TYPE_INDEX_T eNetTypeIdx, IN INT_8 cRssi,
 ...
 */
/*----------------------------------------------------------------------------*/
VOID kalGetChannelList(IN P_GLUE_INFO_T prGlueInfo, IN ENUM_BAND_T eSpecificBand, IN UINT_8 ucMaxChannelNum,
		       IN PUINT_8 pucNumOfChannel, IN P_RF_CHANNEL_INFO_T paucChannelList)
{
	rlmDomainGetChnlList(prGlueInfo->prAdapter, eSpecificBand, FALSE, ucMaxChannelNum, pucNumOfChannel,
			     paucChannelList);
}
 ...
 */
/*----------------------------------------------------------------------------*/
INT_32 kalRequestFirmware(const PUINT_8 pucPath, PUINT_8 pucData, UINT_32 u4Size, PUINT_32 pu4ReadSize,
			  struct device *dev)
{
			...
			IN UINT_8 ucChannelNum, IN INT_32 i4SignalStrength)
{
	...
	if (ucChannelNum <= 14) {
		prChannel = ieee80211_get_channel(wiphy, ieee80211_channel_to_frequency(ucChannelNum, KAL_BAND_2GHZ));
	} else {
		prChannel = ieee80211_get_channel(wiphy, ieee80211_channel_to_frequency(ucChannelNum, KAL_BAND_5GHZ));
	}
	...
	if (prChannel != NULL && prGlueInfo->fgIsRegistered == TRUE) {
		...
		bss = cfg80211_inform_bss_frame(wiphy, prChannel, (struct ieee80211_mgmt *)pucBeaconProbeResp,
 ...
 */
/*----------------------------------------------------------------------------*/
VOID kalReadyOnChannel(IN P_GLUE_INFO_T prGlueInfo, IN UINT_64 u8Cookie, IN ENUM_BAND_T eBand, IN ENUM_CHNL_EXT_T eSco,
			     ...
			     IN PUINT_8 pucFrameBuf, IN UINT_32 u4FrameLen)
{
	do {
		if ((prGlueInfo == NULL) || (pucFrameBuf == NULL) || (u4FrameLen == 0)) {
			DBGLOG(AIS, TRACE, "Unexpected pointer PARAM. 0x%lx, 0x%lx, %d.", prGlueInfo, pucFrameBuf,
			       u4FrameLen);
			ASSERT(FALSE);
			break;
		}

		cfg80211_mgmt_tx_status(prGlueInfo->prDevHandler->ieee80211_ptr, u8Cookie, pucFrameBuf, u4FrameLen,
					fgIsAck, GFP_KERNEL);
...
VOID kalIndicateRxMgmtFrame(IN P_GLUE_INFO_T prGlueInfo, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		if (prGlueInfo->prDevHandler->ieee80211_ptr == NULL) {
			DBGLOG(AIS, WARN, "ieee80211_ptr is NULL!\n");
			break;
		}

		if (prGlueInfo->u4OsMgmtFrameFilter == 0) {
			DBGLOG(AIS, WARN, "The cfg80211 hasn't do mgmt register!\n");
			break;
		}
		...
		cfg80211_rx_mgmt(prGlueInfo->prDevHandler->ieee80211_ptr, i4Freq, /* in MHz */
				 ...
				 prSwRfb->pvHeader, prSwRfb->u2PacketLen, NL80211_RXMGMT_FLAG_ANSWERED);

#elif (KERNEL_VERSION(3, 12, 0) <= CFG80211_VERSION_CODE)
		cfg80211_rx_mgmt(prGlueInfo->prDevHandler->ieee80211_ptr, i4Freq, /* in MHz */
				 RCPI_TO_dBm((UINT_8)nicRxGetRcpiValueFromRxv(RCPI_MODE_WF0, prSwRfb)),
				 prSwRfb->pvHeader, prSwRfb->u2PacketLen, NL80211_RXMGMT_FLAG_ANSWERED, GFP_ATOMIC);
#else
		cfg80211_rx_mgmt(prGlueInfo->prDevHandler->ieee80211_ptr, i4Freq, /* in MHz */
				 RCPI_TO_dBm((UINT_8)nicRxGetRcpiValueFromRxv(RCPI_MODE_WF0, prSwRfb)),
...
BOOLEAN kalSetSdioTestPattern(IN P_GLUE_INFO_T prGlueInfo, IN BOOLEAN fgEn, IN BOOLEAN fgRead)
{
	const UINT_8 aucPattern[] = {
		,
...
#if KERNEL_VERSION(4, 12, 0) <= CFG80211_VERSION_CODE
	cfg80211_sched_scan_results(priv_to_wiphy(prGlueInfo), prGlueInfo->prSchedScanRequest->reqid);
#else
...
#if CFG_SUPPORT_CFG80211_AUTH
void kalIndicateRxAuthToUpperLayer(struct net_device *prDevHandler, PUINT_8 prAuthFrame, UINT_16 u2FrameLen)
{
	DBGLOG(REQ, INFO, "\n");

#if CFG_SUPPORT_CFG80211_QUEUE
	cfg80211AddToPktQueue(prDevHandler, prAuthFrame, u2FrameLen, NULL, CFG80211_RX, MAC_FRAME_AUTH);
#else
...
void kalIndicateRxAssocToUpperLayer(struct net_device *prDevHandler, PUINT_8 prAssocRspFrame, struct cfg80211_bss *bss,
				    UINT_16 u2FrameLen)
{
	DBGLOG(REQ, INFO, "\n");

#if CFG_SUPPORT_CFG80211_QUEUE
	cfg80211AddToPktQueue(prDevHandler, prAssocRspFrame, u2FrameLen, bss, CFG80211_RX, MAC_FRAME_ASSOC_RSP);
#else
#if (KERNEL_VERSION(3, 18, 0) <= CFG80211_VERSION_CODE)
...
void kalIndicateRxDeauthToUpperLayer(struct net_device *prDevHandler, PUINT_8 prDeauthFrame, UINT_16 u2FrameLen)
{
	DBGLOG(REQ, INFO, "\n");

#if CFG_SUPPORT_CFG80211_QUEUE
	cfg80211AddToPktQueue(prDevHandler, prDeauthFrame, u2FrameLen, NULL, CFG80211_RX, MAC_FRAME_DEAUTH);
#else
#if (KERNEL_VERSION(3, 11, 0) <= CFG80211_VERSION_CODE)
...
void kalIndicateRxDisassocToUpperLayer(struct net_device *prDevHandler, PUINT_8 prDisassocFrame, UINT_16 u2FrameLen)
{
	DBGLOG(REQ, INFO, "\n");

#if CFG_SUPPORT_CFG80211_QUEUE
	cfg80211AddToPktQueue(prDevHandler, prDisassocFrame, u2FrameLen, NULL, CFG80211_RX, MAC_FRAME_DISASSOC);
#else
#if (KERNEL_VERSION(3, 11, 0) <= CFG80211_VERSION_CODE)
...
void kalIndicateTxDeauthToUpperLayer(struct net_device *prDevHandler, PUINT_8 prDeauthFrame, UINT_16 u2FrameLen)
{
	DBGLOG(REQ, INFO, "\n");

#if CFG_SUPPORT_CFG80211_QUEUE
	cfg80211AddToPktQueue(prDevHandler, prDeauthFrame, u2FrameLen, NULL, CFG80211_TX, MAC_FRAME_DEAUTH);
#else
#if (KERNEL_VERSION(3, 11, 0) <= CFG80211_VERSION_CODE)
...
void kalIndicateTxDisassocToUpperLayer(struct net_device *prDevHandler, PUINT_8 prDisassocFrame, UINT_16 u2FrameLen)
{
	DBGLOG(REQ, INFO, "\n");

#if CFG_SUPPORT_CFG80211_QUEUE
	cfg80211AddToPktQueue(prDevHandler, prDisassocFrame, u2FrameLen, NULL, CFG80211_TX, MAC_FRAME_DISASSOC);
#else
#if (KERNEL_VERSION(3, 11, 0) <= CFG80211_VERSION_CODE)
...
void cfg80211AddToPktQueue(struct net_device *prDevHandler, void *buf, uint16_t u2FrameLength, struct cfg80211_bss *bss,
			   uint8_t ucflagTx, uint8_t ucFrameType)
{
	...
	DBGLOG(REQ, TRACE, "Alloc prCfg80211Req 0x%x\n", prCfg80211Req);
	...
	kalMemCopy((void *)prCfg80211Req->prFrame, (void *)buf, u2FrameLength);
...
static void kalProcessCfg80211TxPkt(struct PARAM_CFG80211_REQ *prCfg80211Req)
{
	...
	case MAC_FRAME_DEAUTH:
		cfg80211_tx_mlme_mgmt(prCfg80211Req->prDevHandler, (const u8 *)prCfg80211Req->prFrame,
				      prCfg80211Req->frameLen);
		break;
#else
	case MAC_FRAME_DISASSOC:
		cfg80211_send_disassoc(prCfg80211Req->prDevHandler, (const u8 *)prCfg80211Req->prFrame,
				       prCfg80211Req->frameLen);
		break;
	case MAC_FRAME_DEAUTH:
		cfg80211_send_deauth(prCfg80211Req->prDevHandler, (const u8 *)prCfg80211Req->prFrame,
				     prCfg80211Req->frameLen);
		break;
#endif
	default:
...
static void kalProcessCfg80211RxPkt(struct PARAM_CFG80211_REQ *prCfg80211Req)
{
	...
	case MAC_FRAME_DEAUTH:
	case MAC_FRAME_DISASSOC:
		cfg80211_rx_mlme_mgmt(prCfg80211Req->prDevHandler, (PUINT_8)prCfg80211Req->prFrame,
				      prCfg80211Req->frameLen);
		break;
#else
	case MAC_FRAME_AUTH:
		cfg80211_send_rx_auth(prCfg80211Req->prDevHandler, (PUINT_8)prCfg80211Req->prFrame,
				      prCfg80211Req->frameLen);
		break;
	case MAC_FRAME_DEAUTH:
		cfg80211_send_deauth(prCfg80211Req->prDevHandler, (PUINT_8)prCfg80211Req->prFrame,
				     prCfg80211Req->frameLen);
		break;
	case MAC_FRAME_DISASSOC:
		cfg80211_send_disassoc(prCfg80211Req->prDevHandler, (PUINT_8)prCfg80211Req->prFrame,
				       prCfg80211Req->frameLen);
		break;
#endif
	case MAC_FRAME_ASSOC_RSP:
		...
		cfg80211_rx_assoc_resp(prCfg80211Req->prDevHandler, prCfg80211Req->bss,
				       (const u8 *)prCfg80211Req->prFrame, prCfg80211Req->frameLen, 0, NULL, 0);
#elif (KERNEL_VERSION(3, 18, 0) <= CFG80211_VERSION_CODE)
		cfg80211_rx_assoc_resp(prCfg80211Req->prDevHandler, prCfg80211Req->bss, (PUINT_8)prCfg80211Req->prFrame,
				       prCfg80211Req->frameLen, 0);
#elif (KERNEL_VERSION(3, 11, 0) <= CFG80211_VERSION_CODE)
		cfg80211_rx_assoc_resp(prCfg80211Req->prDevHandler, prCfg80211Req->bss, (PUINT_8)prCfg80211Req->prFrame,
				       prCfg80211Req->frameLen);
#else
		cfg80211_send_rx_assoc(prCfg80211Req->prDevHandler, prCfg80211Req->bss, (PUINT_8)prCfg80211Req->prFrame,
				       prCfg80211Req->frameLen);
#endif
		break;
	default:
		...
		while (QUEUE_IS_NOT_EMPTY(prTempQue)) {
			...
			if (prGlueInfo->u4ReadyFlag == 1) {
				if (prCfg80211Req->ucFlagTx == CFG80211_TX) {
					DBGLOG(REQ, INFO, "cfg80211_tx_mlme_mgmt %d %d\n", prCfg80211Req->ucFrameType,
					       prCfg80211Req->frameLen);
					kalProcessCfg80211TxPkt(prCfg80211Req);
				} else if (prCfg80211Req->ucFlagTx == CFG80211_RX) {
					DBGLOG(REQ, INFO, "cfg80211_rx_mlme_mgmt %d %d\n", prCfg80211Req->ucFrameType,
					       prCfg80211Req->frameLen);
			...
			DBGLOG(REQ, TRACE, "Free prCfg80211Req 0x%x\n", prCfg80211Req);

			if (prCfg80211Req->prFrame)
				kalMemFree(prCfg80211Req->prFrame, PHY_MEM_TYPE, prCfg80211Req->prFrameLen);

			kalMemFree(prCfg80211Req, PHY_MEM_TYPE, sizeof(struct PARAM_CFG80211_REQ));
		}
	}
}
#endif
...
BOOLEAN
kalGetIPv4Address(IN struct net_device *prDev, IN UINT_32 u4MaxNumOfAddr, OUT PUINT_8 pucIpv4Addrs,
...
BOOLEAN
kalGetIPv6Address(IN struct net_device *prDev, IN UINT_32 u4MaxNumOfAddr, OUT PUINT_8 pucIpv6Addrs,
...
VOID kalSetNetAddress(IN P_GLUE_INFO_T prGlueInfo, IN UINT_8 ucBssIdx, IN PUINT_8 pucIPv4Addr, IN UINT_32 u4NumIPv4Addr,
		      IN PUINT_8 pucIPv6Addr, IN UINT_32 u4NumIPv6Addr)
{
	...
	for (i = 0; i < u4NumIPv4Addr; i++) {
		...
		prParamNetAddr =
			(P_PARAM_NETWORK_ADDRESS)((ULONG)prParamNetAddr +
						  (ULONG)(u4AddrLen + OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress)));
	}
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetNetworkAddress, (PVOID)prParamNetAddrList, u4Len, FALSE, FALSE, TRUE,
			   &u4SetInfoLen);
...
static int __mt_find_tracing_mark_write_symbol_fn(void *prData, const char *pcNameBuf, struct module *prModule,
...
#endif
const struct file_operations rMetProcCtrlFops = { .write = kalMetCtrlWriteProcfs };

const struct file_operations rMetProcPortFops = { .write = kalMetPortWriteProcfs };
...
BOOLEAN kalIndicateAgpsNotify(P_ADAPTER_T prAdapter, UINT_8 cmd, PUINT_8 data, UINT_16 dataLen)
{
	...
	struct sk_buff *skb = cfg80211_testmode_alloc_event_skb(priv_to_wiphy(prGlueInfo), dataLen, GFP_KERNEL);
...
VOID kalWowInit(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	prGlueInfo->prAdapter->mdns_wake_flag = MDNS_WAKEUP_BY_NO_MATCH_RECORD;
#else
	/* for GVA, wake up devices when get a sub mdns query packet*/
	prGlueInfo->prAdapter->mdns_wake_flag = MDNS_WAKEUP_BY_SUB_REQ;
#endif
...
VOID kalWowProcess(IN P_GLUE_INFO_T prGlueInfo, UINT_8 enable)
{
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucAdvPws == 1 && prGlueInfo->prAdapter->rWowCtrl.fgWowEnable == 0)
		bWake = FALSE;
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucN9Log2HostCtrl > 0) {
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidSetFwLog2Host, &rFwLog2HostCtrl, sizeof(CMD_FW_LOG_2_HOST_CTRL_T),
	...
	rStatus = wlanSendSetQueryCmd(prGlueInfo->prAdapter, CMD_ID_SET_PF_CAPABILITY, TRUE, FALSE, FALSE,
				      kalWowCmdEventSetCb, nicOidCmdTimeoutCommon, sizeof(CMD_PACKET_FILTER_CAP_T),
				      (PUINT_8)&rCmdPacket_Filter_Cap, NULL, 0);
	...
	if (enable) {
		...
		rCmdWowlanParam.u2FilterFlag = WOWLAN_FF_DROP_ALL | WOWLAN_FF_SEND_MAGIC_TO_HOST | WOWLAN_FF_ALLOW_1X |
	...
	rStatus = wlanSendSetQueryCmd(prGlueInfo->prAdapter, CMD_ID_SET_WOWLAN, TRUE, FALSE, FALSE, kalWowCmdEventSetCb,
				      nicOidCmdTimeoutCommon, sizeof(CMD_WOWLAN_PARAM_T), (PUINT_8)&rCmdWowlanParam,
				      NULL, 0);
...
UINT_16 NameParse(IN PUINT_8 prParseName, IN UINT_16 ParseNameLen, OUT PUINT_8 pucBuffer)
{
...
void kalMdnsOffloadInit(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < MAX_MDNS_CACHE_NUM; i++) {
		prMdnsParamEntry = (struct MDNS_PARAM_ENTRY_T *)(&prMdnsInfo->rMdnsEntry[i]);
		LINK_INSERT_TAIL(&prMdnsInfo->rMdnsRecordFreeList, &prMdnsParamEntry->rLinkEntry);
	}
}
...
struct MDNS_PARAM_ENTRY_T *mdnsAllocateParamEntry(IN P_ADAPTER_T prAdapter)
{
	struct MDNS_INFO_T *prMdnsInfo;
	struct MDNS_PARAM_ENTRY_T *prMdnsParamEntry;
	P_LINK_T prMdnsRecordFreeList;
	P_LINK_T prMdnsRecordList;
	...
	LINK_REMOVE_HEAD(prMdnsRecordFreeList, prMdnsParamEntry, struct MDNS_PARAM_ENTRY_T *);

	if (prMdnsParamEntry) {
		kalMemZero(prMdnsParamEntry, sizeof(struct MDNS_PARAM_ENTRY_T));

		prMdnsRecordList = &prMdnsInfo->rMdnsRecordList;

		LINK_INSERT_TAIL(prMdnsRecordList, &prMdnsParamEntry->rLinkEntry);
	}

	return prMdnsParamEntry;
}

VOID kalSendMdnsEventCb(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	ASSERT(prAdapter);
	ASSERT(prCmdInfo);
...
void kalSendMdnsEnableToFw(P_GLUE_INFO_T prGlueInfo)
{
	...
	cmdMdnsParam = kzalloc(sizeof(struct CMD_MDNS_PARAM_T), GFP_KERNEL);
	if (!cmdMdnsParam) {
	...
	DBGLOG(SW4, STATE, "mDNS Enable wake flag %d.\n", cmdMdnsParam->ucWakeFlag);
	...
	/* SA, DA.*/
	COPY_MAC_ADDR(prMacHeader->aucAddr2, prGlueInfo->prDevHandler->dev_addr);
	...
	prMacHeader->u2QosCtrl |= (ACK_POLICY_NORMAL_ACK_IMPLICIT_BA_REQ << MASK_QC_ACK_POLICY_OFFSET);

	/* IPv4 header.*/
	pucIPHeader = &cmdMdnsParam->aucMdnsIPHdr[0];
	/* Version: 4, Length:(5*4) */
	pucIPHeader[0] = (IP_VERSION_4 << IP_VERSION_OFFSET) | 5;
	/* Diff.Services */
	pucIPHeader[1] = 0;
	/* IP length:
	...
	wlanSendSetQueryCmd(prGlueInfo->prAdapter, CMD_ID_SET_MDNS_RECORD, TRUE, FALSE, FALSE, kalSendMdnsEventCb,
			    nicOidCmdTimeoutCommon, sizeof(struct CMD_MDNS_PARAM_T), (PUINT_8)cmdMdnsParam, NULL, 0);
	...
	while (1) {
		...
		if (wait > 100) {
			DBGLOG(INIT, ERROR, "set mdns enable process timeout\n");
			break;
		}
		if (prGlueInfo->prAdapter->fgSetMdnsDone == TRUE) {
			DBGLOG(INIT, LOUD, "send mdns enable cmd done\n");
			break;
		}
		wait++;
	}
...
void kalAddMdnsRecord(P_GLUE_INFO_T prGlueInfo, struct MDNS_INFO_UPLAYER_T *prMdnsUplayerInfo)
{
	struct MDNS_INFO_T *prMdnsInfo;
	struct MDNS_PARAM_ENTRY_T *prMdnsParamEntry;
	P_LINK_T prMdnsRecordList;
	...
	LINK_FOR_EACH_ENTRY(prMdnsParamEntry, prMdnsRecordList, rLinkEntry, struct MDNS_PARAM_ENTRY_T)
	{
		if (kalMemCmp(&prMdnsParamEntry->mdns_param, &prMdnsUplayerInfo->mdns_param,
	...
	if (prMdnsParamEntry == NULL) {
		DBGLOG(REQ, INFO, "mdns record buffer is full replace the first one.\n");
		LINK_REMOVE_HEAD(prMdnsRecordList, prMdnsParamEntry, struct MDNS_PARAM_ENTRY_T *);
		kalMemCopy(&prMdnsParamEntry->mdns_param, &prMdnsUplayerInfo->mdns_param, sizeof(struct MDNS_PARAM_T));
		LINK_INSERT_TAIL(prMdnsRecordList, &prMdnsParamEntry->rLinkEntry);
	} else {
		DBGLOG(REQ, INFO, "add mdns record buffer number %d.\n", prMdnsRecordList->u4NumElem);
		kalMemCopy(&prMdnsParamEntry->mdns_param, &prMdnsUplayerInfo->mdns_param, sizeof(struct MDNS_PARAM_T));
	}
}

void kalDelMdnsRecord(P_GLUE_INFO_T prGlueInfo, struct MDNS_INFO_UPLAYER_T *prMdnsUplayerInfo)
{
	struct MDNS_INFO_T *prMdnsInfo;
	struct MDNS_PARAM_ENTRY_T *prMdnsParamEntry;
	...
	LINK_FOR_EACH_ENTRY_SAFE(prMdnsParamEntry, prMdnsParamEntryNext, prMdnsRecordList, rLinkEntry,
				 struct MDNS_PARAM_ENTRY_T)
	{
		if (kalMemCmp(&prMdnsParamEntry->mdns_param, &prMdnsUplayerInfo->mdns_param,
			      sizeof(struct MDNS_PARAM_T)) == 0) {
			DBGLOG(REQ, ERROR, "del mdns record.\n");
			LINK_REMOVE_KNOWN_ENTRY(prMdnsRecordList, prMdnsParamEntry);
			LINK_INSERT_HEAD(prMdnsRecordFreeList, &prMdnsParamEntry->rLinkEntry);
		}
	}
}
...
void kalSendMdnsDisableToFw(P_GLUE_INFO_T prGlueInfo)
{
	struct CMD_MDNS_PARAM_T *cmdMdnsParam;
	UINT_32 wait;
	...
	while (1) {
		...
		if (wait > 100) {
			DBGLOG(INIT, ERROR, "set mdns disable timeout\n");
			break;
		}
		if (prGlueInfo->prAdapter->fgSetMdnsDone == TRUE) {
			DBGLOG(INIT, LOUD, "send mdns disable cmd done\n");
			break;
		}
		wait++;
	}
...
void kalSendClearRecordToFw(P_GLUE_INFO_T prGlueInfo)
{
	...
	while (1) {
		...
		if (wait > 100) {
			DBGLOG(INIT, ERROR, "clear mdns record timeout\n");
			break;
		}
		if (prGlueInfo->prAdapter->fgSetMdnsDone == TRUE) {
			DBGLOG(INIT, LOUD, "send mdns clear cmd done\n");
			break;
		}
		wait++;
	}
...
void kalSendMdnsRecordToFw(P_GLUE_INFO_T prGlueInfo)
{
	...
	LINK_FOR_EACH_ENTRY(prMdnsParamEntry, prMdnsRecordList, rLinkEntry, struct MDNS_PARAM_ENTRY_T)
	{
		...
		kalMemCopy(&cmdMdnsParam->mdns_param, &prMdnsParamEntry->mdns_param, sizeof(struct MDNS_PARAM_T));

		wlanSendSetQueryCmd(prGlueInfo->prAdapter, CMD_ID_SET_MDNS_RECORD, TRUE, FALSE, FALSE,
				    kalSendMdnsEventCb, nicOidCmdTimeoutCommon, sizeof(struct CMD_MDNS_PARAM_T),
				    (PUINT_8)cmdMdnsParam, NULL, 0);
		...
		while (1) {
			...
			if (wait > 100) {
				DBGLOG(INIT, ERROR, "send mdns record timeout\n");
				break;
			}
			if (prGlueInfo->prAdapter->fgSetMdnsDone == TRUE) {
				DBGLOG(INIT, LOUD, "send mdns record [%d] done\n", i);
				break;
			}
			wait++;
		}
...
void kalMdnsProcess(IN P_GLUE_INFO_T prGlueInfo, IN struct MDNS_INFO_UPLAYER_T *prMdnsUplayerInfo)
{
...
#if CFG_SUPPORT_MDNS_OFFLOAD_GVA
UINT_16 kalMdsnParseName(PUINT_8 pucMdnsHdr, UINT_16 offset, PUINT_8 name, PUINT_16 name_len)
{
	UINT_16 pos = 0;
	UINT_16 i = 0;
	...
	while (pucMdnsHdr[offset] != 0) {
		if ((pucMdnsHdr[offset] & 0xc0) != 0xc0) {
			...
			offset = NTOHS(*(UINT_16 *)(pucMdnsHdr + offset)) & 0x3fff;
		}
	}
...
void kalProcessMdnsRespPkt(P_GLUE_INFO_T prGlueInfo, PUINT_8 pucMdnsHdr)
{
	UINT_8 i;
	UINT_16 pos;
	UINT_16 usMdnsId, usMdnsFlags;
	UINT_16 usQuestionCnt, usAnswerCnt, usAuthCnt, usAddtionCnt;
	UINT_32 ttl;
	UINT_16 type, cl, dataLen;
	UINT_8 domName[MDNS_QUESTION_NAME_MAX_LEN];
	UINT_16 domNameLen;
	...
	usMdnsId = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
	pos += 2;

	usMdnsFlags = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
	pos += 2;

	usQuestionCnt = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
	pos += 2;

	usAnswerCnt = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
	pos += 2;

	usAuthCnt = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
	pos += 2;

	usAddtionCnt = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
	pos += 2;
	...
	if ((usAnswerCnt == 1 && usAddtionCnt == 3) || (usAnswerCnt == 4 && usAddtionCnt == 0)) {
	...
	for (i = 0; i < 4; i++) {
		...
		pos = kalMdsnParseName(pucMdnsHdr, pos, domName, &domNameLen);

		type = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
		pos += 2;

		cl = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
		pos += 2;

		ttl = NTOHL(*(UINT_32 *)(pucMdnsHdr + pos));
		pos += 4;

		dataLen = NTOHS(*(UINT_16 *)(pucMdnsHdr + pos));
		pos += 2;
		...
		case 1:
			...
			break;
		default:
			DBGLOG(SW4, LOUD, "not needed type %d\n", type);
			goto exit;
		}

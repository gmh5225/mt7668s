...
static PUINT_8 apucDebugP2pRoleState[P2P_ROLE_STATE_NUM] = {
	...
	(PUINT_8)DISP_STRING("P2P_ROLE_STATE_REQING_CHANNEL"), (PUINT_8)DISP_STRING("P2P_ROLE_STATE_AP_CHNL_DETECTION"),
#if (CFG_SUPPORT_DFS_MASTER == 1)
	(PUINT_8)DISP_STRING("P2P_ROLE_STATE_GC_JOIN"),	       (PUINT_8)DISP_STRING("P2P_ROLE_STATE_DFS_CAC"),
...
VOID p2pRoleFsmStateTransition(IN P_ADAPTER_T prAdapter, IN P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo,
...
UINT_8 p2pRoleFsmInit(IN P_ADAPTER_T prAdapter, IN UINT_8 ucRoleIdx)
{
	...
	do {
		...
		if (!prP2pRoleFsmInfo) {
			DBGLOG(INIT, ERROR, "Allocate p2p role fsm memory FAILED\n");
			break;
		}
		...
		cnmTimerInitTimer(prAdapter, &(prP2pRoleFsmInfo->rP2pRoleFsmTimeoutTimer),
				  (PFN_MGMT_TIMEOUT_FUNC)p2pRoleFsmRunEventTimeout, (ULONG)prP2pRoleFsmInfo);

#if (CFG_SUPPORT_DFS_MASTER == 1)
		cnmTimerInitTimer(prAdapter, &(prP2pRoleFsmInfo->rDfsShutDownTimer),
				  (PFN_MGMT_TIMEOUT_FUNC)p2pRoleFsmRunEventDfsShutDownTimeout, (ULONG)prP2pRoleFsmInfo);
#endif
		...
		rateGetDataRatesFromRateSet(prP2pBssInfo->u2OperationalRateSet, prP2pBssInfo->u2BSSBasicRateSet,
					    prP2pBssInfo->aucAllSupportedRates, &prP2pBssInfo->ucAllSupportedRatesLen);

		prP2pBssInfo->prBeacon =
			cnmMgtPktAlloc(prAdapter, OFFSET_OF(WLAN_BEACON_FRAME_T, aucInfoElem[0]) + MAX_IE_LENGTH);
			       ...
			       IN ENUM_P2P_ROLE_STATE_T eNextState)
{
	...
	do {
		...
		case P2P_ROLE_STATE_REQING_CHANNEL:
			if (!fgIsTransitionOut) {
				p2pRoleStateInit_REQING_CHANNEL(prAdapter, prP2pRoleFsmInfo->ucBssIndex,
								&(prP2pRoleFsmInfo->rChnlReqInfo));
			} else {
				p2pRoleStateAbort_REQING_CHANNEL(prAdapter, prP2pRoleBssInfo, prP2pRoleFsmInfo,
								 eNextState);
			}
			break;
		case P2P_ROLE_STATE_AP_CHNL_DETECTION:
			if (!fgIsTransitionOut) {
				p2pRoleStateInit_AP_CHNL_DETECTION(prAdapter, prP2pRoleFsmInfo->ucBssIndex,
								   &(prP2pRoleFsmInfo->rScanReqInfo),
								   &(prP2pRoleFsmInfo->rConnReqInfo));
			} else {
				p2pRoleStateAbort_AP_CHNL_DETECTION(prAdapter, prP2pRoleFsmInfo->ucBssIndex,
								    &(prP2pRoleFsmInfo->rConnReqInfo),
								    &(prP2pRoleFsmInfo->rChnlReqInfo),
		...
		case P2P_ROLE_STATE_GC_JOIN:
			if (!fgIsTransitionOut) {
				...
				p2pRoleStateAbort_GC_JOIN(prAdapter, prP2pRoleFsmInfo, &(prP2pRoleFsmInfo->rJoinInfo),
							  eNextState);
			}
			break;
		...
		case P2P_ROLE_STATE_DFS_CAC:
			if (!fgIsTransitionOut) {
				p2pRoleStateInit_DFS_CAC(prAdapter, prP2pRoleFsmInfo->ucBssIndex,
							 &(prP2pRoleFsmInfo->rChnlReqInfo));
			} else {
				p2pRoleStateAbort_DFS_CAC(prAdapter, prP2pRoleBssInfo, prP2pRoleFsmInfo, eNextState);
			}
			break;
		case P2P_ROLE_STATE_SWITCH_CHANNEL:
			if (!fgIsTransitionOut) {
				p2pRoleStateInit_SWITCH_CHANNEL(prAdapter, prP2pRoleFsmInfo->ucBssIndex,
								&(prP2pRoleFsmInfo->rChnlReqInfo));
			} else {
				p2pRoleStateAbort_SWITCH_CHANNEL(prAdapter, prP2pRoleBssInfo, prP2pRoleFsmInfo,
								 eNextState);
			}
			break;
#endif
...
static VOID p2pRoleFsmDeauhComplete(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	P_BSS_INFO_T prP2pBssInfo = (P_BSS_INFO_T)NULL;
	P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo = (P_P2P_ROLE_FSM_INFO_T)NULL;
...
WLAN_STATUS
p2pRoleFsmRunEventDeauthTxDone(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo,
...
VOID p2pRoleFsmRunEventRxDeauthentication(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		case OP_MODE_INFRASTRUCTURE:
			if (authProcessRxDeauthFrame(prSwRfb, prStaRec->aucMacAddr, &u2ReasonCode) ==
			    WLAN_STATUS_SUCCESS) {
		...
		case OP_MODE_ACCESS_POINT:
			...
			if (authProcessRxDeauthFrame(prSwRfb, prP2pBssInfo->aucBSSID, &u2ReasonCode) ==
			    WLAN_STATUS_SUCCESS) {
...
VOID p2pRoleFsmRunEventRxDisassociation(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN P_SW_RFB_T prSwRfb)
{
	...
	case OP_MODE_INFRASTRUCTURE:
		if (assocProcessRxDisassocFrame(prAdapter, prSwRfb, prStaRec->aucMacAddr, &prStaRec->u2ReasonCode) ==
...
#if CFG_WPS_DISCONNECT || (KERNEL_VERSION(4, 4, 0) <= CFG80211_VERSION_CODE)
			/* Indicate disconnect to Host. */
			kalP2PGCIndicateConnectionStatus(prAdapter->prGlueInfo, (UINT_8)prP2pBssInfo->u4PrivateData,
							 NULL, prDisassocFrame->aucInfoElem, u2IELength,
							 prStaRec->u2ReasonCode, WLAN_STATUS_MEDIA_DISCONNECT);
			...
			p2pRoleFsmStateTransition(
				prAdapter, P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, prP2pBssInfo->u4PrivateData),
				P2P_ROLE_STATE_IDLE);
		}
		break;
	case OP_MODE_ACCESS_POINT:
		/* Delete client from client list. */
		if (assocProcessRxDisassocFrame(prAdapter, prSwRfb, prP2pBssInfo->aucBSSID, &u2ReasonCode) ==
		    WLAN_STATUS_SUCCESS) {
#if CFG_SUPPORT_802_11W
			/* AP PMF */
			if (rsnCheckBipKeyInstalled(prAdapter, prStaRec)) {
			...
			if (bssRemoveClient(prAdapter, prP2pBssInfo, prStaRec)) {
				/* Indicate disconnect to Host. */
				p2pFuncDisconnect(prAdapter, prP2pBssInfo, prStaRec, fgSendDeauth, u2ReasonCode);
				/* Deactive BSS if PWR is IDLE and no peer */
				if (IS_NET_PWR_STATE_IDLE(prAdapter, prP2pBssInfo->ucBssIndex) &&
				    (bssGetClientCount(prAdapter, prP2pBssInfo) == 0)) {
					/* All Peer disconnected !! Stop BSS now!! */
...
#if CFG_WPS_DISCONNECT || (KERNEL_VERSION(4, 4, 0) <= CFG80211_VERSION_CODE)
			/* Indicate disconnect to Host. */
			kalP2PGCIndicateConnectionStatus(prAdapter->prGlueInfo, prP2pRoleFsmInfo->ucRoleIndex, NULL,
...
VOID p2pRoleFsmRunEventStartAP(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (prP2pStartAPMsg->u2SsidLen != 0) {
		...
		prP2pConnReqInfo->rSsidStruct.ucSsidLen = prP2pSpecificBssInfo->u2GroupSsidLen =
	...
	if (p2pFuncIsAPMode(prAdapter->rWifiVar.prP2PConnSettings[prP2pStartAPMsg->ucRoleIdx])) {
		...
		if (prAdapter->rWifiVar.ucApChannel && prAdapter->rWifiVar.ucApChnlDefFromCfg) {
	...
	cnmGetDbdcCapability(prAdapter, prP2pBssInfo->ucBssIndex, prP2pConnReqInfo->rChannelInfo.eBand,
			     prP2pConnReqInfo->rChannelInfo.ucChannelNum,
			     wlanGetSupportNss(prAdapter, prP2pBssInfo->ucBssIndex), &rDbdcCap);

	DBGLOG(P2P, INFO, "p2pRoleFsmRunEventStartAP: start AP at CH %u NSS=%u.\n",
	...
	if ((prP2pBssInfo->eCurrentOPMode != OP_MODE_ACCESS_POINT) || (prP2pBssInfo->eIntendOPMode != OP_MODE_NUM)) {
		...
		if (prP2pRoleFsmInfo->rConnReqInfo.rChannelInfo.ucChannelNum != 0) {
			...
			p2pRoleStatePrepare_To_REQING_CHANNEL_STATE(
				prAdapter, GET_BSS_INFO_BY_INDEX(prAdapter, prP2pRoleFsmInfo->ucBssIndex),
			...
			p2pRoleFsmStateTransition(prAdapter, prP2pRoleFsmInfo, P2P_ROLE_STATE_AP_CHNL_DETECTION);
		}
	}
...
VOID p2pRoleFsmRunEventDelIface(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	prP2pRoleFsmInfo = P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, ucRoleIdx);
	if (!prP2pRoleFsmInfo) {
		DBGLOG(P2P, ERROR, "p2pRoleFsmRunEventDelIface: Corresponding P2P Role FSM empty: %d.\n",
...
VOID p2pRoleFsmRunEventStopAP(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (!prP2pBssInfo) {
		DBGLOG(P2P, ERROR, "prP2pBssInfo of prP2pRoleFsmInfo->ucBssIndex %d is NULL!\n",
...
VOID p2pRoleFsmRunEventDfsCac(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (!prP2pRoleFsmInfo) {
		DBGLOG(P2P, ERROR, "p2pRoleFsmRunEventDfsCac: Corresponding P2P Role FSM empty: %d.\n",
		       prP2pDfsCacMsg->ucRoleIdx);
		goto error;
	}
	...
	cnmDbdcEnableDecision(prAdapter, prP2pBssInfo->ucBssIndex, prP2pConnReqInfo->rChannelInfo.eBand);
	cnmGetDbdcCapability(prAdapter, prP2pBssInfo->ucBssIndex, prP2pConnReqInfo->rChannelInfo.eBand,
			     prP2pConnReqInfo->rChannelInfo.ucChannelNum, prAdapter->rWifiVar.ucNSS, &rDbdcCap);
	...
	if (prP2pRoleFsmInfo->rConnReqInfo.rChannelInfo.ucChannelNum != 0) {
		...
		DBGLOG(P2P, ERROR, "prP2pRoleFsmInfo->rConnReqInfo.rChannelInfo.ucChannelNum shouldn't be 0\n");
	}
...
VOID p2pRoleFsmRunEventConnectionRequest(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (!prP2pBssInfo) {
		DBGLOG(P2P, ERROR, "prP2pRoleFsmInfo->ucBssIndex %d of prAdapter->aprBssInfo is NULL!\n",
		       prP2pRoleFsmInfo->ucBssIndex);
		goto error;
	}
	...
	if (prJoinInfo->prTargetBssDesc == NULL) {
		...
		rlmReviseMaxBw(prAdapter, prP2pBssInfo->ucBssIndex, &prChnlReqInfo->eChnlSco,
			       (P_ENUM_CHANNEL_WIDTH_P)&prChnlReqInfo->eChannelWidth, &prChnlReqInfo->ucCenterFreqS1,
		...
		cnmGetDbdcCapability(prAdapter, prP2pBssInfo->ucBssIndex, prChnlReqInfo->eBand,
				     prChnlReqInfo->ucReqChnlNum,
				     wlanGetSupportNss(prAdapter, prP2pBssInfo->ucBssIndex), &rDbdcCap);

		DBGLOG(P2P, INFO, "p2pRoleFsmRunEventConnectionRequest: start GC at CH %u, NSS=%u.\n",
...
VOID p2pRoleFsmRunEventConnectionAbort(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (!prP2pBssInfo) {
		DBGLOG(P2P, ERROR, "prAdapter->aprBssInfo[prP2pRoleFsmInfo->ucBssIndex(%d)] is NULL!",
		       prP2pRoleFsmInfo->ucBssIndex);
		goto error;
	}
	...
	case OP_MODE_INFRASTRUCTURE: {
		...
		if (UNEQUAL_MAC_ADDR(prP2pBssInfo->prStaRecOfAP->aucMacAddr, prDisconnMsg->aucTargetID) &&
			...
			DBGLOG(P2P, TRACE, "Unequal MAC ADDR [" MACSTR ":" MACSTR "]\n",
			       MAC2STR(prP2pBssInfo->prStaRecOfAP->aucMacAddr), MAC2STR(prDisconnMsg->aucTargetID));
			break;
		}

#if CFG_WPS_DISCONNECT || (KERNEL_VERSION(4, 4, 0) <= CFG80211_VERSION_CODE)
		kalP2PGCIndicateConnectionStatus(prAdapter->prGlueInfo, prP2pRoleFsmInfo->ucRoleIndex, NULL, NULL, 0, 0,
						 WLAN_STATUS_MEDIA_DISCONNECT);
#else
		...
		p2pFuncDisconnect(prAdapter, prP2pBssInfo, prStaRec, prDisconnMsg->fgSendDeauth,
		...
		cnmTimerStartTimer(prAdapter, &(prStaRec->rDeauthTxDoneTimer), P2P_DEAUTH_TIMEOUT_TIME_MS);
	...
	case OP_MODE_ACCESS_POINT: {
		...
		if (prCurrStaRec) {
			...
			if (!prDisconnMsg->fgSendDeauth) {
				p2pRoleFsmDeauhComplete(prAdapter, prCurrStaRec);
				break;
			}
			...
			p2pFuncDisconnect(prAdapter, prP2pBssInfo, prCurrStaRec, prDisconnMsg->fgSendDeauth,
					  prDisconnMsg->u2ReasonCode);
...
VOID p2pRoleFsmRunEventJoinComplete(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (!(prStaRec->ucBssIndex < P2P_DEV_BSS_INDEX)) {
		DBGLOG(P2P, ERROR, "prStaRec->ucBssIndex %d should < P2P_DEV_BSS_INDEX(%d)!\n", prStaRec->ucBssIndex,
		       P2P_DEV_BSS_INDEX);
		goto error;
	}
	...
	if (prJoinCompMsg->ucSeqNum == prJoinInfo->ucSeqNumOfReqMsg) {
		...
		if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
			...
			if (prAssocRspSwRfb) {
				p2pFuncUpdateBssInfoForJOIN(prAdapter, prJoinInfo->prTargetBssDesc, prStaRec,
							    prP2pBssInfo, prAssocRspSwRfb);
			} else {
				DBGLOG(P2P, INFO, "prAssocRspSwRfb is NULL! Skip p2pFuncUpdateBssInfoForJOIN\n");
			}
			...
			if (prJoinInfo->prTargetBssDesc)
				scanReportBss2Cfg80211(prAdapter, BSS_TYPE_P2P_DEVICE, prJoinInfo->prTargetBssDesc);
#if CFG_WPS_DISCONNECT || (KERNEL_VERSION(4, 4, 0) <= CFG80211_VERSION_CODE)
...
VOID p2pRoleFsmRunEventScanRequest(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	for (prScanReqInfo->ucSsidNum = 0; prScanReqInfo->ucSsidNum < prP2pScanReqMsg->i4SsidNum;
	     prScanReqInfo->ucSsidNum++) {
		kalMemCopy(prScanReqInfo->arSsidStruct[prScanReqInfo->ucSsidNum].aucSsid, prP2pSsidStruct->aucSsid,
...
VOID p2pRoleFsmRunEventScanDone(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr,
	...
	case P2P_ROLE_STATE_SCAN:
		...
		if (prConnReqInfo->eConnRequest == P2P_CONNECTION_TYPE_GC) {
			prP2pJoinInfo->prTargetBssDesc = p2pFuncKeepOnConnection(
				prAdapter, prAdapter->aprBssInfo[prP2pRoleFsmInfo->ucBssIndex], prConnReqInfo,
				&prP2pRoleFsmInfo->rChnlReqInfo, &prP2pRoleFsmInfo->rScanReqInfo);
...
#endif
#if CFG_SUPPORT_DBDC
				cnmDbdcEnableDecision(prAdapter, prP2pRoleFsmInfo->ucBssIndex, prChnlReqInfo->eBand);
				cnmGetDbdcCapability(prAdapter, prP2pRoleFsmInfo->ucBssIndex, prChnlReqInfo->eBand,
						     prChnlReqInfo->ucReqChnlNum,
						     wlanGetSupportNss(prAdapter, prP2pRoleFsmInfo->ucBssIndex),
						     &rDbdcCap);

				DBGLOG(P2P, INFO, "p2pRoleFsmRunEventScanDone: start GC at CH %u, NSS=%u.\n",
				       prChnlReqInfo->ucReqChnlNum, rDbdcCap.ucNss);
...
VOID p2pRoleFsmRunEventChnlGrant(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr,
				 IN P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo)
{
	P_P2P_CHNL_REQ_INFO_T prChnlReqInfo = (P_P2P_CHNL_REQ_INFO_T)NULL;
	...
	if (prChnlReqInfo->u4MaxInterval != prMsgChGrant->u4GrantInterval) {
		DBGLOG(P2P, WARN, "P2P Role:%d Request Channel Interval:%d, Grant Interval:%d\n",
		       prP2pRoleFsmInfo->ucRoleIndex, prChnlReqInfo->u4MaxInterval, prMsgChGrant->u4GrantInterval);
	...
	if (ucTokenID == prChnlReqInfo->ucSeqNumOfChReq) {
		...
		case P2P_ROLE_STATE_DFS_CAC:
			...
			cnmTimerStartTimer(prAdapter, &(prP2pRoleFsmInfo->rP2pRoleFsmTimeoutTimer), u4CacTimeMs);
		...
		default:
			...
			if (IS_BSS_ACTIVE(prAdapter->aprBssInfo[prP2pRoleFsmInfo->ucBssIndex])) {
				DBGLOG(P2P, WARN, "p2pRoleFsmRunEventChnlGrant: Invalid CurrentState:%d\n",
				       prP2pRoleFsmInfo->eCurrentState);
				ASSERT(FALSE);
			}
			break;
		...
		if (prChnlReqInfo->fgIsChannelRequested)
			DBGLOG(P2P, ERROR, "fgIsChannelRequested is TRUE!Channel was requested, but released!\n");
	}
...
VOID p2pRoleFsmRunEventSwitchOPMode(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (!(prSwitchOpMode->ucRoleIdx < BSS_P2P_NUM)) {
		DBGLOG(P2P, ERROR, "prSwitchOpMode->ucRoleIdx %d should < BSS_P2P_NUM(%d)\n", prSwitchOpMode->ucRoleIdx,
		       BSS_P2P_NUM);
		goto error;
	}
	...
	if (!(prSwitchOpMode->eOpMode < OP_MODE_NUM)) {
		DBGLOG(P2P, ERROR, "prSwitchOpMode->eOpMode %d should < OP_MODE_NUM(%d)\n", prSwitchOpMode->eOpMode,
		       OP_MODE_NUM);
		goto error;
	}
...
VOID p2pRoleFsmRunEventBeaconUpdate(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	p2pFuncBeaconUpdate(prAdapter, prP2pBssInfo, prBcnUpdateInfo, prBcnUpdateMsg->pucBcnHdr,
			    prBcnUpdateMsg->u4BcnHdrLen, prBcnUpdateMsg->pucBcnBody, prBcnUpdateMsg->u4BcnBodyLen);
	...
	if (prBcnUpdateMsg->pucAssocRespIE != NULL && prBcnUpdateMsg->u4AssocRespLen > 0) {
		...
		if (p2pFuncAssocRespUpdate(prAdapter, prP2pBssInfo, prBcnUpdateMsg->pucAssocRespIE,
...
VOID p2pProcessEvent_UpdateNOAParam(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIdx,
 ...
 */
/*----------------------------------------------------------------------------*/
VOID p2pProcessPreSuspendFlow(IN P_ADAPTER_T prAdapter)
{
	UINT_8 idx;
	...
	for (idx = 0; idx < MAX_BSS_INDEX; idx++) {
		...
		if (prBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT) {
			...
			{
				p2pFuncDisconnect(prAdapter, prBssInfo, prCurrStaRec, FALSE,
						  REASON_CODE_DEAUTH_LEAVING_BSS);
				LINK_REMOVE_KNOWN_ENTRY(prClientList, &prCurrStaRec->rLinkEntry);
			}

			p2pChangeMediaState(prAdapter, prBssInfo, PARAM_MEDIA_STATE_DISCONNECTED);
		...
		if (prBssInfo->eCurrentOPMode == OP_MODE_INFRASTRUCTURE) {
...
#if CFG_WPS_DISCONNECT || (KERNEL_VERSION(4, 4, 0) <= CFG80211_VERSION_CODE)
			kalP2PGCIndicateConnectionStatus(prAdapter->prGlueInfo, (UINT_8)prBssInfo->u4PrivateData, NULL,
			...
			p2pFuncDisconnect(prAdapter, prBssInfo, prBssInfo->prStaRecOfAP, FALSE,
					  REASON_CODE_DEAUTH_LEAVING_BSS);
			p2pFuncStopComplete(prAdapter, prBssInfo);
		}
	}

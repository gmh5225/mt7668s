...
/*----------------------------------------------------------------------------*/
P_BSS_DESC_T
scanSearchBssDescByBssidAndSsid(IN P_ADAPTER_T prAdapter, IN UINT_8 aucBSSID[], IN BOOLEAN fgCheckSsid,
 ...
 */
/*----------------------------------------------------------------------------*/
P_BSS_DESC_T scanSearchBssDescByBssidAndChanNum(IN P_ADAPTER_T prAdapter, IN UINT_8 aucBSSID[],
						IN BOOLEAN fgCheckChanNum, IN UINT_8 ucChannelNum)
{
	P_SCAN_INFO_T prScanInfo;
	P_LINK_T prBSSDescList;
...
/*----------------------------------------------------------------------------*/
P_BSS_DESC_T
scanSearchBssDescByTAAndSsid(IN P_ADAPTER_T prAdapter, IN UINT_8 aucSrcAddr[], IN BOOLEAN fgCheckSsid,
			     IN P_PARAM_SSID_T prSsid)
{
	P_SCAN_INFO_T prScanInfo;
	P_LINK_T prBSSDescList;
...
/*----------------------------------------------------------------------------*/
P_BSS_DESC_T
scanSearchExistingBssDesc(IN P_ADAPTER_T prAdapter, IN ENUM_BSS_TYPE_T eBSSType, IN UINT_8 aucBSSID[],
...
scanSearchRoamBssDescBySsid(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc)
{
	...
	{
		if (EQUAL_SSID(prRoamBssDesc->aucSSID, prRoamBssDesc->ucSSIDLen, prBssDesc->aucSSID,
			       prBssDesc->ucSSIDLen)) {
...
VOID scanAddToRoamBssDesc(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc)
{
	...
	if (prRoamBssDesc == NULL) {
		...
		if (prRoamBssDesc != NULL)
			COPY_SSID(prRoamBssDesc->aucSSID, prRoamBssDesc->ucSSIDLen, prBssDesc->aucSSID,
				  prBssDesc->ucSSIDLen);
	}
...
#define SSID_ONLY_EXIST_ONE_AP 1 /* If only exist one same ssid AP, avoid unnecessary scan */
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T)
	{
		if (EQUAL_SSID(prBssDesc->aucSSID, prBssDesc->ucSSIDLen, prAisBssInfo->aucSSID,
...
P_BSS_DESC_T scanAddToBssDesc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (prAdapter->fg5gDisable == TRUE && HAL_RX_STATUS_GET_RF_BAND(prRxStatus) == BAND_5G) {
		DBGLOG(SCN, INFO, "disable 5g so do not add 5g in scan list\n");
		return NULL;
	}
		...
		case ELEM_ID_SSID:
			if (IE_LEN(pucIE) <= ELEM_MAX_LEN_SSID) {
				...
				if (fgIsValidSsid == TRUE) {
					COPY_SSID(rSsid.aucSsid, rSsid.u4SsidLen, SSID_IE(pucIE)->aucSSID,
	...
	prBssDesc = scanSearchExistingBssDescWithSsid(prAdapter, eBSSType, (PUINT_8)prWlanBeaconFrame->aucBSSID,
						      (PUINT_8)prWlanBeaconFrame->aucSrcAddr, fgIsValidSsid,
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset)
	{
...
#if CFG_SUPPORT_BFEE
			prBssDesc->ucVhtCapNumSoundingDimensions = ((((P_IE_VHT_CAP_T)pucIE)->u4VhtCapInfo) &
		...
		case ELEM_ID_VHT_OP:
			...
			rlmModifyVhtBwPara(&prBssDesc->ucCenterFreqS1, &prBssDesc->ucCenterFreqS2,
...
WLAN_STATUS scanAddScanResult(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc, IN P_SW_RFB_T prSwRfb)
{
	...
	kalIndicateBssInfo(prAdapter->prGlueInfo, (PUINT_8)prSwRfb->pvHeader, prSwRfb->u2PacketLen,
...
WLAN_STATUS scanProcessBeaconAndProbeResp(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (prBssDesc) {
		...
		if (((prBssDesc->eBSSType == BSS_TYPE_INFRASTRUCTURE && prConnSettings->eOPMode != NET_TYPE_IBSS) ||
			...
			if (EQUAL_SSID(prBssDesc->aucSSID, prBssDesc->ucSSIDLen, prConnSettings->aucSSID,
				       prConnSettings->ucSSIDLen) &&
			...
			if ((prBssDesc->eBSSType == eBSSType)
				...
				if (eBSSType == BSS_TYPE_INFRASTRUCTURE) {
					if (prBssDesc->u2RawLength != 0) {
						kalIndicateBssInfo(prAdapter->prGlueInfo, (PUINT_8)prBssDesc->aucRawBuf,
								   prBssDesc->u2RawLength, prBssDesc->ucChannelNum,
...
#if CFG_ENABLE_WIFI_DIRECT
					if ((prBssDesc->fgIsP2PReport == TRUE) && prBssDesc->u2RawLength != 0) {
#endif
						...
						kalP2PIndicateBssInfo(prAdapter->prGlueInfo,
								      ...
								      prBssDesc->u2RawLength, &rChannelInfo,
								      RCPI_TO_dBm(prBssDesc->ucRCPI));

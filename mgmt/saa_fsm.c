...
static PUINT_8 apucDebugAAState[AA_STATE_NUM] = {
	(PUINT_8)DISP_STRING("AA_IDLE"),	 (PUINT_8)DISP_STRING("SAA_SEND_AUTH1"),
	(PUINT_8)DISP_STRING("SAA_WAIT_AUTH2"),	 (PUINT_8)DISP_STRING("SAA_SEND_AUTH3"),
	(PUINT_8)DISP_STRING("SAA_WAIT_AUTH4"),	 (PUINT_8)DISP_STRING("SAA_SEND_ASSOC1"),
	(PUINT_8)DISP_STRING("SAA_WAIT_ASSOC2"), (PUINT_8)DISP_STRING("AAA_SEND_AUTH2"),
	(PUINT_8)DISP_STRING("AAA_SEND_AUTH4"),	 (PUINT_8)DISP_STRING("AAA_SEND_ASSOC2"),
...
void saaSendAuthAssoc(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	if (prStaRec->ucTxAuthAssocRetryCount >= prStaRec->ucTxAuthAssocRetryLimit) {
		...
		prStaRec->u2StatusCode = (prConnSettings->fgIsSendAssoc) ? STATUS_CODE_ASSOC_TIMEOUT :
		...
		if (!prConnSettings->fgIsSendAssoc) {
			...
			if (((prAdapter->prGlueInfo->rWpaInfo.u4AuthAlg & AUTH_TYPE_SAE) ||
				...
				kalMemCopy(&u2AuthTransSN, prConnSettings->aucAuthData,
			...
			if (prConnSettings->ucSSIDLen) {
				...
				COPY_SSID(rParamSsid.aucSsid, rParamSsid.u4SsidLen, prConnSettings->aucSSID,
					  prConnSettings->ucSSIDLen);
				prBssDesc = scanSearchBssDescByBssidAndSsid(prAdapter, prStaRec->aucMacAddr, TRUE,
									    &rParamSsid);
				DBGLOG(RSN, INFO,
				       "[RSN] prBssDesc[" MACSTR " ,%s] Searched by BSSID[" MACSTR "] & SSID %s.\n",
				       MAC2STR(prBssDesc->aucBSSID), prBssDesc->aucSSID, MAC2STR(prStaRec->aucMacAddr),
				       prConnSettings->aucSSID);
			} else {
				prBssDesc = scanSearchBssDescByBssidAndChanNum(prAdapter, prStaRec->aucMacAddr, TRUE,
									       prConnSettings->ucChannelNum);
				DBGLOG(RSN, INFO,
				       "[RSN] prBssDesc[" MACSTR " ,%s] Searched by BSSID[" MACSTR "] & ChanNum %d.\n",
				       MAC2STR(prBssDesc->aucBSSID), prBssDesc->aucSSID, MAC2STR(prStaRec->aucMacAddr),
				       prConnSettings->ucChannelNum);
			}
...
#else
		rStatus = authSendAuthFrame(prAdapter, prStaRec, prStaRec->ucBssIndex, NULL, AUTH_TRANSACTION_SEQ_3,
					    STATUS_CODE_RESERVED);
#endif /* CFG_SUPPORT_AAA */
	...
	do {
		...
		case AA_STATE_IDLE:
			...
			if (ePreviousState != prStaRec->eAuthAssocState) { /* Only trigger this event once */
				...
				if (prRetainedSwRfb) {
					if (saaFsmSendEventJoinComplete(prAdapter, WLAN_STATUS_SUCCESS, prStaRec,
					...
					if (saaFsmSendEventJoinComplete(prAdapter, WLAN_STATUS_FAILURE, prStaRec,
...
#else
				rStatus = authSendAuthFrame(prAdapter, prStaRec, prStaRec->ucBssIndex, NULL,
							    AUTH_TRANSACTION_SEQ_1, STATUS_CODE_RESERVED);
#endif /* CFG_SUPPORT_AAA */
				if (rStatus != WLAN_STATUS_SUCCESS) {
					cnmTimerInitTimer(prAdapter, &prStaRec->rTxReqDoneOrRxRespTimer,
...
#else
				rStatus = authSendAuthFrame(prAdapter, prStaRec, prStaRec->ucBssIndex, NULL,
							    AUTH_TRANSACTION_SEQ_3, STATUS_CODE_RESERVED);
#endif /* CFG_SUPPORT_AAA */
				if (rStatus != WLAN_STATUS_SUCCESS) {
					cnmTimerInitTimer(prAdapter, &prStaRec->rTxReqDoneOrRxRespTimer,
							  (PFN_MGMT_TIMEOUT_FUNC)saaFsmRunEventTxReqTimeOut,
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
saaFsmSendEventJoinComplete(IN P_ADAPTER_T prAdapter, IN WLAN_STATUS rJoinStatus, IN P_STA_RECORD_T prStaRec,
...
saaFsmRunEventTxDone(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	if (!IS_STA_IN_P2P(prStaRec)) {
		...
		if ((prStaRec->eAuthAssocSent >= AA_SENT_AUTH1) && (prStaRec->eAuthAssocSent <= AA_SENT_AUTH4)) {
			if (authCheckTxAuthFrame(prAdapter, prMsduInfo, prStaRec->eAuthAssocSent) !=
			    WLAN_STATUS_SUCCESS)
				return WLAN_STATUS_SUCCESS;
...
VOID saaFsmRunEventRxAuth(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if ((authCheckRxAuthFrameStatus(prAdapter, prSwRfb, prStaRec->eAuthAssocSent, &u2StatusCode) ==
		...
		if ((prAuthFrame->u2AuthAlgNum == AUTH_ALGORITHM_NUM_SHARED_KEY) &&
			...
			kalIndicateRxAuthToUpperLayer(prGlueInfo->prDevHandler, (PUINT_8)prAuthFrame,
...
WLAN_STATUS saaFsmRunEventRxAssoc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if ((assocCheckRxReAssocRspFrameStatus(prAdapter, prSwRfb, &u2StatusCode) == WLAN_STATUS_SUCCESS) &&
		...
		if (prConnSettings->bss) {
			kalIndicateRxAssocToUpperLayer(prGlueInfo->prDevHandler, (PUINT_8)prAssocRspFrame,
						       prConnSettings->bss, (size_t)prSwRfb->u2PacketLen);
			DBGLOG(SAA, INFO, "Report RX Assoc to upper layer, Done\n");
...
WLAN_STATUS saaFsmRunEventRxDeauth(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		if (IS_STA_IN_AIS(prStaRec)) {
			...
			if (prStaRec->ucStaState > STA_STATE_1) {
				...
				if (authProcessRxDeauthFrame(prSwRfb, prStaRec->aucMacAddr, &prStaRec->u2ReasonCode) ==
					...
					DBGLOG(SAA, INFO, "notification of RX deauthentication %d\n",
					       prSwRfb->u2PacketLen);
					kalIndicateRxDeauthToUpperLayer(prAdapter->prGlueInfo->prDevHandler,
					...
					g_rDisconnInfoTemp.u2DisassocSeqNum =
						(prDeauthFrame->u2SeqCtrl & MASK_SC_SEQ_NUM) >> MASK_SC_SEQ_NUM_OFFSET;
#endif
			...
			kalIndicateRxDeauthToUpperLayer(prAdapter->prGlueInfo->prP2PInfo[ucRoleIdx]->aprRoleHandler,
				 ...
				 IN ENUM_AA_FRM_TYPE_T eFrmType)
{
	do {
		...
		g_rDisconnInfoTemp.ucDisassocReason = prStaRec->u2ReasonCode;
#endif
	} while (0);
}
...
WLAN_STATUS saaFsmRunEventRxDisassoc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		if (IS_STA_IN_AIS(prStaRec)) {
			...
			if (prStaRec->ucStaState > STA_STATE_1) {
				/* Check if this is the AP we are associated or associating with */
				if (assocProcessRxDisassocFrame(prAdapter, prSwRfb, prStaRec->aucMacAddr,
					...
					DBGLOG(SAA, INFO, "notification of RX disassociation %d\n",
					       prSwRfb->u2PacketLen);
					if (wdev->current_bss)
						kalIndicateRxDisassocToUpperLayer(prAdapter->prGlueInfo->prDevHandler,
...
#if CFG_DISCONN_DEBUG_FEATURE
					g_rDisconnInfoTemp.u2DisassocSeqNum =
						(prDisassocFrame->u2SeqCtrl & MASK_SC_SEQ_NUM) >>

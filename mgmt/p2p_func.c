...
static PUINT_8 apucDfsState[DFS_STATE_NUM] = { (PUINT_8)DISP_STRING("DFS_STATE_INACTIVE"),
					       (PUINT_8)DISP_STRING("DFS_STATE_CHECKING"),
					       (PUINT_8)DISP_STRING("DFS_STATE_ACTIVE"),
					       (PUINT_8)DISP_STRING("DFS_STATE_DETECTED") };

PUINT_8 apucW53RadarType[3] = { (PUINT_8)DISP_STRING("Unknown Type"), (PUINT_8)DISP_STRING("Type 1 (short pulse)"),
				(PUINT_8)DISP_STRING("Type 2 (short pulse)") };
PUINT_8 apucW56RadarType[12] = { (PUINT_8)DISP_STRING("Unknown Type"),
...
static VOID p2pFuncParseBeaconVenderId(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucIE,
...
static VOID p2pFuncProcessP2pProbeRspAction(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucIEBuf, IN UINT_8 ucElemIdType,
					    OUT UINT_8 *ucBssIdx, OUT P_BSS_INFO_T *prP2pBssInfo,
					    OUT BOOLEAN *fgIsWSCIE, OUT BOOLEAN *fgIsP2PIE, OUT BOOLEAN *fgIsWFDIE);
static VOID p2pFuncGetSpecAttriAction(IN P_IE_P2P_T prP2pIE, IN UINT_8 ucOuiType, IN UINT_8 ucAttriID,
...
VOID p2pFuncRequestScan(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, IN P_P2P_SCAN_REQ_INFO_T prScanReqInfo)
{
	...
	do {
		...
		prScanReqV2 = (P_MSG_SCN_SCAN_REQ_V2)cnmMemAlloc(
			...
			(sizeof(MSG_SCN_SCAN_REQ_V2) + (sizeof(PARAM_SSID_T) * prScanReqInfo->ucSsidNum)));
		if (!prScanReqV2) {
		...
		case SCAN_CHANNEL_P2P_SOCIAL: {
			...
			for (prScanReqV2->ucSSIDNum = 0; prScanReqV2->ucSSIDNum < prScanReqInfo->ucSsidNum;
			     prScanReqV2->ucSSIDNum++) {
				COPY_SSID(prParamSsid->aucSsid, prParamSsid->u4SsidLen,
...
VOID p2pFuncGCJoin(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prP2pBssInfo, IN P_P2P_JOIN_INFO_T prP2pJoinInfo)
{
	...
	do {
		...
		if (prBssDesc->ucSSIDLen) {
			COPY_SSID(prP2pBssInfo->aucSSID, prP2pBssInfo->ucSSIDLen, prBssDesc->aucSSID,
				  prBssDesc->ucSSIDLen);
		}
		...
		prStaRec = bssCreateStaRecFromBssDesc(
			prAdapter, (prBssDesc->fgIsP2PPresent ? (STA_TYPE_P2P_GO) : (STA_TYPE_LEGACY_AP)),
...
WLAN_STATUS
p2pFuncTxMgmtFrame(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, IN P_MSDU_INFO_T prMgmtTxMsdu,
		   IN BOOLEAN fgNonCckRate)
{
	...
	do {
		...
		case MAC_FRAME_PROBE_RSP:
			...
			prMgmtTxMsdu->ucStaRecIndex = (prStaRec != NULL) ? (prStaRec->ucIndex) :
		...
		TX_SET_MMPDU(prAdapter, prMgmtTxMsdu, prMgmtTxMsdu->ucBssIndex,
			     ...
			     WLAN_MAC_MGMT_HEADER_LEN, prMgmtTxMsdu->u2FrameLength, p2pDevFsmRunEventMgmtFrameTxDone,
			     MSDU_RATE_MODE_AUTO);
		    ...
		    IN P_P2P_CONNECTION_REQ_INFO_T prP2pConnReqInfo, IN P_P2P_CHNL_REQ_INFO_T prP2pChnlReqInfo)
{
	...
	do {
...
#if (CFG_SUPPORT_DFS_MASTER == 1)
		prCmdRddOnOffCtrl =
			(P_CMD_RDD_ON_OFF_CTRL_T)cnmMemAlloc(prAdapter, RAM_TYPE_MSG, sizeof(*prCmdRddOnOffCtrl));
		...
		wlanSendSetQueryCmd(prAdapter, CMD_ID_RDD_ON_OFF_CTRL, TRUE, FALSE, FALSE, NULL, NULL,
		...
		/* 4 <1.1> Assign SSID */
		COPY_SSID(prBssInfo->aucSSID, prBssInfo->ucSSIDLen, prP2pConnReqInfo->rSsidStruct.aucSsid,
		...
		} else if ((prP2pConnReqInfo->eConnRequest == P2P_CONNECTION_TYPE_PURE_AP) || fgIs11bRate) {
			/* Depend on eBand */
		...
		bssDetermineApBssInfoPhyTypeSet(
			prAdapter, (prP2pConnReqInfo->eConnRequest == P2P_CONNECTION_TYPE_PURE_AP) ? TRUE : FALSE,
			prBssInfo);

		prBssInfo->ucNonHTBasicPhyType =
			(UINT_8)rNonHTApModeAttributes[prBssInfo->ucConfigAdHocAPMode].ePhyTypeIndex;
...
VOID p2pFuncSwitchOPMode(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prP2pBssInfo, IN ENUM_OP_MODE_T eOpMode,
	...
	do {
		...
		if (prP2pBssInfo->eCurrentOPMode != eOpMode) {
			...
			if (1) {
				...
				wlanSendSetQueryCmd(prAdapter, CMD_ID_P2P_ABORT, TRUE, FALSE, FALSE, NULL, NULL,
...
VOID p2pFuncReleaseCh(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIdx, IN P_P2P_CHNL_REQ_INFO_T prChnlReqInfo)
{
	...
	do {
		...
		DBGLOG(P2P, INFO, "p2pFuncReleaseCh: P2P abort channel on band %u.\n", prMsgChRelease->eDBDCBand);
...
VOID p2pFuncAcquireCh(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIdx, IN P_P2P_CHNL_REQ_INFO_T prChnlReqInfo)
{
	...
	do {
		...
		DBGLOG(P2P, INFO, "p2pFuncAcquireCh: P2P Request channel on band %u.\n", prMsgChReq->eDBDCBand);
#endif /*CFG_SUPPORT_DBDC*/
...
VOID p2pFuncStartRdd(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIdx)
{
	...
	prP2pRoleFsmInfo = P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, prAdapter->aprBssInfo[ucBssIdx]->u4PrivateData);
...
VOID p2pFuncDfsSwitchCh(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prBssInfo, IN P2P_CHNL_REQ_INFO_T rP2pChnlReqInfo)
{
	 ...
	 */
	cfg80211_ch_switch_notify(prGlueInfo->prP2PInfo[role_idx]->aprRoleHandler,
...
VOID p2pFuncShowRadarInfo(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIdx)
{
	...
	if (g_rP2pRadarInfo.ucRadarReportMode == 1) {
		prP2pRoleFsmInfo =
			P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, prAdapter->aprBssInfo[ucBssIdx]->u4PrivateData);
		...
		if (g_rP2pRadarInfo.ucPeriodicDetected) {
			...
			for (ucCnt = 1; ucCnt < g_rP2pRadarInfo.ucPPBNum; ucCnt++) {
				DBGLOG(P2P, INFO, "%-10d    %-11d    %d\n",
				       ...
				       g_rP2pRadarInfo.arPpbContent[ucCnt].u2PeriodicPulseWidth,
				       (g_rP2pRadarInfo.arPpbContent[ucCnt].u4PeriodicStartTime -
					g_rP2pRadarInfo.arPpbContent[ucCnt - 1].u4PeriodicStartTime) *
					       2 / 5);
			}
			...
			for (ucCnt = 1; ucCnt < g_rP2pRadarInfo.ucLPBNum; ucCnt++) {
				DBGLOG(P2P, INFO, "%-10d    %-11d    %d\n",
				       ...
				       g_rP2pRadarInfo.arLpbContent[ucCnt].u2LongPulseWidth,
				       (g_rP2pRadarInfo.arLpbContent[ucCnt].u4LongStartTime -
					g_rP2pRadarInfo.arLpbContent[ucCnt - 1].u4LongStartTime) *
					       2 / 5);
			}
		}
	}
...
{
	DBGLOG(P2P, INFO, "[DFS_STATE] TRANSITION: [%s] -> [%s]\n", apucDfsState[g_ucDfsState],
	       apucDfsState[ucDfsState]);
		    ...
		    IN PUINT_8 pucNewBcnBody, IN UINT_32 u4NewBodyLen)
{
	...
	do {
		...
		p2pFuncParseBeaconContent(prAdapter, prP2pBssInfo, (PUINT_8)prBcnFrame->aucInfoElem,
...
#else
		nicUpdateBeaconIETemplate(prAdapter, IE_UPD_METHOD_UPDATE_ALL, NETWORK_TYPE_P2P_INDEX,
					  prBcnFrame->u2CapInfo, (PUINT_8)prBcnFrame->aucInfoElem,
					  (prBcnMsduInfo->u2FrameLength - OFFSET_OF(WLAN_BEACON_FRAME_T, aucInfoElem)));
#endif
	} while (FALSE);
...
WLAN_STATUS
p2pFuncAssocRespUpdate(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prP2pBssInfo, IN PUINT_8 AssocRespIE,
		       IN UINT_32 u4AssocRespLen)
{
	UINT_8 ucOuiType = 0;
	...
	do {
		...
		case OP_MODE_INFRASTRUCTURE:
			...
			if (prP2pBssInfo->prStaRecOfAP) {
				...
				p2pFuncDisconnect(prAdapter, prP2pBssInfo, prP2pBssInfo->prStaRecOfAP, fgSendDeauth,
						  u2ReasonCode);
			}
		...
		case OP_MODE_ACCESS_POINT:
			 ...
			 */
			/* Send deauth. */
			authSendDeauthFrame(prAdapter, prP2pBssInfo, NULL, (P_SW_RFB_T)NULL, u2ReasonCode,
...
/*----------------------------------------------------------------------------*/
BOOLEAN
p2pFuncValidateAuth(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prP2pBssInfo, IN P_SW_RFB_T prSwRfb,
		    IN PP_STA_RECORD_T pprStaRec, OUT PUINT_16 pu2StatusCode)
{
	...
	*pu2StatusCode = STATUS_CODE_REQ_DECLINED;
	...
	if (!prStaRec) {
		prStaRec =
			cnmStaRecAlloc(prAdapter, STA_TYPE_P2P_GC, prP2pBssInfo->ucBssIndex, prAuthFrame->aucSrcAddr);
...
BOOLEAN p2pFuncParseCheckForTKIPInfoElem(IN PUINT_8 pucBuf)
{
	...
	if (prWpaIE->ucOuiType == VENDOR_OUI_TYPE_WPA && u4GroupKeyCipher == WPA_CIPHER_SUITE_TKIP)
		return TRUE;
	else
		return FALSE;
...
BOOLEAN p2pFuncParseCheckForP2PInfoElem(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucBuf, OUT PUINT_8 pucOuiType)
{
	...
	do {
		...
		if (IE_LEN(pucBuf) <= ELEM_MIN_LEN_WFA_OUI_TYPE_SUBTYPE) {
			break;
		} else if (prWfaIE->aucOui[0] != aucWfaOui[0] || prWfaIE->aucOui[1] != aucWfaOui[1] ||
			...
			IN BOOLEAN fgIsDevInterface, IN UINT_8 ucRoleIdx)
{
	...
	do {
		...
		/* TODO: */
		if ((fgApplyp2PDevFilter && (prAdapter->u4OsPacketFilter & PARAM_PACKET_FILTER_PROBE_REQ)) ||
		    (!fgApplyp2PDevFilter && (prP2pRoleFsmInfo->u4P2pPacketFilter & PARAM_PACKET_FILTER_PROBE_REQ))) {
...
VOID p2pFuncParseBeaconContent(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prP2pBssInfo, IN PUINT_8 pucIEInfo,
			       IN UINT_32 u4IELen)
{
	...
	do {
		...
		IE_FOR_EACH(pucIE, u4IELen, u2Offset)
		{
			...
			case ELEM_ID_SUP_RATES: /* 1 */ /* V */ /* Done */
			{
				...
				kalMemCopy(prP2pBssInfo->aucAllSupportedRates, SUP_RATES_IE(pucIE)->aucSupportedRates,
			...
			case ELEM_ID_ERP_INFO: /* 42 */ /* V */
			{
				...
				prP2pBssInfo->fgErpProtectMode =
					((ERP_INFO_IE(pucIE)->ucERP &
					  (ERP_INFO_USE_PROTECTION | ERP_INFO_NON_ERP_PRESENT)) ?
						 TRUE :
						 FALSE);
#endif
			...
			case ELEM_ID_HT_CAP: /* 45 */ /* V */
			{
				...
				DBGLOG(P2P, TRACE, "HT Extended Cap:%x, TX Beamforming Cap:%x, Ant Selection Cap:%x\n",
				...
				ASSERT(HT_CAP_IE(pucIE)->u2HtExtendedCap ==
				       (HT_EXT_CAP_DEFAULT_VAL & ~(HT_EXT_CAP_PCO | HT_EXT_CAP_PCO_TRANS_TIME_NONE)));
			...
			case ELEM_ID_RSN: /* 48 */ /* V */
				...
				for (i = 0; i < rRsnIe.u4AuthKeyMgtSuiteCount; i++) {
					if ((rRsnIe.au4AuthKeyMgtSuite[i] == RSN_AKM_SUITE_PSK_SHA256) ||
						...
						prP2pBssInfo->u4RsnSelectedAKMSuite = rRsnIe.au4AuthKeyMgtSuite[i];
	...
	do {
		...
		if (rsnParseCheckForWFAInfoElem(prAdapter, pucIE, &ucOuiType, &u2SubTypeVersion)) {
			...
			if (ucOuiType == VENDOR_OUI_TYPE_P2P) {
				kalMemCopy(
					&prP2pSpecificBssInfo->aucAttributesCache[prP2pSpecificBssInfo->u2AttributeLen],
					pucIE, IE_SIZE(pucIE));
			...
			IN P_P2P_SCAN_REQ_INFO_T prScanReqInfo)
{
	...
	do {
		ASSERT_BREAK((prAdapter != NULL) && (prBssInfo != NULL) && (prConnReqInfo != NULL) &&
...
VOID p2pFuncStoreAssocRspIEBuffer(IN P_ADAPTER_T prAdapter, IN P_P2P_JOIN_INFO_T prP2pJoinInfo, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		i2IELen = prSwRfb->u2PacketLen -
			  (WLAN_MAC_HEADER_LEN + CAP_INFO_FIELD_LEN + STATUS_CODE_FIELD_LEN + AID_FIELD_LEN);
 ...
 */
/*----------------------------------------------------------------------------*/
VOID p2pFuncMgmtFrameRegister(IN P_ADAPTER_T prAdapter, IN UINT_16 u2FrameType, IN BOOLEAN fgIsRegistered,
			      OUT PUINT_32 pu4P2pPacketFilter)
{
	...
	do {
		...
		wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_RX_FILTER, TRUE, FALSE, FALSE, nicCmdEventSetCommon,
				    nicOidCmdTimeoutCommon, sizeof(CMD_RX_PACKET_FILTER), (PUINT_8)&rSetRxPacketFilter,
...
VOID p2pFuncUpdateMgmtFrameRegister(IN P_ADAPTER_T prAdapter, IN UINT_32 u4OsFilter)
{
	...
	do {
		...
		if ((prAdapter->u4OsPacketFilter & PARAM_PACKET_FILTER_P2P_MASK) ^ u4OsFilter) {
			...
			wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_RX_FILTER, TRUE, FALSE, FALSE, nicCmdEventSetCommon,
					    nicOidCmdTimeoutCommon, sizeof(CMD_RX_PACKET_FILTER),
...
P_MSDU_INFO_T p2pFuncProcessP2pProbeRsp(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIdx, IN P_MSDU_INFO_T prMgmtTxMsdu)
{
	...
	do {
		...
		u2ProbeRspHdrLen = (WLAN_MAC_MGMT_HEADER_LEN + TIMESTAMP_FIELD_LEN + BEACON_INTERVAL_FIELD_LEN +
				    CAP_INFO_FIELD_LEN);
...
#endif
		kalP2PResetP2P_IE(prGlueInfo, (UINT_8)prP2pBssInfo->u4PrivateData);
		IE_FOR_EACH(pucIEBuf, u2IELength, u2Offset)
		{
			switch (IE_ID(pucIEBuf)) {
			case ELEM_ID_SSID: {
				p2pFuncProcessP2pProbeRspAction(prAdapter, pucIEBuf, ELEM_ID_SSID, &ucBssIdx,
								&prP2pBssInfo, &fgIsWSCIE, &fgIsP2PIE, &fgIsWFDIE);
			} break;
			case ELEM_ID_VENDOR: {
				p2pFuncProcessP2pProbeRspAction(prAdapter, pucIEBuf, ELEM_ID_VENDOR, &ucBssIdx,
								&prP2pBssInfo, &fgIsWSCIE, &fgIsP2PIE, &fgIsWFDIE);
			} break;
			default:
				break;
		...
		u2EstimateSize = WLAN_MAC_MGMT_HEADER_LEN + TIMESTAMP_FIELD_LEN + BEACON_INTERVAL_FIELD_LEN +
				 CAP_INFO_FIELD_LEN + (ELEM_HDR_LEN + ELEM_MAX_LEN_SSID) +
		...
		for (u4Idx = 0; u4Idx < u4IeArraySize; u4Idx++) {
			if (txProbeRspIETable[u4Idx].u2EstimatedFixedIELen) {
				...
				u2EstimatedExtraIELen += (UINT_16)(txProbeRspIETable[u4Idx].pfnCalculateVariableIELen(
		...
		if (fgIsWSCIE)
			u2EstimatedExtraIELen +=
				kalP2PCalWSC_IELen(prAdapter->prGlueInfo, 2, (UINT_8)prP2pBssInfo->u4PrivateData);

		if (fgIsP2PIE) {
			for (i = 0; i < MAX_MULTI_P2P_IE_COUNT; i++) {
				u2EstimatedExtraIELen +=
					kalP2PCalP2P_IELen(prGlueInfo, i, (UINT_8)prP2pBssInfo->u4PrivateData);
			}
		...
		bssComposeBeaconProbeRespFrameHeaderAndFF(
			(PUINT_8)((ULONG)(prRetMsduInfo->prPacket) + MAC_TX_RESERVED_FIELD),
			prProbeRspFrame->aucDestAddr, prProbeRspFrame->aucSrcAddr, prProbeRspFrame->aucBSSID,
			prProbeRspFrame->u2BeaconInterval, prProbeRspFrame->u2CapInfo);

		prRetMsduInfo->u2FrameLength = (WLAN_MAC_MGMT_HEADER_LEN + TIMESTAMP_FIELD_LEN +
		...
		if (fgIsWSCIE) {
			kalP2PGenWSC_IE(prAdapter->prGlueInfo, 2,
					(PUINT_8)((ULONG)prRetMsduInfo->prPacket + (ULONG)prRetMsduInfo->u2FrameLength),
					(UINT_8)prP2pBssInfo->u4PrivateData);

			prRetMsduInfo->u2FrameLength += (UINT_16)kalP2PCalWSC_IELen(
				prAdapter->prGlueInfo, 2, (UINT_8)prP2pBssInfo->u4PrivateData);
		}
		...
		if (fgIsWFDIE > 0) {
			...
			kalMemCopy((PUINT_8)((ULONG)prRetMsduInfo->prPacket + (ULONG)prRetMsduInfo->u2FrameLength),
					    ...
					    OUT BOOLEAN *fgIsWSCIE, OUT BOOLEAN *fgIsP2PIE, OUT BOOLEAN *fgIsWFDIE)
{
		...
		if (SSID_IE(pucIEBuf)->ucLength > 7) {
			for ((*ucBssIdx) = 0; (*ucBssIdx) < MAX_BSS_INDEX; (*ucBssIdx)++) {
				*prP2pBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter, *ucBssIdx);
				if (!(*prP2pBssInfo))
					continue;
				if (EQUAL_SSID((*prP2pBssInfo)->aucSSID, (*prP2pBssInfo)->ucSSIDLen,
			...
			*prP2pBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter, P2P_DEV_BSS_INDEX);
			COPY_SSID((*prP2pBssInfo)->aucSSID, (*prP2pBssInfo)->ucSSIDLen, SSID_IE(pucIEBuf)->aucSSID,
	...
	case ELEM_ID_VENDOR:
		if (rsnParseCheckForWFAInfoElem(prAdapter, pucIEBuf, &ucOuiType, &u2SubTypeVersion)) {
			...
			if (ucOuiType == VENDOR_OUI_TYPE_P2P) {
				kalP2PUpdateP2P_IE(prAdapter->prGlueInfo, pucIEBuf, IE_SIZE(pucIEBuf),
						   (UINT_8)((P_BSS_INFO_T)*prP2pBssInfo)->u4PrivateData);
				*fgIsP2PIE = TRUE;
			}
#if CFG_SUPPORT_WFD
				...
				if ((sizeof(prAdapter->prGlueInfo
						    ->prP2PInfo[((P_BSS_INFO_T)*prP2pBssInfo)->u4PrivateData]
						    ->aucWFDIE) >=
...
VOID p2pFuncGenerateP2p_IEForAssocRsp(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	if (IS_STA_IN_P2P(prStaRec)) {
		...
		p2pFuncGenerateP2P_IE(prAdapter, prMsduInfo->ucBssIndex, TRUE, &prMsduInfo->u2FrameLength,
				      prMsduInfo->prPacket, 1500, txAssocRspAttributesTable,
				      sizeof(txAssocRspAttributesTable) / sizeof(APPEND_VAR_ATTRI_ENTRY_T));
	} else {
	...
	do {
		...
		for (i = 0; i < u4AttriTableSize; i++) {
			if (arAppendAttriTable[i].pfnAppendAttri) {
				u4AttriLen = arAppendAttriTable[i].pfnAppendAttri(prAdapter, ucBssIndex, fgIsAssocFrame,
				...
				if (u4OverallAttriLen > P2P_MAXIMUM_ATTRIBUTE_LEN) {
					...
					pucBuffer = (PUINT_8)((ULONG)prIeP2P +
		    ...
		    IN UINT_8 ucAttriID)
{
	...
	do {
		...
		do {
			fgIsMore = FALSE;
			prP2pIE = (P_IE_P2P_T)p2pFuncGetSpecIE(prAdapter, pucIE, u2BufferLenLeft, ELEM_ID_VENDOR,
							       &fgIsMore);
			if (prP2pIE) {
				      ...
				      OUT P_ATTRIBUTE_HDR_T *prTargetAttri)
{
	...
	if (prP2pIE->ucOuiType == ucOuiType) {
		...
		if ((prP2pIE->aucOui[0] == aucWfaOui[0]) && (prP2pIE->aucOui[1] == aucWfaOui[1]) &&
...
WLAN_STATUS
p2pFuncGenerateBeaconProbeRsp(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prBssInfo, IN P_MSDU_INFO_T prMsduInfo,
			      IN BOOLEAN fgIsProbeRsp)
{
	...
	do {
		...
		return nicUpdateBeaconIETemplate(prAdapter, IE_UPD_METHOD_UPDATE_ALL, prBssInfo->ucBssIndex,
						 prBssInfo->u2CapInfo, (PUINT_8)prBcnFrame->aucInfoElem,
...
WLAN_STATUS
p2pFuncComposeBeaconProbeRspTemplate(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prP2pBssInfo, IN PUINT_8 pucBcnBuffer,
				     IN UINT_32 u4BcnBufLen, IN BOOLEAN fgIsProbeRsp,
...
VOID wfdFuncGenerateWfdIEForAssocRsp(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	if (u2EstimatedExtraIELen > 0 && u2EstimatedExtraIELen < VENDOR_SPECIFIC_IE_LENGTH) {
		kalMemCopy((prMsduInfo->prPacket + prMsduInfo->u2FrameLength), prP2PInfo->aucWFDIE,
			   u2EstimatedExtraIELen);
...
VOID p2pFuncComposeNoaAttribute(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, OUT PUINT_8 aucNoaAttrArray,
				OUT PUINT_32 pu4Len)
{
	P_BSS_INFO_T prBssInfo = NULL;
...
BOOLEAN p2pFuncIsBufferableMMPDU(IN P_MSDU_INFO_T prMgmtTxMsdu)
{
	...
	prWlanHdr = (P_WLAN_MAC_HEADER_T)((unsigned long)prMgmtTxMsdu->prPacket + MAC_TX_RESERVED_FIELD);

...
VOID secInit(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex)
{
	...
	prAdapter->rMib.dot11RSNAConfigAuthenticationSuitesTable[13].dot11RSNAConfigAuthenticationSuite =
		RSN_AKM_SUITE_SAE;
	prAdapter->rMib.dot11RSNAConfigAuthenticationSuitesTable[14].dot11RSNAConfigAuthenticationSuite =
		RSN_AKM_SUITE_OWE;
#endif
...
#if CFG_SUPPORT_802_11W
	cnmTimerInitTimer(prAdapter, &prAisSpecBssInfo->rSaQueryTimer, (PFN_MGMT_TIMEOUT_FUNC)rsnStartSaQueryTimer,
			  (ULONG)NULL);
#endif
...
BOOL secCheckClassError(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, IN P_STA_RECORD_T prStaRec)
{
	...
	if (!prStaRec || (prRxStatus->u2StatusFlag & RXS_DW2_RX_CLASSERR_BITMAP) == RXS_DW2_RX_CLASSERR_VALUE) {
		DBGLOG(RSN, TRACE, "prStaRec=%p RX Status = 0x%x RX_CLASSERR check!\n", prStaRec,
...
BOOLEAN secIsUsedByStaRecEntry(IN P_ADAPTER_T prAdapter, IN UINT_8 Index)
{
	...
	for (i = 0; i < CFG_STA_REC_NUM; i++) {
		...
		if (prStaRec->fgIsInUse && prStaRec->ucWlanIndex == Index) {
			fgIsUsed = TRUE;
			DBGLOG(RSN, INFO, "[Wlan index]: Duplicated entry #%d\n", Index);
			break;
		}
	}
	return fgIsUsed;
...
BOOL secPrivacySeekForEntry(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prSta)
{
	...
	if (ucEntry < NIC_TX_DEFAULT_WLAN_INDEX) {
		...
		DBGLOG(RSN, INFO,
		       ...
		       prSta->ucBssIndex, 0, prWtbl[ucEntry].ucPairwise, ucEntry, prSta->ucIndex,
		       MAC2STR(prSta->aucMacAddr), prSta->eStaType);
#if 1 /* DBG */
...
VOID secRemoveBssBcEntry(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prBssInfo, IN BOOL fgRoam)
{
	...
	if (fgRoam) {
		if (IS_BSS_AIS(prBssInfo) && prBssInfo->prStaRecOfAP &&
		    (prConnSettings->eAuthMode >= AUTH_MODE_WPA && prConnSettings->eAuthMode != AUTH_MODE_WPA_NONE)) {
...
/*----------------------------------------------------------------------------*/
UINT_8
secPrivacySeekForBcEntry(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, IN PUINT_8 pucAddr, IN UINT_8 ucStaIdx,
			 IN UINT_8 ucAlg, IN UINT_8 ucKeyId)
{
	...
	if (ucAlg == CIPHER_SUITE_WPI || /* CIPHER_SUITE_GCM_WPI || */
	    ucAlg == CIPHER_SUITE_WEP40 || ucAlg == CIPHER_SUITE_WEP104 || ucAlg == CIPHER_SUITE_WEP128 ||
	...
	DBGLOG(INIT, INFO, "OpMode:%d, NetworkType:%d, CheckKeyId:%d\n", prBSSInfo->eCurrentOPMode,
	       prBSSInfo->eNetworkType, fgCheckKeyId);
	...
	if (ucEntry < NIC_TX_DEFAULT_WLAN_INDEX) {
		...
		DBGLOG(RSN, INFO, "[Wlan index] BSS#%d keyid#%d P=%d use WlanIndex#%d STAIdx=%d " MACSTR "\n",
...
void secPrivacyDumpWTBL(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < WTBL_SIZE; i++) {
		if (prWtbl[i].ucUsed)
			DBGLOG(RSN, INFO, "#%d Used=%d  BSSIdx=%d keyid=%d P=%d STA=%d Addr=" MACSTR "\n", i,
			       prWtbl[i].ucUsed, prWtbl[i].ucBssIndex, prWtbl[i].ucKeyId, prWtbl[i].ucPairwise,
			       prWtbl[i].ucStaIndex, MAC2STR(prWtbl[i].aucMacAddr));
	}
}
...
BOOL secCheckSetKeyDone(IN P_STA_RECORD_T prStaRec)
{
	...
	/* Check WEP flow */
	if (prStaRec->fgIsTxPairwiseKeyReady == FALSE && prStaRec->fgIsTxGroupKeyReady == TRUE &&
	    prStaRec->ucGroupKeyCount == 2)
		return TRUE;
	/* Check TKIP/CCMP flow */
	else if (prStaRec->fgIsTxPairwiseKeyReady == TRUE && prStaRec->fgIsTxGroupKeyReady == TRUE)
		return TRUE;
	else
		return FALSE;
}

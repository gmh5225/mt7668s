			       ...
			       OUT P_ENUM_P2P_DEV_STATE_T peNextState)
{
	...
	do {
		...
		cnmGetDbdcCapability(prAdapter, prBssInfo->ucBssIndex, prP2pMsgChnlReq->rChannelInfo.eBand,
				     ...
				     wlanGetSupportNss(prAdapter, prBssInfo->ucBssIndex), &rDbdcCap);
				  ...
				  IN P_P2P_DEV_FSM_INFO_T prP2pDevFsmInfo, IN P_P2P_CHNL_REQ_INFO_T prChnlReqInfo)
{
	do {
		...
		kalP2PIndicateChannelReady(prAdapter->prGlueInfo, prChnlReqInfo->u8Cookie, prChnlReqInfo->ucReqChnlNum,
				   ...
				   IN P_P2P_DEV_FSM_INFO_T prP2pDevFsmInfo, IN P_P2P_CHNL_REQ_INFO_T prChnlReqInfo)
{
	do {
		...
		kalP2PIndicateChannelExpired(prAdapter->prGlueInfo, prChnlReqInfo->u8Cookie,
					     prChnlReqInfo->ucReqChnlNum, prChnlReqInfo->eBand,
			    ...
			    OUT P_ENUM_P2P_DEV_STATE_T peNextState)
{
	...
	do {
		ASSERT_BREAK((prAdapter != NULL) && (prP2pMgmtTxInfo != NULL) && (peNextState != NULL));

		if (!LINK_IS_EMPTY(&(prP2pMgmtTxInfo->rP2pTxReqLink))) {
			prP2pOffChnlTxPkt = LINK_PEEK_HEAD(&(prP2pMgmtTxInfo->rP2pTxReqLink),
			...
			if (prChnlReqInfo->ucReqChnlNum != prP2pOffChnlTxPkt->rChannelInfo.ucChannelNum) {
				...
				LINK_REMOVE_HEAD(&(prP2pMgmtTxInfo->rP2pTxReqLink), prP2pOffChnlTxPkt,
	...
	if (eNextState != P2P_DEV_STATE_OFF_CHNL_TX) {
		while (!LINK_IS_EMPTY(&(prP2pMgmtTxInfo->rP2pTxReqLink))) {
			...
			if (prP2pOffChnlTxPkt)
				kalP2PIndicateMgmtTxStatus(prAdapter->prGlueInfo, prP2pOffChnlTxPkt->prMgmtTxMsdu,
							   FALSE);
			else

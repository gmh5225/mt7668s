...
VOID rlmObssInit(P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < BSS_INFO_NUM; i++) {
		...
		cnmTimerInitTimer(prAdapter, &prBssInfo->rObssScanTimer, (PFN_MGMT_TIMEOUT_FUNC)rlmObssScanTimeout,
				  (ULONG)prBssInfo);
	}
}
...
VOID rlmObssScanDone(P_ADAPTER_T prAdapter, P_MSG_HDR_T prMsgHdr)
{
	...
	if (prBssInfo->auc2G_20mReqChnlList[0] > 0 || prBssInfo->auc2G_NonHtChnlList[0] > 0) {
		...
		if (prMsduInfo) {
			prTxFrame =
				(P_ACTION_20_40_COEXIST_FRAME)((ULONG)(prMsduInfo->prPacket) + MAC_TX_RESERVED_FIELD);
			...
			prTxFrame->rBssCoexist.ucData = (prBssInfo->auc2G_20mReqChnlList[0] > 0) ? BSS_COEXIST_20M_REQ :
												   0;
			...
			TX_SET_MMPDU(prAdapter, prMsduInfo, prBssInfo->ucBssIndex, prBssInfo->prStaRecOfAP->ucIndex,
				     WLAN_MAC_MGMT_HEADER_LEN, WLAN_MAC_MGMT_HEADER_LEN + u2PayloadLen, NULL,
				     MSDU_RATE_MODE_AUTO);
...
VOID rlmObssAbortScan(P_ADAPTER_T prAdapter)
{
	...
	if (prBssInfo && (prScanParam->ucSeqNum == 0x33) && (prScanParam->fgIsObssScan == TRUE)) {
		...
		prBssInfo->u2ObssScanInterval = 0;
		DBGLOG(RLM, STATE, "Obss can timeout change IDLE\n");

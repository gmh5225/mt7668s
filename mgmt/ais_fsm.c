 ...
 *******************************************************************************/
static PUINT_8 apucDebugAisState[AIS_STATE_NUM] = { (PUINT_8)DISP_STRING("IDLE"),
...
VOID aisFsmInit(IN P_ADAPTER_T prAdapter)
{
	...
	cnmTimerInitTimer(prAdapter, &prAisFsmInfo->rBGScanTimer, (PFN_MGMT_TIMEOUT_FUNC)aisFsmRunEventBGSleepTimeOut,
			  (ULONG)NULL);

	cnmTimerInitTimer(prAdapter, &prAisFsmInfo->rIbssAloneTimer,
			  (PFN_MGMT_TIMEOUT_FUNC)aisFsmRunEventIbssAloneTimeOut, (ULONG)NULL);

	cnmTimerInitTimer(prAdapter, &prAisFsmInfo->rIndicationOfDisconnectTimer,
	...
	cnmTimerInitTimer(prAdapter, &prAisFsmInfo->rJoinTimeoutTimer, (PFN_MGMT_TIMEOUT_FUNC)aisFsmRunEventJoinTimeout,
			  (ULONG)NULL);

	cnmTimerInitTimer(prAdapter, &prAisFsmInfo->rChannelTimeoutTimer,
			  (PFN_MGMT_TIMEOUT_FUNC)aisFsmRunEventChannelTimeout, (unsigned long)NULL);
	...
	prAisBssInfo->ucWmmQueSet = (prAdapter->rWifiVar.ucDbdcMode == DBDC_MODE_DISABLED) ? DBDC_5G_WMM_INDEX :
											     DBDC_2G_WMM_INDEX;
#endif
	...
	g_prDisconnInfo = (struct AIS_DISCONN_INFO_T *)kalMemAlloc(
		sizeof(struct AIS_DISCONN_INFO_T) * MAX_DISCONNECT_RECORD, VIR_MEM_TYPE);
	if (g_prDisconnInfo != NULL) {
		kalMemZero(g_prDisconnInfo, sizeof(struct AIS_DISCONN_INFO_T) * MAX_DISCONNECT_RECORD);
	} else {
		DBGLOG(AIS, ERROR, "allocate memory for g_prDisconnInfo failed!\n");
	}
	g_DisconnInfoIdx = 0;
...
#if CFG_DISCONN_DEBUG_FEATURE
	if (g_prDisconnInfo != NULL) {
		kalMemFree(g_prDisconnInfo, VIR_MEM_TYPE, sizeof(struct AIS_DISCONN_INFO_T) * MAX_DISCONNECT_RECORD);
...
VOID aisFsmStateInit_JOIN(IN P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc)
{
	...
	prStaRec = bssCreateStaRecFromBssDesc(prAdapter, STA_TYPE_LEGACY_AP, prAdapter->prAisBssInfo->ucBssIndex,
					      prBssDesc);
	if (!prStaRec) {
...
VOID aisFsmStateInit_IBSS_MERGE(IN P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc)
{
	...
	/* 4 <2> Setup corresponding STA_RECORD_T */
	prStaRec = bssCreateStaRecFromBssDesc(prAdapter, STA_TYPE_ADHOC_PEER, prAdapter->prAisBssInfo->ucBssIndex,
					      prBssDesc);
	if (!prStaRec) {
		DBGLOG(AIS, ERROR, "prStaRec is NULL!\n");
		return;
	...
	do {
		...
		case AIS_STATE_IDLE:
			...
			if (prAisReq == NULL || prAisReq->eReqType == AIS_REQUEST_RECONNECT) {
				...
				if (prConnSettings->fgIsConnReqIssued == TRUE &&
				    prConnSettings->fgIsDisconnectedByNonRequest == FALSE &&
				    !atomic_read(&prAdapter->prGlueInfo->cfgSuspend)) {
		...
		case AIS_STATE_SEARCH:
			...
			if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_DISCONNECTED) {
				...
				if (prBssDesc) {
					...
					cnmGetDbdcCapability(prAdapter, prAisBssInfo->ucBssIndex, prBssDesc->eBand,
				...
				/* 4 <2.b> If we don't have the matched one */
					...
					/* 4 <B> We've do SCAN already, now wait in some STATE. */
						...
						eNextState = aisFsmStateSearchAction(
							prAdapter, AIS_FSM_STATE_SEARCH_ACTION_PHASE_0);
			...
			/* 4 <3> We are under Roaming Condition. */
				...
				/* 4 <3.b> Try to roam out for JOIN this BSS_DESC_T. */
					...
					if (prBssDesc == NULL) {
						fgIsTransition = TRUE;
						eNextState = aisFsmStateSearchAction(
							prAdapter, AIS_FSM_STATE_SEARCH_ACTION_PHASE_1);
					} else {
		...
		case AIS_STATE_LOOKING_FOR:
			...
			prScanReqMsg = (P_MSG_SCN_SCAN_REQ_V2)cnmMemAlloc(
				prAdapter, RAM_TYPE_MSG, OFFSET_OF(MSG_SCN_SCAN_REQ_V2, aucIE) + u2ScanIELen);
			if (!prScanReqMsg) {
		...
		case AIS_STATE_DISCONNECTING:
			...
			authSendDeauthFrame(prAdapter, prAisBssInfo, prAisBssInfo->prStaRecOfAP, (P_SW_RFB_T)NULL,
					    ucReasonCode, aisDeauthXmitComplete);
...
VOID aisFsmRunEventScanDone(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (ucSeqNumOfCompMsg != prAisFsmInfo->ucSeqNumOfScanReq) {
		...
		case AIS_STATE_DISCONNECTING:
			...
			prConnSettings->fgIsScanReqIssued = FALSE;
			prAisFsmInfo->u4ScanIELength = 0;
			kalIndicateStatusAndComplete(prAdapter->prGlueInfo, WLAN_STATUS_SCAN_ABORT, NULL, 0);
...
VOID aisFsmRunEventAbort(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (ucReasonOfDisconnect == DISCONNECT_REASON_CODE_REASSOCIATION &&
		...
		if (prAdapter->prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
			...
			kalSetMediaStateIndicated(prGlueInfo, PARAM_MEDIA_STATE_CONNECTED);
		}
...
#if DBG
		else DBGLOG(AIS, WARN, "SEQ NO of AIS JOIN COMP MSG is not matched.\n");
#endif /* DBG */
	}
	if (eNextState != prAisFsmInfo->eCurrentState)
		aisFsmSteps(prAdapter, eNextState);
...
enum _ENUM_AIS_STATE_T aisFsmJoinCompleteAction(IN struct _ADAPTER_T *prAdapter, IN struct _MSG_HDR_T *prMsgHdr)
{
	...
	if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
		...
		if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
			...
			if ((prAisBssInfo->prStaRecOfAP) && (prAisBssInfo->prStaRecOfAP != prStaRec) &&
			...
			nicUpdateRSSI(prAdapter, prAdapter->prAisBssInfo->ucBssIndex,
	...
	/* 4 <2> JOIN was not successful */
		...
		if (aisFsmStateInit_RetryJOIN(prAdapter, prStaRec) == FALSE) {
			...
			if (prAisBssInfo->ucSSIDLen) {
				...
				COPY_SSID(rParamSsid.aucSsid, rParamSsid.u4SsidLen, prAisBssInfo->aucSSID,
					  prAisBssInfo->ucSSIDLen);
				prBssDesc = scanSearchBssDescByBssidAndSsid(prAdapter, prAisBssInfo->aucBSSID, TRUE,
									    &rParamSsid);
			} else {
#if CFG_SUPPORT_CFG80211_AUTH
				prBssDesc = scanSearchBssDescByBssidAndChanNum(prAdapter, prConnSettings->aucBSSID,
			...
			if (prBssDesc == NULL) {
				DBGLOG(AIS, WARN, "prBssDesc == NULL ->JOIN FAIL");
				/* Free STA-REC */
			...
			if (prBssDesc->ucJoinFailureCount > SCN_BSS_JOIN_FAIL_THRESOLD) {
				...
				DBGLOG(AIS, INFO, "Bss " MACSTR " join fail %d times,", MAC2STR(prBssDesc->aucBSSID),
 ...
 */
/*----------------------------------------------------------------------------*/
VOID aisIndicationOfMediaStateToHost(IN P_ADAPTER_T prAdapter, ENUM_PARAM_MEDIA_STATE_T eConnectionState,
				     BOOLEAN fgDelayIndication)
{
	...
	if (!fgDelayIndication) {
		...
		cnmTimerStartTimer(prAdapter, &prAisFsmInfo->rIndicationOfDisconnectTimer,
...
#if CFG_SUPPORT_CFG80211_AUTH
		prBssDesc = scanSearchBssDescByBssidAndChanNum(prAdapter, prConnSettings->aucBSSID, TRUE,
...
VOID aisUpdateBssInfoForMergeIBSS(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	if (!prAisBssInfo->fgIsBeaconActivated) {
		...
		/* 4 <1.2> Setup SSID */
		COPY_SSID(prAisBssInfo->aucSSID, prAisBssInfo->ucSSIDLen, prConnSettings->aucSSID,
			  prConnSettings->ucSSIDLen);
	...
	rateGetDataRatesFromRateSet(prAisBssInfo->u2OperationalRateSet, prAisBssInfo->u2BSSBasicRateSet,
...
VOID aisCollectDisconnInfo(IN P_ADAPTER_T prAdapter)
{
	...
	_wlanoidAdvCtrl(prAdapter, &prDisconn->rNoise, sizeof(struct CMD_NOISE_HISTOGRAM_REPORT), &u4BufLen, FALSE);
#endif

	/* Query Beacon RSSI info */
	_wlanoidQueryRssi(prAdapter, &prDisconn->rBcnRssi, sizeof(PARAM_RSSI), &u4BufLen, FALSE);

	/* Store target sta rec and get wlan index */
	if (prAisBssInfo->prStaRecOfAP) {
		kalMemCopy(&prDisconn->rStaRec, prAisBssInfo->prStaRecOfAP, sizeof(STA_RECORD_T));
	...
	_wlanoidQueryWlanInfo(prAdapter, &prDisconn->rHwInfo, sizeof(PARAM_HW_WLAN_INFO_T), &u4BufLen, FALSE);
	...
	if (pucMacAddr) {
		COPY_MAC_ADDR(prDisconn->rStaStatistics.aucMacAddr, pucMacAddr);

		_wlanoidQueryStaStatistics(prAdapter, &prDisconn->rStaStatistics, sizeof(PARAM_GET_STA_STA_STATISTICS),
					   &u4BufLen, FALSE);
	}
...
VOID aisFsmDisconnect(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgDelayIndication)
{
...
#if CFG_SUPPORT_ADHOC
	if (prAisBssInfo->fgIsBeaconActivated) {
		nicUpdateBeaconIETemplate(prAdapter, IE_UPD_METHOD_DELETE_ALL, prAdapter->prAisBssInfo->ucBssIndex, 0,
					  NULL, 0);
	...
	if (!fgDelayIndication) {
		...
		GLUE_ACQUIRE_SPIN_LOCK(prAdapter->prGlueInfo, SPIN_LOCK_STA_RECOFAP);
		if (prAisBssInfo->prStaRecOfAP) {
			/* cnmStaRecChangeState(prAdapter, prAisBssInfo->prStaRecOfAP, STA_STATE_1); */
			prAisBssInfo->prStaRecOfAP = (P_STA_RECORD_T)NULL;
		}
		GLUE_RELEASE_SPIN_LOCK(prAdapter->prGlueInfo, SPIN_LOCK_STA_RECOFAP);
	}
#if CFG_SUPPORT_ROAMING
...
VOID aisFsmScanRequest(IN P_ADAPTER_T prAdapter, IN P_PARAM_SSID_T prSsid, IN PUINT_8 pucIe, IN UINT_32 u4IeLength)
{
	...
	if (!prConnSettings->fgIsScanReqIssued) {
		...
		if (prSsid == NULL) {
			...
			COPY_SSID(prAisFsmInfo->arScanSSID[0].aucSsid, prAisFsmInfo->arScanSSID[0].u4SsidLen,
...
VOID aisFsmScanRequestAdv(IN P_ADAPTER_T prAdapter, IN UINT_8 ucSsidNum, IN P_PARAM_SSID_T prSsid,
			  IN UINT_8 ucChannelListNum, IN P_RF_CHANNEL_INFO_T prChnlInfoList, IN PUINT_8 pucIe,
	...
	if (!prConnSettings->fgIsScanReqIssued) {
		...
		if (ucSsidNum == 0) {
			...
			for (i = 0; i < ucSsidNum; i++) {
				COPY_SSID(prAisFsmInfo->arScanSSID[i].aucSsid, prAisFsmInfo->arScanSSID[i].u4SsidLen,
...
VOID aisFsmRunEventChGrant(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (prAisFsmInfo->eCurrentState == AIS_STATE_REQ_CHANNEL_JOIN && prAisFsmInfo->ucSeqNumOfChReq == ucTokenID) {
		...
		kalReadyOnChannel(prAdapter->prGlueInfo, prAisFsmInfo->rChReqInfo.u8Cookie,
				  prAisFsmInfo->rChReqInfo.eBand, prAisFsmInfo->rChReqInfo.eSco,
...
BOOLEAN aisFsmIsRequestPending(IN P_ADAPTER_T prAdapter, IN ENUM_AIS_REQUEST_TYPE_T eReqType, IN BOOLEAN bRemove)
{
	...
	LINK_FOR_EACH_ENTRY_SAFE(prPendingReqHdr, prPendingReqHdrNext, &(prAisFsmInfo->rPendingReqList), rLinkEntry,
				 AIS_REQ_HDR_T)
	{
...
VOID aisFsmRunEventMgmtFrameTx(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		aisFuncTxMgmtFrame(prAdapter, &prAisFsmInfo->rMgmtTxInfo, prMgmtTxMsg->prMgmtMsduInfo,
...
VOID aisFsmRunEventChannelTimeout(IN P_ADAPTER_T prAdapter, ULONG ulParamPtr)
{
	...
	if (prAisFsmInfo->eCurrentState == AIS_STATE_REMAIN_ON_CHANNEL) {
		...
		kalRemainOnChannelExpired(prAdapter->prGlueInfo, prAisFsmInfo->rChReqInfo.u8Cookie,
					  prAisFsmInfo->rChReqInfo.eBand, prAisFsmInfo->rChReqInfo.eSco,
...
WLAN_STATUS
aisFsmRunEventMgmtFrameTxDone(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo,
			      IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	do {
		...
		if (prMgmtTxReqInfo->prMgmtTxMsdu == prMsduInfo) {
			kalIndicateMgmtTxStatus(prAdapter->prGlueInfo, prMgmtTxReqInfo->u8Cookie, fgIsSuccess,
	...
	do {
		...
		if (prMgmtTxReqInfo->fgIsMgmtTxRequested) {
			...
			if (prTxMsduInfo != NULL) {
				kalIndicateMgmtTxStatus(prAdapter->prGlueInfo, prMgmtTxReqInfo->u8Cookie, FALSE,
		...
		TX_SET_MMPDU(prAdapter, prMgmtTxMsdu,
			     (prStaRec != NULL) ? (prStaRec->ucBssIndex) : (prAdapter->prAisBssInfo->ucBssIndex),
			     (prStaRec != NULL) ? (prStaRec->ucIndex) : (STA_REC_INDEX_NOT_FOUND),
			     WLAN_MAC_MGMT_HEADER_LEN, prMgmtTxMsdu->u2FrameLength, aisFsmRunEventMgmtFrameTxDone,
			     MSDU_RATE_MODE_AUTO);
...
struct AIS_BLACKLIST_ITEM *aisQueryBlackList(P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc)
{
	...
	LINK_FOR_EACH_ENTRY(prEntry, prBlackList, rLinkEntry, struct AIS_BLACKLIST_ITEM)
	{
		if (EQUAL_MAC_ADDR(prBssDesc->aucBSSID, prEntry->aucBSSID) &&
		    EQUAL_SSID(prBssDesc->aucSSID, prBssDesc->ucSSIDLen, prEntry->aucSSID, prEntry->ucSSIDLen)) {
			prBssDesc->prBlack = prEntry;
			return prEntry;
		}
	}
...
struct AIS_BLACKLIST_ITEM *aisQueryBlackListByBssid(P_ADAPTER_T prAdapter, UINT_8 aucBSSID[])
{
	...
	LINK_FOR_EACH_ENTRY(prEntry, prBlackList, rLinkEntry, struct AIS_BLACKLIST_ITEM)
	{
		if (EQUAL_MAC_ADDR(aucBSSID, prEntry->aucBSSID) &&
		    EQUAL_SSID(prConnSettings->aucSSID, prConnSettings->ucSSIDLen, prEntry->aucSSID,
			       prEntry->ucSSIDLen))
			return prEntry;
	}
	DBGLOG(AIS, TRACE, "%pM is not in blacklist\n", aucBSSID);

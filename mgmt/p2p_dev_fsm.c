...
UINT_8 p2pDevFsmInit(IN P_ADAPTER_T prAdapter)
{
	...
	do {
		...
		cnmTimerInitTimer(prAdapter, &(prP2pDevFsmInfo->rP2pFsmTimeoutTimer),
...
#else
			prP2pBssInfo->ucWmmQueSet = (prAdapter->rWifiVar.ucDbdcMode == DBDC_MODE_DISABLED) ?
							    DBDC_5G_WMM_INDEX :
							    DBDC_2G_WMM_INDEX;
#endif
...
VOID p2pDevFsmStateTransition(IN P_ADAPTER_T prAdapter, IN P_P2P_DEV_FSM_INFO_T prP2pDevFsmInfo,
			      IN ENUM_P2P_DEV_STATE_T eNextState)
{
	...
	if (prP2pDevFsmInfo->ucBssIndex != P2P_DEV_BSS_INDEX) {
		DBGLOG(P2P, ERROR, "prP2pDevFsmInfo->ucBssIndex %d should be P2P_DEV_BSS_INDEX(%d)!\n",
	...
	do {
		...
		case P2P_DEV_STATE_IDLE:
			if (!fgIsLeaveState) {
				fgIsLeaveState =
					p2pDevStateInit_IDLE(prAdapter, &prP2pDevFsmInfo->rChnlReqInfo, &eNextState);
			} else {
		...
		case P2P_DEV_STATE_SCAN:
			if (!fgIsLeaveState) {
				p2pDevStateInit_SCAN(prAdapter, prP2pDevFsmInfo->ucBssIndex,
		...
		case P2P_DEV_STATE_REQING_CHANNEL:
			if (!fgIsLeaveState) {
				...
				p2pDevStateAbort_REQING_CHANNEL(prAdapter, &(prP2pDevFsmInfo->rChnlReqInfo),
								eNextState);
			}
			break;
		...
		case P2P_DEV_STATE_OFF_CHNL_TX:
			if (!fgIsLeaveState) {
				fgIsLeaveState = p2pDevStateInit_OFF_CHNL_TX(prAdapter, prP2pDevFsmInfo,
									     &(prP2pDevFsmInfo->rChnlReqInfo),
									     &(prP2pDevFsmInfo->rMgmtTxInfo),
									     &eNextState);
			} else {
				p2pDevStateAbort_OFF_CHNL_TX(prAdapter, &(prP2pDevFsmInfo->rMgmtTxInfo),
...
VOID p2pDevFsmRunEventTimeout(IN P_ADAPTER_T prAdapter, IN ULONG ulParamPtr)
{
	...
	do {
		...
		default:
			ASSERT(FALSE);
			DBGLOG(P2P, ERROR, "Current P2P Dev State %d is unexpected for FSM timeout event.\n",
...
VOID p2pDevFsmRunEventScanDone(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr,
			       IN P_P2P_DEV_FSM_INFO_T prP2pDevFsmInfo)
{
	...
	do {
		...
		if (prScanDoneMsg->ucSeqNum != prP2pScanReqInfo->ucSeqNumOfScnMsg) {
			DBGLOG(P2P, TRACE, "P2P Scan Done SeqNum:%d  <->   P2P Dev FSM Scan SeqNum:%d",
...
VOID p2pDevFsmRunEventChannelRequest(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		if ((!fgIsChnlFound) && (prChnlReqInfo->eChnlReqType == CH_REQ_TYPE_P2P_LISTEN) &&
...
VOID p2pDevFsmRunEventChnlGrant(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr,
				IN P_P2P_DEV_FSM_INFO_T prP2pDevFsmInfo)
{
...
VOID p2pDevFsmRunEventMgmtTx(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if ((prMgmtTxMsg->ucBssIdx != P2P_DEV_BSS_INDEX) && (IS_NET_ACTIVE(prAdapter, prMgmtTxMsg->ucBssIdx))) {
		...
		p2pFuncTxMgmtFrame(prAdapter, prMgmtTxMsg->ucBssIdx, prMgmtTxMsg->prMgmtMsduInfo,
	...
	if ((!prMgmtTxMsg->fgIsOffChannel) || ((prP2pDevFsmInfo->eCurrentState == P2P_DEV_STATE_OFF_CHNL_TX) &&
					       (LINK_IS_EMPTY(&prP2pMgmtTxReqInfo->rP2pTxReqLink)))) {
		p2pFuncTxMgmtFrame(prAdapter, prP2pDevFsmInfo->ucBssIndex, prMgmtTxMsg->prMgmtMsduInfo,
				   prMgmtTxMsg->fgNoneCckRate);
	} else {
		...
		kalMemCopy(&prOffChnlTxReq->rChannelInfo, &prMgmtTxMsg->rChannelInfo, sizeof(RF_CHANNEL_INFO_T));
		...
		if (prP2pDevFsmInfo->eCurrentState != P2P_DEV_STATE_OFF_CHNL_TX) {
			...
			kalMemCopy(&prP2pMsgChnlReq->rChannelInfo, &prMgmtTxMsg->rChannelInfo,
...
WLAN_STATUS
p2pDevFsmRunEventMgmtFrameTxDone(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo,

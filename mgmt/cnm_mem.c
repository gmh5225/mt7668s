...
static PUINT_8 apucStaRecType[STA_TYPE_INDEX_NUM] = { (PUINT_8) "LEGACY", (PUINT_8) "P2P", (PUINT_8) "BOW" };
...
static VOID cnmStaSendRemoveCmd(P_ADAPTER_T prAdapter, ENUM_STA_REC_CMD_ACTION_T eActionType, UINT_8 ucStaRecIndex,
				UINT_8 ucBssIndex);
...
P_MSDU_INFO_T cnmPktAlloc(P_ADAPTER_T prAdapter, UINT_32 u4Length)
{
	...
	if (prMsduInfo == NULL) {
		...
		DBGLOG(MEM, WARN, "rMgtBufInfo: alloc#=%ld, free#=%ld, null#=%ld\n",
		       prAdapter->rMgtBufInfo.u4AllocCount, prAdapter->rMgtBufInfo.u4FreeCount,
...
VOID cnmMemFree(IN P_ADAPTER_T prAdapter, IN PVOID pvMemory)
{
	...
	if (((ULONG)pvMemory >= (ULONG)&prAdapter->aucMsgBuf[0]) &&
		...
		prBufInfo = &prAdapter->rMsgBufInfo;
		u4BlockIndex = ((ULONG)pvMemory - (ULONG)prBufInfo->pucBuf) >> MSG_BUF_BLOCK_SIZE_IN_POWER_OF_2;
		...
		prBufInfo = &prAdapter->rMgtBufInfo;
		u4BlockIndex = ((ULONG)pvMemory - (ULONG)prBufInfo->pucBuf) >> MGT_BUF_BLOCK_SIZE_IN_POWER_OF_2;
		ASSERT(u4BlockIndex < MAX_NUM_OF_BUF_BLOCKS);
		eRamType = RAM_TYPE_BUF;
	} else {
#ifdef LINUX
...
VOID cnmStaFreeAllStaByNetwork(P_ADAPTER_T prAdapter, UINT_8 ucBssIndex, UINT_8 ucStaRecIndexExcluded)
{
	...
	cnmStaSendRemoveCmd(prAdapter,
			    (ucStaRecIndexExcluded < CFG_STA_REC_NUM) ? STA_REC_CMD_ACTION_BSS_EXCLUDE_STA :
 ...
 */
/*----------------------------------------------------------------------------*/
P_STA_RECORD_T cnmGetAnyStaRecByAddress(P_ADAPTER_T prAdapter, PUINT_8 pucPeerMacAddr)
{
	P_STA_RECORD_T prStaRec;
	UINT_16 i;
	...
	for (i = 0; i < CFG_STA_REC_NUM; i++) {
		...
		if (prStaRec->fgIsInUse && EQUAL_MAC_ADDR(prStaRec->aucMacAddr, pucPeerMacAddr)) {
			break;
		}
	}
			 ...
			 BOOLEAN fgNeedResp)
{
	...
	if (prAdapter->rWifiVar.eRateSetting != FIXED_RATE_NONE) {
		...
		nicUpdateRateParams(prAdapter, prAdapter->rWifiVar.eRateSetting, &(prCmdContent->ucDesiredPhyTypeSet),
				    &(prCmdContent->u2DesiredNonHTRateSet), &(prCmdContent->u2BSSBasicRateSet),
				    &(prCmdContent->ucMcsSet), &(prCmdContent->ucSupMcs32),
	...
	DBGLOG(REQ, INFO, "Update StaRec[%u] WIDX[%u] State[%u] Type[%u] BssIdx[%u] AID[%u]\n",
	       prCmdContent->ucStaIndex, prCmdContent->ucWlanIndex, prCmdContent->ucStaState, prCmdContent->ucStaType,
...
VOID cnmDumpStaRec(IN P_ADAPTER_T prAdapter, IN UINT_8 ucStaRecIdx)
{
	...
	DBGLOG(SW4, INFO, "STA_IDX[%u] BSS_IDX[%u] MAC[" MACSTR "] TYPE[%s %s] WTBL[%u] USED[%u] State[%u]\n",
	       prStaRec->ucIndex, prStaRec->ucBssIndex, MAC2STR(prStaRec->aucMacAddr),
	       cnmStaRecGetTypeString(prStaRec->eStaType), cnmStaRecGetRoleString(prStaRec->eStaType), ucWTEntry,
	...
	DBGLOG(SW4, INFO,
	       ...
	       prBssInfo->u2BSSBasicRateSet, prStaRec->u2OperationalRateSet, prStaRec->u2DesiredNonHTRateSet,
	       prStaRec->u2HwDefaultFixedRateCode);

	DBGLOG(SW4, INFO, "HT Cap[0x%04x] ExtCap[0x%04x] BeemCap[0x%08x] MCS[0x%02x] MCS32[%u]\n",
	       prStaRec->u2HtCapInfo, prStaRec->u2HtExtendedCap, prStaRec->u4TxBeamformingCap, prStaRec->ucMcsSet,
	       prStaRec->fgSupMcs32);

	DBGLOG(SW4, INFO, "VHT Cap[0x%08x] TxMCS[0x%04x] RxMCS[0x%04x] VhtOpMode[0x%02x]\n", prStaRec->u4VhtCapInfo,
	       prStaRec->u2VhtTxMcsMap, prStaRec->u2VhtRxMcsMap, prStaRec->ucVhtOpMode);

	DBGLOG(SW4, INFO, "RCPI[%u] InPS[%u] TxAllowed[%u] KeyRdy[%u] AMPDU T/R[%u/%u]\n", prStaRec->ucRCPI,
	       prStaRec->fgIsInPS, prStaRec->fgIsTxAllowed, prStaRec->fgIsTxKeyReady, prStaRec->fgTxAmpduEn,
	       prStaRec->fgRxAmpduEn);

	DBGLOG(SW4, INFO, "TxQ LEN TC[0~5] [%03u:%03u:%03u:%03u]\n", prStaRec->aprTargetQueue[0]->u4NumElem,
	       prStaRec->aprTargetQueue[1]->u4NumElem, prStaRec->aprTargetQueue[2]->u4NumElem,
	...
	for (i = 0; i < CFG_RX_MAX_BA_TID_NUM; i++) {
		if (prStaRec->aprRxReorderParamRefTbl[i]) {
			...
			DBGLOG(SW4, INFO, " Valid[%u] WinStart/End[%u/%u] WinSize[%u] ReOrderQueLen[%u]\n",
			...
			DBGLOG(SW4, INFO, " Bubble Exist[%u] SN[%u]\n",
...
UINT_32 cnmDumpMemoryStatus(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucBuf, IN UINT_32 u4Max)
{
	...
	LOGBUF(pucBuf, u4Max, u4Len, "MSG memory count: alloc[%u] free[%u] null[%u] bitmap[0x%08x]\n",
	       prBufInfo->u4AllocCount, prBufInfo->u4FreeCount, prBufInfo->u4AllocNullCount,
	       (UINT_32)prBufInfo->rFreeBlocksBitmap);
...
cnmPeerAdd(P_ADAPTER_T prAdapter, PVOID pvSetBuffer, UINT_32 u4SetBufferLen, PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(CMD_PEER_ADD_T);
	...
	if (prStaRec == NULL) {
		prStaRec = cnmStaRecAlloc(prAdapter, STA_TYPE_DLS_PEER, (UINT_8)prAdapter->prAisBssInfo->ucBssIndex,
					  prCmd->aucPeerMac);
		...
		prStaRec->ucNonHTBasicPhyType = prAisBssInfo->ucNonHTBasicPhyType;
...
P_STA_RECORD_T cnmGetTdlsPeerByAddress(P_ADAPTER_T prAdapter, UINT_8 ucBssIndex, UINT_8 aucPeerMACAddress[])
{
	...
	for (i = 0; i < CFG_STA_REC_NUM; i++) {
		prStaRec = &prAdapter->arStaRec[i];
		if (prStaRec) {
			if (prStaRec->fgIsInUse && prStaRec->eStaType == STA_TYPE_DLS_PEER &&

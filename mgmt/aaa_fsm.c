...
VOID aaaFsmRunEventRxAuth(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		if (prBssInfo && prBssInfo->fgIsNetActive) {
			...
			if (WLAN_STATUS_SUCCESS == authProcessRxAuthFrame(prAdapter, prSwRfb, prBssInfo, &u2StatusCode))
#else
...
#endif
			{
				if (u2StatusCode == STATUS_CODE_SUCCESSFUL) {
					...
					fgReplyAuth = p2pFuncValidateAuth(prAdapter, prBssInfo, prSwRfb, &prStaRec,
									  &u2StatusCode);
...
#define IS_SOFTAP_MODE(role_index) \
	((gprP2pRoleWdev[role_index]) && (gprP2pRoleWdev[role_index]->iftype == NL80211_IFTYPE_AP))

		if (IS_SOFTAP_MODE(prBssInfo->u4PrivateData)) {
			kalP2PIndicateRxMgmtFrame(prAdapter->prGlueInfo, prSwRfb, FALSE,
						  (uint8_t)prBssInfo->u4PrivateData);
			return;
		} else {
#endif
			if (WLAN_STATUS_SUCCESS != authSendAuthFrame(prAdapter, prStaRec, prBssInfo->ucBssIndex,
...
WLAN_STATUS aaaFsmRunEventRxAssoc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
			...
			if (prBssInfo->fgIsNetActive) {
				...
				if (WLAN_STATUS_SUCCESS ==
					...
					if (u2StatusCode == STATUS_CODE_SUCCESSFUL) {
						...
						fgReplyAssocResp = p2pFuncValidateAssocReq(prAdapter, prSwRfb,
	...
	if (fgReplyAssocResp) {
		...
		if ((((P_WLAN_ASSOC_REQ_FRAME_T)(prSwRfb->pvHeader))->u2FrameCtrl & MASK_FRAME_TYPE) ==
			...
			u2IELength =
				prSwRfb->u2PacketLen - (UINT_16)OFFSET_OF(WLAN_REASSOC_REQ_FRAME_T, aucInfoElem[0]);

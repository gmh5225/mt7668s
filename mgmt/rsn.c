...
BOOLEAN rsnParseRsnIE(IN P_ADAPTER_T prAdapter, IN P_RSN_INFO_ELEM_T prInfoElem, OUT P_RSN_INFO_T prRsnInfo)
{
	...
	do {
		...
		if (u4RemainRsnIeLen < 2) {
			 ...
			 */
			DBGLOG(RSN, TRACE, "Ignore truncated RSN capabilities (IE len: %d)\n", prInfoElem->ucLength);
			break;
		}
		...
		if (u4RemainRsnIeLen < 2) {
			 ...
			 */
			DBGLOG(RSN, TRACE, "Ignore truncated PMKID count (IE len: %d)\n", prInfoElem->ucLength);
			break;
		}
		...
		if (u2DesiredPmkidCnt > MAX_NUM_SUPPORTED_PMKID) {
			...
			DBGLOG(RSN, WARN, "Support maximum PMKID Cnt = %d with desired PMKID Cnt = %d\n",
	...
	DBGLOG(RSN, LOUD, "RSN: version %d, group mgmt key cipher suite %02x-%02x-%02x-%02x\n", u2Version,
	       (UCHAR)(u4GroupMgmtSuite & 0x000000FF), (UCHAR)((u4GroupMgmtSuite >> 8) & 0x000000FF),
	       (UCHAR)((u4GroupMgmtSuite >> 16) & 0x000000FF), (UCHAR)((u4GroupMgmtSuite >> 24) & 0x000000FF));
...
BOOLEAN rsnPerformPolicySelection(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBss)
{
	...
	if (fgIsWpsActive && (prAdapter->rWifiVar.rConnSettings.eAuthMode < AUTH_MODE_WPA) &&
	...
	if (prBssRsnInfo->u4PairwiseKeyCipherSuiteCount == 1 &&
		...
		DBGLOG(RSN, TRACE, "eEncStatus %d %d 0x%08x\n", prAdapter->rWifiVar.rConnSettings.eEncStatus,
	...
	DBGLOG(RSN, TRACE, "Selected pairwise/group cipher: %02x-%02x-%02x-%02x/%02x-%02x-%02x-%02x\n",
	       (UINT_8)(u4PairwiseCipher & 0x000000FF), (UINT_8)((u4PairwiseCipher >> 8) & 0x000000FF),
	       (UINT_8)((u4PairwiseCipher >> 16) & 0x000000FF), (UINT_8)((u4PairwiseCipher >> 24) & 0x000000FF),
...
VOID rsnGenerateRSNIE(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	if (
		...
		WLAN_SET_FIELD_32(&RSN_IE(pucBuffer)->u4GroupKeyCipherSuite,
				  GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->u4RsnSelectedGroupCipher); /* Group key
													      suite */
		...
		if (GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eNetworkType == NETWORK_TYPE_AIS &&
		    (prStaRec != NULL) && rsnSearchPmkidEntry(prAdapter, prStaRec->aucMacAddr, &u4Entry)) {
#if 0
			...
			if (prAdapter->rWifiVar.rAisSpecificBssInfo.arPmkidCache[u4Entry].fgPmkidExist) {
				...
				DBGLOG(RSN, TRACE, "use PMKID " MACSTR "\n",
				       MAC2STR(prAdapter->rWifiVar.rAisSpecificBssInfo.arPmkidCache[u4Entry]
						       .rBssidInfo.arPMKID));
				kalMemCopy(cp,
					   (PVOID)prAdapter->rWifiVar.rAisSpecificBssInfo.arPmkidCache[u4Entry]
						   .rBssidInfo.arPMKID,
...
/*----------------------------------------------------------------------------*/
BOOLEAN
rsnParseCheckForWFAInfoElem(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucBuf, OUT PUINT_8 pucOuiType,
	...
	if (rsnParseRsnIE(prAdapter, prIe, &rRsnIe)) {
		...
		*pu2StatusCode = WLAN_STATUS_SUCCESS;
		...
		DBGLOG(RSN, INFO, "STA Assoc req mfpc:%d, mfpr:%d, sha256:%d, bssIndex:%d, applyPmf:%d\n",
		       prStaRec->rPmfCfg.fgMfpc, prStaRec->rPmfCfg.fgMfpr, prStaRec->rPmfCfg.fgSha256,
		       prStaRec->ucBssIndex, prStaRec->rPmfCfg.fgApplyPmf);
...
VOID rsnGenMicErrorEvent(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgFlags)
{
	...
	kalIndicateStatusAndComplete(prAdapter->prGlueInfo, WLAN_STATUS_MEDIA_SPECIFIC_INDICATION, (PVOID)prAuthEvent,
...
VOID rsnSelectPmkidCandidateList(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc)
{
	...
	if (UNEQUAL_SSID(prBssDesc->aucSSID, prBssDesc->ucSSIDLen, prConnSettings->aucSSID,
	...
	for (i = 0; i < prAisSpecBssInfo->u4PmkidCandicateCount; i++) {
		...
		if (j == prAisSpecBssInfo->u4PmkidCacheCount &&
			...
			DBGLOG(RSN, TRACE, "Add " MACSTR " to PMKID cache!!\n",
			       MAC2STR(prAisSpecBssInfo->arPmkidCandicate[i].aucBssid));
			kalMemCopy((PVOID)prAisSpecBssInfo->arPmkidCache[prAisSpecBssInfo->u4PmkidCacheCount]
					   .rBssidInfo.arBSSID,
...
VOID rsnGeneratePmkidIndication(IN P_ADAPTER_T prAdapter)
{
	...
	u4LenOfUsedBuffer =
		sizeof(ENUM_STATUS_TYPE_T) + (2 * sizeof(UINT_32)) + (count * sizeof(PARAM_PMKID_CANDIDATE_T));
...
void rsnStartSaQueryTimer(IN P_ADAPTER_T prAdapter, IN ULONG ulParamPtr)
{
	...
	prTxFrame = (P_ACTION_SA_QUERY_FRAME)((ULONG)(prMsduInfo->prPacket) + MAC_TX_RESERVED_FIELD);
	...
	if (pucTmp) {
		...
		kalMemCopy(&prBssSpecInfo->pucSaQueryTransId[(prBssSpecInfo->u4SaQueryCount - 1) *
	...
	TX_SET_MMPDU(prAdapter, prMsduInfo, prBssInfo->prStaRecOfAP->ucBssIndex, prBssInfo->prStaRecOfAP->ucIndex,
...
void rsnSaQueryAction(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	for (i = 0; i < prBssSpecInfo->u4SaQueryCount; i++) {
		if (kalMemCmp(prBssSpecInfo->pucSaQueryTransId + i * ACTION_SA_QUERY_TR_ID_LEN, prRxFrame->ucTransId,
...
VOID rsnGenerateWSCIEForAssocRsp(P_ADAPTER_T prAdapter, P_MSDU_INFO_T prMsduInfo)
{
	...
	kalP2PGenWSC_IE(prAdapter->prGlueInfo, 3,
			(PUINT_8)((ULONG)prMsduInfo->prPacket + (ULONG)prMsduInfo->u2FrameLength),
...
VOID rsnPmfGenerateTimeoutIE(P_ADAPTER_T prAdapter, P_MSDU_INFO_T prMsduInfo)
{
	...
	prTimeout = (IE_TIMEOUT_INTERVAL_T *)(((PUINT_8)prMsduInfo->prPacket) + prMsduInfo->u2FrameLength);
...
void rsnApStartSaQueryTimer(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN ULONG ulParamPtr)
{
	...
	if (prStaRec->rPmfCfg.u4SAQueryCount > 0 && rsnApCheckSaQueryTimeout(prAdapter, prStaRec)) {
		DBGLOG(RSN, INFO, "MFP: retry max timeout, u4SaQueryCount count =%d\n",
...
void rsnApStartSaQuery(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	cnmTimerStopTimer(prAdapter, &prStaRec->rPmfCfg.rSAQueryTimer);
	cnmTimerInitTimer(prAdapter, &prStaRec->rPmfCfg.rSAQueryTimer, (PFN_MGMT_TIMEOUT_FUNC)rsnApStartSaQueryTimer,
			  (ULONG)prStaRec);
...
void rsnApSaQueryRequest(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (!rsnCheckBipKeyInstalled(prAdapter, prStaRec)) {
		DBGLOG(RSN, INFO, "IEEE 802.11: AP Ignore SA Query Request non-PMF STA " MACSTR "\n",
		       MAC2STR(prStaRec->aucMacAddr));
		return;
	}
...
void rsnGenerateOWEIE(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	pucBuffer = (UINT_8 *)((unsigned long)prMsduInfo->prPacket + (unsigned long)prMsduInfo->u2FrameLength);
	ASSERT(pucBuffer);

	/* if (eNetworkId != NETWORK_TYPE_AIS_INDEX) */
	/* return; */
	kalMemCopy(pucBuffer, &(prAdapter->rWifiVar.rConnSettings.rOweInfo), ucLength);
	prMsduInfo->u2FrameLength += IE_SIZE(pucBuffer);
 ...
 */
/*----------------------------------------------------------------------------*/
uint32_t rsnCalOweIELen(IN P_ADAPTER_T prAdapter, IN UINT_8 ucBssIndex, P_STA_RECORD_T prStaRec)
{

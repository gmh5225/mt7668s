...
VOID roamingFsmInit(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, LOUD, "->roamingFsmInit(): Current Time = %d\n", kalGetTimeTick());
...
VOID roamingFsmUninit(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, LOUD, "->roamingFsmUninit(): Current Time = %d\n", kalGetTimeTick());
...
VOID roamingFsmSendCmd(IN P_ADAPTER_T prAdapter, IN P_CMD_ROAMING_TRANSIT_T prTransit)
{
	...
	DBGLOG(ROAMING, LOUD, "->roamingFsmSendCmd(): Current Time = %d\n", kalGetTimeTick());
...
VOID roamingFsmScanResultsUpdate(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, LOUD, "->roamingFsmScanResultsUpdate(): Current Time = %d\n", kalGetTimeTick());
...
static BOOLEAN roamingFsmIsNeedScan(IN P_ADAPTER_T prAdapter)
{
	...
	{
		if (EQUAL_SSID(prRoamBssDesc->aucSSID, prRoamBssDesc->ucSSIDLen, prAisBssInfo->aucSSID,
	...
	if (!fgIsRoamingSSID) {
		...
		if (prBssDesc) {
			...
			if (prAisBssInfo->ucRoamSkipTimes == 0) {
				...
				wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_ROAMING_SKIP, TRUE, FALSE, FALSE, NULL, NULL,
						    sizeof(CMD_ROAMING_SKIP_ONE_AP_T), (PUINT_8)&rCmdRoamingSkipOneAP,
						    NULL, 0);
		} else
...
VOID roamingFsmRunEventStart(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, EVENT, "EVENT-ROAMING START: Current Time = %d\n", kalGetTimeTick());
...
VOID roamingFsmRunEventDiscovery(IN P_ADAPTER_T prAdapter, IN P_CMD_ROAMING_TRANSIT_T prTransit)
{
	...
	DBGLOG(ROAMING, EVENT, "EVENT-ROAMING DISCOVERY: Current Time = %d\n", kalGetTimeTick());
...
VOID roamingFsmRunEventRoam(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, EVENT, "EVENT-ROAMING ROAM: Current Time = %d\n", kalGetTimeTick());
...
VOID roamingFsmRunEventFail(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Param)
{
	...
	DBGLOG(ROAMING, EVENT, "EVENT-ROAMING FAIL: reason %x Current Time = %d\n", u4Param, kalGetTimeTick());
...
VOID roamingFsmRunEventAbort(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(ROAMING, EVENT, "EVENT-ROAMING ABORT: Current Time = %d\n", kalGetTimeTick());
...
{
	DBGLOG(ROAMING, LOUD, "ROAMING Process Events: Current Time = %d\n", kalGetTimeTick());

...
VOID nicCmdEventQueryRxStatistics(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid) {
		...
		DBGLOG(INIT, ERROR, "MT7668S : RX Statistics Test SeqNum = %d, TotalNum = %d\n",
...
VOID nicCmdEventPfmuTagRead(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	DBGLOG(INIT, INFO, "Nrow = %d, Ncol = %d, Ng = %d, LM = %d\n",
	       prEventPfmuTagRead->ru4TxBfPFMUTag1.rField.ucNrow, prEventPfmuTagRead->ru4TxBfPFMUTag1.rField.ucNcol,
	...
	if (prCmdInfo->fgIsOid) {
		...
		kalOidComplete(prAdapter->prGlueInfo, prCmdInfo->fgSetQuery,
			       OFFSET_OF(PARAM_NETWORK_ADDRESS_LIST, arAddress) +
				       u4Count * (OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress) +
...
VOID nicCmdEventEnterRfTest(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	aisFsmStateAbort(prAdapter, DISCONNECT_REASON_CODE_RESERVED, FALSE);
#if 0
...
INT_32 GetIQData(INT_32 **prIQAry, UINT_32 *prDataLen, UINT_32 u4IQ, UINT_32 u4GetWf1)
{
	...
	*prIQAry = g_au4IQData;
	...
	snprintf(aucPath, sizeof(aucPath), "/tmp/dump_out_%05hu_WF%d.txt", (g_u2DumpIndex - 1), u4GetWf1);
	if (kalCheckPath(aucPath) == -1)
		snprintf(aucPath, sizeof(aucPath), "/data/dump_out_%05hu_WF%d.txt", (g_u2DumpIndex - 1), u4GetWf1);
	...
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT GetIQData prDataLen = %d\n", *prDataLen);
	DBGLOG(INIT, INFO, "MT7668S : QA_AGENT GetIQData i = %d\n", i);
...
UINT_32 TsfRawData2IqFmt(P_EVENT_DUMP_MEM_T prEventDumpMem)
{
	...
	if (prEventDumpMem->ucFragNum == 1) {
		...
		scnprintf(aucPathWF0, sizeof(aucPathWF0), "/tmp/dump_out_%05hu_WF0.txt", g_u2DumpIndex);
		scnprintf(aucPathWF1, sizeof(aucPathWF1), "/tmp/dump_out_%05hu_WF1.txt", g_u2DumpIndex);
		if (kalCheckPath(aucPathWF0) == -1) {
			kalMemSet(aucPathWF0, 0x00, sizeof(aucPathWF0));
			scnprintf(aucPathWF0, sizeof(aucPathWF0), "/data/dump_out_%05hu_WF0.txt", g_u2DumpIndex);
		} else
			kalTrunkPath(aucPathWF0);

		if (kalCheckPath(aucPathWF1) == -1) {
			kalMemSet(aucPathWF1, 0x00, sizeof(aucPathWF1));
			scnprintf(aucPathWF1, sizeof(aucPathWF1), "/data/dump_out_%05hu_WF1.txt", g_u2DumpIndex);
		} else
			kalTrunkPath(aucPathWF1);

		scnprintf(aucPathRAWWF0, sizeof(aucPathRAWWF0), "/dump_RAW_%05hu_WF0.txt", g_u2DumpIndex);
		scnprintf(aucPathRAWWF1, sizeof(aucPathRAWWF1), "/dump_RAW_%05hu_WF1.txt", g_u2DumpIndex);
		if (kalCheckPath(aucPathRAWWF0) == -1) {
			kalMemSet(aucPathRAWWF0, 0x00, sizeof(aucPathRAWWF0));
			scnprintf(aucPathRAWWF0, sizeof(aucPathRAWWF0), "/data/dump_RAW_%05hu_WF0.txt", g_u2DumpIndex);
		} else
			kalTrunkPath(aucPathRAWWF0);

		if (kalCheckPath(aucPathRAWWF1) == -1) {
			kalMemSet(aucPathRAWWF1, 0x00, sizeof(aucPathRAWWF1));
			scnprintf(aucPathRAWWF1, sizeof(aucPathRAWWF1), "/data/dump_RAW_%05hu_WF1.txt", g_u2DumpIndex);
		} else
	...
	for (u4SrcOffset = 0, u4RemainByte = prEventDumpMem->u4Length; u4RemainByte > 0;) {
		...
		if (prEventDumpMem->eIcapContent == ICAP_CONTENT_FIIQ ||
			...
			u4DataWLenF1 = scnprintf(pucDataWF1, u4DataWBufSize, "%8d,%8d\n",
						 icapBusData.rIqcBusData.u4Iqc1I, icapBusData.rIqcBusData.u4Iqc1Q);
		...
		u4DataRAWWLenF0 =
			scnprintf(pucDataRAWWF0, u4DataWBufSize, "%08x%08x%08x\n", *(ptr + 2), *(ptr + 1), *ptr);
...
VOID nicCmdEventQueryCalBackupV2(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prEventCalBackupDataV2->ucReason == 0 && prEventCalBackupDataV2->ucAction == 0) {
		...
		if (prEventCalBackupDataV2->u4Address == 0xFFFFFFFF) {
			...
			wlanoidQueryCalBackupV2(prAdapter, prCalBackupDataV2Info, sizeof(PARAM_CAL_BACKUP_STRUCT_V2_T),
						&u4QueryInfo);
		} else {
			wlanoidSendCalBackupV2Cmd(prAdapter, prCmdInfo->pvInformationBuffer,
						  prCmdInfo->u4InformationBufferLength);
		}
...
VOID nicEventQueryMemDump(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucEventBuf)
{
	...
	if (kalCheckPath(aucPath) == -1) {
		...
		snprintf(aucPath, sizeof(aucPath), "/data/dump_%05hu.hex", g_u2DumpIndex);
	}
...
VOID nicCmdEventQueryMemDump(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (1) {
		...
		if (prEventDumpMem->u4RemainLength == 0 || prEventDumpMem->u4Address == 0xFFFFFFFF) {
			...
			wlanSendMemDumpCmd(prAdapter, prCmdInfo->pvInformationBuffer,
					   prCmdInfo->u4InformationBufferLength);
#endif
		}
	}
 ...
 */
/*----------------------------------------------------------------------------*/
VOID nicCmdEventQueryTemperature(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	UINT_32 u4QueryInfoLen;
...
VOID nicCmdEventQueryWlanInfo(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	DBGLOG(RSN, INFO, "MT7668S : nicCmdEventQueryWlanInfo\n");
	...
	if (prEventWlanInfo && prWlanInfo) {
		...
		kalMemCopy(&prWlanInfo->rWtblRxCounter, &prEventWlanInfo->rWtblRxCounter,
			   sizeof(PARAM_PEER_RX_COUNTER_ALL_T));
		kalMemCopy(&prWlanInfo->rWtblTxCounter, &prEventWlanInfo->rWtblTxCounter,
...
VOID nicCmdEventQueryMibInfo(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	DBGLOG(RSN, INFO, "MT7668S : nicCmdEventQueryMibInfo\n");
	...
	if (prCmdInfo->fgIsOid) {
		...
		if (prEventMibInfo && prMibInfo) {
			...
			kalMemCopy(&prMibInfo->rHwTxAmpduMts, &prEventMibInfo->rHwTxAmpduMts,
	...
	if ((prEvent->ucExtenEID) == EXT_EVENT_ID_CMD_RESULT) {
		...
		if (prCmdInfo != NULL) {
			if ((prCmdInfo->fgIsOid) != 0) {
				kalOidComplete(prAdapter->prGlueInfo, prCmdInfo->fgSetQuery, u4QueryInfoLen,
					       WLAN_STATUS_SUCCESS);
				/* return prCmdInfo */
				cmdBufFreeCmdInfo(prAdapter, prCmdInfo);
			}
...
VOID nicEventBeaconTimeout(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	if (prAdapter->fgDisBcnLostDetection == FALSE) {
		...
		if (prEventBssBeaconTimeout->ucBssIndex == prAdapter->prAisBssInfo->ucBssIndex) {
			...
			g_rDisconnInfoTemp.ucBcnTimeoutReason = prEventBssBeaconTimeout->ucReasonCode;
#endif
...
VOID nicEventUpdateBwcsStatus(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	kalIndicateStatusAndComplete(prAdapter->prGlueInfo, WLAN_STATUS_BWCS_UPDATE, (PVOID)prEventBwcsStatus,
				     sizeof(PTA_IPC_T));
}
...
VOID nicEventAddPkeyDone(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	if (prStaRec) { /* EVENT_ID_ADD_PKEY_DONE for pairwise key */
		...
		for (ucIdx = 0; ucIdx < CFG_STA_REC_NUM; ucIdx++) {
			...
			if (prStaRec && prStaRec->fgIsInUse && prStaRec->ucBssIndex == ucBSSIndex &&
			    prStaRec->fgIsTxKeyReady != TRUE) {
				DBGLOG(RSN, EVENT, "STA " MACSTR " Add Group Key Done!!\n",
				       MAC2STR(prStaRec->aucMacAddr));
...
VOID nicEventCalAllDone(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	if (prEventCalBackupDataV2->ucReason == 1 && prEventCalBackupDataV2->ucAction == 2) {
		...
		DBGLOG(RFTEST, INFO, "Thermal Temp = %d\n", g_rBackupCalDataAllV2.u4ThermalInfo);
		wlanoidQueryCalBackupV2(prAdapter, &g_rCalBackupDataV2, sizeof(PARAM_CAL_BACKUP_STRUCT_V2_T),
					&u4QueryInfo);
	}
}
#endif
...
VOID nicEventWakeUpReason(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	if (prWakeUpReason->reason != ENUM_PF_CMD_TYPE_UNDEFINED) {
		...
		DBGLOG(RX, INFO, "upload EVENT ID[0x%02X] to upper layer!!\n", prEvent->ucEID);
		mtk_cfg80211_vendor_event_wowlan_magic_pkt(wiphy, prGlueInfo->prDevHandler->ieee80211_ptr, 0);
	}
#endif
}
#endif
...
VOID nicEventCSIData(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	INT_32 i4EventLen = prEvent->u2PacketLength - EVENT_HDR_WITHOUT_RXD_SIZE;
	INT_16 i2Idx = 0;
	PINT_8 prBuf = NULL;
	UINT_32 u4IsCck = 0;
...
#define CSI_EVENT_MAX_SIZE 1500
	...
	while (i4EventLen >= u2Offset) {
		...
		case CSI_EVENT_IS_CCK:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid IS_CCK len %u", prCSITlvData->body_len);
				goto out;
			}

			u4IsCck = le32_to_cpup((PUINT_32)prCSITlvData->aucbody);
			prCSIData->bIsCck = (u4IsCck) ? TRUE : FALSE;
			break;
		case CSI_EVENT_CBW:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid CBW len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->ucBw = le32_to_cpup((PUINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_RSSI:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid RSSI len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->cRssi = le32_to_cpup((PUINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_SNR:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid SNR len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->ucSNR = le32_to_cpup((PUINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_BAND:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid BAND len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->ucDbdcIdx = le32_to_cpup((PUINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_CSI_NUM:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid CSI num len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->u2DataCount = le32_to_cpup((PUINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_CSI_I_DATA:
			if (prCSIData->u2DataCount > CSI_MAX_DATA_COUNT) {
				DBGLOG(NIC, WARN, "Invalid CSI count %u\n", prCSIData->u2DataCount);
				goto out;
			}
			...
			kalMemZero(prCSIData->ac2IData, sizeof(prCSIData->ac2IData));

			pru2Tmp = (PINT_16)prCSITlvData->aucbody;
			for (i2Idx = 0; i2Idx < prCSIData->u2DataCount; i2Idx++)
				prCSIData->ac2IData[i2Idx] = le16_to_cpup(pru2Tmp + i2Idx);
			break;
		case CSI_EVENT_CSI_Q_DATA:
			...
			kalMemZero(prCSIData->ac2QData, sizeof(prCSIData->ac2QData));

			pru2Tmp = (PINT_16)prCSITlvData->aucbody;
			for (i2Idx = 0; i2Idx < prCSIData->u2DataCount; i2Idx++)
				prCSIData->ac2QData[i2Idx] = le16_to_cpup(pru2Tmp + i2Idx);
			break;
		case CSI_EVENT_DBW:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid DBW len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->ucDataBw = le32_to_cpup((PINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_CH_IDX:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid CH IDX len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->ucPrimaryChIdx = le32_to_cpup((PINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_TA:
			/*
			 ...
			 */
			if (prCSITlvData->body_len != ALIGN_4(sizeof(prCSIData->aucTA))) {
				DBGLOG(NIC, WARN, "Invalid TA len %u", prCSITlvData->body_len);
				goto out;
			}
			kalMemCopy(prCSIData->aucTA, prCSITlvData->aucbody, sizeof(prCSIData->aucTA));
			break;
		case CSI_EVENT_EXTRA_INFO:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid Error len %u", prCSITlvData->body_len);
				goto out;
			}
			prCSIData->u4ExtraInfo = le32_to_cpup((PINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_RX_MODE:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid Rx Mode len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->ucRxMode = le32_to_cpup((PINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_RSVD1:
			if (prCSITlvData->body_len > sizeof(INT_32) * CSI_MAX_RSVD1_COUNT) {
				DBGLOG(NIC, WARN, "Invalid RSVD1 len %u", prCSITlvData->body_len);
				goto out;
			}

			kalMemCopy(prCSIData->ai4Rsvd1, prCSITlvData->aucbody, prCSITlvData->body_len);

			prCSIData->ucRsvd1Cnt = prCSITlvData->body_len / sizeof(INT_32);
			break;
		case CSI_EVENT_RSVD2:
			if (prCSITlvData->body_len > sizeof(INT_32) * CSI_MAX_RSVD1_COUNT) {
				DBGLOG(NIC, WARN, "Invalid RSVD2 len %u", prCSITlvData->body_len);
				goto out;
			}

			kalMemCopy(prCSIData->au4Rsvd2, prCSITlvData->aucbody, prCSITlvData->body_len);

			prCSIData->ucRsvd1Cnt = prCSITlvData->body_len / sizeof(INT_32);
			break;
		case CSI_EVENT_RSVD3:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid RSVD3 len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->i4Rsvd3 = le32_to_cpup((PINT_32)prCSITlvData->aucbody);
			break;
		case CSI_EVENT_RSVD4:
			if (prCSITlvData->body_len != sizeof(UINT_32)) {
				DBGLOG(NIC, WARN, "Invalid RSVD4 len %u", prCSITlvData->body_len);
				goto out;
			}

			prCSIData->ucRsvd4 = le32_to_cpup((PUINT_32)prCSITlvData->aucbody);
			break;
		default:
	...
	if ((prCSIInfo->ucValue1[CSI_CONFIG_OUTPUT_FORMAT] == CSI_OUTPUT_TONE_MASKED ||
	     prCSIInfo->ucValue1[CSI_CONFIG_OUTPUT_FORMAT] == CSI_OUTPUT_TONE_MASKED_SHIFTED) &&
	    !prCSIData->bIsCck) {
		wlanApplyCSIToneMask(prCSIData->ucRxMode, prCSIData->ucBw, prCSIData->ucDataBw,
				     prCSIData->ucPrimaryChIdx, prCSIData->ac2IData, prCSIData->ac2QData);
	}
	if (prCSIInfo->ucValue1[CSI_CONFIG_OUTPUT_FORMAT] == CSI_OUTPUT_TONE_MASKED_SHIFTED && !prCSIData->bIsCck) {
		kalMemCopy(prCSIInfo->ai2TempIData, prCSIData->ac2IData, sizeof(INT_16) * prCSIData->u2DataCount);
		kalMemCopy(prCSIInfo->ai2TempQData, prCSIData->ac2QData, sizeof(INT_16) * prCSIData->u2DataCount);
		wlanShiftCSI(prCSIData->ucRxMode, prCSIData->ucBw, prCSIData->ucDataBw, prCSIData->ucPrimaryChIdx,
			     prCSIInfo->ai2TempIData, prCSIInfo->ai2TempQData, prCSIData->ac2IData,
			     prCSIData->ac2QData);
...
VOID nicCmdEventGetTxPwrTbl(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	UINT_32 u4QueryInfoLen;
	P_GLUE_INFO_T prGlueInfo;
...
VOID nicEventGetTemperature(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	P_CMD_INFO_T prCmdInfo;

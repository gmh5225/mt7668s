	...
	if (!(u2FrameCtrl & MASK_FC_MORE_FRAG)) {
		...
		if (ucFragNum) {
			DBGLOG(RX, LOUD, "FC %04x M %04x SQ %04x\n", u2FrameCtrl,
			       (UINT_16)(u2FrameCtrl & MASK_FC_MORE_FRAG), u2SeqCtrl);
			fgLast = TRUE;
		}
		/* Non-fragment frame */
		else
	...
	if (RXM_IS_QOS_DATA_FRAME(u2FrameCtrl)) { /* WLAN header shall exist when doing duplicate detection */
		...
		if (prSwRfb->prStaRec->ucDesiredPhyTypeSet & (PHY_TYPE_BIT_HT | PHY_TYPE_BIT_VHT)) {
			...
			if (prSwRfb->ucTid < 8) { /* UP = 0~7 */
				...
				DBGLOG(RX, WARN, "RXM: (Warning) Unknown QoS Data with TID=%d\n", prSwRfb->ucTid);
...
VOID nicRxProcessPktWithoutReorder(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (kalProcessRxPacket(prAdapter->prGlueInfo, prSwRfb->pvPacket, prSwRfb->pvHeader,
	...
	/* Return RFB */
	if ((!timerPendingTimer(&prAdapter->rPacketDelaySetupTimer)) && nicRxSetupRFB(prAdapter, prSwRfb)) {
		DBGLOG(RX, WARN, "Allocate buf failed, Start IndicatedRfb Timer (%u)\n",
...
VOID nicRxProcessForwardPkt(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (prMsduInfo &&
	    kalProcessRxPacket(prAdapter->prGlueInfo, prSwRfb->pvPacket, prSwRfb->pvHeader,
...
VOID nicRxProcessGOBroadcastPkt(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (prRxCtrl->rFreeSwRfbList.u4NumElem >=
		...
		if (prSwRfbDuplicated) {
			kalMemCopy(prSwRfbDuplicated->pucRecvBuff, prSwRfb->pucRecvBuff,
...
VOID nicRxFillRadiotapMCS(IN OUT P_MONITOR_RADIOTAP_T prMonitorRadiotap, IN P_HW_MAC_RX_STS_GROUP_3_T prRxStatusGroup3)
{
	...
	prMonitorRadiotap->ucMcsFlags = ((ucFrMode) | (ucShortGI << 2) | ((ucRxMode & BIT(0)) << 3) | (ucLDPC << 4) |
					 (ucSTBC << 5) | ((ucNess & BIT(0)) << 7));
...
VOID nicRxFillRadiotapVHT(IN OUT P_MONITOR_RADIOTAP_T prMonitorRadiotap, IN P_HW_MAC_RX_STS_GROUP_3_T prRxStatusGroup3)
{
	...
	prMonitorRadiotap->ucVhtFlags = ((ucSTBC) | (ucTxopPsNotAllow << 1) | (ucShortGI << 2) | (ucNsym << 3) |
					 (ucLdpcExtraOfdmSym << 4) | (ucBeamFormed << 5));
...
VOID nicRxProcessMonitorPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	rRadiotapFieldVendor.ucData =
		(((prRxStatusGroup3)->u4RxVector[0] & RX_VT_FR_MODE_MASK) >> RX_VT_FR_MODE_OFFSET);
...
VOID nicRxProcessDataPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	/* non BA session */
		...
		if (!HAL_RX_STATUS_IS_ICV_ERROR(prRxStatus) && HAL_RX_STATUS_IS_TKIP_MIC_ERROR(prRxStatus)) {
		...
		else if (HAL_RX_STATUS_IS_LLC_MIS(prRxStatus) && !HAL_RX_STATUS_IS_ERROR(prRxStatus) &&
			...
			if (prSwRfb->u2HeaderLen >= ETH_HLEN && *pu2EtherType == NTOHS(ETH_P_VLAN))
				fgDrop = FALSE;
		}
#else
	...
	if (HAL_RX_STATUS_IS_CIPHER_MISMATCH(prRxStatus) && (prSwRfb->fgDataFrame == TRUE)) {
		PUINT_16 pu2EtherType;
		...
		if (prSwRfb->u2HeaderLen >= ETH_HLEN && (*pu2EtherType == NTOHS(ETH_P_1X)
#if CFG_SUPPORT_WAPI
			...
			fgDrop = TRUE;
			DBGLOG(RSN, INFO, "Drop plain text during security connection\n");
		}
	}
	...
	if (prAdapter->fgTestMode == FALSE && fgDrop == FALSE) {
		 ...
		 */
		if (prSwRfb->u2HeaderLen >= ETH_HLEN && *pu2EtherType == NTOHS(ETH_P_1X) && prStaRec != NULL &&
		    prStaRec->ucStaState != STA_STATE_3 && prStaRec->eapol_re_enqueue_cnt++ < MAX_EAPOL_REQUE) {
			KAL_SPIN_LOCK_DECLARATION();
			DBGLOG(RSN, WARN, "Re-enqueue EAPOL frame: State[%u], retry[%d]\n", prStaRec->ucStaState,
		...
		if ((prSwRfb->prStaRec != NULL) && (prSwRfb->ucStaRecIdx < CFG_STA_REC_NUM) &&
...
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
...
#if DBG
	{
		...
		if (authSendDeauthFrame(prAdapter, NULL, NULL, prSwRfb, REASON_CODE_CLASS_3_ERR,
	...
	case EVENT_ID_TDLS:
		...
		TdlsexEventHandle(prAdapter->prGlueInfo, (UINT_8 *)prEvent->aucBuffer,
...
VOID nicRxProcessMgmtPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
		...
		if (prAdapter->rRxCtrl.u4RxPktsDumpTypeMask & BIT(HIF_RX_PKT_TYPE_MANAGEMENT)) {
			if (u2TxFrameCtrl == MAC_FRAME_BEACON || u2TxFrameCtrl == MAC_FRAME_PROBE_RSP) {
				...
				DBGLOG(SW4, INFO,
				       ...
				       prSwRfb->prStaRec->ucBssIndex, prSwRfb->ucStaRecIdx, prSwRfb->ucWlanIdx,
	...
	if (prAdapter->fgTestMode == FALSE) {
		...
		if ((prGlueInfo == NULL) || (prGlueInfo->u4ReadyFlag == 0)) {
			DBGLOG(RX, WARN, "Bypass this mgmt frame without wlanProbe done\n");
			...
			while (QUEUE_IS_NOT_EMPTY(prTempRfbList)) {
				...
				case RX_PKT_TYPE_SW_DEFINED:
					...
					/* case HIF_RX_PKT_TYPE_MANAGEMENT: */
						...
						DBGLOG(RX, ERROR, "Rcv Pkt format wrong.!!!\n");
						nicRxReturnRFB(prAdapter, prSwRfb);
...
#define SPRINTF_RX_COUNTER(eCounter) \
	{ \
		u4CurrCount = scnprintf(pucCurrBuf, *pu4Count, "%-30s : %u\n", #eCounter, \
					(UINT_32)prRxCtrl->au8Statistics[eCounter]); \
		pucCurrBuf += (UINT_8)u4CurrCount; \
		*pu4Count -= u4CurrCount; \
	}
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
nicRxWaitResponse(IN P_ADAPTER_T prAdapter, IN UINT_8 ucPortIdx, OUT PUINT_8 pucRspBuffer,
		  IN UINT_32 u4MaxRespBufferLen, OUT PUINT_32 pu4Length)
{
	...
	if (u4Status == WLAN_STATUS_SUCCESS) {
		DBGLOG(RX, TRACE, "Dump Response buffer, length = %u\n", *pu4Length);
...
WLAN_STATUS nicRxProcessActionFrame(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if ((prActFrame->ucCategory <= CATEGORY_PROTECTED_DUAL_OF_PUBLIC_ACTION &&
	     ...
	     prActFrame->ucCategory != CATEGORY_HT_ACTION) /* At 11W
							      spec
							      Code 7
							      is
							      reserved
...
/* Sigma */
#else
		if (prAdapter->prGlueInfo && prAdapter->prGlueInfo->fgIsRegistered && prAdapter->prAisBssInfo &&

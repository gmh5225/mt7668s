			...
			if (HAL_RX_STATUS_GET_SEC_MODE(prRxStatus) == CIPHER_SUITE_TKIP_WO_MIC) { \
				if (prCurrSwRfb->prStaRec) { \
					...
					if (pucMicKey == NULL) { \
						...
						fgMicErr = TRUE; \
					} else if (tkipMicDecapsulateInRxHdrTransMode(prCurrSwRfb, pucMicKey) == \
						   FALSE) { \
						fgMicErr = TRUE; \
					} \
				} \
...
VOID qmUpdateStaRec(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec)
{
	...
	if ((IS_FEATURE_ENABLED(prAdapter->rWifiVar.ucStaVhtBfer) ||
	    ...
	    prAdapter->fgIsHwSupportBfer && fgIsTxAllowed && (prStaRec->ucStaState == STA_STATE_3)) {
...
VOID qmSetStaRecTxAllowed(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN BOOLEAN fgIsTxAllowed)
{
	...
	DBGLOG(QM, INFO, "Set Sta[%u] TxAllowed from [%u] to [%u] %s TxQ\n", prStaRec->ucIndex, prStaRec->fgIsTxAllowed,
	       fgIsTxAllowed, fgIsTxAllowed ? "normal" : "pending");
...
/*----------------------------------------------------------------------------*/
UINT_32
qmDequeueTxPacketsFromPerStaQueues(IN P_ADAPTER_T prAdapter, OUT P_QUE_T prQue, IN UINT_8 ucTC,
 ...
 */
/*----------------------------------------------------------------------------*/
VOID qmDequeueTxPacketsFromPerTypeQueues(IN P_ADAPTER_T prAdapter, OUT P_QUE_T prQue, IN UINT_8 ucTC,
					 IN UINT_32 u4CurrentQuota, IN UINT_32 u4TotalQuota)
{
...
/*----------------------------------------------------------------------------*/
UINT_32
qmDequeueTxPacketsFromGlobalQueue(IN P_ADAPTER_T prAdapter, OUT P_QUE_T prQue, IN UINT_8 ucTC,
				  IN UINT_32 u4CurrentQuota, IN UINT_32 u4TotalQuota)
{
	P_BSS_INFO_T prBssInfo;
	P_QUE_T prCurrQueue;
...
P_MSDU_INFO_T qmDequeueTxPacketsMthread(IN P_ADAPTER_T prAdapter, IN P_TX_TCQ_STATUS_T prTcqStatus)
{
	...
	while (prMsduInfo) {
		...
		nicTxAcquireResource(prAdapter, prMsduInfo->ucTC, nicTxGetPageCount(prMsduInfo->u2FrameLength, FALSE),
				     FALSE);
...
VOID qmAllocateResidualTcResource(IN P_ADAPTER_T prAdapter, IN PINT_32 ai4TcResDemand, IN PUINT_32 pu4ResidualResource,
		...
		for (u4Tc = 0; u4Tc < QM_ACTIVE_TC_NUM; u4Tc++) {
			if (QM_GET_TX_QUEUE_LEN(prAdapter, u4Tc) >= 100) {
				DBGLOG(QM, LOUD, "QM: QueLen [%ld %ld %ld %ld %ld %ld]\n",
				       QM_GET_TX_QUEUE_LEN(prAdapter, 0), QM_GET_TX_QUEUE_LEN(prAdapter, 1),
				       QM_GET_TX_QUEUE_LEN(prAdapter, 2), QM_GET_TX_QUEUE_LEN(prAdapter, 3),
...
UINT_32
gmGetDequeueQuota(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN P_BSS_INFO_T prBssInfo,
	...
	do {
		...
		if (prAdapter->rRxCtrl.u4RxPktsDumpTypeMask & BIT(HIF_RX_PKT_TYPE_DATA)) {
			DBGLOG(SW4, INFO, "QM RX DATA: net _u sta idx %u wlan idx %u ssn _u tid %u ptype %u 11 %u\n",
			       ...
			       HAL_RX_STATUS_GET_TID(prRxStatus), prCurrSwRfb->ucPacketType,
		...
		if (HAL_RX_STATUS_GET_HEADER_TRAN(prRxStatus) == TRUE) {
			...
			if (prCurrSwRfb->prRxStatusGroup4 == NULL) {
				...
				prCurrSwRfb->eDst = RX_PKT_DESTINATION_NULL;
				QUEUE_INSERT_TAIL(prReturnedQue, (P_QUE_ENTRY_T)prCurrSwRfb);
				DBGLOG_MEM8(RX, WARN, (uint8_t *)prRxStatus, sizeof(*prRxStatus));
...
VOID qmProcessBarFrame(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, OUT P_QUE_T prReturnedQue)
{
	...
	prSwRfb->u2SSN = (*((PUINT_16)((PUINT_8)prBarCtrlFrame + CTRL_BAR_BAR_INFORMATION_OFFSET))) >>
			 OFFSET_BAR_SSC_SN;
...
VOID qmInsertReorderPkt(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, IN P_RX_BA_ENTRY_T prReorderQueParm,
			OUT P_QUE_T prReturnedQue)
{
	...
	/* Case 2: Fall ahead */
		...
		prReorderQueParm->u2WinStart =
			(prReorderQueParm->u2WinEnd + MAX_SEQ_NO_COUNT - prReorderQueParm->u2WinSize + 1) %
			MAX_SEQ_NO_COUNT;
#if CFG_SUPPORT_RX_AMSDU
		/* RX reorder for one MSDU in AMSDU issue */
		prReorderQueParm->u8LastAmsduSubIdx = RX_PAYLOAD_FORMAT_MSDU;
...
VOID qmInsertFallWithinReorderPkt(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, IN P_RX_BA_ENTRY_T prReorderQueParm,
				  OUT P_QUE_T prReturnedQue)
{
...
VOID qmInsertFallAheadReorderPkt(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, IN P_RX_BA_ENTRY_T prReorderQueParm,
				 OUT P_QUE_T prReturnedQue)
{
	P_QUE_T prReorderQue;
	...
	while (TRUE) {
		...
		/* SN > WinStart, break to update WinEnd */
			...
			if (!prReorderQueParm->fgHasBubble) {
				...
				DBGLOG(QM, TRACE, "QM:(Bub Timer) STA[%u] TID[%u] BubSN[%u] Win{%u, %u}\n",
			...
			if (fgMissing && CHECK_FOR_TIMEOUT(rCurrentTime, *prMissTimeout,
				...
				DBGLOG(QM, TRACE, "QM:RX BA Timout Next Tid %u SSN %u\n", prReorderQueParm->ucTid,
...
VOID qmHandleReorderBubbleTimeout(IN P_ADAPTER_T prAdapter, IN ULONG ulParamPtr)
{
	...
	if (prSwRfb) {
		...
		DBGLOG(QM, TRACE, "QM:(Bub Timer Restart) STA[%u] TID[%u] BubSN[%u] Win{%u,%u}\n",
...
VOID qmHandleEventCheckReorderBubble(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	if (prReorderQueParm->u2FirstBubbleSn == prReorderQueParm->u2WinStart) {
		...
		DBGLOG(QM, TRACE, "QM:(Bub Flush) STA[%u] TID[%u] BubSN[%u] Win{%u,%u}\n",
		       prReorderQueParm->ucStaRecIdx, prReorderQueParm->ucTid, prReorderQueParm->u2FirstBubbleSn,
		       prReorderQueParm->u2WinStart, prReorderQueParm->u2WinEnd);
...
BOOL qmAddRxBaEntry(IN P_ADAPTER_T prAdapter, IN UINT_8 ucStaRecIdx, IN UINT_8 ucTid, IN UINT_16 u2WinStart,
		    IN UINT_16 u2WinSize)
{
	...
	if (prRxBaEntry) {
		...
		DBGLOG(QM, INFO, "QM: +RxBA(STA=%u TID=%u WinStart=%u WinEnd=%u WinSize=%u)\n", ucStaRecIdx, ucTid,
		...
		if (IS_BSS_ACTIVE(prBssInfo)) {
			if (prBssInfo->eCurrentOPMode == OP_MODE_INFRASTRUCTURE &&
				...
				if (EQUAL_MAC_ADDR(prBssInfo->aucBSSID,
					...
					fgNewParameter =
						mqmParseEdcaParameters(prAdapter, prSwRfb, pucIE, u2IELength, FALSE);
				}
			}
	...
	do {
		...
		for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
			...
			DBGLOG(QM, INFO,
			       ...
			       prAcQueParams->u2CWmin, prAcQueParams->u2CWmax, prAcQueParams->u2TxopLimit,
			       fgNewParameter);
		}
	} while (FALSE);
			...
			if ((WMM_IE_OUI_TYPE(pucIE) == VENDOR_OUI_TYPE_WMM) &&
				...
				case VENDOR_OUI_SUBTYPE_WMM_PARAM:
					...
					prStaRec->fgIsUapsdSupported =
						((((P_IE_WMM_PARAM_T)pucIE)->ucQosInfo) & WMM_QOS_INFO_UAPSD) ? TRUE :
														FALSE;
					break;
				...
				case VENDOR_OUI_SUBTYPE_WMM_INFO:
					...
					prStaRec->fgIsWmmSupported = TRUE;
					prStaRec->fgIsUapsdSupported =
						((((P_IE_WMM_INFO_T)pucIE)->ucQosInfo) & WMM_QOS_INFO_UAPSD) ? TRUE :
													       FALSE;
					break;
...
/*----------------------------------------------------------------------------*/
UINT_32
mqmFillWmmInfoIE(P_UINT_8 pucOutBuf, BOOLEAN fgSupportUAPSD, UINT_8 ucBmpDeliveryAC, UINT_8 ucBmpTriggerAC,
		 UINT_8 ucUapsdSp)
{
	P_IE_WMM_INFO_T prIeWmmInfo;
	UINT_32 ucUapsd[] = { WMM_QOS_INFO_BE_UAPSD, WMM_QOS_INFO_BK_UAPSD, WMM_QOS_INFO_VI_UAPSD,
...
VOID mqmGenerateWmmInfoIE(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo)
{
	...
	u4Length = mqmGenerateWmmInfoIEByStaRec(prAdapter, prBssInfo, prStaRec,
	...
	for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
		...
		if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.fgTdlsBufferSTASleep)) {
			...
			if (prBssInfo->arACQueParms[eAci].ucIsACMSet)
				prAcParam->ucAciAifsn |= WMM_ACIAIFSN_ACM;
			/* AIFSN */
			prAcParam->ucAciAifsn |= (prBssInfo->arACQueParms[eAci].u2Aifsn & WMM_ACIAIFSN_AIFSN);

			/* ECW Min */
			prAcParam->ucEcw = (prBssInfo->arACQueParms[eAci].u2CWmin & WMM_ECW_WMIN_MASK);
			/* ECW Max */
			prAcParam->ucEcw |=
				((prBssInfo->arACQueParms[eAci].u2CWmax << WMM_ECW_WMAX_OFFSET) & WMM_ECW_WMAX_MASK);

			/* Txop limit */
			WLAN_SET_FIELD_16(&prAcParam->u2TxopLimit, prBssInfo->arACQueParms[eAci].u2TxopLimit);
		}
	}
...
#if (CFG_SUPPORT_DFS_MASTER == 1)
		if (prP2pRoleFsmInfo &&
		    prAdapter->prGlueInfo->prP2PInfo[prP2pRoleFsmInfo->ucRoleIndex]->fgIsNetDevDetach) {
			netif_device_attach(
				prAdapter->prGlueInfo->prP2PInfo[prP2pRoleFsmInfo->ucRoleIndex]->prDevHandler);
			prAdapter->prGlueInfo->prP2PInfo[prP2pRoleFsmInfo->ucRoleIndex]->fgIsNetDevDetach = FALSE;
		}
#endif
	}
}
...
VOID qmHandleEventStaChangePsMode(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	if (prStaRec) {
		...
		qmUpdateFreeQuota(prAdapter, prStaRec, prEventStaChangePsMode->ucUpdateMode,
 ...
 */
/*----------------------------------------------------------------------------*/
VOID qmUpdateFreeQuota(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN UINT_8 ucUpdateMode,
...
UINT_32 qmDumpQueueStatus(IN P_ADAPTER_T prAdapter, IN PUINT_8 pucBuf, IN UINT_32 u4Max)
{
	...
	LOGBUF(pucBuf, u4Max, u4Len, "ToT ResCount: Max[%02u/%03u] Free[%02u/%03u]\n", u4TotalBufferCount,
	       u4TotalPageCount, u4CurBufferCount, u4CurPageCount);
 ...
 */
/*----------------------------------------------------------------------------*/
VOID mqmRxModifyBaEntryStatus(IN P_ADAPTER_T prAdapter, IN P_RX_BA_ENTRY_T prRxBaEntry,
			      IN ENUM_BA_ENTRY_STATUS_T eStatus)
{
	...
	if (fgResetScoreBoard) {
		...
		prCmdBody = (P_CMD_RESET_BA_SCOREBOARD_T)cnmMemAlloc(prAdapter, RAM_TYPE_BUF,
...
VOID mqmHandleAddBaReq(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		if ((((rAddBaReqBody.u2BAParameterSet) & BA_PARAM_SET_ACK_POLICY_MASK) >>
		     BA_PARAM_SET_ACK_POLICY_MASK_OFFSET) != BA_PARAM_SET_ACK_POLICY_IMMEDIATE_BA) { /* Only
													Immediate_BA is
													supported */
		...
		if (fgIsReqAccepted) {
			...
			if (!prRxBaEntry) {
				...
				if (prQM->ucRxBaCount < CFG_NUM_OF_RX_BA_AGREEMENTS) {
					fgIsNewEntryAdded = qmAddRxBaEntry(prAdapter, (UINT_8)u4StaRecIdx,
...
BOOLEAN qmHandleRxReplay(P_ADAPTER_T prAdapter, P_SW_RFB_T prSwRfb)
{
	...
	DBGLOG(QM, TRACE, "BC packet 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x--0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\n", pucPN[0],
	       pucPN[1], pucPN[2], pucPN[3], pucPN[4], pucPN[5], prDetRplyInfo->arReplayPNInfo[ucKeyID].auPN[0],

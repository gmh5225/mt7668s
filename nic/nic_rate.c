...
BOOLEAN
nicGetRateIndexFromRateSetWithLimit(IN UINT_16 u2RateSet, IN UINT_32 u4PhyRateLimit, IN BOOLEAN fgGetLowest,
				    OUT PUINT_8 pucRateSwIndex)
{
	...
	for (i = RATE_1M_SW_INDEX; i <= RATE_54M_SW_INDEX; i++) {
		if (u2RateSet & BIT(i)) {
			...
			u4CurPhyRate = nicRateCode2DataRate(u2CurRateCode, MAC_BW_20, MAC_GI_NORMAL);

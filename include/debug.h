...
typedef enum _ENUM_DBG_MODULE_T {
	DBG_INIT_IDX = 0,
	/* 0x00 */ /* For driver initial */
	DBG_HAL_IDX,
	/* 0x01 */ /* For HAL(HW) Layer */
	DBG_INTR_IDX,
	/* 0x02 */   /* For Interrupt */
	DBG_REQ_IDX, /* 0x03 */
	DBG_TX_IDX,  /* 0x04 */
	DBG_RX_IDX,  /* 0x05 */
	DBG_RFTEST_IDX,
	/* 0x06 */ /* For RF test mode */
	DBG_EMU_IDX,
	/* 0x07 */ /* Developer specific */
	DBG_SW1_IDX,
	/* 0x08 */ /* Developer specific */
	DBG_SW2_IDX,
	/* 0x09 */ /* Developer specific */
	DBG_SW3_IDX,
	/* 0x0A */ /* Developer specific */
	DBG_SW4_IDX,
	/* 0x0B */ /* Developer specific */
	DBG_HEM_IDX,
	/* 0x0C */ /* HEM */
	DBG_AIS_IDX,
	/* 0x0D */ /* AIS */
...
} ENUM_DBG_ASSERT_LEVEL_CTRL_T,
	*P_ENUM_DBG_ASSERT_LEVEL_CTRL_T;
...
#define DBGLOG(_Module, _Class, _Fmt, ...) \
	do { \
		if ((aucDebugModule[DBG_##_Module##_IDX] & DBG_CLASS_##_Class) == 0) \
			break; \
		LOG_FUNC("[%u]%s:(" #_Module " " #_Class ") " _Fmt, KAL_GET_CURRENT_THREAD_ID(), __func__, \
			 ##__VA_ARGS__); \
	} while (0)
#define TOOL_PRINTLOG(_Module, _Class, _Fmt, ...) \
	do { \
...
#define LIMITED_DBGLOG(_Module, _Class, _Fmt, ...) \
	do { \
		if ((aucDebugModule[DBG_##_Module##_IDX] & DBG_CLASS_##_Class) == 0) \
			break; \
		LIMITED_LOG_FUNC("[%u]%s:(" #_Module " " #_Class ") " _Fmt, KAL_GET_CURRENT_THREAD_ID(), __func__, \
				 ##__VA_ARGS__); \
	} while (0)
#endif

...
#define CHANNEL_PWR_LIMIT(_channel, _pwrLimit_cck, _pwrLimit_bw20, _pwrLimit_bw40, _pwrLimit_bw80, _pwrLimit_bw160, \
			  _ucFlag) \
	{ \
		.ucCentralCh = (_channel), .cPwrLimitCCK = (_pwrLimit_cck), .cPwrLimit20 = (_pwrLimit_bw20), \
		.cPwrLimit40 = (_pwrLimit_bw40), .cPwrLimit80 = (_pwrLimit_bw80), .cPwrLimit160 = (_pwrLimit_bw160), \
		.ucFlag = (_ucFlag), .aucReserved = { \
			0 \
		} \
	}
...
enum regd_control_flag { REGD_CTRL_FLAG_SUPPORT_LOCAL_REGD_DB = (0x1 << 0) };
 ...
 *                                 M A C R O S
 *******************************************************************************/
#define CAL_CH_OFFSET_80M(_PRIMARY_CH, _CENTRAL_CH) (((_PRIMARY_CH - _CENTRAL_CH) + 6) >> 2)

#define CAL_CH_OFFSET_160M(_PRIMARY_CH, _CENTRAL_CH) (((_PRIMARY_CH - _CENTRAL_CH) + 14) >> 2)
...
BOOLEAN rlmDomainIsValidRfSetting(P_ADAPTER_T prAdapter, ENUM_BAND_T eBand, UINT_8 ucPriChannel,
				  ENUM_CHNL_EXT_T eExtend, ENUM_CHANNEL_WIDTH_T eChannelWidth, UINT_8 ucChannelS1,
				  UINT_8 ucChannelS2);

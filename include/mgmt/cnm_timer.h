...
#define SYSTIME_TO_TU(_systime) ((SYSTIME_TO_USEC(_systime) + ((USEC_PER_TU / 2) - 1)) / USEC_PER_TU)
...
#define timerRenewTimer(adapter, tmr, interval) timerStartTimer(adapter, tmr, interval, (tmr)->function, (tmr)->data)
...
VOID cnmTimerInitTimer(IN P_ADAPTER_T prAdapter, IN P_TIMER_T prTimer, IN PFN_MGMT_TIMEOUT_FUNC pfFunc,
		       IN ULONG ulDataPtr);

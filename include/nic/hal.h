...
#define HAL_READ_RX_PORT(prAdapter, u4PortId, u4Len, pvBuf, _u4ValidBufSize) \
	{ \
		ASSERT(u4PortId < 2); \
		HAL_PORT_RD(prAdapter, u4PortId, u4Len, pvBuf, _u4ValidBufSize /*temp!!*/ /*4Kbyte*/) \
	}

#define HAL_WRITE_TX_PORT(_prAdapter, _u4PortId, _u4Len, _pucBuf, _u4ValidBufSize) \
	{ \
		HAL_PORT_WR(_prAdapter, _u4PortId, _u4Len, _pucBuf, _u4ValidBufSize /*temp!!*/ /*4KByte*/) \
	}
...
#define HAL_LP_OWN_CLR(_prAdapter, _pfgResult) \
	{ \
		...
		*_pfgResult = FALSE; \
		...
		HAL_MCR_WR(_prAdapter, CFG_PCIE_LPCR_HOST, PCIE_LPCR_HOST_CLR_OWN); \
		HAL_MCR_RD(_prAdapter, CFG_PCIE_LPCR_HOST, &u4RegValue); \
		if (u4RegValue == 0) { \
			*_pfgResult = TRUE; \
		} \
...
#define HAL_IS_TX_DONE_INTR(u4IntrStatus) \
	((u4IntrStatus & (WPDMA_TX_DONE_INT0 | WPDMA_TX_DONE_INT1 | WPDMA_TX_DONE_INT2 | WPDMA_TX_DONE_INT3)) ? TRUE : \
														FALSE)

#define HAL_IS_RX_DONE_INTR(u4IntrStatus) ((u4IntrStatus & (WPDMA_RX_DONE_INT0 | WPDMA_RX_DONE_INT1)) ? TRUE : FALSE)

#define HAL_IS_ABNORMAL_INTR(u4IntrStatus)

#define HAL_IS_FW_OWNBACK_INTR(u4IntrStatus) ((u4IntrStatus & WPDMA_FW_CLR_OWN_INT) ? TRUE : FALSE)
...
#define HAL_READ_RX_PORT(prAdapter, u4PortId, u4Len, pvBuf, _u4ValidBufSize) \
	{ \
		ASSERT(u4PortId < 2); \
		HAL_PORT_RD(prAdapter, ((u4PortId == 0) ? MCR_WRDR0 : MCR_WRDR1), u4Len, pvBuf, \
...
#define HAL_WRITE_TX_PORT(_prAdapter, _u4PortId, _u4Len, _pucBuf, _u4ValidBufSize) \
	{ \
		...
		HAL_PORT_WR(_prAdapter, MCR_WTDR1, _u4Len, _pucBuf, _u4ValidBufSize /*temp!!*/ /*4KByte*/) \
	}
...
#define HAL_WIFI_FUNC_GET_STATUS(_prAdapter, _u4Result) halGetMailbox(_prAdapter, 0, &_u4Result)

#define HAL_INTR_DISABLE(_prAdapter) HAL_MCR_WR(_prAdapter, MCR_WHLPCR, WHLPCR_INT_EN_CLR)

#define HAL_INTR_ENABLE(_prAdapter) HAL_MCR_WR(_prAdapter, MCR_WHLPCR, WHLPCR_INT_EN_SET)

#define HAL_INTR_ENABLE_AND_LP_OWN_SET(_prAdapter) \
	HAL_MCR_WR(_prAdapter, MCR_WHLPCR, (WHLPCR_INT_EN_SET | WHLPCR_FW_OWN_REQ_SET))
...
#define HAL_DISABLE_RX_ENHANCE_MODE(_prAdapter) \
	{ \
		...
		HAL_MCR_WR(_prAdapter, MCR_WHCR, u4Value & ~WHCR_RX_ENHANCE_MODE_EN); \
	}
...
#define HAL_SET_INTR_STATUS_READ_CLEAR(prAdapter) \
	{ \
		...
		HAL_MCR_WR(prAdapter, MCR_WHCR, u4Value & ~WHCR_W_INT_CLR_CTRL); \
 ...
 */
#define HAL_READ_INTR_STATUS(prAdapter, length, pvBuf) HAL_PORT_RD(prAdapter, MCR_WHISR, length, pvBuf, length)
...
#define HAL_IS_TX_DONE_INTR(u4IntrStatus) ((u4IntrStatus & WHISR_TX_DONE_INT) ? TRUE : FALSE)

#define HAL_IS_RX_DONE_INTR(u4IntrStatus) ((u4IntrStatus & (WHISR_RX0_DONE_INT | WHISR_RX1_DONE_INT)) ? TRUE : FALSE)

#define HAL_IS_ABNORMAL_INTR(u4IntrStatus) ((u4IntrStatus & WHISR_ABNORMAL_INT) ? TRUE : FALSE)

#define HAL_IS_FW_OWNBACK_INTR(u4IntrStatus) ((u4IntrStatus & WHISR_FW_OWN_BACK_INT) ? TRUE : FALSE)

#define HAL_PUT_MAILBOX(prAdapter, u4MboxId, u4Data) \
	{ \
		ASSERT(u4MboxId < 2); \
		HAL_MCR_WR(prAdapter, ((u4MboxId == 0) ? MCR_H2DSM0R : MCR_H2DSM1R), u4Data); \
	}

#define HAL_GET_MAILBOX(prAdapter, u4MboxId, pu4Data) \
	{ \
		ASSERT(u4MboxId < 2); \
		HAL_MCR_RD(prAdapter, ((u4MboxId == 0) ? MCR_D2HRM0R : MCR_D2HRM1R), pu4Data); \
	}
...
#define HAL_SET_MAILBOX_READ_CLEAR(prAdapter, fgEnableReadClear) \
	{ \
		...
		HAL_MCR_WR(prAdapter, MCR_WHCR, \
			   (fgEnableReadClear) ? (u4Value | WHCR_RECV_MAILBOX_RD_CLR_EN) : \
...
#define HAL_WIFI_FUNC_POWER_ON(_prAdapter) \
	mtk_usb_vendor_request(_prAdapter->prGlueInfo, 0, DEVICE_VENDOR_REQUEST_OUT, VND_REQ_POWER_ON_WIFI, 0, 1, \
			       NULL, 0)

#define HAL_WIFI_FUNC_CHIP_RESET(_prAdapter) \
	mtk_usb_vendor_request(_prAdapter->prGlueInfo, 0, DEVICE_VENDOR_REQUEST_OUT, VND_REQ_POWER_ON_WIFI, 1, 1, \
			       NULL, 0)
...
#define HAL_IS_TX_DIRECT(_prAdapter) ((CFG_TX_DIRECT_USB) ? TRUE : FALSE)

#define HAL_IS_RX_DIRECT(_prAdapter) ((CFG_RX_DIRECT_USB) ? TRUE : FALSE)

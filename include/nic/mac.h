...
#define DOT11_RTS_THRESHOLD_DEFAULT DOT11_RTS_THRESHOLD_MAX
...
#define DOT11_FRAGMENTATION_THRESHOLD_DEFAULT DOT11_FRAGMENTATION_THRESHOLD_MAX
...
#define INVALID_SEQ_CTRL_NUM \
	0x000F /* According to 6.2.1.1.2 \
...
#define STATUS_CODE_ASSOC_DENIED_LARGE_LIS_INTERVAL \
...
#define VENDOR_OUI_WFA \
	{ \
		0x00, 0x50, 0xF2 \
	}
#define VENDOR_OUI_WFA_SPECIFIC \
	{ \
		0x50, 0x6F, 0x9A \
	}
...
#define dBm_TO_RCPI(_dbm) \
	(RCPI)(((((PARAM_RSSI)(_dbm) + NDBM_LOW_BOUND_FOR_RCPI) << 1) > RCPI_HIGH_BOUND) ? \
		       RCPI_HIGH_BOUND : \
		       ((((PARAM_RSSI)(_dbm) + NDBM_LOW_BOUND_FOR_RCPI) << 1) < RCPI_LOW_BOUND ? \
				RCPI_LOW_BOUND : \
...
#define IS_BMCAST_MAC_ADDR(_pucDestAddr) ((BOOLEAN)(((PUINT_8)(_pucDestAddr))[0] & BIT(0)))

/* The macro to check if the MAC address is UCAST Address */
#define IS_UCAST_MAC_ADDR(_pucDestAddr) ((BOOLEAN) !(((PUINT_8)(_pucDestAddr))[0] & BIT(0)))

/* The macro to copy the MAC address */
#define COPY_MAC_ADDR(_pucDestAddr, _pucSrcAddr) kalMemCopy(_pucDestAddr, _pucSrcAddr, MAC_ADDR_LEN)

/* The macro to check if two MAC addresses are equal */
#define EQUAL_MAC_ADDR(_pucDestAddr, _pucSrcAddr) (!kalMemCmp(_pucDestAddr, _pucSrcAddr, MAC_ADDR_LEN))

/* The macro to check if two MAC addresses are not equal */
#define UNEQUAL_MAC_ADDR(_pucDestAddr, _pucSrcAddr) (kalMemCmp(_pucDestAddr, _pucSrcAddr, MAC_ADDR_LEN))

/* The macro to check whether two SSIDs are equal */
#define EQUAL_SSID(pucSsid1, ucSsidLen1, pucSsid2, ucSsidLen2) \
	((ucSsidLen1 <= ELEM_MAX_LEN_SSID) && (ucSsidLen2 <= ELEM_MAX_LEN_SSID) && ((ucSsidLen1) == (ucSsidLen2)) && \
...
#define COPY_IE(pucDestIE, pucSrcIE) \
	do { \
		kalMemCopy((PUINT_8)pucDestIE, (PUINT_8)pucSrcIE, IE_SIZE(pucSrcIE)); \
	} while (FALSE)

...
#define LINK_MGMT_GET_ENTRY(prLinkMgmt, prEntry, EntryType, memType) \
	do { \
		LINK_REMOVE_HEAD(&((struct LINK_MGMT *)prLinkMgmt)->rFreeLink, prEntry, EntryType *); \
		if (!prEntry) \
...
#define LINK_PEEK_HEAD(prLink, _type, _member) \
	(LINK_IS_EMPTY(prLink) ? NULL : LINK_ENTRY((prLink)->prNext, _type, _member))

/* Peek tail entry, but keep still in link list */
#define LINK_PEEK_TAIL(prLink, _type, _member) \
	(LINK_IS_EMPTY(prLink) ? NULL : LINK_ENTRY((prLink)->prPrev, _type, _member))
...
#define LINK_FOR_EACH(prEntry, prLink) \
	for (prEntry = (prLink)->prNext; prEntry != (P_LINK_ENTRY_T)(prLink); prEntry = (P_LINK_ENTRY_T)prEntry->prNext)

/* Iterate over a link list backwards */
#define LINK_FOR_EACH_PREV(prEntry, prLink) \
	for (prEntry = (prLink)->prPrev; prEntry != (P_LINK_ENTRY_T)(prLink); prEntry = (P_LINK_ENTRY_T)prEntry->prPrev)

/* Iterate over a link list safe against removal of link entry */
#define LINK_FOR_EACH_SAFE(prEntry, prNextEntry, prLink) \
	for (prEntry = (prLink)->prNext, prNextEntry = prEntry->prNext; prEntry != (P_LINK_ENTRY_T)(prLink); \
...
#define LINK_FOR_EACH_ENTRY(prObj, prLink, rMember, _TYPE) \
	for (prObj = LINK_ENTRY((prLink)->prNext, _TYPE, rMember); &prObj->rMember != (P_LINK_ENTRY_T)(prLink); \
...
#define LINK_FOR_EACH_ENTRY_PREV(prObj, prLink, rMember, _TYPE) \
	for (prObj = LINK_ENTRY((prLink)->prPrev, _TYPE, rMember); &prObj->rMember != (P_LINK_ENTRY_T)(prLink); \
...
#define LINK_FOR_EACH_ENTRY_SAFE(prObj, prNextObj, prLink, rMember, _TYPE) \
	for (prObj = LINK_ENTRY((prLink)->prNext, _TYPE, rMember), \
	    prNextObj = LINK_ENTRY(prObj->rMember.prNext, _TYPE, rMember); \
	     ...
	     prObj = prNextObj, prNextObj = LINK_ENTRY(prNextObj->rMember.prNext, _TYPE, rMember))

...
wlanoidSendSetQueryBowCmd(IN P_ADAPTER_T prAdapter, IN UINT_8 ucCID, IN UINT_8 ucBssIdx, IN BOOLEAN fgSetQuery,
			  IN BOOLEAN fgNeedResp, IN PFN_CMD_DONE_HANDLER pfCmdDoneHandler,
...
WLAN_STATUS bowCmdGetMacStatus(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	if (cnmPreferredChannel(prAdapter, &eBand, &ucPrimaryChannel, &eBssSCO)) {
		...
		rlmDomainGetChnlList(prAdapter, BAND_2G4, FALSE, MAX_BOW_NUMBER_OF_CHANNEL_2G4, &ucNumOfChannel,
				     aucChannelList);
		...
		rlmDomainGetChnlList(prAdapter, BAND_5G, FALSE, MAX_BOW_NUMBER_OF_CHANNEL_5G, &ucNumOfChannel,
				     aucChannelList);
...
WLAN_STATUS bowCmdSetupConnection(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	if (g_u4LinkCount == 1) {
		...
		cnmTimerInitTimer(prAdapter, &prBowFsmInfo->rStartingBeaconTimer, (PFN_MGMT_TIMEOUT_FUNC)bowSendBeacon,
				  (ULONG)NULL);

		cnmTimerInitTimer(prAdapter, &prBowFsmInfo->rChGrantedTimer, (PFN_MGMT_TIMEOUT_FUNC)bowChGrantedTimeout,
				  (ULONG)NULL);
...
WLAN_STATUS bowCmdDestroyConnection(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	return wlanoidSendSetQueryBowCmd(prAdapter, CMD_ID_CMD_BT_OVER_WIFI, prBowFsmInfo->ucBssIndex, TRUE, FALSE,
					 wlanbowCmdEventLinkDisconnected, wlanbowCmdTimeoutHandler,
					 sizeof(CMD_BT_OVER_WIFI), (PUINT_8)&rCmdBtOverWifi,
...
WLAN_STATUS bowCmdSetPTK(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	return wlanoidSendSetQueryBowCmd(prAdapter, CMD_ID_ADD_REMOVE_KEY, prBowFsmInfo->ucBssIndex, TRUE, FALSE,
					 wlanbowCmdEventSetCommon, wlanbowCmdTimeoutHandler, sizeof(CMD_802_11_KEY),
...
WLAN_STATUS bowCmdReadRSSI(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	return wlanoidSendSetQueryBowCmd(prAdapter, CMD_ID_GET_LINK_QUALITY, prBowFsmInfo->ucBssIndex, FALSE, TRUE,
					 wlanbowCmdEventReadRssi, wlanbowCmdTimeoutHandler, 0, NULL,
					 prCmd->rHeader.ucSeqNumber);
...
WLAN_STATUS bowCmdReadLinkQuality(IN P_ADAPTER_T prAdapter, IN P_AMPC_COMMAND prCmd)
{
	...
	return wlanoidSendSetQueryBowCmd(prAdapter, CMD_ID_GET_LINK_QUALITY, prBowFsmInfo->ucBssIndex, FALSE, TRUE,
					 wlanbowCmdEventReadLinkQuality, wlanbowCmdTimeoutHandler, 0, NULL,
					 prCmd->rHeader.ucSeqNumber);
...
VOID bowStopping(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(BOW, EVENT, "bowStoping, prBowBssInfo->aucBSSID, %x:%x:%x:%x:%x:%x.\n", prBowBssInfo->aucBSSID[0],
	       prBowBssInfo->aucBSSID[1], prBowBssInfo->aucBSSID[2], prBowBssInfo->aucBSSID[3],
...
VOID bowStarting(IN P_ADAPTER_T prAdapter)
{
	...
	if (g_u4LinkCount == 1) {
		...
		DBGLOG(BOW, EVENT, "prBssInfo->aucBSSID, %x:%x:%x:%x:%x:%x.\n", prBssInfo->aucBSSID[0],
		       prBssInfo->aucBSSID[1], prBssInfo->aucBSSID[2], prBssInfo->aucBSSID[3], prBssInfo->aucBSSID[4],
...
VOID bowResponderScanDone(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	if (eScanStatus == SCAN_STATUS_CANCELLED) {
		...
		if (eFsmState == BOW_DEVICE_STATE_DISCONNECTING) {
			wlanoidSendSetQueryBowCmd(prAdapter, CMD_ID_CMD_BT_OVER_WIFI, prBowFsmInfo->ucBssIndex, TRUE,
						  FALSE, wlanbowCmdEventLinkDisconnected, wlanbowCmdTimeoutHandler, 0,
						  NULL, 0);
		}
		return;
...
VOID bowResponderCancelScan(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgIsChannelExtention)
{
	...
	do {
		...
		if (TRUE) {
			...
			prScanCancel = (P_MSG_SCN_SCAN_CANCEL)cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
	...
	if (prJoinCompMsg->ucSeqNum == prBowFsmInfo->ucSeqNumOfReqMsg) {
		...
		if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
			...
			if (prStaRec->u2DesiredNonHTRateSet == 0) {
				...
				if (rateGetHighestRateIndexFromRateSet(prBowBssInfo->u2OperationalRateSet,
								       &ucHighestRateIndex)) {
			...
			wlanoidSendSetQueryBowCmd(prAdapter, CMD_ID_CMD_BT_OVER_WIFI, prBowFsmInfo->ucBssIndex, TRUE,
						  FALSE, wlanbowCmdEventLinkConnected, wlanbowCmdTimeoutHandler, 0,
						  NULL, 0);
		}
 ...
 */
/*----------------------------------------------------------------------------*/
VOID bowIndicationOfMediaStateToHost(IN P_ADAPTER_T prAdapter, IN ENUM_PARAM_MEDIA_STATE_T eConnectionState,
...
/*----------------------------------------------------------------------------*/
BOOLEAN
bowValidateAuth(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, IN PP_STA_RECORD_T pprStaRec,
		OUT PUINT_16 pu2StatusCode)
{
	...
	if (!prStaRec) {
		...
		prStaRec = cnmStaRecAlloc(prAdapter, STA_TYPE_BOW_CLIENT, prBowFsmInfo->ucBssIndex,
...
#else
	cnmTimerStartTimer(prAdapter, &prBowFsmInfo->rChGrantedTimer,
	...
	for (idx = 0; idx < CFG_BOW_PHYSICAL_LINK_NUM; idx++) {
		if (arBowTable[idx].fgIsValid && EQUAL_MAC_ADDR(arBowTable[idx].aucPeerAddress, aucPeerAddress)) {
			DBGLOG(BOW, EVENT, "kalCheckBowifVaild, aucPeerAddress %x, %x:%x:%x:%x:%x:%x.\n", idx,
			       aucPeerAddress[0], aucPeerAddress[1], aucPeerAddress[2], aucPeerAddress[3],

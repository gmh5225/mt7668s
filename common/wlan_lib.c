...
const UINT_8 aucPriorityParam2TC[] = { TC1_INDEX, TC0_INDEX, TC0_INDEX, TC1_INDEX,
				       TC2_INDEX, TC2_INDEX, TC3_INDEX, TC3_INDEX };
 ...
 *                                 M A C R O S
 *******************************************************************************/
#define SIGNED_EXTEND(n, _sValue) (((_sValue)&BIT((n)-1)) ? ((_sValue) | BITS(n, 31)) : ((_sValue) & ~BITS(n, 31)))
...
PFN_OID_HANDLER_FUNC apfnOidSetHandlerAllowedInRFTest[] = { wlanoidRftestSetTestMode, wlanoidRftestSetAbortTestMode,
							    wlanoidRftestSetAutoTest, wlanoidSetMcrWrite,
							    wlanoidSetEepromWrite };

/* OID query handlers allowed in RF test mode */
PFN_OID_HANDLER_FUNC apfnOidQueryHandlerAllowedInRFTest[] = { wlanoidRftestQueryAutoTest, wlanoidQueryMcrRead,
...
WLAN_STATUS wlanAdapterStart(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	do {
		...
		cnmTimerInitTimer(prAdapter, &prAdapter->rOidTimeoutTimer, (PFN_MGMT_TIMEOUT_FUNC)wlanReleasePendingOid,
				  (ULONG)NULL);
		...
		cnmTimerInitTimer(prAdapter, &prAdapter->rPacketDelaySetupTimer,
...
#else
	if (prAdapter->rAcpiState == ACPI_STATE_D0 && !wlanIsChipNoAck(prAdapter) &&
...
WLAN_STATUS wlanProcessCommandQueue(IN P_ADAPTER_T prAdapter, IN P_QUE_T prCmdQue)
{
	...
	while (prQueueEntry) {
		...
		case COMMAND_TYPE_SECURITY_FRAME:
			...
			eFrameAction = qmGetFrameAction(prAdapter, prMsduInfo->ucBssIndex, prMsduInfo->ucStaRecIndex,
							NULL, FRAME_TYPE_802_1X, prCmdInfo->u2InfoBufLen);
			break;
		...
		case COMMAND_TYPE_MANAGEMENT_FRAME:
			...
			eFrameAction = qmGetFrameAction(prAdapter, prMsduInfo->ucBssIndex, prMsduInfo->ucStaRecIndex,
							prMsduInfo, FRAME_TYPE_MMPDU, prMsduInfo->u2FrameLength);
			break;
...
VOID wlanClearTxCommandQueue(IN P_ADAPTER_T prAdapter)
{
	...
	while (prQueueEntry) {
		...
		nicTxReleaseResource(prAdapter, nicTxGetCmdResourceType(prCmdInfo), nicTxGetCmdPageCount(prCmdInfo),
				     TRUE);
...
VOID wlanReleaseCommand(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	case COMMAND_TYPE_MANAGEMENT_FRAME:
		...
		DBGLOG(INIT, INFO, "Free %s Frame: BSS[%u] WIDX:PID[%u:%u] SEQ[%u] STA[%u] RSP[%u] CMDSeq[%u]\n",
		       prCmdInfo->eCmdType == COMMAND_TYPE_SECURITY_FRAME ? "SEC" : "MGMT", prMsduInfo->ucBssIndex,
		       prMsduInfo->ucWlanIndex, prMsduInfo->ucPID, prMsduInfo->ucTxSeqNum, prMsduInfo->ucStaRecIndex,
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanQueryInformation(IN P_ADAPTER_T prAdapter, IN PFN_OID_HANDLER_FUNC pfnOidQryHandler, IN PVOID pvInfoBuf,
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanSetInformation(IN P_ADAPTER_T prAdapter, IN PFN_OID_HANDLER_FUNC pfnOidSetHandler, IN PVOID pvInfoBuf,
...
WLAN_STATUS wlanKeepFullPwr(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgEnable)
{
	...
	return wlanSendSetQueryCmd(prAdapter, CMD_ID_KEEP_FULL_PWR, TRUE, FALSE, FALSE, NULL, NULL,
...
#if CFG_ENABLE_FW_DOWNLOAD
VOID wlanImageSectionGetFwInfo(IN P_ADAPTER_T prAdapter, IN PVOID pvFwImageMapFile, IN UINT_32 u4FwImageFileLength,
	...
	*pu4DataMode = u4DataMode;
	...
	if (ucCurSecNum == 0) {
		DBGLOG(INIT, INFO, "%s INFO: chip_info[%u:E%u] feature[0x%02X]\n",
		       (eDlIdx == IMG_DL_IDX_N9_FW) ? "N9" : "CR4", prTailer->chip_info, prTailer->eco_code + 1,
		       prTailer->feature_set);
		DBGLOG(INIT, INFO, "date[%s] version[%c%c%c%c%c%c%c%c%c%c]\n", prTailer->ram_built_date,
		       prTailer->ram_version[0], prTailer->ram_version[1], prTailer->ram_version[2],
		       prTailer->ram_version[3], prTailer->ram_version[4], prTailer->ram_version[5],
		       prTailer->ram_version[6], prTailer->ram_version[7], prTailer->ram_version[8],
		       prTailer->ram_version[9]);
	}
...
VOID wlanImageSectionGetCompressFwInfo(IN P_ADAPTER_T prAdapter, IN PVOID pvFwImageMapFile,
				       IN UINT_32 u4FwImageFileLength, IN UINT_8 ucTotSecNum, IN UINT_8 ucCurSecNum,
				       IN ENUM_IMG_DL_IDX_T eDlIdx, OUT PUINT_32 pu4StartOffset, OUT PUINT_32 pu4Addr,
				       OUT PUINT_32 pu4Len, OUT PUINT_32 pu4DataMode, OUT PUINT_32 pu4BlockSize,
				       OUT PUINT_32 pu4CRC, OUT PUINT_32 pu4UncompressedLength)
{
	...
	*pu4DataMode = u4DataMode;
	...
	if (ucCurSecNum == 0) {
		DBGLOG(INIT, INFO, "%s INFO: chip_info[%u:E%u] feature[0x%02X]\n",
		       (eDlIdx == IMG_DL_IDX_N9_FW) ? "N9" : "CR4", prTailer->chip_info, prTailer->eco_code,
		       prTailer->feature_set);
		DBGLOG(INIT, INFO, "date[%s] version[%c%c%c%c%c%c%c%c%c%c]\n", prTailer->ram_built_date,
		       prTailer->ram_version[0], prTailer->ram_version[1], prTailer->ram_version[2],
		       prTailer->ram_version[3], prTailer->ram_version[4], prTailer->ram_version[5],
...
#endif
VOID wlanImageSectionGetPatchInfo(IN P_ADAPTER_T prAdapter, IN PVOID pvFwImageMapFile, IN UINT_32 u4FwImageFileLength,
				  IN UINT_8 ucTotSecNum, IN UINT_8 ucCurSecNum, IN ENUM_IMG_DL_IDX_T eDlIdx,
				  OUT PUINT_32 pu4StartOffset, OUT PUINT_32 pu4Addr, OUT PUINT_32 pu4Len,
				  OUT PUINT_32 pu4DataMode)
{
	...
	*pu4DataMode = u4DataMode;
	...
	DBGLOG(INIT, INFO, "PATCH INFO: platform[%s] HW/SW ver[0x%04X] ver[0x%04X]\n", aucBuffer,
	       prPatchFormat->u4SwHwVersion, prPatchFormat->u4PatchVersion);
...
VOID wlanImageSectionGetInfo(IN P_ADAPTER_T prAdapter, IN PVOID pvFwImageMapFile, IN UINT_32 u4FwImageFileLength,
			     IN UINT_8 ucTotSecNum, IN UINT_8 ucCurSecNum, IN ENUM_IMG_DL_IDX_T eDlIdx,
			     OUT PUINT_32 pu4StartOffset, OUT PUINT_32 pu4Addr, OUT PUINT_32 pu4Len,
			     OUT PUINT_32 pu4DataMode)
{
	if (eDlIdx == IMG_DL_IDX_PATCH) {
		wlanImageSectionGetPatchInfo(prAdapter, pvFwImageMapFile, u4FwImageFileLength, ucTotSecNum, ucCurSecNum,
					     eDlIdx, pu4StartOffset, pu4Addr, pu4Len, pu4DataMode);
	} else {
		wlanImageSectionGetFwInfo(prAdapter, pvFwImageMapFile, u4FwImageFileLength, ucTotSecNum, ucCurSecNum,
					  eDlIdx, pu4StartOffset, pu4Addr, pu4Len, pu4DataMode);
	}
}
#if CFG_SUPPORT_COMPRESSION_FW_OPTION

BOOLEAN wlanImageSectionCheckFwCompressInfo(IN P_ADAPTER_T prAdapter, IN PVOID pvFwImageMapFile,
					    IN UINT_32 u4FwImageFileLength, IN ENUM_IMG_DL_IDX_T eDlIdx)
{
...
WLAN_STATUS wlanImageSectionDownloadStage(IN P_ADAPTER_T prAdapter, IN PVOID pvFwImageMapFile,
					  ...
					  IN ENUM_IMG_DL_IDX_T eDlIdx, OUT PUINT_8 pucIsCompressed,
					  OUT P_INIT_CMD_WIFI_DECOMPRESSION_START prFwImageInFo)
{
	...
	for (i = 0; i < ucSectionNumber; ++i) {
		if (wlanImageSectionCheckFwCompressInfo(prAdapter, pvFwImageMapFile, u4FwImageFileLength, eDlIdx) ==
		    TRUE) {
			wlanImageSectionGetCompressFwInfo(prAdapter, pvFwImageMapFile, u4FwImageFileLength,
							  ucSectionNumber, i, eDlIdx, &u4StartOffset, &u4Addr, &u4Len,
							  &u4DataMode, &u4BlockSize, &u4CRC, &u4UnCompressedLength);
			u4offset = 0;
			if (i == 0) {
			...
			while (i4TotalLen) {
				...
				if (wlanImageSectionConfig(prAdapter, (u4Addr + u4offset), u4ChunkSize, u4DataMode,
			...
			*pucIsCompressed = TRUE;
			...
			wlanImageSectionGetInfo(prAdapter, pvFwImageMapFile, u4FwImageFileLength, ucSectionNumber, i,
						eDlIdx, &u4StartOffset, &u4Addr, &u4Len, &u4DataMode);
			pucStartPtr = (PUINT_8)pvFwImageMapFile + u4StartOffset;
					  ...
					  IN ENUM_IMG_DL_IDX_T eDlIdx)
{
	...
	/* 3a. parse file header for decision of divided firmware download or not */
	for (i = 0; i < ucSectionNumber; ++i) {
		wlanImageSectionGetInfo(prAdapter, pvFwImageMapFile, u4FwImageFileLength, ucSectionNumber, i, eDlIdx,
					&u4StartOffset, &u4Addr, &u4Len, &u4DataMode);

		pucStartPtr = (PUINT_8)pvFwImageMapFile + u4StartOffset;

		DBGLOG(INIT, INFO, "DL Offset[%u] addr[0x%08x] len[%u] datamode[0x%08x]\n", u4FileOffset, u4Addr, u4Len,
		       u4DataMode);
...
WLAN_STATUS wlanPatchRecvSemaResp(IN P_ADAPTER_T prAdapter, IN UINT_8 ucCmdSeqNum, OUT PUINT_8 pucPatchStatus)
{
	...
	if (nicRxWaitResponse(prAdapter, 0, aucBuffer, sizeof(INIT_HIF_RX_HEADER_T) + sizeof(INIT_EVENT_CMD_RESULT),
...
WLAN_STATUS wlanImageQueryStatus(IN P_ADAPTER_T prAdapter)
{
	...
	do {
		if (kalIsCardRemoved(prAdapter->prGlueInfo) == TRUE || fgIsBusAccessFailed == TRUE) {
			u4Status = WLAN_STATUS_FAILURE;
		} else if (nicRxWaitResponse(prAdapter, 0, aucBuffer,
...
WLAN_STATUS wlanImageSectionDownloadStatus(IN P_ADAPTER_T prAdapter, IN UINT_8 ucCmdSeqNum)
{
	...
	do {
		if (kalIsCardRemoved(prAdapter->prGlueInfo) == TRUE || fgIsBusAccessFailed == TRUE) {
			u4Status = WLAN_STATUS_FAILURE;
		} else if (nicRxWaitResponse(prAdapter, ucPortIdx, aucBuffer,
...
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
WLAN_STATUS
wlanCompressedFWConfigWifiFunc(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgEnable, IN UINT_32 u4StartAddress,
			       IN UINT_8 ucPDA, IN P_INIT_CMD_WIFI_DECOMPRESSION_START prFwImageInFo)
{
	P_CMD_INFO_T prCmdInfo;
	P_INIT_HIF_TX_HEADER_T prInitHifTxHeader;
	...
	kalMemZero(prCmdInfo->pucInfoBuffer, sizeof(INIT_HIF_TX_HEADER_T) + sizeof(INIT_CMD_WIFI_DECOMPRESSION_START));
	prCmdInfo->u2InfoBufLen = sizeof(INIT_HIF_TX_HEADER_T) + sizeof(INIT_CMD_WIFI_DECOMPRESSION_START);
...
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
		rDlStatus = wlanImageSectionDownloadStage(prAdapter, prFwBuffer, u4FwSize, CR4_FWDL_SECTION_NUM,
							  IMG_DL_IDX_CR4_FW, &fgIsCompressed, &rFwImageInFo);
...
WLAN_STATUS wlanWakeUpWiFi(IN P_ADAPTER_T prAdapter)
{
	...
	HAL_WIFI_FUNC_READY_CHECK(prAdapter, WIFI_FUNC_READY_BITS, &fgReady);
	if (fgReady) {
		DBGLOG(INIT, WARN, "Wi-Fi is already ON!, turn off before FW DL!\n");
#if 0
...
WLAN_STATUS wlanDownloadPatch(IN P_ADAPTER_T prAdapter)
{
	...
	do {
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
		wlanImageSectionDownloadStage(prAdapter, prFwBuffer, u4FwSize, 1, IMG_DL_IDX_PATCH, &ucIsCompressed,
					      NULL);
#else
...
WLAN_STATUS wlanGetPatchInfo(IN P_ADAPTER_T prAdapter)
{
	...
	wlanImageSectionGetInfo(prAdapter, prFwBuffer, u4FwSize, 1, 1, IMG_DL_IDX_PATCH, &u4StartOffset, &u4Addr,
				&u4Len, &u4DataMode);
 ...
 */
/*----------------------------------------------------------------------------*/
WLAN_STATUS wlanAccessRegister(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Addr, IN UINT_32 *pru4Result, IN UINT_32 u4Data,
	...
	if (ucSetQuery == 0) {
		...
		if (prInitEventAccessReg->u4Address != u4Addr) {
			DBGLOG(INIT, ERROR, "Event reports address incorrect. 0x%08x, 0x%08x.\n", u4Addr,
			       prInitEventAccessReg->u4Address);
			u4Status = WLAN_STATUS_FAILURE;
		}
 ...
 */
/*----------------------------------------------------------------------------*/
WLAN_STATUS wlanAccessRegisterStatus(IN P_ADAPTER_T prAdapter, IN UINT_8 ucCmdSeqNum, IN UINT_8 ucSetQuery,
				     IN PVOID prEvent, IN UINT_32 u4EventLen)
{
	...
	do {
		if (kalIsCardRemoved(prAdapter->prGlueInfo) == TRUE || fgIsBusAccessFailed == TRUE) {
			u4Status = WLAN_STATUS_FAILURE;
		} else if (nicRxWaitResponse(prAdapter, ucPortIdx, prEvent, u4EventLen, &u4RxPktLength) !=
			   WLAN_STATUS_SUCCESS) {
			u4Status = WLAN_STATUS_FAILURE;
		} else {
			...
			if (((prInitHifRxHeader->rInitWifiEvent.ucEID != INIT_EVENT_ID_CMD_RESULT) &&
			    ...
			    ((prInitHifRxHeader->rInitWifiEvent.ucEID != INIT_EVENT_ID_ACCESS_REG) &&
			     (ucSetQuery == 0))) {
				u4Status = WLAN_STATUS_FAILURE;
				DBGLOG(INIT, ERROR, "wlanAccessRegisterStatus: incorrect ucEID. ucSetQuery = 0x%x\n",
				       ucSetQuery);
			} else if (prInitHifRxHeader->rInitWifiEvent.ucSeqNum != ucCmdSeqNum) {
				u4Status = WLAN_STATUS_FAILURE;
				DBGLOG(INIT, ERROR, "wlanAccessRegisterStatus: incorrect ucCmdSeqNum. = 0x%x\n",
				       ucCmdSeqNum);
			} else {
			}
		}
	...
	if (kalQoSFrameClassifierAndPacketInfo(prAdapter->prGlueInfo, prPacket, &rTxPacketInfo)) {
		...
		if (rTxPacketInfo.u2Flag) {
			if (rTxPacketInfo.u2Flag & BIT(ENUM_PKT_1X)) {
				...
				prStaRec = cnmGetStaRecByAddress(prAdapter, GLUE_GET_PKT_BSS_IDX(prPacket),
								 rTxPacketInfo.aucEthDestAddr);

				GLUE_SET_PKT_FLAG(prPacket, ENUM_PKT_1X);
				/*
				 *				if (secIsProtected1xFrame(prAdapter, prStaRec) &&
				 *!kalIs24Of4Packet(prPacket)) GLUE_SET_PKT_FLAG(prPacket, ENUM_PKT_PROTECTED_1X);
				 */
			}
			...
			if (rTxPacketInfo.u2Flag & BIT(ENUM_PKT_VLAN_EXIST) && FEAT_SUP_LLC_VLAN_TX(prChipInfo))
...
BOOLEAN wlanProcessSecurityFrame(IN P_ADAPTER_T prAdapter, IN P_NATIVE_PACKET prPacket)
{
	...
	if (prCmdInfo && prMsduInfo) {
		...
		/* Fill-up MSDU_INFO */
		nicTxSetDataPacket(prAdapter, prMsduInfo, ucBssIndex, ucStaRecIndex, 0, u4PacketLen, nicTxDummyTxDone,
...
VOID wlanSecurityFrameTxDone(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (GET_BSS_INFO_BY_INDEX(prAdapter, prMsduInfo->ucBssIndex)->eNetworkType == NETWORK_TYPE_AIS &&
		...
		if (prSta) {
			kalMsleep(10);
			if (authSendDeauthFrame(prAdapter, GET_BSS_INFO_BY_INDEX(prAdapter, prMsduInfo->ucBssIndex),
						prSta, (P_SW_RFB_T)NULL, REASON_CODE_MIC_FAILURE,
...
VOID wlanEnableATGO(IN P_ADAPTER_T prAdapter)
{
	...
	prMsgConnReq = (P_MSG_P2P_CONNECTION_REQUEST_T)cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
...
VOID wlanPrintVersion(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(SW4, INFO, "N9 FW version %s-%u.%u.%u[DEC] (%s)\n", aucBuf, (UINT_32)(prVerInfo->u2FwOwnVersion >> 8),
	       (UINT_32)(prVerInfo->u2FwOwnVersion & BITS(0, 7)), prVerInfo->ucFwBuildNumber, prVerInfo->aucFwDateCode);
#if CFG_SUPPORT_COMPRESSION_FW_OPTION
...
WLAN_STATUS wlanQueryNicCapability(IN P_ADAPTER_T prAdapter)
{
	...
	while (TRUE) {
		if (nicRxWaitResponse(prAdapter, 1, aucBuffer, sizeof(WIFI_EVENT_T) + sizeof(EVENT_NIC_CAPABILITY_T),
				      &u4RxPktLength) != WLAN_STATUS_SUCCESS) {
	...
	prAdapter->fgIsHwACDisabled = (BOOLEAN)prEventNicCapability->ucHwNotSupportAC;
	prAdapter->fgIsEepromUsed = (BOOLEAN)prEventNicCapability->ucEepromUsed;
	prAdapter->fgIsEmbbededMacAddrValid =
		(BOOLEAN)(!IS_BMCAST_MAC_ADDR(prEventNicCapability->aucMacAddr) &&
	...
	if (prEventNicCapability->ucHwNotSupportAC) {
		...
		prAdapter->rWifiVar.aucMtkFeature[0] &= ~(MTK_SYNERGY_CAP_SUPPORT_24G_MCS89);
		DBGLOG(INIT, WARN, "Disable 2.4G 256QAM support if N only chip\n");
#endif
	}
	...
	if ((!prEventNicCapability->ucHwNotSupportDBDC) && (!prEventNicCapability->ucHwSetNss1x1) &&
		...
		prAdapter->rWifiFemCfg.u2WifiPath =
		   ...
		   (!prEventNicCapability->ucHwWiFiZeroOnly)) {
		prAdapter->rWifiFemCfg.u2WifiPath = (WLAN_FLAG_5G_WF0 | WLAN_FLAG_2G4_WF1);
	} else if ((prEventNicCapability->ucHwNotSupportDBDC) && (prEventNicCapability->ucHwSetNss1x1) &&
		   (!prEventNicCapability->ucHwWiFiZeroOnly)) {
		prAdapter->rWifiFemCfg.u2WifiPath = (WLAN_FLAG_5G_WF0 | WLAN_FLAG_2G4_WF1);
	} else if ((prEventNicCapability->ucHwNotSupportDBDC) && (!prEventNicCapability->ucHwSetNss1x1) &&
		...
		prAdapter->rWifiFemCfg.u2WifiPath =
		   ...
		   (prEventNicCapability->ucHwWiFiZeroOnly)) {
		prAdapter->rWifiFemCfg.u2WifiPath = (WLAN_FLAG_2G4_WF0 | WLAN_FLAG_5G_WF0);
	} else {
...
WLAN_STATUS wlanQueryPdMcr(IN P_ADAPTER_T prAdapter, P_PARAM_MCR_RW_STRUCT_T prMcrRdInfo)
{
	...
	if (nicRxWaitResponse(prAdapter, 1, aucBuffer, sizeof(WIFI_EVENT_T) + sizeof(CMD_ACCESS_REG), &u4RxPktLength) !=
	    WLAN_STATUS_SUCCESS) {
		return WLAN_STATUS_FAILURE;
	}
	/* header checking .. */
...
WLAN_STATUS wlanLoadManufactureData_5G(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	if (pr5GBandEdge->uc5GBandEdgePwrUsed != 0) {
		...
		wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_EDGE_TXPWR_LIMIT_5G, TRUE, FALSE, FALSE, NULL, NULL,
	...
	if (prRegInfo->prOldEfuseMapping->uc5GChannelOffsetVaild) {
		...
		wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_CHANNEL_PWR_OFFSET, TRUE, FALSE, FALSE, NULL, NULL,
	...
	if (prRegInfo->prOldEfuseMapping->uc11AcTxPwrValid) {
		...
		wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_80211AC_TX_PWR, TRUE, FALSE, FALSE, NULL, NULL,
				    sizeof(CMD_TX_AC_PWR_T), (PUINT_8)&rCmdAcPwr, NULL, 0);
...
WLAN_STATUS wlanLoadManufactureData(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	/* 1. Version Check */
	kalGetConfigurationVersion(prAdapter->prGlueInfo, &(prAdapter->rVerInfo.u2Part1CfgOwnVersion),
	...
	wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_PHY_PARAM, TRUE, FALSE, FALSE, NULL, NULL, sizeof(CMD_PHY_PARAM_T),
	...
	if (prRegInfo->ucRssiPathCompasationUsed) {
		...
		wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_PATH_COMPASATION, TRUE, FALSE, FALSE, NULL, NULL,
	...
	prAdapter->rWifiVar.rConnSettings.u2CountryCode = (((UINT_16)prRegInfo->au2CountryCode[0]) << 8) |
	...
	if (prRegInfo->fg2G4BandEdgePwrUsed) {
		...
		wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_EDGE_TXPWR_LIMIT, TRUE, FALSE, FALSE, NULL, NULL,
				    sizeof(CMD_EDGE_TXPWR_LIMIT_T), (PUINT_8)&rCmdEdgeTxPwrLimit, NULL, 0);
	}
	/*8. Set 2.4G AC power */
	...
	kalMemCopy(&rCmdNvramSettings.rNvramSettings, &prRegInfo->prNvramSettings->u2Part1OwnVersion,
		   sizeof(WIFI_CFG_PARAM_STRUCT));
	ASSERT(sizeof(WIFI_CFG_PARAM_STRUCT) == 512);
	wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_NVRAM_SETTINGS, TRUE, FALSE, FALSE, NULL, NULL,
...
WLAN_STATUS
wlanoidQueryBssStatistics(IN P_ADAPTER_T prAdapter, IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen,
...
VOID wlanDumpBssStatistics(IN P_ADAPTER_T prAdapter, UINT_8 ucBssIdx)
{
	...
	for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
		DBGLOG(SW4, INFO, "LLS BSS[%u] %s: T[%06u] R[%06u] T_D[%06u] T_F[%06u]\n", prBssInfo->ucBssIndex,
		       apucACI2Str[eAci], arLLStats[eAci].u4TxMsdu, arLLStats[eAci].u4RxMsdu,
		       arLLStats[eAci].u4TxDropMsdu, arLLStats[eAci].u4TxFailMsdu);
	}
}
...
WLAN_STATUS
wlanoidQueryStaStatistics(IN P_ADAPTER_T prAdapter, IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen,
			  OUT PUINT_32 pu4QueryInfoLen)
{
	return _wlanoidQueryStaStatistics(prAdapter, pvQueryBuffer, u4QueryBufferLen, pu4QueryInfoLen, g_fgIsOid);
} /* wlanoidQueryStaStatistics */

WLAN_STATUS
_wlanoidQueryStaStatistics(IN P_ADAPTER_T prAdapter, IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen,
	...
	do {
		...
		*pu4QueryInfoLen = sizeof(PARAM_GET_STA_STA_STATISTICS);
		...
		if (prAdapter->u4FwCompileFlag0 & COMPILE_FLAG0_GET_STA_LINK_STATUS) {
			...
			rResult = wlanSendSetQueryCmd(prAdapter, CMD_ID_GET_STA_STATISTICS, FALSE, TRUE, fgIsOid,
						      nicCmdEventQueryStaStatistics, nicOidCmdTimeoutCommon,
						      sizeof(CMD_GET_STA_STATISTICS_T),
						      (PUINT_8)&rQueryCmdStaStatistics, pvQueryBuffer,
						      u4QueryBufferLen);
...
WLAN_STATUS wlanQueryNicCapabilityV2(IN P_ADAPTER_T prAdapter)
{
	...
	if (prAdapter->u4FwFeatureFlag0 & FEATURE_FLAG0_NIC_CAPABILITY_V2) {
		...
		/* get event */
		while (TRUE) {
			if (nicRxWaitResponse(prAdapter, 1, prEventBuff, CFG_RX_MAX_PKT_SIZE, &u4RxPktLength) !=
			    WLAN_STATUS_SUCCESS) {
				DBGLOG(INIT, WARN, "%s: wait for event failed!\n", __func__);
...
VOID wlanSetNicResourceParameters(IN P_ADAPTER_T prAdapter)
{
	...
	for (idx = 0; idx < TC_NUM; idx++) {
		...
		prWifiVar->au4TcPageCount[idx] =
			(UINT_32)wlanCfgGetUint32(prAdapter, string, prWifiVar->au4TcPageCount[idx]);
	}
	...
	for (idx = 0; idx < TC_NUM; idx++) {
		...
		prQM->au4GuaranteedTcResource[idx] =
			(UINT_32)wlanCfgGetUint32(prAdapter, string, prQM->au4GuaranteedTcResource[idx]);
	}
...
VOID wlanInitFeatureOption(IN P_ADAPTER_T prAdapter)
{
	...
	prWifiVar->ucApAllowHtVhtTkip = (UINT_8)wlanCfgGetUint32(prAdapter, "ApAllowHtVhtTkip", FEATURE_DISABLED);
	...
	prWifiVar->ucAdvPws = (UINT_8)wlanCfgGetUint32(prAdapter, "AdvPws", FEATURE_DISABLED);
	...
	prWifiVar->ucCtrlFlagAssertPath = (UINT_8)wlanCfgGetUint32(prAdapter, "AssertPath", DBG_ASSERT_PATH_DEFAULT);
	prWifiVar->ucCtrlFlagDebugLevel =
		(UINT_8)wlanCfgGetUint32(prAdapter, "AssertLevel", DBG_ASSERT_CTRL_LEVEL_DEFAULT);
	prWifiVar->u4ScanCtrl = (UINT_8)wlanCfgGetUint32(prAdapter, "ScanCtrl", SCN_CTRL_DEFAULT_SCAN_CTRL);
	prWifiVar->ucScanChannelListenTime = (UINT_8)wlanCfgGetUint32(prAdapter, "ScnChListenTime", 0);

	/* Wake lock related configuration */
	prWifiVar->u4WakeLockRxTimeout = wlanCfgGetUint32(prAdapter, "WakeLockRxTO", WAKE_LOCK_RX_TIMEOUT);
	...
	prWifiVar->uDeQuePercentEnable = (UINT_8)wlanCfgGetUint32(prAdapter, "DeQuePercentEnable", 1);
	...
	prWifiVar->u4AisJoinChReqIntervel =
		(UINT_32)wlanCfgGetUint32(prAdapter, "AisJoinChReqIntervel", AIS_JOIN_CH_REQUEST_INTERVAL);
...
#endif
#if 0
	prWifiVar->fgEfuseCheck = (UINT_8)wlanCfgGetUint32(prAdapter, "EfuseCheck", FEATURE_DISABLED);
#endif
}
...
VOID wlanCfgSetDebugLevel(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < WLAN_CFG_SET_DEBUG_LEVEL_LEN_MAX; i++) {
		...
		if (u4DbgIdx == 0xFFFFFFFF) {
			...
			DBGLOG(INIT, INFO, "Set DBG module[%u] log level to [0x%02x]!", u4DbgIdx, (UINT_8)u4DbgMask);
		}
	}
}
...
VOID wlanCfgSetCountryCode(IN P_ADAPTER_T prAdapter)
{
	...
	if (wlanCfgGet(prAdapter, "Country", aucValue, "", 0) == WLAN_STATUS_SUCCESS) {
		prAdapter->rWifiVar.rConnSettings.u2CountryCode = (((UINT_16)aucValue[0]) << 8) |
								  ((UINT_16)aucValue[1]);
...
enum { STATE_EOF = 0, STATE_TEXT = 1, STATE_NEWLINE = 2 };
...
WLAN_STATUS
wlanCfgParseAddEntry(IN P_ADAPTER_T prAdapter, PUINT_8 pucKeyHead, PUINT_8 pucKeyTail, PUINT_8 pucValueHead,
		     PUINT_8 pucValueTail)
{
	...
	if ((pucKeyHead == NULL) || (pucValueHead == NULL))
		return WLAN_STATUS_FAILURE;
...
enum { WAIT_KEY_HEAD = 0, WAIT_KEY_TAIL, WAIT_VALUE_HEAD, WAIT_VALUE_TAIL, WAIT_COMMENT_TAIL };
...
#endif
	if (buffer == NULL || args_size[ED_STRING_SITE] == 0 || args_size[ED_VALUE_SITE] == 0 ||
...
VOID wlanFeatureToFw(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < WLAN_CFG_ENTRY_NUM_MAX; i++) {
		...
		if (prWlanCfgEntry) {
			...
			kalMemCopy(((P_CMD_FORMAT_V1_T)rCmdV1Header.buffer) + ucTimes, &rCmd_v1,
				   sizeof(CMD_FORMAT_V1_T));
...
WLAN_STATUS wlanCfgParse(IN P_ADAPTER_T prAdapter, PUINT_8 pucConfigBuf, UINT_32 u4ConfigBufLen, BOOLEAN isFwConfig)
{
	...
	for (;;) {
		...
		case STATE_EOF:
			...
			if (i4Nargs == 3 && !isFwConfig && arcArgv_size[0] == 1) {
				...
				kalMemSet(apcArgv, 0, WLAN_CFG_ARGV_MAX * sizeof(int8_t *));
				i4Nargs = 0;
				goto exit;
			}
		...
		case STATE_TEXT:
			if (i4Nargs < WLAN_CFG_ARGV_MAX) {
				...
				DBGLOG(INIT, INFO, " nargs= %d STATE_TEXT = %s, SIZE = %d\n", i4Nargs - 1,
				       ppcArgs[i4Nargs - 1], arcArgv_size[i4Nargs - 1]);
			}
			break;
		}
	}
	...
	for (i4BssIdx = 0; i4BssIdx < HW_BSSID_NUM; i4BssIdx++) {
		...
		if (prBssInfo->eNetworkType == NETWORK_TYPE_AIS) {
			...
			if (prBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT) {
				...
				LINK_FOR_EACH_ENTRY_SAFE(prCurrStaRec, prNextCurrStaRec, prClientList, rLinkEntry,
							 STA_RECORD_T)
				{
					kalP2PGOStationUpdate(prAdapter->prGlueInfo, (UINT_8)prBssInfo->u4PrivateData,
							      prCurrStaRec, FALSE);
				...
				kalP2PGCIndicateConnectionStatus(prGlueInfo, (UINT_8)prBssInfo->u4PrivateData, NULL,
...
WLAN_STATUS wlan1xTxDone(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
...
VOID wlanNotifyFwSuspend(P_GLUE_INFO_T prGlueInfo, struct net_device *prDev, BOOLEAN fgSuspend)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidNotifyFwSuspend, (PVOID)&rSuspendCmd, sizeof(rSuspendCmd), FALSE, FALSE,
			   TRUE, &u4SetInfoLen);
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanoidQueryLteSafeChannel(IN P_ADAPTER_T prAdapter, IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen,
			   OUT PUINT_32 pu4QueryInfoLen)
{
	WLAN_STATUS rResult = WLAN_STATUS_FAILURE;
	...
	do {
		...
		rResult = wlanSendSetQueryCmd(prAdapter, CMD_ID_GET_LTE_CHN, FALSE, TRUE, g_fgIsOid, /* Query ID */
					      nicCmdEventQueryLteSafeChn, /* The handler to receive firmware
									     notification */
					      nicOidCmdTimeoutCommon, sizeof(CMD_GET_LTE_SAFE_CHN_T),
 ...
 */
/*----------------------------------------------------------------------------*/
static VOID wlanAddDirtinessToAffectedChannels(P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc, UINT_32 u4Dirtiness,
					       UINT_8 ucCentralChannel, UINT_8 ucCoveredRange)
{
	UINT_8 ucIdx, ucStart, ucEnd;
	BOOL bIs5GChl = ucCentralChannel > 14;
	UINT_8 ucLeftNeighborChannel, ucRightNeighborChannel, ucLeftNeighborChannel2 = 0, ucRightNeighborChannel2 = 0,
							      ucLeftestCoveredChannel, ucRightestCoveredChannel;
	P_PARAM_GET_CHN_INFO prGetChnLoad = &(prAdapter->rWifiVar.rChnLoadInfo);

	ucLeftestCoveredChannel = ucCentralChannel > ucCoveredRange ? ucCentralChannel - ucCoveredRange : 1;

	ucLeftNeighborChannel = ucLeftestCoveredChannel ? ucLeftestCoveredChannel - 1 : 0;
	...
	if (bIs5GChl) {
		...
		ucLeftNeighborChannel2 = ucLeftNeighborChannel > 1 ? ucLeftNeighborChannel - 1 : 0;
	}
	...
	if (bIs5GChl) {
		...
		ucRightNeighborChannel2 = ucRightNeighborChannel < 13 ? ucRightNeighborChannel + 1 : 0;
	}
	...
	for (ucIdx = ucStart; ucIdx <= ucEnd; ucIdx++) {
		...
		DBGLOG(SCN, LOUD, "Add dirtiness %d, to covered ch %d\n", u4Dirtiness,
	...
	if (ucLeftNeighborChannel != 0) {
		...
		DBGLOG(SCN, LOUD, "Add dirtiness %d, to neighbor ch %d\n", u4Dirtiness >> 1,
		       prGetChnLoad->rEachChnLoad[ucIdx].ucChannel);
	}
 ...
 */
/*----------------------------------------------------------------------------*/
static VOID wlanCalculateChannelDirtiness(IN P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc, UINT_32 u4Dirtiness,
					  BOOL bIsIndexOne)
{
	UINT_8 ucCoveredRange = 0, ucCentralChannel = 0, ucCentralChannel2 = 0;

	if (bIsIndexOne) {
		DBGLOG(SCN, LOUD, "Process dirtiness index 1\n");
		ucCentralChannel = prBssDesc->ucChannelNum;
		ucCoveredRange = 2;
	} else {
		DBGLOG(SCN, LOUD, "Process dirtiness index 2, ");
		switch (prBssDesc->eChannelWidth) {
		case CW_20_40MHZ:
			if (prBssDesc->eSco == CHNL_EXT_SCA) {
				DBGLOG(SCN, LOUD, "BW40\n");
				...
				DBGLOG(SCN, LOUD, "BW20\n");
				ucCentralChannel = prBssDesc->ucChannelNum;
				ucCoveredRange = 2;
			}
			break;
		case CW_80MHZ:
			DBGLOG(SCN, LOUD, "BW80\n");
			ucCentralChannel = prBssDesc->ucCenterFreqS1;
			ucCoveredRange = 8;
			break;
		case CW_160MHZ:
			DBGLOG(SCN, LOUD, "BW160\n");
			ucCentralChannel = prBssDesc->ucCenterFreqS1;
			ucCoveredRange = 16;
			break;
		case CW_80P80MHZ:
			DBGLOG(SCN, LOUD, "BW8080\n");
			ucCentralChannel = prBssDesc->ucCenterFreqS1;
	...
	wlanAddDirtinessToAffectedChannels(prAdapter, prBssDesc, u4Dirtiness, ucCentralChannel, ucCoveredRange);

	/* 80 + 80 secondary 80 case */
	if (bIsIndexOne || ucCentralChannel2 == 0)
		return;

	wlanAddDirtinessToAffectedChannels(prAdapter, prBssDesc, u4Dirtiness, ucCentralChannel2, ucCoveredRange);
}
...
VOID wlanInitChnLoadInfoChannelList(IN P_ADAPTER_T prAdapter)
{
	...
	for (ucIdx = 0; ucIdx < MAX_CHN_NUM; ucIdx++)
		prGetChnLoad->rEachChnLoad[ucIdx].ucChannel = wlanGetChannelNumFromIndex(ucIdx);
}
...
wlanCalculateAllChannelDirtiness(IN P_ADAPTER_T prAdapter)
{
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T)
	{
		...
		DBGLOG(SCN, LOUD, "Found an AP(%s), primary ch %d\n", prBssDesc->aucSSID, prBssDesc->ucChannelNum);

		/* dirtiness index1 */
		wlanCalculateChannelDirtiness(prAdapter, prBssDesc, u4Dirtiness, TRUE);

		/* dirtiness index2 */
		wlanCalculateChannelDirtiness(prAdapter, prBssDesc, u4Dirtiness >> 1, FALSE);
	}
...
VOID wlanSortChannel(IN P_ADAPTER_T prAdapter)
{
	...
	for (ucIdx = 0; ucIdx < MAX_CHN_NUM; ++ucIdx) {
		prChnLoadInfo->rChnRankList[ucIdx].ucChannel = prChnLoadInfo->rEachChnLoad[ucIdx].ucChannel;
		prChnLoadInfo->rChnRankList[ucIdx].u4Dirtiness = prChnLoadInfo->rEachChnLoad[ucIdx].u4Dirtiness;
	}
	...
	for (ucIdx = MAX_CHN_NUM / 2 - 1; ucIdx >= 0; --ucIdx) {
		for (ucRoot = ucIdx; ucRoot * 2 + 1 < MAX_CHN_NUM; ucRoot = ucChild) {
			ucChild = ucRoot * 2 + 1;
			if (ucChild < MAX_CHN_NUM - 1 && prChnLoadInfo->rChnRankList[ucChild + 1].u4Dirtiness >
	...
	for (ucIdx = MAX_CHN_NUM - 1; ucIdx > 0; ucIdx--) {
		...
		for (ucRoot = 0; ucRoot * 2 + 1 < ucIdx; ucRoot = ucChild) {
			ucChild = ucRoot * 2 + 1;
			if (ucChild < ucIdx - 1 && prChnLoadInfo->rChnRankList[ucChild + 1].u4Dirtiness >
							   prChnLoadInfo->rChnRankList[ucChild].u4Dirtiness)
				ucChild += 1;
...
UINT_8
wlanAntPathFavorSelect(enum ENUM_WF_PATH_FAVOR_T eWfPathFavor)
{
	UINT_8 ucRetValSpeIdx = 0;

	if ((eWfPathFavor == ENUM_WF_NON_FAVOR) || (eWfPathFavor == ENUM_WF_0_ONE_STREAM_PATH_FAVOR) ||
...
{
	if (bReqFw)
		return kalRequestFirmware(pcFileName, pucBuf, u4MaxFileLen, pu4ReadFileLen,
					  prAdapter->prGlueInfo->prDev);

	return kalReadToFile(pcFileName, pucBuf, u4MaxFileLen, pu4ReadFileLen);
}
...
WLAN_STATUS wlanUpdateExtInfo(IN P_ADAPTER_T prAdapter)
{
	...
	return kalIoctl(prGlueInfo, wlanoidSetSwCtrlWrite, &rSwCtrlInfo, sizeof(rSwCtrlInfo), FALSE, FALSE, TRUE,
			&u4BufLen);
}
#endif
...
int wlanSuspendRekeyOffload(P_GLUE_INFO_T prGlueInfo, IN UINT_8 ucRekeyMode)
{
	...
	if ((ucRekeyMode == GTK_REKEY_CMD_MODE_SET_BCMC_PN) && (ucRpyOffload == FALSE)) {
		DBGLOG(RSN, INFO, "Set PN to fw, but feature off. no action\n");
		return WLAN_STATUS_SUCCESS;
	}

	if ((ucRekeyMode == GTK_REKEY_CMD_MODE_GET_BCMC_PN) && (ucRpyOffload == FALSE)) {
	...
	if ((ucRekeyMode == GTK_REKEY_CMD_MODE_SET_BCMC_PN) && (ucRpyOffload == TRUE)) {
		prGtkData->ucBssIndex = prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex;
		prBssInfo =
			GET_BSS_INFO_BY_INDEX(prGlueInfo->prAdapter, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex);
		...
		kalMemCopy(prGtkData->aucReplayCtr, prDetRplyInfo->arReplayPNInfo[ucCurKeyId].auPN,
	...
	if ((ucRekeyMode == GTK_REKEY_CMD_MODE_GET_BCMC_PN) && (ucRpyOffload == TRUE)) {
		prGtkData->ucBssIndex = prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex;
	}
#endif

	rStatus = kalIoctl(prGlueInfo, wlanoidSetGtkRekeyData, prGtkData, sizeof(PARAM_GTK_REKEY_DATA), FALSE, FALSE,
			   TRUE, &u4BufLen);
...
VOID wlanSuspendPmHandle(P_GLUE_INFO_T prGlueInfo)
{
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucWow &&
	    (prGlueInfo->prAdapter->rWowCtrl.fgWowEnable || prGlueInfo->prAdapter->rWifiVar.ucAdvPws)) {
...
VOID wlanResumePmHandle(P_GLUE_INFO_T prGlueInfo)
{
	...
	/* if wifi.cfg EAPOL offload is disble,
	 *we disable FW offload when leave wow
	 */
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucWow &&
		...
		if (kalGetMediaStateIndicated(prGlueInfo) == PARAM_MEDIA_STATE_CONNECTED) {
			...
			nicConfigPowerSaveWowProfile(prGlueInfo->prAdapter,
						     prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex, ePwrMode, FALSE,
						     FALSE);
		}
	}
...
VOID setRekeyOffloadEnterWow(P_GLUE_INFO_T prGlueInfo)
{
...
#endif
	/* if wifi.cfg EAPOL offload is 0,
	 *we set rekey offload when enter wow
	 */
	if (!prGlueInfo->prAdapter->rWifiVar.ucEapolOffload) {
		...
		if ((ucKekZeroCnt == NL80211_KCK_LEN) || (ucKckZeroCnt == NL80211_KCK_LEN)) {
			...
			wlanSuspendRekeyOffload(prGlueInfo, GTK_REKEY_CMD_MODE_RPY_OFFLOAD_OFF);
		}
...
void disconnect_sta(P_ADAPTER_T prAdapter, P_STA_RECORD_T sta_rec)
{
	...
	case STA_TYPE_LEGACY_AP:
		...
		ais_abort_msg = (P_MSG_AIS_ABORT_T)cnmMemAlloc(prAdapter, RAM_TYPE_MSG, sizeof(MSG_AIS_ABORT_T));
		if (ais_abort_msg) {
			ais_abort_msg->rMsgHdr.eMsgId = MID_OID_AIS_FSM_JOIN_REQ;
			ais_abort_msg->ucReasonOfDisconnect = DISCONNECT_REASON_CODE_DISASSOCIATED;
			ais_abort_msg->fgDelayIndication = FALSE;

			DBGLOG(AIS, INFO, "Disconnect STA[" MACSTR "] type:0x%x\n", MAC2STR(sta_rec->aucMacAddr),
			       sta_rec->eStaType);

			mboxSendMsg(prAdapter, MBOX_ID_0, (P_MSG_HDR_T)ais_abort_msg, MSG_SEND_METHOD_UNBUF);
#define DISCONNECT_STATUS WLAN_STATUS_MEDIA_DISCONNECT
			/* indicate for disconnection */
			if (kalGetMediaStateIndicated(glue_info) == PARAM_MEDIA_STATE_CONNECTED) {
				kalIndicateStatusAndComplete(glue_info, DISCONNECT_STATUS, NULL, 0);
			}
#undef DISCONNECT_STATUS
	...
	case STA_TYPE_P2P_GO:
		p2p_abot_msg = (P_MSG_P2P_CONNECTION_ABORT_T)cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
									 sizeof(MSG_P2P_CONNECTION_ABORT_T));

		if (p2p_abot_msg) {
			p2p_bss_info = GET_BSS_INFO_BY_INDEX(prAdapter, sta_rec->ucBssIndex);
			role_idx = p2p_bss_info->u4PrivateData;
			p2p_abot_msg->rMsgHdr.eMsgId = MID_MNY_P2P_CONNECTION_ABORT;
			COPY_MAC_ADDR(p2p_abot_msg->aucTargetID, sta_rec->aucMacAddr);

			p2p_abot_msg->u2ReasonCode = REASON_CODE_UNSPECIFIED;
			p2p_abot_msg->ucRoleIdx = role_idx;
			p2p_abot_msg->fgSendDeauth = FALSE;

			DBGLOG(P2P, INFO, "Disconnect STA[" MACSTR "] type:0x%x\n", MAC2STR(sta_rec->aucMacAddr),
			       sta_rec->eStaType);

			mboxSendMsg(prAdapter, MBOX_ID_0, (P_MSG_HDR_T)p2p_abot_msg, MSG_SEND_METHOD_UNBUF);
		}
		break;
	default:
		break;
...
bool wlanPushCSIData(P_ADAPTER_T prAdapter, struct CSI_DATA_T *prCSIData)
{
	...
	kalMemCopy(&(prCSIInfo->arCSIBuffer[prCSIInfo->u4CSIBufferTail]), prCSIData, sizeof(struct CSI_DATA_T));
...
bool wlanPopCSIData(P_ADAPTER_T prAdapter, struct CSI_DATA_T *prCSIData)
{
	...
	kalMemCopy(prCSIData, &(prCSIInfo->arCSIBuffer[prCSIInfo->u4CSIBufferHead]), sizeof(struct CSI_DATA_T));
...
VOID wlanApplyCSIToneMask(UINT_8 ucRxMode, UINT_8 ucCBW, UINT_8 ucDBW, UINT_8 ucPrimaryChIdx, INT_16 *ai2IData,
			  INT_16 *ai2QData)
{
...
#define ZERO_RANGE(start, end) \
	...
	} else if (ucRxMode == RX_VT_MIXED_MODE || ucRxMode == RX_VT_GREEN_MODE || ucRxMode == RX_VT_VHT_MODE) {
		if (ucCBW == RX_VT_FR_MODE_20) {
			ZERO(0);
			ZERO_RANGE(29, 35);
		} else if (ucCBW == RX_VT_FR_MODE_40) {
...
VOID wlanShiftCSI(UINT_8 ucRxMode, UINT_8 ucCBW, UINT_8 ucDBW, UINT_8 ucPrimaryChIdx, INT_16 *ai2IData,
		  INT_16 *ai2QData, INT_16 *ai2ShiftIData, INT_16 *ai2ShiftQData)
{
	UINT_8 ucSize = sizeof(INT_16);
#define COPY_RANGE(dest, start, end) \
	{ \
		kalMemCopy(&ai2ShiftIData[dest], &ai2IData[start], ucSize *(end - start + 1)); \
		kalMemCopy(&ai2ShiftQData[dest], &ai2QData[start], ucSize *(end - start + 1)); \
	}

#define COPY(dest, src) \
	{ \
		ai2ShiftIData[dest] = ai2IData[src]; \
		ai2ShiftQData[dest] = ai2QData[src]; \
	}

...
static int __init preallocInit(void)
{
	...
	u4Size = u4Size > HIF_RX_COALESCING_BUFFER_SIZE ? u4Size : HIF_RX_COALESCING_BUFFER_SIZE;
	...
	preallocAddBlock(MEM_ID_RX_DATA, "RX DATA", HIF_RX_COALESCING_BUF_COUNT, HIF_RX_COALESCING_BUFFER_SIZE,
			 GFP_KERNEL);
#endif
#if 0
...
#if CFG_USB_TX_AGG
	preallocAddBlock(MEM_ID_TX_DATA, "TX AGG DATA", (USB_TC_NUM * USB_REQ_TX_DATA_CNT), USB_TX_DATA_BUFF_SIZE,
			 GFP_KERNEL);
#else
